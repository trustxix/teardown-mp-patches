...
-RECOIL_BASE_HIP   = 0.95   -- per-shot pitch in hip
-RECOIL_BASE_ADS   = 0.85   -- per-shot pitch in ADS
...
-	RegisterTool("rmwxm7", "SIG Spear", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwxm7.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwxm7", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwxm7" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwxm7", "SIG Spear", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwxm7.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
