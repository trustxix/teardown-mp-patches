...
-RECOIL_BASE_ADS   = 0.6   -- per-shot pitch in ADS
-RECOIL_YAW_RANGE  = 0.7   -- +/- horizontal drift per shot (random)
...
-VISUAL_SCALE_ADS  = 0.8   -- viewmodel tilt multiplier (ADS)
...
-	RegisterTool("rmwmp7", "MP7", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwmp7.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwmp7", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
+
+tion draw()
 if GetString("game.player.tool") == "rmwmp7" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwmp7", "MP7", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwmp7.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
