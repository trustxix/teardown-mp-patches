...
**Rule:** Do NOT use `ClientCall(0, "client.playSound", ...)` or `ClientCall(p, "client.playSound", ...)` for game sounds. Just call `PlaySound()` on the server. Only use `UiSound()` for client-only UI feedback (button clicks, menu transitions).

---

## Base Game Pattern: ToolAnimator must run for ALL players
**Source:** Official snowball.lua client.tick
**Pattern:**
```lua
function client.tick(dt)
    ...
    for p in Players() do
        if GetPlayerTool(p) == "mytool" then
            tickToolAnimator(toolAnimators[p], dt, nil, p)
        end
    end
end
```
**Why:** `tickToolAnimator` internally detects FP vs TP: `IsPlayerLocal(p)` → first-person poses, remote → third-person poses. If you only call it for the local player, remote players see a static tool.
**Rule:** Always call `tickToolAnimator` inside a `for p in Players()` loop. Create animators in `PlayersAdded()`, destroy in `PlayersRemoved()`.

---

## Base Game Pattern: shared.* tables for client-readable state
**Source:** Official mpcampaign/stats.lua, spawn.lua, teams.lua, hud.lua
**Pattern:** Server writes to `shared.*` tables. Client reads them every frame. No RPC needed.
```lua
-- Server writes:
shared.respawnTimeLeft[p] = math.ceil(respawnTime - deadTime)
shared._statsState[p] = { kills=N, deaths=N }

-- Client reads (automatically synced):
local timeLeft = shared.respawnTimeLeft[GetLocalPlayer()]
```
**Rule:** For any state the client needs to display (ammo, settings, scores, timers), prefer `shared.*` over ServerCall/ClientCall. Zero RPC cost, automatic sync.

---

## Base Game Pattern: Event-driven death/damage detection
**Source:** Official mpcampaign/stats.lua, tools.lua, hud.lua
**Pattern:**
```lua
...
```
**Rule:** Use `GetEventCount("playerdied")`/`GetEvent()` instead of polling `GetPlayerHealth()` every tick. Events fire exactly once per death — no duplicate processing, no missed deaths between ticks.

---

## Base Game Pattern: Registry broadcast with `true` flag
**Source:** Official toolutilities.lua, toolspawn.lua, mp/campaign.lua
**Pattern:**
```lua
SetInt("game.tool."..toolId..".ammo", value, true)     -- broadcasts to all clients
SetBool("game.tool."..toolId..".enabled", true, true)   -- broadcasts to all clients
SetFloat("level.health", healthValue, true)              -- broadcasts to all clients
```
**Rule:** For any state that all clients need, use the `true` flag on `SetInt/SetFloat/SetBool/SetString`. This replaces per-tick RPC for continuous state sync.

---

## Base Game Pattern: ClientCall(0, ...) for world-visible events
**Source:** Official playerbody.lua
**Pattern:**
```lua
-- Server → ALL clients (player 0 = broadcast)
ClientCall(0, "client.character_die", victim)
ClientCall(0, "client.character_live", victim)

-- Server → ONE client (personal feedback only)
ClientCall(p, "client.setAmmo", ammo)
```
**Rule:** Use `ClientCall(0, ...)` for events all players should see (explosions, deaths, pickups). Use `ClientCall(p, ...)` only for personal feedback (camera shake, ammo HUD, recoil).

---

## Base Game Pattern: PlayersRemoved cleanup for per-player state
**Source:** Official snowball.lua, mpcampaign/stats.lua, playerbody.lua
**Pattern:**
```lua
for p in PlayersAdded() do
    toolAnimators[p] = ToolAnimator()
...
```
**Why:** Without cleanup, per-player state tables leak when players disconnect. New players joining may inherit stale state from old player IDs (wrong settings, stuck abilities, ghost projectiles). The base game ALWAYS pairs `PlayersAdded()` with `PlayersRemoved()`.
**Auto-fix available:** `python -m tools.fix --only missing-players-removed` adds a stub `PlayersRemoved()` cleanup loop.
**Rule:** Every `PlayersAdded()` MUST have a matching `PlayersRemoved()` that cleans up all per-player tables, animators, timers, and projectile lists.
