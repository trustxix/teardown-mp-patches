...
-function tick()
-
-    -- Fix broken sound + timer after quicksave reload
-    if not sounds or not sounds.play or not sounds.play.poltergeist then
-        Poltergeist.init()
-        initSounds()
...
+#version 2
 function initDebug()
     db = regGetBool('impactDamage.debugMode')
 end
...
# Migration Report: scripts\impact_damage.lua
...
```diff
--- original/scripts\impact_damage.lua
+++ patched/scripts\impact_damage.lua
@@ -1,8 +1,4 @@
-ImpactDamage = {}
-ImpactDamage.blacklist = {}
-ImpactDamage.blacklistCounter = {}
-ImpactDamage.floorBody = nil
-
+#version 2
 function ImpactDamage.run()
 
     -- Get all objects colliding with the player
@@ -16,7 +12,7 @@
         local damages, source = ImpactDamage.getImpactDamage(body, impacts[body])
         -- dbp("Body "..body.." blacklisted "..(ImpactDamage.blacklist[body] and "true" or "false"))
         -- dbp("Body "..body.." damages "..damages)
-        if not ImpactDamage.blacklist[body] and damages > 0 then
+        if not ImpactDamage.blacklist[body] and damages ~= 0 then
             if source == "player" then
                 -- If player movement does damage, stores the max damages taken
                 playerDamages = math.max(playerDamages, damages)
@@ -27,7 +23,7 @@
             end
         end
         -- Add object to blacklist if damages were done, remove if no damage for 10 frames
-        if damages > 0 then
+        if damages ~= 0 then
             ImpactDamage.blacklistBody(body)
         elseif ImpactDamage.blacklist[body] then
             local unblCount = ImpactDamage.blacklistCounter[body]
@@ -38,7 +34,7 @@
             end
         end
     end
-    if playerDamages > 0 then
+    if playerDamages ~= 0 then
         dbw("Impact damages from player movement", playerDamages)
         -- If player movement does damage, inflict them
         ImpactDamage.damagePlayer(playerDamages)
@@ -46,20 +42,16 @@
 
 end
 
-
 function ImpactDamage.blacklistBody(body)
     ImpactDamage.blacklist[body] = true
     ImpactDamage.blacklistCounter[body] = 0
 end
 
-
 function ImpactDamage.unblacklistBody(body)
     ImpactDamage.blacklist[body] = nil
     ImpactDamage.blacklistCounter[body] = nil
 end
 
-
--- Finds the bodies colliding with player
 function ImpactDamage.getBodiesCollidingPlayer()
...
-    local feetPos = GetPlayerTransform().pos
+    local feetPos = GetPlayerTransform(playerId).pos
     local headPos = GetPlayerEyeTransform().pos
     local trajectory = VecSub(headPos, feetPos)
     local direction = VecNormalize(trajectory)
     local maxDist = VecLength(trajectory)
     local radius = 0.5
     -- If driving, always ignore driven vehicle
...
+    if vehicle ~= 0 then
         ...
         ImpactDamage.blacklistBody(vehicleBody)
     end
     -- If grabbing an object, ignore it
-    local grabbedBody = GetPlayerGrabBody()
-    if grabbedBody > 0 then
+    local grabbedBody = GetPlayerGrabBody(playerId)
+    if grabbedBody ~= 0 then
         ImpactDamage.blacklistBody(grabbedBody)
     end
     -- If interacting with an object, ignore it
-    local interactBody = GetPlayerInteractBody()
-    if interactBody > 0 then
+    local interactBody = GetPlayerInteractBody(playerId)
+    if interactBody ~= 0 then
         ImpactDamage.blacklistBody(interactBody)
     end
     -- Do repeated trace with the player radius to find actually colliding objects
@@ -115,7 +107,6 @@
 
 end
 
--- Remove from blacklist objects not colliding any more
 function ImpactDamage.cleanupBlacklist(collidingBodies)
...
--- Computes the damage this body should do to the player
 function ImpactDamage.getImpactDamage(body, impactPos)
     
     local bodyPos = GetBodyTransform(body).pos
     local bodyVel = GetBodyVelocity(body)
     local playerPos = AabbGetPlayerCenter()
-    local playerVel = GetPlayerVelocity()
+    local playerVel = GetPlayerVelocity(playerId)
 
     local objectsHitSelf = regGetBool('impactDamage.objectsHitSelf')
     local selfHitObject = regGetBool('impactDamage.selfHitObject')
@@ -167,7 +157,7 @@
                 end
                 if scaleWithMass then
                     local bodyMass = GetBodyMass(body)
-                    if bodyMass > 0 then
+                    if bodyMass ~= 0 then
                         objectDamages = objectDamages * bodyMass / 100
                     end
                 end
@@ -176,7 +166,7 @@
                 end
...
-        if db and objectDamages > 0 then
+        if db and objectDamages ~= 0 then
             dbl(bodyPos, impactPos, 0, 0, 1, 1)
         else
             dbl(bodyPos, impactPos, 0, 1, 0, 1)
@@ -185,7 +175,7 @@
     if selfHitObject and VecLength(playerVel) > 0 then
         local bodyDir = VecSub(impactPos, playerPos)
         local playerEyePos = GetPlayerEyeTransform().pos
-        local playerFeetPos = GetPlayerTransform().pos
+        local playerFeetPos = GetPlayerTransform(playerId).pos
         -- Only consider low impacts if they are above max step height
         local impactPosInPlayer = (playerEyePos[2] >= impactPos[2]) and (impactPos[2] >= playerFeetPos[2] + 0.7)
         -- dbp("impactY="..impactPos[2]..", feetY="..playerFeetPos[2])
@@ -213,7 +203,7 @@
                 end
             end
         end
-        if db and playerDamages > 0 then
+        if db and playerDamages ~= 0 then
             dbl(playerPos, impactPos, 1, 0, 0, 1)
         else
             dbl(playerPos, impactPos, 1, 1, 1, 1)
@@ -228,10 +218,9 @@
 
 end
 
-
--- Apply damages to the player (assuming player has 100 HP)
 function ImpactDamage.damagePlayer(damages)
-    local health = GetPlayerHealth()
+    local health = GetPlayerHealth(playerId)
     local newHealth = math.max(health - (damages / 100), 0)
...
# Migration Report: scripts\poltergeist.lua
...
```diff
--- original/scripts\poltergeist.lua
+++ patched/scripts\poltergeist.lua
@@ -1,12 +1,8 @@
-Poltergeist = {}
-Poltergeist.poltergeists = {} -- Holds common data for all poltergeists.
-Poltergeist.poltergeistsList = {}
-
+#version 2
 function Poltergeist.init()
     Poltergeist.poltergeists.colony.timers.spawnDelay.delay = regGetFloat('poltergeist.spawnDelay')
     TimerResetTime(Poltergeist.poltergeists.colony.timers.spawnDelay)
 end
-
 
 function Poltergeist.run()
...
-
 function Poltergeist.getHeightProgress(poltergeist)
     local progress = poltergeist.targetTr.pos[2] - poltergeist.sourceTr.pos[2]
     return progress/poltergeist.maxHeight
 end
...
-
-Poltergeist.score = {}
-Poltergeist.score.time = 0
-Poltergeist.score.kills = 0
-Poltergeist.score.playerDead = false
-
-
 function Poltergeist.score.updateTime()
     if not Poltergeist.score.playerDead and regGetBool('poltergeist.spawnEnabled') then
         Poltergeist.score.time = Poltergeist.score.time + GetTimeStep()
     end
 end
 
-
 function Poltergeist.score.updateKills()
     if not Poltergeist.score.playerDead and regGetBool('poltergeist.spawnEnabled') then
         Poltergeist.score.kills = Poltergeist.score.kills + 1
     end
 end
 
-
 function Poltergeist.score.checkPlayerStatus()
     
     if regGetBool('poltergeist.spawnEnabled') then
-        local playerDead = (GetPlayerHealth() == 0)
+        local playerDead = (GetPlayerHealth(playerId) == 0)
         if playerDead and not Poltergeist.score.playerDead then
             Poltergeist.score.onPlayerDie()
         end
@@ -86,11 +67,9 @@
...
-
 function Poltergeist.score.onPlayerDie()
     dbp("player died")
 end
-
 
 function Poltergeist.score.onPlayerRevive()
     dbp("player respawned")
@@ -98,32 +77,18 @@
     local oldKills = regGetInt('poltergeist.bestKills')
     local newKills = Poltergeist.score.kills
     if newKills > oldKills then
-        regSetInt('poltergeist.bestKills', newKills)
+        regSetInt('poltergeist.bestKills', newKills, true)
     end
     local oldTime = regGetInt('poltergeist.bestTime')
     local newTime = Poltergeist.score.time
     if newTime > oldTime then
-        regSetInt('poltergeist.bestTime', newTime)
+        regSetInt('poltergeist.bestTime', newTime, true)
     end
...
-
-
-Poltergeist.poltergeists.colony = {}
-Poltergeist.poltergeists.colony.possessedBodies = {}
-Poltergeist.poltergeists.colony.timers = {
-    spawnDelay = {time = 0, delay = 0}
-}
-Poltergeist.poltergeists.colony.lastSpawnDelay = 10
-
---- Check timer and generate new poltegeist if needed
 function Poltergeist.poltergeists.colony.generate()
     -- No spawn if disabled
     if not regGetBool('poltergeist.spawnEnabled') then
@@ -173,7 +138,7 @@
                 bodiesInRange[#bodiesInRange+1] = body
             end
         end
-        if #bodiesInRange > 0 then
+        if #bodiesInRange ~= 0 then
             local index = rdm(1, #bodiesInRange)
             local bodyToPossess = bodiesInRange[index]
             -- Possess this body with a poltergeist
@@ -190,7 +155,6 @@
 
 end
 
---- Create a poltergeist object and store it in the list of poltergeists.
 function Poltergeist.poltergeists.colony.create(body)
...
---- Spawn a poltergeist into the world.
 function Poltergeist.poltergeists.colony.spawn(body)
     -- No visual effect or sound on spawn, nothing else to do here
     Poltergeist.poltergeists.colony.create(body)
 
 end
 
---- Mark a poltergeist for removal to safely remove it near the end of the frame.
 function Poltergeist.poltergeists.colony.markPoltergeistForRemoval(poltergeist)
     poltergeist.removePoltergeist = true
     dbp('Poltergeist marked for removal: ' .. poltergeist.id .. ' ' ..sfnTime())
 end
 
---- Safely delete all poltergeists that are marked for removal.
 function Poltergeist.poltergeists.colony.removeMarkedPoltergeists()
 
     TableRemove(Poltergeist.poltergeistsList, function(t, i, j)
@@ -258,7 +219,6 @@
     
 end
 
---- Find the parent body of a group of bodies (the largest of the group)
 function Poltergeist.poltergeists.colony.findParentBody(body)
     local parentBody = nil
     local maxSize = 0
@@ -275,14 +235,6 @@
     return parentBody
 end
...
-
-
-Poltergeist.poltergeists.behavior = {}
-
----Process the behavior of a single poltergeist.
 function Poltergeist.poltergeists.behavior.process(poltergeist)
 
     local state = poltergeist.state
@@ -308,8 +260,8 @@
             local hit, dist, normal, shape = QueryRaycast(center, VecNormalize(bodyToPlayer), VecLength(bodyToPlayer))
             if hit then
                 playerInSight = false
-                local vehicle = GetPlayerVehicle()
-                if vehicle > 0 then
+                local vehicle = GetPlayerVehicle(playerId)
+                if vehicle ~= 0 then
                     local hitVehicle = GetBodyVehicle(GetShapeBody(shape))
                     if vehicle == hitVehicle then
                         playerInSight = true
@@ -346,7 +298,7 @@
             end
             if db then
                 local playerCenter = AabbGetPlayerCenter()
-                local playerVel = GetPlayerVelocity()
+                local playerVel = GetPlayerVelocity(playerId)
                 local dist = VecDist(center, playerCenter)
                 local speed = VecLength(playerVel)
                 local scaledSpeed = dist * speed / 500
@@ -368,7 +320,7 @@
         r = 1
         if IsBodyValid(poltergeist.possessedBody) then
             local playerCenter = AabbGetPlayerCenter()
-            local playerVel = GetPlayerVelocity()
+            local playerVel = GetPlayerVelocity(playerId)
             local dist = VecDist(center, playerCenter)
...
---- Deterimine how high a poltergeist should float before attacking
 function Poltergeist.poltergeists.behavior.getMaxHeight(body)
 
     return regGetFloat('poltergeist.maxHeight')
...
-
-
-Poltergeist.poltergeists.effects = {}
 
 function Poltergeist.poltergeists.effects.process()
...
-
 ...
 function Poltergeist.poltergeists.effects.renderPoltergeistIdle(poltergeist)
...
-
 function Poltergeist.poltergeists.effects.renderPoltergeistExorcised(pos)
 
     ParticleReset()
@@ -521,4 +463,5 @@
     -- Play exorcised sound
     sounds.play.exorcised(pos, 0.5)
...
+#version 2
 function modReset()
     ...
     -- Impact Damages
-    regSetBool('impactDamage.debugMode'      , false)
-    regSetBool('impactDamage.objectsHitSelf' , true)
-    regSetBool('impactDamage.selfHitObject'  , false)
-    regSetBool('impactDamage.scaleWithSpeed' , false)
-    regSetBool('impactDamage.scaleWithMass'  , false)
-    regSetInt('impactDamage.fixedDamages'    , 40)
+    regSetBool('impactDamage.debugMode'      , false, true)
+    regSetBool('impactDamage.objectsHitSelf' , true, true)
+    regSetBool('impactDamage.selfHitObject'  , false, true)
+    regSetBool('impactDamage.scaleWithSpeed' , false, true)
+    regSetBool('impactDamage.scaleWithMass'  , false, true)
+    regSetInt('impactDamage.fixedDamages'    , 40, true)
     regInitKey('impactDamage.optionsKey'     , '')
 
     -- Poltergeist
-    regSetBool('poltergeist.spawnEnabled'       , true)
-    regSetFloat('poltergeist.spawnDelay'        , 10)
-    regSetFloat('poltergeist.spawnDelayIncrease', 1.3)
-    regSetFloat('poltergeist.spawnDelayDecrease', 0.9)
-    regSetBool('poltergeist.soundOn'            , true)
-    regSetBool('poltergeist.spawnFaster'        , true)
-    regSetBool('poltergeist.noobMode'           , false)
-    regSetFloat('poltergeist.minBodySize'       , 0)
-    regSetFloat('poltergeist.maxBodySize'       , 0)
-    regSetFloat('poltergeist.maxHeight'         , 10)
-    regSetFloat('poltergeist.ascendSpeed'       , 1)
-    regSetFloat('poltergeist.attackDelay'       , 5)
-    regSetFloat('poltergeist.releaseDelay'      , 3)
-    regSetInt('poltergeist.spawnRadiusMin'      , 10)
-    regSetInt('poltergeist.spawnRadiusMax'      , 50)
-    regSetFloat('poltergeist.maxShiverAngle'    , 5)
-    regSetFloat('poltergeist.throwSpeed'        , 100)
+    regSetBool('poltergeist.spawnEnabled'       , true, true)
+    regSetFloat('poltergeist.spawnDelay'        , 10, true)
+    regSetFloat('poltergeist.spawnDelayIncrease', 1.3, true)
+    regSetFloat('poltergeist.spawnDelayDecrease', 0.9, true)
+    regSetBool('poltergeist.soundOn'            , true, true)
+    regSetBool('poltergeist.spawnFaster'        , true, true)
+    regSetBool('poltergeist.noobMode'           , false, true)
+    regSetFloat('poltergeist.minBodySize'       , 0, true)
+    regSetFloat('poltergeist.maxBodySize'       , 0, true)
+    regSetFloat('poltergeist.maxHeight'         , 10, true)
+    regSetFloat('poltergeist.ascendSpeed'       , 1, true)
+    regSetFloat('poltergeist.attackDelay'       , 5, true)
+    regSetFloat('poltergeist.releaseDelay'      , 3, true)
+    regSetInt('poltergeist.spawnRadiusMin'      , 10, true)
+    regSetInt('poltergeist.spawnRadiusMax'      , 50, true)
+    regSetFloat('poltergeist.maxShiverAngle'    , 5, true)
+    regSetFloat('poltergeist.throwSpeed'        , 100, true)
     if not regGetInt('poltergeist.bestKills') then
-        regSetInt('poltergeist.bestKills'       , 0)
+        regSetInt('poltergeist.bestKills'       , 0, true)
     end
     if not regGetInt('poltergeist.bestTime') then
-        regSetInt('poltergeist.bestTime'        , 0)
+        regSetInt('poltergeist.bestTime'        , 0, true)
     end
-    regSetBool('poltergeist.showScore'          , true)
+    regSetBool('poltergeist.showScore'          , true, true)
...
-            do UiPush()
-
-                ui.checkBox.create('You take damages from objects hitting player', 'impactDamage.objectsHitSelf')
-                ui.padding.create(0, 60)
-                UiColor(0.6,0.6,0.6, 1)
-                UiAlign('left middle')
-                UiText('Note: Poltergeists cannot damage you if this is disabled')
-                ui.padding.create(0, 36)
-
-                ui.checkBox.create('You take damages from player hitting objects', 'impactDamage.selfHitObject')
-                ui.padding.create(0, 96)
-
-                ui.checkBox.create('Damages scale with object speed', 'impactDamage.scaleWithSpeed')
-                ui.padding.create(0, 96)
-
-                ui.checkBox.create('Damages scale with object mass', 'impactDamage.scaleWithMass')
-                ui.padding.create(0, 96)
-
-                ui.slider.create('Damages taken if not scaling', 'impactDamage.fixedDamages', 'HP', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 96)
-
-            UiPop() end
-
...
-
-                ui.checkBox.create('Debug Mode', 'impactDamage.debugMode')
-                ui.padding.create(0, 96)
-
-            UiPop() end
-
...
-
-                do UiPush()
-                    ui.checkBox.create('Poltergeists enabled', 'poltergeist.spawnEnabled')
-                UiPop() end
-                do UiPush()
-                    ui.padding.create(300, 0)
-                    ui.checkBox.create('Poltergeists appear faster if not exorcised', 'poltergeist.spawnFaster')
-                UiPop() end
-                ui.padding.create(0, 120)
-
-                ui.slider.create('Poltergeists appear every', 'poltergeist.spawnDelay', 'Seconds', 0, 10)
-                ui.padding.create(0, 80)
-
-                ui.slider.create('Poltergeists only posses objects larger than', 'poltergeist.minBodySize', 'Meters', 0, 10)
-                ui.padding.create(0, 80)
-
-                ui.slider.create('Poltergeists only posses objects smaller than (0 = infinite)', 'poltergeist.maxBodySize', 'Meters', 0, 10)
-                ui.padding.create(0, 80)
-
-                ui.slider.create('Delay before attack', 'poltergeist.attackDelay', 'Seconds', 0, 5)
-                ui.padding.create(0, 40)
-                UiColor(0.6,0.6,0.6, 1)
-                UiAlign('left middle')
-                UiText('Note: This is also the only time when you can exorcise them')
-                ui.padding.create(0, 40)
-
-            UiPop() end
-
...
-                ui.padding.create(800, 0)
-
-                ui.checkBox.create('Sound', 'poltergeist.soundOn')
-                ui.padding.create(0, 80)
-
-                ui.checkBox.create('Noob mode (highlight poltergeists about to attack)', 'poltergeist.noobMode')
-                ui.padding.create(0, 80)
-
-                UiColor(1,1,1, 1)
-                UiFont('bold.ttf', 32)
...
-                UiText('SCORES')
-                do UiPush()
-                    ui.padding.create(440, 0)
-                    local resetScoreButton = UiTextButton('Reset Scores')
-                    if resetScoreButton then
-                        regSetInt('poltergeist.bestKills', 0)
-                        regSetInt('poltergeist.bestTime' , 0)
-                        beep()
-                    end
...
-                do UiPush()
-
-                    UiText('Poltergeists exorcised:')
-                    ui.padding.create(250, 0)
-                    local score = Poltergeist and Poltergeist.score.kills or 0
-                    UiText(score)
-                    ui.padding.create(200, 0)
-                    UiText('Best:')
-                    ui.padding.create(100, 0)
-                    UiText(regGetInt('poltergeist.bestKills'))
-        
-                UiPop() end
-                ui.padding.create(0, 40)
-
-                do UiPush()
-
-                    UiText('Time survived:')
-                    ui.padding.create(250, 0)
-                    local score = Poltergeist and FormatTime(Poltergeist.score.time) or FormatTime(0)
-                    UiText(score)
-                    ui.padding.create(200, 0)
-                    UiText('Best:')
-                    ui.padding.create(100, 0)
-                    UiText(FormatTime(regGetInt('poltergeist.bestTime')))
-        
-                UiPop() end
-                ui.padding.create(0, 80)
-
-                ui.checkBox.create('Show scores ingame', 'poltergeist.showScore')
-                ui.padding.create(0, 80)
-
-               
-            UiPop() end
...
-            UiFont('regular.ttf', font_normal)
-            UiText('- Poltergeists take control of an object and launch it at you')
-            ui.padding.create(0, 50)
-            UiText('- If the object they control is damaged, they will be exorcised')
-            ui.padding.create(0, 50)
-            UiText('- Poltergeist spawn can be disabled so you can\nuse this mod only for impact damages on player')
-            ui.padding.create(0, 50)
-
-        UiPop() end
-
...
-            ui.padding.create(800, 0)
-
-            ui.keyBinding.create('Options key (to open this menu ingame)', 'impactDamage.optionsKey')
-            ui.padding.create(0, 80)
-           
-        UiPop() end
-        
...
-
-
---- Draw current scores at top left
 function uiDrawScores()
 
     if regGetBool('poltergeist.showScore') and regGetBool('poltergeist.spawnEnabled') then
@@ -390,8 +202,6 @@
  
 end
 
-
---- Draw game over screen with score and best score
 function uiDrawGameOverScreen()
 
     -- Only display this screen if player dead and poltergeists spawn is active
@@ -467,10 +277,6 @@
...
---- Manage when to open and close the options menu.
 function uiManageGameOptions()
     ...
     if InputPressed(regGetString('impactDamage.optionsKey')) then UI_GAME = not UI_GAME end
@@ -481,22 +287,6 @@
     end
