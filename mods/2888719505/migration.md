...
+    regSetInt('tool.range'     , 0, true)
+    regSetBool('tool.debugMode', false, true)
+    regSetBool('tool.soundOn', true, true)
+    regSetBool('tool.showAbilityText', true, true)
+    regSetBool('tool.showAbilityIcon', true, true)
 
     -- Shard
-    regSetFloat('shard.deploySpeed'  , 100.0)
-    regSetFloat('shard.density'      , 100.0)
-    regSetInt('shard.deploySpace'    , 3)
-    regSetInt('shard.minVDeployAngle', 0.0)
-    regSetInt('shard.maxVDeployAngle', 15.0)
-    regSetInt('shard.minHDeployAngle', 0.0)
-    regSetInt('shard.maxHDeployAngle', 90.0)
-    regSetFloat('shard.moveSpeed'    , 10.0)
-    regSetFloat('shard.retractDelay' , 1.0)
-    regSetFloat('shard.spellDuration', 1.0)
-    regSetInt('shard.waveAngle'      , 30)
-    regSetInt('shard.maxShards'      , 0)
-    regSetFloat('shard.spawnDelay'   , 1.0)
-    regSetBool('shard.burning'       , false)
-    regSetBool('shard.mimeticBurn'   , false)
-    regSetBool('shard.explosive'     , false)
-    regSetInt('shard.wallSize'       , 10)
-    regSetInt('shard.pierceDistance' , 0)
-    regSetInt('shard.startDistance'  , 0)
+    regSetFloat('shard.deploySpeed'  , 100.0, true)
+    regSetFloat('shard.density'      , 100.0, true)
+    regSetInt('shard.deploySpace'    , 3, true)
+    regSetInt('shard.minVDeployAngle', 0.0, true)
+    regSetInt('shard.maxVDeployAngle', 15.0, true)
+    regSetInt('shard.minHDeployAngle', 0.0, true)
+    regSetInt('shard.maxHDeployAngle', 90.0, true)
+    regSetFloat('shard.moveSpeed'    , 10.0, true)
+    regSetFloat('shard.retractDelay' , 1.0, true)
+    regSetFloat('shard.spellDuration', 1.0, true)
+    regSetInt('shard.waveAngle'      , 30, true)
+    regSetInt('shard.maxShards'      , 0, true)
+    regSetFloat('shard.spawnDelay'   , 1.0, true)
+    regSetBool('shard.burning'       , false, true)
+    regSetBool('shard.mimeticBurn'   , false, true)
+    regSetBool('shard.explosive'     , false, true)
+    regSetInt('shard.wallSize'       , 10, true)
+    regSetInt('shard.pierceDistance' , 0, true)
+    regSetInt('shard.startDistance'  , 0, true)
 
     -- Skin
-    regSetString('shard.preset'     , "stone")
-    regSetString('shard.material'   , "masonry")
-    regSetInt('shard.color.r'       , 145)
-    regSetInt('shard.color.g'       , 142)
-    regSetInt('shard.color.b'       , 133)
-    regSetInt('shard.color.a'       , 255)
-    regSetString('shard.texture'    , "scorched")
-    regSetInt('shard.textureOpacity', 100)
-    regSetInt('shard.reflective'    , 0)
-    regSetInt('shard.roughness'     , 0)
-    regSetInt('shard.metallic'      , 0)
-    regSetInt('shard.emissive'      , 0)
-    regSetBool('shard.transparent'  , false)
-    regSetBool('shard.mimetic'      , false)
+    regSetString('shard.preset'     , "stone", true)
+    regSetString('shard.material'   , "masonry", true)
+    regSetInt('shard.color.r'       , 145, true)
+    regSetInt('shard.color.g'       , 142, true)
+    regSetInt('shard.color.b'       , 133, true)
+    regSetInt('shard.color.a'       , 255, true)
+    regSetString('shard.texture'    , "scorched", true)
+    regSetInt('shard.textureOpacity', 100, true)
+    regSetInt('shard.reflective'    , 0, true)
+    regSetInt('shard.roughness'     , 0, true)
+    regSetInt('shard.metallic'      , 0, true)
+    regSetInt('shard.emissive'      , 0, true)
+    regSetBool('shard.transparent'  , false, true)
+    regSetBool('shard.mimetic'      , false, true)
 
     -- Key bindings
     regInitKey('key.summon', "lmb")
@@ -74,21 +75,22 @@
...
# Migration Report: scripts\shard.lua
...
```diff
--- original/scripts\shard.lua
+++ patched/scripts\shard.lua
@@ -1,14 +1,4 @@
-Shard = {}
-Shard.shardsList = {}
-Shard.tags = {
-    state = "shardState",
-    retractedPos = "shardRetracted",
-    deployedPos = "shardDeployed",
-    tr = "shardTr",
-    timer = "shardTimer"
-}
--- states = {"onHold", "deploying", "deployed", "retracting"}
-
+#version 2
 function Shard.run()
     -- Do registry read only once to hopefully save some access time
     Shard.shardsList = FindBodies(Shard.tags.state, true)
@@ -121,7 +111,6 @@
     end
 end
 
---- Burns a position and applies visual effects
 function Shard.burnPosition(pos)
 
     for i = 1, 10 do
@@ -149,7 +138,7 @@
     }
     if regGetBool("shard.mimeticBurn") then
         local shapes = GetBodyShapes(shard)
-        if #shapes > 0 then
+        if #shapes ~= 0 then
             local type, r, g, b, a = GetShapeMaterialAtPosition(shapes[1], pos)
             colors = {
                 r0 = r,
@@ -319,4 +308,5 @@
     Shard.SetShardState(shard, "retracting")
     Shard.SetShardTimer(shard, {time = 0})
     SetBodyDynamic(shard, true)
-end+end
+
...
-
-        input = {
-            didStartSpell = function() return tool.tool.active() and InputPressed(regGetString('key.summon')) end,
-            didStopSpell = function() return tool.tool.active() and InputReleased(regGetString('key.summon')) end,
-            didReset = function() return tool.tool.active() and InputPressed(regGetString('key.remove')) end,
-        },
-
-        setup = {
-            name = 'shards',
-            title = 'Shards',
-            voxPath = 'MOD/vox/hand.vox',
-        },
-
-        active = function()
...
-    tool.tool.playerPos = GetPlayerTransform().pos -- Use feet pos for spell start
+    local playerTr = GetPlayerCameraTransform(playerId)
+    tool.tool.playerPos = GetPlayerTransform(playerId).pos -- Use feet pos for spell start
     local direction = TransformToParentVec(playerTr, Vec(0, 0, -1))
     tool.tool.aimDir = direction
     local hit, dist, normal, shape = QueryRaycast(playerTr.pos, direction, maxDist)
@@ -128,7 +95,6 @@
...
-    
+
 function tool.anim.animate(dt)
     -- Init animation (done here to handle quicksave reload properly)
     if not tool.anim.animations then
@@ -250,7 +216,7 @@
     SetToolTransform(targetTr)
     local toolTr = GetBodyTransform(toolBody)
     local effectPos = TransformToParentPoint(toolTr, effectLocalPos)
-    local vel = GetPlayerVelocity()
+    local vel = GetPlayerVelocity(playerId)
     tool.anim.renderSpellParticles(effectPos, vel, thirdPerson)
 end
...
-        {
-            name = "Line",
-            icon = "MOD/img/line.png",
-            color = Vec(1, 1, 1)
-        },
-        {
-            name = "Paintbrush",
-            icon = "MOD/img/paintbrush.png",
-            color = Vec(128/255, 128/255, 0)
-        },
-        {
-            name = "Wave",
-            icon = "MOD/img/wave.png",
-            color = Vec(0, 0, 127/255)
-        },
-        {
-            name = "Trap",
-            icon = "MOD/img/trap.png",
-            color = Vec(85/255, 85/255, 85/255)
-        },
-        {
-            name = "Ring",
-            icon = "MOD/img/ring.png",
-            color = Vec(1, 0, 0)
-        },
-        {
-            name = "Sphere",
-            icon = "MOD/img/sphere.png",
-            color = Vec(1, 0, 1)
-        },
-        {
-            name = "Blade",
-            icon = "MOD/img/blade.png",
-            color = Vec(1, 1, 0)
-        },
-        {
-            name = "Wall",
-            icon = "MOD/img/wall.png",
-            color = Vec(0, 1, 0)
-        },
-        {
-            name = "Whip",
-            icon = "MOD/img/whip.png",
-            color = Vec(0, 0, 0)
-        },
-        {
-            name = "Rain",
-            icon = "MOD/img/rain.png",
-            color = Vec(127/255, 127/255, 1)
-        },
-        {
-            name = "Storm",
-            icon = "MOD/img/storm.png",
-            color = Vec(1, 1, 128/255)
-        },
-        {
-            name = "Tornado",
-            icon = "MOD/img/tornado.png",
-            color = Vec(170/255, 170/255, 170/255)
-        }
-    },
-    indexByName = {},
-    texturesIndex = {},
-    stopTimer = {time = 0, delay = regGetFloat('shard.spellDuration')},
-    shardsCount = 0,
-    casting = false,
-    startCastPos = nil,
-    curCastPos = nil,
...
-    castSpawnPoints = {},
-    targetShape = nil,
-    curSpawnTimer = {time = 0, delay = regGetFloat('shard.spawnDelay')},
-    scanRange = 10
-}
-
-for i=1,#tool.ability.list do
-    local item = tool.ability.list[i]
-    tool.ability.indexByName[item.name] = i
-end
-
 function tool.ability.getType()
     return tool.ability.list[tool.ability.selected].name
 end
@@ -445,7 +322,7 @@
     tool.ability.stopTimer.delay = regGetFloat('shard.spellDuration')
     dbw("stopTimer", tool.ability.stopTimer.time)
     -- Check if spell should stop after a delay
-    if tool.ability.stopTimer.time > 0 then
+    if tool.ability.stopTimer.time ~= 0 then
         TimerRunTime(tool.ability.stopTimer)
         if tool.ability.stopTimer.time <= 0 then
             tool.ability.stop()
@@ -498,7 +375,7 @@
                 local spawnPoint = VecAdd(tool.ability.lastSpawnPos, VecResize(tool.ability.castDirection, spaceBetweenShards))
                 tool.ability.addShardsToSpawn(spawnPoint, tool.ability.lastSpawnPos, shardsToSpawn)
                 --dbp("out castWaveAngle="..tool.ability.castWaveAngle)
-                if tool.ability.castWaveAngle > 0 then
+                if tool.ability.castWaveAngle ~= 0 then
                     -- Wave pattern is in progress, wait for next loop to continue
                     break
                 end
@@ -535,7 +412,7 @@
 
     elseif tool.ability.getType() == "Ring" or tool.ability.getType() == "Sphere" or tool.ability.getType() == "Blade" or tool.ability.getType() == "Wall" then
         --dbp("castSpawnPoints="..#tool.ability.castSpawnPoints)
-        if #tool.ability.castSpawnPoints > 0 then
+        if #tool.ability.castSpawnPoints ~= 0 then
             local doSpawn = false
             --dbp("curSpawnTimer.delay="..tool.ability.curSpawnTimer.delay..", curSpawnTimer.time="..tool.ability.curSpawnTimer.time)
             if tool.ability.curSpawnTimer.time == 0 then
@@ -544,7 +421,7 @@
                 doSpawn = true
             else
                 -- Run timer between spawns
-                if tool.ability.curSpawnTimer.time > 0 then
+                if tool.ability.curSpawnTimer.time ~= 0 then
                     TimerRunTime(tool.ability.curSpawnTimer)
                     if tool.ability.curSpawnTimer.time <= 0 then
                         tool.ability.curSpawnTimer.delay = tool.ability.curSpawnTimer.delay * 0.9
@@ -578,7 +455,7 @@
     for i=1,#shardsToSpawn do
         local shardSpawnInfo = shardsToSpawn[i]
         local material = regGetString('shard.material')
-        regSetBool('shard.transparent', material == "glass")
+        regSetBool('shard.transparent', material == "glass", true)
         local r = regGetInt('shard.color.r')/255
         local g = regGetInt('shard.color.g')/255
         local b = regGetInt('shard.color.b')/255
@@ -681,7 +558,7 @@
         end
     end
     local maxShards = regGetInt('shard.maxShards')
-    if maxShards > 0 then
+    if maxShards ~= 0 then
         if tool.ability.shardsCount >= maxShards then
             tool.ability.stop()
         end
@@ -808,7 +685,6 @@
     end
...
--- Get shard ring points
 function tool.ability.getRingInfos()
     local spaceBetweenShards = regGetInt('shard.deploySpace') * 0.1
     local ringSize = tool.ability.castDistance
@@ -827,7 +703,6 @@
     return visual.drawpolygon(centerTr, ringSize, 0, ringSamples, db and infoDebug or info, true)
 end
 
--- Get shard sphere points
 function tool.ability.getSphereInfos()
     local spaceBetweenShards = regGetInt('shard.deploySpace') * 0.1
     local sphereSize = tool.ability.castDistance
@@ -846,7 +721,6 @@
     return visual.drawsphere(centerTr, sphereSize, 0, sphereSamples, db and infoDebug or info)
 end
 
--- Get shard blade points
 function tool.ability.getBladeInfos()
     local spaceBetweenShards = regGetInt('shard.deploySpace') * 0.1
     local bladeSize = regGetInt('shard.wallSize')
@@ -872,7 +746,6 @@
     return points
 end
 
--- Get shard wall points
 function tool.ability.getWallInfos()
     local spaceBetweenShards = regGetInt('shard.deploySpace') * 0.1
     local wallSize = regGetInt('shard.wallSize')
@@ -900,7 +773,6 @@
     return points
 end
...
--- Finds the number of points to draw a sphere with the minimum distance between any two point being lower or equal than maxDist
 function tool.ability.findSamples(transform, radius, maxDist)
     local sampleStart = 2
...
-
-presets = {"stone", "steel", "gold", "ice", "wood", "bone", "sand", "lava", "blood", "shadow", "light"}
-
-options_tabs_render = {
-
-    Tool = function()
...
-            do UiPush()
-
-                ui.slider.create('Max cursor distance (0 = infinite)', 'tool.range', 'Meters', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                ui.checkBox.create('Show summoning pattern type', 'tool.showAbilityText')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Show summoning pattern icon', 'tool.showAbilityIcon')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-        do UiPush()
-
-            -- Balance min/max angles
-            local minVAngle = regGetInt('shard.minVDeployAngle')
-            local maxVAngle = regGetInt('shard.maxVDeployAngle')
-            local minHAngle = regGetInt('shard.minHDeployAngle')
-            local maxHAngle = regGetInt('shard.maxHDeployAngle')
-
-            UiColor(1,1,1, 1)
-            UiFont('bold.ttf', 32)
-
-            do UiPush()
-
-                ui.slider.create('Space between shards (higher = less lag)', 'shard.deploySpace', 'Voxels', 1, 20, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                ui.slider.create('Min vertical deploy angle', 'shard.minVDeployAngle', 'Deg', 0, 90, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                ui.slider.create('Max vertical deploy angle', 'shard.maxVDeployAngle', 'Deg', 0, 90, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                ui.slider.create('Min horizontal deploy angle', 'shard.minHDeployAngle', 'Deg', 0, 180, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                ui.slider.create('Max horizontal deploy angle', 'shard.maxHDeployAngle', 'Deg', 0, 180, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                ui.slider.create('Propagation speed (lower = less lag)', 'shard.moveSpeed', 'm/s', 0, 100)
-                ui.padding.create(0, 30)
-                ui.slider.create('Retractation delay (10 = never retract)', 'shard.retractDelay', 's', 0, 10)
-                ui.padding.create(0, 30)
-                ui.slider.create('Propagation duration after mouse release', 'shard.spellDuration', 's', 0, 10)
-                ui.padding.create(0, 30)
-                ui.slider.create('Wave angle around player / Trap radius', 'shard.waveAngle', 'Deg', 0, 360, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                ui.slider.create('Stop after summoning (0 = never)', 'shard.maxShards', 'Shards', 0, 100, nil, nil, nil, nil, true)
-
-            UiPop() end
-
-            do UiPush()
-
-                ui.padding.create(580, 0)
-
-                ui.slider.create('Material (physics and sound)', 'shard.material', '', nil, nil, nil, nil, nil, nil, nil, materials)
-                ui.padding.create(0, 50)
-
-                ui.colorPicker.create('Color', 'shard.color', nil, nil, nil, false)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Texture (visual and sound)', 'shard.texture', '', nil, nil, nil, nil, nil, nil, nil, textures)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Texture opacity', 'shard.textureOpacity', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Reflective', 'shard.reflective', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Roughness (reflection blur)', 'shard.roughness', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Metallic (color tint)', 'shard.metallic', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Emissive (glow)', 'shard.emissive', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-            UiPop() end
-
-            do UiPush()
-
-                ui.padding.create(1150, 0)
-
-                ui.slider.create('Preset', 'shard.preset', '', nil, nil, 200, nil, nil, nil, nil, presets)
-                do UiPush()
-                    ui.padding.create(320, 0)
-                    local whiteColor = {r=1, g=1, b=1, a=1}
-                    local applyButton = ui.button.create('Apply', whiteColor, whiteColor)
-                    if applyButton then
-                        applyPreset()
-                    end
-                UiPop() end
-                ui.padding.create(0, 50)
-                ui.checkBox.create('Mimetic shards (copy ground color + material)', 'shard.mimetic')
-                ui.padding.create(0, 60)
-                ui.slider.create('Extra start distance', 'shard.startDistance', 'Vox', 0, 300, 350, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                ui.slider.create('Extra pierce distance', 'shard.pierceDistance', 'Vox', 0, 300, 350, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                ui.slider.create('Ring/Sphere/Blade/Wall spawn delay', 'shard.spawnDelay', 's', 0, 5, 350)
-                ui.padding.create(0, 30)
-                ui.slider.create('Blade/Wall side size', 'shard.wallSize', 'Shards', 2, 100, 350, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                do UiPush()
-                    ui.padding.create(170, 0)
-                    ui.checkBox.create('Mimetic fire (use shard color)', 'shard.mimeticBurn')
-                UiPop() end
-                ui.checkBox.create('Burning shards', 'shard.burning')
-                ui.padding.create(0, 60)
-                ui.checkBox.create('Explosive shards', 'shard.explosive')
-                ui.padding.create(0, 60)
-
-            UiPop() end
-
-            -- Balance min/max angles
-            local newMinVAngle = regGetInt('shard.minVDeployAngle')
-            local newMaxVAngle = regGetInt('shard.maxVDeployAngle')
-            if newMinVAngle ~= minVAngle then
-                if newMaxVAngle < newMinVAngle then
-                    regSetFloat('shard.maxVDeployAngle', newMinVAngle)
-                end
-            elseif newMaxVAngle ~= maxVAngle then
-                if newMaxVAngle < newMinVAngle then
-                    regSetFloat('shard.minVDeployAngle', newMaxVAngle)
-                end
-            end
-            local newMinHAngle = regGetInt('shard.minHDeployAngle')
-            local newMaxHAngle = regGetInt('shard.maxHDeployAngle')
-            if newMinHAngle ~= minHAngle then
-                if newMaxHAngle < newMinHAngle then
-                    regSetFloat('shard.maxHDeployAngle', newMinHAngle)
-                end
-            elseif newMaxHAngle ~= maxHAngle then
-                if newMaxHAngle < newMinHAngle then
-                    regSetFloat('shard.minHDeployAngle', newMaxHAngle)
-                end
-            end
-
-        UiPop() end
...
-            ui.keyBinding.create('Summon shards (hold)', 'key.summon', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Remove all shards', 'key.remove', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Change summoning pattern (hold)', 'key.pattern', true)
-            ui.padding.create(0, 50)
...
-            UiFont('regular.ttf', font_normal)
-            ui.padding.create(0, 50)
-            UiText('- Shards are unbreakable')
-            ui.padding.create(0, 50)
-            UiText('- Retractation delay also includes the time left for the shards to settle down after deployment,')
-            ui.padding.create(0, 30)
-            UiText('  so reducing it too much may prevent shards from deploying fully before they retract')
-            ui.padding.create(0, 50)
-            UiText('- Spawning too many Shards will lag your game, and eventually crash it (use that clear button)')
-            ui.padding.create(0, 50)
-            UiText('- A single click can lead to interesting results with Sphere pattern')
-            ui.padding.create(0, 50)
-            UiText('- Be careful when increasing propagation speed, or also increase space between Shards')
-
-        UiPop() end
-        
-    end,
...
-
 function applyPreset()
     local preset = regGetString('shard.preset')
     if preset == "stone" then
-        regSetString('shard.material'   , "masonry")
-        regSetInt('shard.color.r'       , 145)
-        regSetInt('shard.color.g'       , 142)
-        regSetInt('shard.color.b'       , 133)
...
+        regSetInt('shard.metallic'      , 0, true)
+        regSetInt('shard.emissive'      , 0, true)
     elseif preset == "steel" then
-        regSetString('shard.material'   , "metal")
-        regSetInt('shard.color.r'       , 136)
-        regSetInt('shard.color.g'       , 139)
-        regSetInt('shard.color.b'       , 141)
-        regSetString('shard.texture'    , "none")
-        regSetInt('shard.textureOpacity', 0)
-        regSetInt('shard.reflective'    , 100)
-        regSetInt('shard.roughness'     , 50)
-        regSetInt('shard.metallic'      , 50)
-        regSetInt('shard.emissive'      , 0)
+        regSetString('shard.material'   , "metal", true)
+        regSetInt('shard.color.r'       , 136, true)
+        regSetInt('shard.color.g'       , 139, true)
+        regSetInt('shard.color.b'       , 141, true)
+        regSetString('shard.texture'    , "none", true)
+        regSetInt('shard.textureOpacity', 0, true)
+        regSetInt('shard.reflective'    , 100, true)
+        regSetInt('shard.roughness'     , 50, true)
+        regSetInt('shard.metallic'      , 50, true)
+        regSetInt('shard.emissive'      , 0, true)
     elseif preset == "gold" then
-        regSetString('shard.material'   , "metal")
-        regSetInt('shard.color.r'       , 212)
-        regSetInt('shard.color.g'       , 175)
-        regSetInt('shard.color.b'       , 55)
-        regSetString('shard.texture'    , "none")
-        regSetInt('shard.textureOpacity', 0)
-        regSetInt('shard.reflective'    , 90)
-        regSetInt('shard.roughness'     , 0)
-        regSetInt('shard.metallic'      , 100)
-        regSetInt('shard.emissive'      , 0)
+        regSetString('shard.material'   , "metal", true)
+        regSetInt('shard.color.r'       , 212, true)
+        regSetInt('shard.color.g'       , 175, true)
+        regSetInt('shard.color.b'       , 55, true)
+        regSetString('shard.texture'    , "none", true)
+        regSetInt('shard.textureOpacity', 0, true)
+        regSetInt('shard.reflective'    , 90, true)
+        regSetInt('shard.roughness'     , 0, true)
+        regSetInt('shard.metallic'      , 100, true)
+        regSetInt('shard.emissive'      , 0, true)
     elseif preset == "ice" then
-        regSetString('shard.material'   , "glass")
-        regSetInt('shard.color.r'       , 200)
-        regSetInt('shard.color.g'       , 200)
-        regSetInt('shard.color.b'       , 255)
-        regSetString('shard.texture'    , "none")
-        regSetInt('shard.textureOpacity', 0)
-        regSetInt('shard.reflective'    , 100)
-        regSetInt('shard.roughness'     , 100)
-        regSetInt('shard.metallic'      , 100)
-        regSetInt('shard.emissive'      , 0)
+        regSetString('shard.material'   , "glass", true)
+        regSetInt('shard.color.r'       , 200, true)
+        regSetInt('shard.color.g'       , 200, true)
+        regSetInt('shard.color.b'       , 255, true)
+        regSetString('shard.texture'    , "none", true)
+        regSetInt('shard.textureOpacity', 0, true)
+        regSetInt('shard.reflective'    , 100, true)
+        regSetInt('shard.roughness'     , 100, true)
+        regSetInt('shard.metallic'      , 100, true)
+        regSetInt('shard.emissive'      , 0, true)
     elseif preset == "wood" then
-        regSetString('shard.material'   , "wood")
-        regSetInt('shard.color.r'       , 186)
-        regSetInt('shard.color.g'       , 142)
-        regSetInt('shard.color.b'       , 74)
-        regSetString('shard.texture'    , "scorched")
-        regSetInt('shard.textureOpacity', 100)
-        regSetInt('shard.reflective'    , 0)
-        regSetInt('shard.roughness'     , 0)
-        regSetInt('shard.metallic'      , 0)
-        regSetInt('shard.emissive'      , 0)
+        regSetString('shard.material'   , "wood", true)
+        regSetInt('shard.color.r'       , 186, true)
+        regSetInt('shard.color.g'       , 142, true)
+        regSetInt('shard.color.b'       , 74, true)
+        regSetString('shard.texture'    , "scorched", true)
+        regSetInt('shard.textureOpacity', 100, true)
+        regSetInt('shard.reflective'    , 0, true)
+        regSetInt('shard.roughness'     , 0, true)
+        regSetInt('shard.metallic'      , 0, true)
+        regSetInt('shard.emissive'      , 0, true)
     elseif preset == "bone" then
-        regSetString('shard.material'   , "plastic")
-        regSetInt('shard.color.r'       , 250)
-        regSetInt('shard.color.g'       , 235)
-        regSetInt('shard.color.b'       , 215)
-        regSetString('shard.texture'    , "none")
-        regSetInt('shard.textureOpacity', 0)
-        regSetInt('shard.reflective'    , 0)
-        regSetInt('shard.roughness'     , 0)
-        regSetInt('shard.metallic'      , 0)
-        regSetInt('shard.emissive'      , 0)
+        regSetString('shard.material'   , "plastic", true)
+        regSetInt('shard.color.r'       , 250, true)
+        regSetInt('shard.color.g'       , 235, true)
+        regSetInt('shard.color.b'       , 215, true)
+        regSetString('shard.texture'    , "none", true)
+        regSetInt('shard.textureOpacity', 0, true)
+        regSetInt('shard.reflective'    , 0, true)
+        regSetInt('shard.roughness'     , 0, true)
+        regSetInt('shard.metallic'      , 0, true)
+        regSetInt('shard.emissive'      , 0, true)
     elseif preset == "sand" then
-        regSetString('shard.material'   , "dirt")
-        regSetInt('shard.color.r'       , 194)
-        regSetInt('shard.color.g'       , 178)
-        regSetInt('shard.color.b'       , 128)
-        regSetString('shard.texture'    , "sand")
-        regSetInt('shard.textureOpacity', 100)
-        regSetInt('shard.reflective'    , 0)
-        regSetInt('shard.roughness'     , 0)
-        regSetInt('shard.metallic'      , 0)
-        regSetInt('shard.emissive'      , 0)
+        regSetString('shard.material'   , "dirt", true)
+        regSetInt('shard.color.r'       , 194, true)
+        regSetInt('shard.color.g'       , 178, true)
+        regSetInt('shard.color.b'       , 128, true)
+        regSetString('shard.texture'    , "sand", true)
+        regSetInt('shard.textureOpacity', 100, true)
+        regSetInt('shard.reflective'    , 0, true)
+        regSetInt('shard.roughness'     , 0, true)
+        regSetInt('shard.metallic'      , 0, true)
+        regSetInt('shard.emissive'      , 0, true)
     elseif preset == "lava" then
-        regSetString('shard.material'   , "masonry")
-        regSetInt('shard.color.r'       , 51)
-        regSetInt('shard.color.g'       , 21)
-        regSetInt('shard.color.b'       , 1)
-        regSetString('shard.texture'    , "concrete")
-        regSetInt('shard.textureOpacity', 100)
-        regSetInt('shard.reflective'    , 100)
-        regSetInt('shard.roughness'     , 0)
-        regSetInt('shard.metallic'      , 0)
-        regSetInt('shard.emissive'      , 100)
+        regSetString('shard.material'   , "masonry", true)
+        regSetInt('shard.color.r'       , 51, true)
+        regSetInt('shard.color.g'       , 21, true)
+        regSetInt('shard.color.b'       , 1, true)
+        regSetString('shard.texture'    , "concrete", true)
+        regSetInt('shard.textureOpacity', 100, true)
+        regSetInt('shard.reflective'    , 100, true)
+        regSetInt('shard.roughness'     , 0, true)
+        regSetInt('shard.metallic'      , 0, true)
+        regSetInt('shard.emissive'      , 100, true)
     elseif preset == "blood" then
-        regSetString('shard.material'   , "foliage")
-        regSetInt('shard.color.r'       , 34)
-        regSetInt('shard.color.g'       , 0)
-        regSetInt('shard.color.b'       , 0)
-        regSetString('shard.texture'    , "scorched")
-        regSetInt('shard.textureOpacity', 100)
-        regSetInt('shard.reflective'    , 100)
-        regSetInt('shard.roughness'     , 90)
-        regSetInt('shard.metallic'      , 90)
-        regSetInt('shard.emissive'      , 100)
+        regSetString('shard.material'   , "foliage", true)
+        regSetInt('shard.color.r'       , 34, true)
+        regSetInt('shard.color.g'       , 0, true)
+        regSetInt('shard.color.b'       , 0, true)
+        regSetString('shard.texture'    , "scorched", true)
+        regSetInt('shard.textureOpacity', 100, true)
+        regSetInt('shard.reflective'    , 100, true)
+        regSetInt('shard.roughness'     , 90, true)
+        regSetInt('shard.metallic'      , 90, true)
+        regSetInt('shard.emissive'      , 100, true)
     elseif preset == "shadow" then
-        regSetString('shard.material'   , "none")
-        regSetInt('shard.color.r'       , 0)
-        regSetInt('shard.color.g'       , 0)
-        regSetInt('shard.color.b'       , 0)
-        regSetString('shard.texture'    , "none")
-        regSetInt('shard.textureOpacity', 0)
...
-        regSetInt('shard.metallic'      , 100)
-        regSetInt('shard.emissive'      , 100)
+        regSetString('shard.material'   , "none", true)
+        regSetInt('shard.color.r'       , 0, true)
+        regSetInt('shard.color.g'       , 0, true)
+        regSetInt('shard.color.b'       , 0, true)
+        regSetString('shard.texture'    , "none", true)
+        regSetInt('shard.textureOpacity', 0, true)
...
-        regSetString('shard.material'   , "glass")
-        regSetInt('shard.color.r'       , 255)
-        regSetInt('shard.color.g'       , 255)
-        regSetInt('shard.color.b'       , 255)
-        regSetString('shard.texture'    , "none")
-        regSetInt('shard.textureOpacity', 0)
-        regSetInt('shard.reflective'    , 0)
...
---- Draw keys hint
+        regSetString('shard.material'   , "glass", true)
+        regSetInt('shard.color.r'       , 255, true)
+        regSetInt('shard.color.g'       , 255, true)
+        regSetInt('shard.color.b'       , 255, true)
+        regSetString('shard.texture'    , "none", true)
+        regSetInt('shard.textureOpacity', 0, true)
+        regSetInt('shard.reflective'    , 0, true)
...
@@ -603,7 +367,7 @@
             ...
             UiPush()
-                if tool.ability.highlightTimer > 0 then
+                if tool.ability.highlightTimer ~= 0 then
                     local a = tool.ability.highlightTimer
                     UiColor(1, 1, 1, a)
