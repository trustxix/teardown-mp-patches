...
-	RegisterTool("rmwmp443", "MP443", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwmp443.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwmp443", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
@@ -230,7 +142,7 @@
 			if GetString("game.player.tool") == "rmwmp443" then --- When Zero then its empty
 			
 			local optic = opticssight[10]
-                if charginghandletime > 0 then ---Charginghandle
+                if charginghandletime ~= 0 then ---Charginghandle
 					SetShapeLocalTransform(optic, Transform(Vec(-0.025, 0.0905, -0.2), QuatEuler(-90, 0, 0)))
 				else
 					SetShapeLocalTransform(optic, Transform(Vec(-0.025, 0.0905, -0.3), QuatEuler(-90, 0, 0)))
@@ -453,7 +365,7 @@
...
+
+tion draw()
 if GetString("game.player.tool") == "rmwmp443" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwmp443", "MP443", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwmp443.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
