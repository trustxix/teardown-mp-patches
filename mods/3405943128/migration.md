...
+++ patched/main.lua
@@ -1,101 +1,13 @@
---rmwstoeger custom tool example
-
...
-	RegisterTool("rmwstoeger", "Stoeger Double Defense", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwstoeger.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwstoeger", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
+tion tick(dt)
 	...
 	--Check if rmwstoeger is selected
@@ -184,7 +94,7 @@
 		local d = GetToolBody()
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwstoeger" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwstoeger", "Stoeger Double Defense", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwstoeger.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
