...
-	RegisterTool("rmwak5", "Bofors AK5", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwak5.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwak5", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
@@ -246,7 +155,7 @@
 			if GetString("game.player.tool") == "rmwak5" then --- When Zero then its empty
 			
 			local optic = opticssight[10]
-                if charginghandletime > 0 then ---Charginghandle
+                if charginghandletime ~= 0 then ---Charginghandle
 					SetShapeLocalTransform(optic, Transform(Vec(0, 0.1, -0.19), QuatEuler(-90, 0, 0)))
 				else
 					SetShapeLocalTransform(optic, Transform(Vec(0, 0.1, -0.29), QuatEuler(-90, 0, 0)))
@@ -474,7 +383,7 @@
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwak5" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwak5", "Bofors AK5", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwak5.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
...
-
-	--Check if rmwak5 is selected
-	if GetString("game.player.tool") == "rmwak5" then
...
+                        RemoveTag(optic, "invisible")
+
+    					SetShapeLocalTransform(optic, Transform(Vec(-0.05, 0.14, -0.1), QuatEuler(-90, 0, 0)))
+    				else
+    				SetTag(optic, "invisible")
+
+                    end
...
+                        RemoveTag(optic, "invisible")
+
+    					SetShapeLocalTransform(optic, Transform(Vec(-0.025, 0.145, -0.15), QuatEuler(-90, 0, 0)))
+    				else
+    				SetTag(optic, "invisible")
+
+                    end
...
-		end
-		if aiming then
-		toolAnimator.offsetTransform = (Transform(Vec(-0.1085, 0.075, recoil/2), QuatEuler(0, 0, 0)))
-		tickToolAnimator(toolAnimator, dt)
-		if optics == 2 then
-		toolAnimator.offsetTransform = (Transform(Vec(-0.108, 0.027, recoil/2), QuatEuler(0, 0, 0)))
-		tickToolAnimator(toolAnimator, dt)
-		end
-		if optics == 3 then
-		toolAnimator.offsetTransform = (Transform(Vec(-0.108, 0.04, recoil/2), QuatEuler(0, 0, 0)))
-		tickToolAnimator(toolAnimator, dt)
-		end
...
-    end
-	
-if GetString("game.player.tool") == "rmwak5" and not fireswitch and not reloading and not attachmentmenu then
-		UiPush()
...
-			UiText(magazine.." (5.56×45mm NATO) Auto")
-				
-				
-			end
-
-if GetString("game.player.tool") == "rmwak5" and fireswitch and not reloading and not attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(1, 1, 1)
-			UiFont("bold.ttf", 32)
-			UiTextOutline(0,0,0,1,0.1)
-			UiText(magazine.." (5.56×45mm NATO) Semi")
-			
-			end
-			
-if GetString("game.player.tool") == "rmwak5" and reloading and not attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(1, 1, 1)
...
-				
-			end
-if GetString("game.player.tool") == "rmwak5" and not reloading and attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(0.8, 0.8, 0.8)
