...
-	RegisterTool("rmwfnscar", "FN SCAR", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwfnscar.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwfnscar", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
+tion tick(dt)
 	...
 	--Check if rmwfnscar is selected
@@ -186,7 +94,7 @@
 		local d = GetToolBody()
...
@@ -253,7 +161,7 @@
 			if GetString("game.player.tool") == "rmwfnscar" then --- When Zero then its empty
 			
 			local optic = opticssight[10]
-                if charginghandletime > 0 then ---Charginghandle
+                if charginghandletime ~= 0 then ---Charginghandle
 					SetShapeLocalTransform(optic, Transform(Vec(-0.06, 0.08, -0.13), QuatEuler(-90, 0, 0)))
 				else
 					SetShapeLocalTransform(optic, Transform(Vec(-0.06, 0.08, -0.23), QuatEuler(-90, 0, 0)))
@@ -489,7 +397,7 @@
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwfnscar" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwfnscar", "FN SCAR", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwfnscar.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
