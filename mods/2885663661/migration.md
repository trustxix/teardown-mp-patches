...
-            emissive = 0,
-        },
-        -- texture is impossible to obtain in the current version of the game :/
-        texture = nil,
-    },
-}
-Debug.values = {
-    player = DeepCopy(Debug.default.player),
-    vehicle = DeepCopy(Debug.default.vehicle),
-    body = DeepCopy(Debug.default.body),
-    shape = DeepCopy(Debug.default.shape),
-    voxel = DeepCopy(Debug.default.voxel),
-}
-
 function Debug.getTagsStr(handle)
...
-        if #value > 0 then
+        if #value ~= 0 then
             listStr = listStr..tag.."="..value
         else
             listStr = listStr..tag
@@ -196,10 +63,10 @@
 
 function Debug.updateData(shape, hitPos, hitNormal)
     -- Player
-    Debug.values.player.feetTr = TransformCopy(GetPlayerTransform())
+    Debug.values.player.feetTr = TransformCopy(GetPlayerTransform(playerId))
     Debug.values.player.centerTr = Transform(AabbGetPlayerCenter(), QuatCopy(Debug.values.player.feetTr.rot))
     Debug.values.player.cameraTr = TransformCopy(GetPlayerEyeTransform())
-    Debug.values.player.velocity = VecCopy(GetPlayerVelocity())
+    Debug.values.player.velocity = VecCopy(GetPlayerVelocity(playerId))
     Debug.values.player.aabb.min, Debug.values.player.aabb.max = AabbGetPlayerBounds()
     Debug.values.player.obb.tr, Debug.values.player.obb.min, Debug.values.player.obb.max = GetPlayerObb()
     Debug.values.player.hitPos = VecCopy(hitPos)
@@ -256,7 +123,7 @@
 
         -- Vehicle
         Debug.values.vehicle.id = GetBodyVehicle(Debug.values.body.id)
-        if Debug.values.vehicle.id > 0 then
+        if Debug.values.vehicle.id ~= 0 then
             Debug.values.vehicle.tr = TransformCopy(GetVehicleTransform(Debug.values.vehicle.id))
             Debug.values.vehicle.aabb.min, Debug.values.vehicle.aabb.max = GetVehicleBounds(Debug.values.vehicle.id)
             Debug.values.vehicle.center = AabbGetCenterPos(Debug.values.vehicle.aabb.min, Debug.values.vehicle.aabb.max)
@@ -549,3 +416,4 @@
         end
     end
 end
...
-    regSetBool('tool.debugMode', true)
+    regSetInt('tool.range'     , 0, true)
+    regSetBool('tool.debugMode', true, true)
 
     -- Player
-    regSetBool('debug.player.text'    , false)
-    regSetBool('debug.player.aabb'    , false)
-    regSetBool('debug.player.obb'     , false)
-    regSetBool('debug.player.aabs'    , false)
-    regSetBool('debug.player.obs'     , false)
-    regSetBool('debug.player.origin'  , false)
-    regSetBool('debug.player.center'  , false)
-    regSetBool('debug.player.head'    , false)
-    regSetBool('debug.player.velocity', false)
-    regSetBool('debug.player.orient'  , false)
-    regSetBool('debug.player.camFront', false)
+    regSetBool('debug.player.text'    , false, true)
+    regSetBool('debug.player.aabb'    , false, true)
+    regSetBool('debug.player.obb'     , false, true)
+    regSetBool('debug.player.aabs'    , false, true)
+    regSetBool('debug.player.obs'     , false, true)
+    regSetBool('debug.player.origin'  , false, true)
+    regSetBool('debug.player.center'  , false, true)
+    regSetBool('debug.player.head'    , false, true)
+    regSetBool('debug.player.velocity', false, true)
+    regSetBool('debug.player.orient'  , false, true)
+    regSetBool('debug.player.camFront', false, true)
 
     -- Body
-    regSetBool('debug.body.text'        , true)
-    regSetBool('debug.body.outline'     , false)
-    regSetBool('debug.body.aabb'        , false)
-    regSetBool('debug.body.obb'         , false)
-    regSetBool('debug.body.aabs'        , false)
-    regSetBool('debug.body.obs'         , false)
-    regSetBool('debug.body.origin'      , false)
-    regSetBool('debug.body.center'      , false)
-    regSetBool('debug.body.centerOfMass', true)
-    regSetBool('debug.body.velocity'    , true)
-    regSetBool('debug.body.orient'      , false)
+    regSetBool('debug.body.text'        , true, true)
+    regSetBool('debug.body.outline'     , false, true)
+    regSetBool('debug.body.aabb'        , false, true)
+    regSetBool('debug.body.obb'         , false, true)
+    regSetBool('debug.body.aabs'        , false, true)
+    regSetBool('debug.body.obs'         , false, true)
+    regSetBool('debug.body.origin'      , false, true)
+    regSetBool('debug.body.center'      , false, true)
+    regSetBool('debug.body.centerOfMass', true, true)
+    regSetBool('debug.body.velocity'    , true, true)
+    regSetBool('debug.body.orient'      , false, true)
 
     -- Shape
-    regSetBool('debug.shape.text'    , true)
-    regSetBool('debug.shape.outline' , true)
-    regSetBool('debug.shape.aabb'    , false)
-    regSetBool('debug.shape.obb'     , true)
-    regSetBool('debug.shape.aabs'    , false)
-    regSetBool('debug.shape.obs'     , false)
-    regSetBool('debug.body.origin'   , false)
-    regSetBool('debug.shape.center'  , true)
-    regSetBool('debug.shape.orient'  , true)
+    regSetBool('debug.shape.text'    , true, true)
+    regSetBool('debug.shape.outline' , true, true)
+    regSetBool('debug.shape.aabb'    , false, true)
+    regSetBool('debug.shape.obb'     , true, true)
+    regSetBool('debug.shape.aabs'    , false, true)
+    regSetBool('debug.shape.obs'     , false, true)
+    regSetBool('debug.body.origin'   , false, true)
+    regSetBool('debug.shape.center'  , true, true)
+    regSetBool('debug.shape.orient'  , true, true)
 
     -- Voxel
-    regSetBool('debug.voxel.text'    , true)
-    regSetBool('debug.voxel.aabb'    , false)
-    regSetBool('debug.voxel.obb'     , true)
-    regSetBool('debug.voxel.aabs'    , false)
-    regSetBool('debug.voxel.obs'     , false)
-    regSetBool('debug.voxel.origin'  , false)
-    regSetBool('debug.voxel.center'  , false)
+    regSetBool('debug.voxel.text'    , true, true)
+    regSetBool('debug.voxel.aabb'    , false, true)
+    regSetBool('debug.voxel.obb'     , true, true)
+    regSetBool('debug.voxel.aabs'    , false, true)
+    regSetBool('debug.voxel.obs'     , false, true)
+    regSetBool('debug.voxel.origin'  , false, true)
+    regSetBool('debug.voxel.center'  , false, true)
 
     -- Vehicle
-    regSetBool('debug.vehicle.text'     , false)
-    regSetBool('debug.vehicle.outline'  , false)
-    regSetBool('debug.vehicle.aabb'     , false)
-    regSetBool('debug.vehicle.obb'      , false)
-    regSetBool('debug.vehicle.aabs'     , false)
-    regSetBool('debug.vehicle.obs'      , false)
-    regSetBool('debug.vehicle.origin'   , false)
-    regSetBool('debug.vehicle.center'   , false)
-    regSetBool('debug.vehicle.orient'   , false)
-    regSetBool('debug.vehicle.driverPos', false)
+    regSetBool('debug.vehicle.text'     , false, true)
+    regSetBool('debug.vehicle.outline'  , false, true)
+    regSetBool('debug.vehicle.aabb'     , false, true)
+    regSetBool('debug.vehicle.obb'      , false, true)
+    regSetBool('debug.vehicle.aabs'     , false, true)
+    regSetBool('debug.vehicle.obs'      , false, true)
+    regSetBool('debug.vehicle.origin'   , false, true)
+    regSetBool('debug.vehicle.center'   , false, true)
+    regSetBool('debug.vehicle.orient'   , false, true)
+    regSetBool('debug.vehicle.driverPos', false, true)
 
     -- Key bindings
-    regSetBool('debug.keysHint'  , true)
+    regSetBool('debug.keysHint'  , true, true)
     regInitKey('debug.optionsKey', 'p')
     regInitKey('debug.enableKey' , 'n')
     regInitKey('debug.freezeKey' , 'b')
@@ -93,21 +94,22 @@
...
-
-        input = {
-            didEnable = function() return InputPressed(regGetString('debug.enableKey')) end,
-            didFreeze = function() return tool.tool.active() and InputPressed(regGetString('debug.freezeKey')) end,
-            didAttach = function() return tool.tool.active() and InputPressed(regGetString('debug.attachKey')) end,
-        },
-
-        active = function()
-            return regGetBool('tool.debugMode') and (GetPlayerVehicle() == 0)
-        end,
-
-    }
-}
...
+#version 2
 function tool.run()
     ...
     -- Get shape at cursor at all times
@@ -25,7 +6,7 @@
     if maxDist == 0 then
         maxDist = 1000000
     end
...
@@ -40,7 +21,7 @@
     ...
     if tool.tool.input.didEnable() then -- Enable debug display.
         if not UI_GAME then
-            regSetBool('tool.debugMode', not regGetBool('tool.debugMode'))
+            regSetBool('tool.debugMode', not regGetBool('tool.debugMode'), true)
         end
     end
...
-                ui.checkBox.create('Text', 'debug.player.text', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('AABB', 'debug.player.aabb', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('OBB', 'debug.player.obb', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('AABS', 'debug.player.aabs', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('OBS', 'debug.player.obs', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Head', 'debug.player.head', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Center', 'debug.player.center', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Origin', 'debug.player.origin', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Velocity', 'debug.player.velocity', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Orientation', 'debug.player.orient', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Camera front vec', 'debug.player.camFront', true)
-            UiPop() end
-
-            -- Body
...
-                UiText('BODY')
-                ui.padding.create(0, 30)
-                ui.checkBox.create('Text', 'debug.body.text', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Outline', 'debug.body.outline', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('AABB', 'debug.body.aabb', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('OBB', 'debug.body.obb', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('AABS', 'debug.body.aabs', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('OBS', 'debug.body.obs', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Origin', 'debug.body.origin', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Center', 'debug.body.center', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Center of mass', 'debug.body.centerOfMass', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Velocity', 'debug.body.velocity', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Orientation', 'debug.body.orient', true)
-            UiPop() end              
-
-            -- Shape
-            ui.padding.create(350, 0)
...
-                UiText('SHAPE')
-                ui.padding.create(0, 30)
-                ui.checkBox.create('Text', 'debug.shape.text', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Outline', 'debug.shape.outline', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('AABB', 'debug.shape.aabb', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('OBB', 'debug.shape.obb', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('AABS', 'debug.shape.aabs', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('OBS', 'debug.shape.obs', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Origin', 'debug.shape.origin', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Center', 'debug.shape.center', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Orientation', 'debug.shape.orient', true)
-            UiPop() end
-
-            -- Voxel
-            ui.padding.create(330, 0)
-            do UiPush()
-                UiText('VOXEL')
-                ui.padding.create(0, 30)
-                ui.checkBox.create('Text', 'debug.voxel.text', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('AABB', 'debug.voxel.aabb', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('OBB', 'debug.voxel.obb', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('AABS', 'debug.voxel.aabs', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('OBS', 'debug.voxel.obs', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Origin', 'debug.voxel.origin', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Center', 'debug.voxel.center', true)
-                ui.padding.create(0, 65)
-            UiPop() end
-
-            -- Vehicle
...
-                UiText('VEHICLE')
-                ui.padding.create(0, 30)
-                ui.checkBox.create('Text', 'debug.vehicle.text', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Outline', 'debug.vehicle.outline', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('AABB', 'debug.vehicle.aabb', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('OBB', 'debug.vehicle.obb', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('AABS', 'debug.vehicle.aabs', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('OBS', 'debug.vehicle.obs', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Origin', 'debug.vehicle.origin', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Center', 'debug.vehicle.center', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Orientation', 'debug.vehicle.orient', true)
-                ui.padding.create(0, 45)
-                ui.checkBox.create('Driver pos', 'debug.vehicle.driverPos', true)
-                ui.padding.create(0, 45)
-            UiPop() end
-
-        UiPop() end
...
-            ui.checkBox.create('Show keys hint', 'debug.keysHint')
-            ui.padding.create(0, 80)
-            ui.keyBinding.create('Enable key (to enable/disable scanner)', 'debug.enableKey')
-            ui.padding.create(0, 80)
-            ui.keyBinding.create('Freeze key (to freeze/unfreeze debug data updates)', 'debug.freezeKey')
-            ui.padding.create(0, 80)
-            ui.keyBinding.create('Attach key (to attach/detach scanner to object)', 'debug.attachKey')
-            ui.padding.create(0, 80)
-            ui.keyBinding.create('Options key (to open this menu ingame)', 'debug.optionsKey')
-
-        UiPop() end
-
-        do UiPush()
...
-            ui.padding.create(0, 50)
-            UiFont('regular.ttf', font_normal)
-            UiText('- AABB: Axis Aligned Bounding Box (minimal box containing the object aligned with world axis)')
-            ui.padding.create(0, 50)
-            UiText('- OBB: Oriented Bounding Box (minimal box containing the object aligned with object axis)')
-            ui.padding.create(0, 50)
-            UiText('- AABS: Axis Aligned Bounding Sphere (minimal sphere containing AABB)')
-            ui.padding.create(0, 50)
-            UiText('- OBS: Oriented Bounding Sphere (minimal sphere containing OBB)')
-            ui.padding.create(0, 60)
-            UiFont('bold.ttf', 32)
-            UiText('NOTES')
-            ui.padding.create(0, 50)
-            UiFont('regular.ttf', font_normal)
-            UiText('- Vectors color code: Up = Green, Front = Blue, Right = Red, Camera = Black, Velocity = White')
-            ui.padding.create(0, 50)
-            UiText('- If the scanner is attached and the object is deleted, the scanner will detach automatically')
-            ui.padding.create(0, 50)
-            UiText('- The shape of the player bounds in this mod is totally arbitrary:')
-            ui.padding.create(0, 25)
-            UiText('  the actual true shape is more like a "pill" of radius 1 voxel and the same height as my OBB')
-            ui.padding.create(0, 50)
-            UiText('- Be careful when reading voxel data after attaching the scanner to a shape,')
-            ui.padding.create(0, 25)
-            UiText('  as the selected voxel will change if the original one is destroyed')
-
-        UiPop() end
-        
-    end,
...
---- Draw keys hint
 function uiDrawKeyHint()
     ...
     if not regGetBool('tool.debugMode') then
@@ -417,7 +205,6 @@
 
 end
 
--- Draw debug infos selected
 function uiDrawDebugInfo()
...
---- Manage when to open and close the options menu.
 function uiManageGameOptions()
     ...
     if InputPressed(regGetString("debug.optionsKey")) then UI_GAME = not UI_GAME end
@@ -648,21 +434,6 @@
     end
...
-        local playerFeetTr = GetPlayerTransform()
+        local playerFeetTr = GetPlayerTransform(playerId)
         local playerWidth = 1 -- Player width is arbitrary 1
