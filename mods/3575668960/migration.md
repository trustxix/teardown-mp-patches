...
-RECOIL_BASE_HIP   = 0.75   -- per-shot pitch in hip
-RECOIL_BASE_ADS   = 0.65   -- per-shot pitch in ADS
...
-VISUAL_SCALE_HIP  = 1   -- viewmodel tilt multiplier (hip)
-VISUAL_SCALE_ADS  = 0.9   -- viewmodel tilt multiplier (ADS)
-CAMERA_SCALE_HIP  = 0.8   -- camera tilt multiplier (hip)
-CAMERA_SCALE_ADS  = 0.7   -- camera tilt multiplier (ADS)
-
...
+#version 2
 function SpentCasing()
 	...
 	local casingpos = TransformToParentPoint(gt, Vec(0, -0.3, -0.4))
 	local fwdpos = TransformToParentPoint(gt, Vec(0, 0, 0))
...
-	RegisterTool("rmwp90", "FN P90", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwp90.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwp90", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
+
+tion draw()
 if GetString("game.player.tool") == "rmwp90" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwp90", "FN P90", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwp90.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
