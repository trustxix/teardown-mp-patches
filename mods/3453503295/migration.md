...
-	
-	reticlesprite = LoadSprite("MOD/sprites/reticlesprite3.png")
...
-	RegisterTool("rmwfamas", "FAMAS F1", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwfamas.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwfamas", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
-                if handletimer > 0 then ---Carry Handle
+                if handletimer ~= 0 then ---Carry Handle
                     RemoveTag(optic, "invisible")
 					
 					SetShapeLocalTransform(optic, Transform(Vec(-0.008, 0.16, -0.2), QuatEuler(-90, 0, 0)))
@@ -470,7 +377,7 @@
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwfamas" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwfamas", "FAMAS F1", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwfamas.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
