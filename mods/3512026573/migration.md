...
-	RegisterTool("rmwm16a2", "M16A2", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwm16a2.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwm16a2", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwm16a2" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwm16a2", "M16A2", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwm16a2.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
...
-function init()
-	--Register tool and enable it
-	RegisterTool("rmwm16", "M16", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwm16.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	shootHaptic = LoadHaptic("MOD/haptic/gun_fire.xml")
-	local toolHaptic = LoadHaptic("MOD/haptic/background.xml")
-	SetToolHaptic("rmwm16", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
-	
-
-	--Check if rmwm16 is selected
-	if GetString("game.player.tool") == "rmwm16" then
...
-		end
-		
-		if InputReleased("usetool") then burst = 0 
-		end
-		if InputReleased("usetool") then semi = 0 
-		end
-		
-		if burst > 3 then burst = 3 
-		end
...
-			end
-			
-			if GetString("game.player.tool") == "rmwm16" then --- When Zero then its empty
-			
...
-
-		--Check if firing
-		if GetBool("game.player.canusetool") and InputDown("usetool") and not attachmentmenu and not reloading and GetInt("game.tool.gun.ammo") > 0 and not low and magazine > 0 and not fireswitch and burst < 3 then
-			angVel = math.min(1000, angVel + dt*9000)	
-			if angVel == 1000 then
-				if coolDown < 0 then
-                    local _, p, _, d = GetPlayerAimInfo(mt.pos)
...
-function draw()
-
-if GetString("game.player.tool") == "rmwm16" and not fireswitch and not reloading and not attachmentmenu then
-		UiPush()
...
-			UiText(magazine.." (5.56mm Nato) Burst")
-				
-				
-			end
-
-if GetString("game.player.tool") == "rmwm16" and fireswitch and not reloading and not attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(1, 1, 1)
...
-			end
-			
-if GetString("game.player.tool") == "rmwm16" and reloading and not attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(1, 1, 1)
...
-				
-			end
-if GetString("game.player.tool") == "rmwm16" and not reloading and attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(0.8, 0.8, 0.8)
...
+function server.init()
+    RegisterTool("rmwm16", "M16", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwm16.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
