...
## Reference Docs — When to Read Each

| Doc | WHEN to read it |
|-----|-----------------|
| `docs/RESEARCH.md` | **Before upgrading any mod's aim, damage, or sync system.** Contains 34 findings on official API patterns (GetPlayerAimInfo, Shoot, QueryShot, SetToolAmmoPickupAmount, etc.) with exact function signatures and code examples from official mods. This is the source of truth for HOW to implement Shoot/AimInfo/AmmoPickup upgrades. |
| `docs/V2_SYNC_PATTERNS.md` | **Before writing any mod with custom entities (drones, NPCs, projectiles, thrown objects).** Covers registry sync, RPC (ServerCall/ClientCall), shared table, local-prediction + server-synced-remote pattern, quaternion sync, and interpolation tuning. Required reading for Attack Drone, Guided Missile, Black Hole, or any mod that tracks objects beyond the player. |
| `docs/AUDIT_REPORT.md` | **When deciding what to work on next.** Generated by `python -m tools.audit`. Shows which mods have/lack Shoot, AimInfo, AmmoPickup, options menu, keybind hints, etc. Regenerate with `python -m tools.audit --output docs/AUDIT_REPORT.md` after changes. |
| `ISSUES_AND_FIXES.md` | **Before debugging a new bug.** Contains 32 resolved issues with root causes, fixes, and rules. Check if the bug matches a known pattern before investigating from scratch. **After fixing a new bug**, append it here. |
| `MASTER_MOD_LIST.md` | **When checking which mods are patched vs remaining.** Lists all 45+ patched mods by batch with workshop IDs. Update after converting new mods. |
| `C:/Users/trust/Documents/Teardown/TEARDOWN_V2_API_REFERENCE.md` | **When you need the exact signature of a Teardown API function.** Full v2 API reference (550+ functions). Use when the rules above don't cover a specific function. |
| `QUICKSTART.md` | Not needed if you read this CLAUDE.md. Exists as a fallback for quick orientation. |
