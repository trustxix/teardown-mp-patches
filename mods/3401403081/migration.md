...
-	RegisterTool("rmwg17", "G17", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwg17.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwg17", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
@@ -232,7 +143,7 @@
 			if GetString("game.player.tool") == "rmwg17" then --- When Zero then its empty
 			
 			local optic = opticssight[10]
-                if charginghandletime > 0 then ---Charginghandle
+                if charginghandletime ~= 0 then ---Charginghandle
 					SetShapeLocalTransform(optic, Transform(Vec(-0.013, 0.0905, -0.18), QuatEuler(-90, 0, 0)))
 				else
 					SetShapeLocalTransform(optic, Transform(Vec(-0.013, 0.0905, -0.28), QuatEuler(-90, 0, 0)))
@@ -457,7 +368,7 @@
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwg17" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwg17", "G17", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwg17.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
