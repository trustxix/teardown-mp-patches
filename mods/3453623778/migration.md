...
-VISUAL_SCALE_HIP  = 1.7   -- viewmodel tilt multiplier (hip)
-VISUAL_SCALE_ADS  = 1.3   -- viewmodel tilt multiplier (ADS)
...
-	RegisterTool("rmwak74", "AK74", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwak74.enabled", true)
-	
...
-	SetToolHaptic("rmwak74", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
@@ -271,7 +176,7 @@
 			if GetString("game.player.tool") == "rmwak74" then --- When Zero then its empty
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwak74" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwak74", "AK74", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwak74.enabled", true, true)
+     SetToolTransform(Transform(Vec(0, 0, 0)), 0) 
+       tool_initialized = false
+    angle = 0
+    angVel = 0
...
-function init()
-	--Register tool and enable it
-	RegisterTool("rmwak74m", "AK74M", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwak74m.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	shootHaptic = LoadHaptic("MOD/haptic/gun_fire.xml")
-	local toolHaptic = LoadHaptic("MOD/haptic/background.xml")
-	SetToolHaptic("rmwak74m", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
-	
-
-	--Check if rmwak74m is selected
-	if GetString("game.player.tool") == "rmwak74m" then
...
-			end
-			
-			if GetString("game.player.tool") == "rmwak74m" then --- When Zero then its empty
-			
-			local optic = opticssight[14]
-                if charginghandletime > 0 then ---Charginghandle
-					SetShapeLocalTransform(optic, Transform(Vec(0.01, 0.125, -0.1), QuatEuler(-90, 0, 0)))
-				else
-					SetShapeLocalTransform(optic, Transform(Vec(0.01, 0.125, -0.2), QuatEuler(-90, 0, 0)))
...
-                    RemoveTag(optic, "invisible")
-					SetShapeLocalTransform(optic, Transform(Vec(0.0078, 0.228, -0.15), QuatEuler(-90, 0, 45)))
-				else
-				SetTag(optic, "invisible")
-                end
...
-                    RemoveTag(optic, "invisible")
-					SetShapeLocalTransform(optic, Transform(Vec(0.0078, 0.227, -0.145), QuatEuler(-90, 0, 45)))
-				else
-				SetTag(optic, "invisible")
-                end
...
-function draw()
-
-if GetString("game.player.tool") == "rmwak74m" and not fireswitch and not reloading and not attachmentmenu then
-		UiPush()
...
-			UiText(magazine.." (5.56x39mm) Auto")
-				
-				
-			end
-
-if GetString("game.player.tool") == "rmwak74m" and fireswitch and not reloading and not attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(1, 1, 1)
-			UiFont("bold.ttf", 32)
-			UiTextOutline(0,0,0,1,0.1)
-			UiText(magazine.." (5.56x39mm) Semi")
-			
-			end
-			
-if GetString("game.player.tool") == "rmwak74m" and reloading and not attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(1, 1, 1)
...
-				
-			end
-if GetString("game.player.tool") == "rmwak74m" and not reloading and attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(0.8, 0.8, 0.8)
...
+function server.init()
+    RegisterTool("rmwak74m", "AK74M", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwak74m.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
