...
+        if not sounds or not sounds.play or not sounds.play.place then
+            initSounds()
+        end
+        tool.run()
+        Magnet.run()
...
+
 function dds(transform, radius, sides, info)
     if db then
         local tr1 = TransformCopy(transform)
@@ -62,7 +68,7 @@
             dbl(center, nextPos, r+0.5, g+0.5, b+0.5, 1)
         end
 
-        if #Magnet.magnetsList > 0 then
+        if #Magnet.magnetsList ~= 0 then
             dbw('MAGNET[1] speed', VecLength(GetBodyVelocity(Magnet.magnetsList[1])))
         end
...
# Migration Report: scripts\magnet.lua
...
```diff
--- original/scripts\magnet.lua
+++ patched/scripts\magnet.lua
@@ -1,11 +1,4 @@
-Magnet = {}
-Magnet.magnetsList = {}
-Magnet.force = 0
-Magnet.factor = 0
-Magnet.polarityTag = "magnetPolarity"
--- Magnet.cappedSpeed = 2.5
--- Magnet.closestMagnets = {}
-
+#version 2
 function Magnet.run()
     -- Do registry read only once to hopefully save some access time
     local showPolarity = regGetBool('magnet.showPolarity')
@@ -45,7 +38,6 @@
     return #joints > 0
 end
 
--- Draw blue/red outline to represent polarity
 function Magnet.DrawMagneticField(body, polarity)
     local r, g, b = Magnet.GetColor(polarity)
     DrawBodyOutline(body, r, g, b, 1)
@@ -61,7 +53,6 @@
     return r, g, b
 end
 
--- Apply magentic force between this magnet and all other magnets on the map starting at nextIndex
 function Magnet.applyMagnetism(magnet, nextIndex)
     local magnetPos = AabbGetBodyCenterPos(magnet)
     local magnetPolarity = Magnet.GetPolarity(magnet)
@@ -108,23 +99,9 @@
     -- end
 end
 
--- Apply a given magnetic force to a magnet in the given direction
 function Magnet.applyMagneticForce(magnet, dir, force)
     local currentVelocity = GetBodyVelocity(magnet)
     local newVelocity = VecAdd(currentVelocity, VecResize(dir, force * GetTimeStep()))
     SetBodyVelocity(magnet, newVelocity)
 end
 
--- Sequentially computes the closest other magnet for each magnet
--- function Magnet.updateClosestMagnet(magnet, dist, otherMagnet)
---     if Magnet.closestMagnets[magnet] == nil then
---         Magnet.closestMagnets[magnet] = {nil, nil}
---     end
---     local closestMagnet = Magnet.closestMagnets[magnet][1]
---     local closestDist = Magnet.closestMagnets[magnet][2]
---     if (closestDist == nil) or (dist < closestDist) then
---         closestDist = dist
---         closestMagnet = otherMagnet
---     end
---     Magnet.closestMagnets[magnet] = {closestMagnet, closestDist}
--- end
```
...
+    regSetBool('tool.debugMode', false, true)
     ...
     -- Magnet
-    regSetBool('magnet.soundOn'     , true)
-    regSetBool('magnet.showHint'    , true)
-    regSetInt('magnet.force'        , 4)
-    regSetInt('magnet.factor'       , 1)
-    regSetBool('magnet.showPolarity', false)
-    -- regSetBool('magnet.capVelocity' , true)
+    regSetBool('magnet.soundOn'     , true, true)
+    regSetBool('magnet.showHint'    , true, true)
+    regSetInt('magnet.force'        , 4, true)
+    regSetInt('magnet.factor'       , 1, true)
+    regSetBool('magnet.showPolarity', false, true)
+    -- regSetBool('magnet.capVelocity' , true, true)
...
-
-        input = {
-            didStick = function() return tool.tool.active() and InputPressed('lmb') end,
-            didDrop = function() return tool.tool.active() and InputPressed('mmb') end,
...
-            didChangePolarity = function() return tool.tool.selected() and InputPressed('e') end,
-            didRemove = function() return tool.tool.selected() and InputPressed('r') end,
-            didAttachDetach = function() return tool.tool.selected() and InputPressed('t') end,
-        },
-
-        setup = {
-            name = 'magnet',
-            title = 'Magnet',
-            voxPath = 'MOD/vox/magnet.vox',
-        },
-
-        active = function()
...
-
-        selected = function()
-            return (tool.tool.selectedMagnet ~= nil) and (GetPlayerVehicle() == 0)
-        end,
-
...
+#version 2
 function tool.run()
     ...
     -- Get selected magnet at all times (even if tool not active)
@@ -48,7 +6,7 @@
     if maxDist == 0 then
         maxDist = 1000000
     end
...
+    local playerTr = GetPlayerCameraTransform(playerId)
     ...
     tool.tool.selectedMagnet = nil
@@ -66,7 +24,7 @@
         tool.tool.aimPoint = VecAdd(playerTr.pos, VecResize(direction, maxDist))
         tool.tool.aimNormal = VecResize(direction, -1)
         tool.tool.spawnTr.pos = tool.tool.aimPoint
-        tool.tool.spawnTr.rot = GetPlayerCameraTransform().rot
+        tool.tool.spawnTr.rot = GetPlayerCameraTransform(playerId).rot
     end
     dbw("playerTr", TransformStr(playerTr))
     dbw("spawnTr", TransformStr(tool.tool.spawnTr))
@@ -84,17 +42,17 @@
     end
 
     if tool.tool.active() then -- Lock tool change on scroll when editing
-        SetBool("game.input.locktool", tool.tool.input.isEditing())
+        SetBool("game.input.locktool", tool.tool.input.isEditing(), true)
     end
 
     local scroll = tool.tool.input.getScroll()
     if scroll ~= 0 then -- Mouse wheel was scrolled, change tool magnet polarity
         if tool.tool.magnetPolarity == "S" then
             tool.tool.magnetPolarity = "N"
-            sounds.play.north(GetPlayerCameraTransform().pos, 0.5)
+            sounds.play.north(GetPlayerCameraTransform(playerId).pos, 0.5)
         else
             tool.tool.magnetPolarity = "S"
-            sounds.play.south(GetPlayerCameraTransform().pos, 0.5)
+            sounds.play.south(GetPlayerCameraTransform(playerId).pos, 0.5)
         end
     end
...
-
 function tool.placeMagnet(stick)
     -- If sticky magnet, aim must touch an object to allow magnet spawn
     local doSpawn = false
@@ -217,4 +174,5 @@
         -- Play sound
         sounds.play.place(spawnTr.pos, 0.5)
     end
-end+end
+
...
-            do UiPush()
-
-                ui.slider.create('Max placement distance (0 = infinite)', 'tool.range', 'Meters', 0, 100, nil, nil, nil, nil, true)
...
-            do UiPush()
-
-                ui.slider.create('Max magnet force (when magnets touching)', 'magnet.force', '', 1, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 40)
-                UiColor(0.6,0.6,0.6, 1)
-                UiAlign('left middle')
-                UiText('Note: Higher force will make detached magnets more unstable')
-                ui.padding.create(0, 60)
-
-                ui.slider.create('Magnetic curve ratio', 'magnet.factor', '', 1, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 40)
-                UiColor(0.6,0.6,0.6, 1)
-                UiAlign('left middle')
-                UiText('Note: Higher ratio = higher range + higher force')
-                ui.padding.create(0, 60)
-
-                ui.checkBox.create('Show magnets polarity', 'magnet.showPolarity')
-                ui.padding.create(0, 80)
-
-                -- ui.checkBox.create('Limit magnets velocity when close to each other', 'magnet.capVelocity')
-                -- ui.padding.create(0, 60)
-                -- UiColor(0.6,0.6,0.6, 1)
-                -- UiAlign('left middle')
-                -- UiText('Note: Improves Magnets collision at high speed')
-                -- ui.padding.create(0, 40)
-
-            UiPop() end
-
...
-                ui.padding.create(800, 0)
-
-                ui.checkBox.create('Sound', 'magnet.soundOn')
-                ui.padding.create(0, 80)
-
-                ui.checkBox.create('Show key hint when mouse on a magnet', 'magnet.showHint')
-                ui.padding.create(0, 80)
-
-            UiPop() end
-
...
-            UiText('[LMB]: Stick Magnet')
-            ui.padding.create(0, 50)
-            UiText('[MMB]: Drop Magnet')
-            ui.padding.create(0, 50)
-            UiText('[RMB]+[Scroll]: Change held Magnet polarity')
-            ui.padding.create(0, 50)
-            UiText('[K]: Remove all magnets')
-            ui.padding.create(0, 50)
-            UiText('[E]: Change selected Magnet polarity')
-            ui.padding.create(0, 50)
-            UiText('[R]: Remove selected Magnet')
-            ui.padding.create(0, 50)
-            UiText('[T]: Attach/Detach selected Magnet')
-            ui.padding.create(0, 50)
-            UiText('[O]: Options')
-            ui.padding.create(0, 50)
-
...
-            ui.padding.create(0, 50)
-            UiFont('regular.ttf', font_normal)
-            UiText('- Magnets of same polarity repel each other')
-            ui.padding.create(0, 50)
-            UiText('- Magnets of opposite polarity attract each other')
-            ui.padding.create(0, 50)
-            UiText('- Magnets with North polarity have a red outline')
-            ui.padding.create(0, 50)
-            UiText('- Magnets with South polarity have a blue outline')
-            ui.padding.create(0, 50)
-            UiText('- Detached Magnets fail to stick together when force is too strong')
-            ui.padding.create(0, 50)
-            UiText('- All magnets interact with each other all the time,\nso spawning too many magnets will cause lag')
-            ui.padding.create(0, 50)
-
-        UiPop() end
-        
...
--- Draw key hint on currently hovered magnet
 function uiDrawMagnetHint()
     if tool.tool.selected() and regGetBool('magnet.showHint') then
         UiPush()
@@ -394,7 +244,6 @@
     end
 end
