...
-RECOIL_BASE_HIP   = 0.78   -- per-shot pitch in hip
-RECOIL_BASE_ADS   = 0.68   -- per-shot pitch in ADS
...
-	RegisterTool("rmw416", "HK416", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmw416.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmw416", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmw416" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmw416", "HK416", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmw416.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
