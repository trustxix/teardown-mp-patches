...
# Migration Report: scripts\blob.lua
...
```diff
--- original/scripts\blob.lua
+++ patched/scripts\blob.lua
@@ -1,19 +1,4 @@
-Blob = {}
--- Nested object to make sure quick save will keep state
-Blob_info = {
-    {
-        timers = {},       -- Ordered list of future times when one or more blobs should be updated
-        blobsByTime = {},  -- Hashmap indexed by time from the timers list containing the list of blob shapes to update
-        clusters = {},     -- List of blob clusters (index is cluster id). Each cluster contains a list of blobs and the blobTarget for all of them
-        freeClusters = {}, -- List of indexes of the clusters list that are empty and can be reused
-        texturesIndex = {},
-        spawnSound = LoadSound(''),
-        physicsBodies = {},
-        trailsWasActive = false;
-    }
-}
-
-
+#version 2
 function Blob.run()
     local bi = Blob_info[1]
     -- Reverse mapping for texture indexes (index starts at 0)
@@ -116,7 +101,7 @@
     -- Manage self-replication (spawn one blob per tick near an existing blob)
     if regGetBool('blob.selfClone') then
         local maxAmount = regGetInt('blob.maxSpawn')
-        if maxAmount > 0 then
+        if maxAmount ~= 0 then
             local blobs = FindShapes("blob", true)
             if #blobs >= maxAmount then
                 return
@@ -368,6 +353,7 @@
 function Blob.isBlobPart(shape)
     return (GetEntityType(shape) == "shape") and HasTag(shape, "blobCluster")
 end
+
 function Blob.isBlobTrail(shape)
     return (GetEntityType(shape) == "shape") and HasTag(shape, "blobCluster") and not HasTag(shape, "blob")
 end
@@ -382,7 +368,7 @@
     -- If cluster id is not defined, find a free cluster to use
     if clusterId == nil then
         local nbFreeClusters = #bi.freeClusters
-        if nbFreeClusters > 0 then
+        if nbFreeClusters ~= 0 then
             clusterId = bi.freeClusters[nbFreeClusters]
             bi.freeClusters[nbFreeClusters] = nil
         else
@@ -679,7 +665,7 @@
             blobsToRemove[#blobsToRemove+1] = clusterBlob
         end
     end
-    if #blobsToRemove > 0 then
+    if #blobsToRemove ~= 0 then
         dbp("cleaning up "..#blobsToRemove.." non blob shapes")
         Blob.removeFromCluster(blobsToRemove, clusterId)
     end
@@ -1288,4 +1274,5 @@
             bi.trailsWasActive = false
         end
     end
-end+end
+
...
# Migration Report: scripts\blob_bugged.lua
...
```diff
--- original/scripts\blob_bugged.lua
+++ patched/scripts\blob_bugged.lua
@@ -1,17 +1,4 @@
-Blob = {}
--- Nested object to make sure quick save will keep state
-Blob_info = {
...
@@ -578,7 +566,7 @@
     ...
     -- If we are here then blobs are probably dynamic, so check if any target nearby became closer than previous target
     local newBlobTarget = Blob.findBlobTarget(Blob.getClusterCenter(clusterId))
-    if newBlobTarget > 0 then
+    if newBlobTarget ~= 0 then
         bi.clusters[clusterId].blobTarget = newBlobTarget
     end
     -- Find a shape nearby that is not from the current body to stick to
@@ -653,4 +641,5 @@
         Delete(body)
         dbp("empty blob body deleted")
     end
-end+end
+
...
+#version 2
 function modReset()
     ...
     -- Global parameters
-    regSetBool('blob.debugMode'     , false)
-    regSetBool('blob.soundOn'       , true)
-    regSetInt('tool.pour.rate'      , 30)
-    regSetFloat('tool.pour.spread'  , 10)
-    regSetInt('tool.pour.velocity'  , 30)
+    regSetBool('blob.debugMode'     , false, true)
+    regSetBool('blob.soundOn'       , true, true)
+    regSetInt('tool.pour.rate'      , 30, true)
+    regSetFloat('tool.pour.spread'  , 10, true)
+    regSetInt('tool.pour.velocity'  , 30, true)
 
     -- Physical characteristics and appearance
-    regSetFloat('blob.size'        , 6)
-    regSetBool('blob.solid'        , true)
-    regSetBool('blob.unbreakable'  , false)
-    regSetBool('blob.explosive'    , false)
-    regSetBool('blob.igniting'     , false)
-    regSetString('blob.corrosive'  , "hard")
-    regSetString('blob.material'   , "dirt")
-    regSetInt('blob.color.r'       , 252)
-    regSetInt('blob.color.g'       , 246)
-    regSetInt('blob.color.b'       , 16)
-    regSetString('blob.texture'    , "soaked")
-    regSetInt('blob.textureOpacity', 75)
-    regSetInt('blob.reflective'    , 10)
-    regSetInt('blob.roughness'     , 100)
-    regSetInt('blob.metallic'      , 100)
-    regSetInt('blob.emissive'      , 0)
-    regSetString('blob.loadSelected'    , "none")
+    regSetFloat('blob.size'        , 6, true)
+    regSetBool('blob.solid'        , true, true)
+    regSetBool('blob.unbreakable'  , false, true)
+    regSetBool('blob.explosive'    , false, true)
+    regSetBool('blob.igniting'     , false, true)
+    regSetString('blob.corrosive'  , "hard", true)
+    regSetString('blob.material'   , "dirt", true)
+    regSetInt('blob.color.r'       , 252, true)
+    regSetInt('blob.color.g'       , 246, true)
+    regSetInt('blob.color.b'       , 16, true)
+    regSetString('blob.texture'    , "soaked", true)
+    regSetInt('blob.textureOpacity', 75, true)
+    regSetInt('blob.reflective'    , 10, true)
+    regSetInt('blob.roughness'     , 100, true)
+    regSetInt('blob.metallic'      , 100, true)
+    regSetInt('blob.emissive'      , 0, true)
+    regSetString('blob.loadSelected'    , "none", true)
     local savedblobSkins = regListKeys('blob.saved')
-    if #savedblobSkins > 0 then
-        regSetString('blob.loadSelected', savedblobSkins[1])
+    if #savedblobSkins ~= 0 then
+        regSetString('blob.loadSelected', savedblobSkins[1], true)
     end
-    regSetString('blob.saveName'        , "")
-    regSetString('blob.physics'         , "low")
-    regSetInt('blob.maxSpawn'           , 50)
+    regSetString('blob.saveName'        , "", true)
+    regSetString('blob.physics'         , "low", true)
+    regSetInt('blob.maxSpawn'           , 50, true)
 
     -- Movement parameters
-    regSetFloat('blob.movementDelay'  , 0.1)
-    regSetBool('blob.leaveSplash'     , false)
-    regSetBool('blob.splashStatic'    , false) 
-    regSetBool('blob.leaveTrail'      , false)
-    regSetBool('blob.permanentTrail'  , false)
-    regSetBool('blob.selfClone'       , false)
+    regSetFloat('blob.movementDelay'  , 0.1, true)
+    regSetBool('blob.leaveSplash'     , false, true)
+    regSetBool('blob.splashStatic'    , false, true) 
+    regSetBool('blob.leaveTrail'      , false, true)
+    regSetBool('blob.permanentTrail'  , false, true)
+    regSetBool('blob.selfClone'       , false, true)
...
-            name = 'blight_gun',
-            title = 'Blight Gun',
-            voxPath = 'MOD/vox/blightgun.vox',
-        },
-        -- Tool projectiles
-        shootTimer = 0,
-        openSnd = LoadSound("MOD/snd/open.ogg"),
-        closeSnd = LoadSound("MOD/snd/close.ogg"),
-        shootingSound = LoadLoop("MOD/snd/shooting.ogg"),
-        shootSound = LoadSound("MOD/snd/shoot0.ogg"),
-        targetSound = LoadSound("warning-beep.ogg"),
-        deleteSound = LoadSound("water/splash-m0.ogg"),
-        hitShape = nil,
-        hitPos = Vec(),
-
...
+
 function tool.input.didTarget()
     return InputPressed(regGetString('key.target'))
 end
+
 function tool.input.didKillOne()
     return InputDown(regGetString('key.killOne'))
 end
+
 function tool.input.didKillAll()
     return InputPressed(regGetString('key.killAll'))
 end
...
-			if ti.ready > 0 then
+			if ti.ready ~= 0 then
 				local p = TransformToParentPoint(toolTr, Vec(0, 0, -2))
-				local sprite = VecAdd(TransformToParentPoint(toolTr, Vec(0.675, -0.4, -2.1)), VecScale(GetPlayerVelocity(), dt*1))
+				local sprite = VecAdd(TransformToParentPoint(toolTr, Vec(0.675, -0.4, -2.1)), VecScale(GetPlayerVelocity(playerId), dt*1))
 				PointLight(p, 1, 1, 0, ti.ready * math.random(10, 15) / 10)
 				local rnd = math.random()*3/4+1
 				DrawSprite(ti.muzzleflash,{pos=sprite,rot=GetCameraTransform().rot},4.5*rnd,4.5*rnd, 1,0.8,0.3, 1,0.3,1, 0.01,false, true)
@@ -171,7 +145,6 @@
 function tool.getShootDelay()
     return 1.0 / regGetInt('tool.pour.rate')
 end
-
 
 function tool.shootBlobs()
     local ti = tool_info[1]
@@ -252,7 +225,7 @@
     QueryClearTags()
...
-
 function tool.getRandPosInCylinder(center, radius, height)
 	local s = rdmf()
 	local theta = rdmf(0, 2*math.pi)
@@ -364,4 +336,5 @@
 	local y = r * math.sin(theta)
 	local z = rdmf(0, height)
...
-            do UiPush()
-
-                ui.slider.create('Blight shoot speed', 'tool.pour.velocity', 'm/s', 1, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 40)
-                ui.slider.create('Blight per second', 'tool.pour.rate', 'BPS', 1, 50, nil, nil, nil, nil, true)
-                ui.padding.create(0, 40)
-                ui.slider.create('Blight spread', 'tool.pour.spread', nil, 0, 100)
-                ui.padding.create(0, 40)
-                ui.slider.create('Blight max amount (0 = infinite)', 'blob.maxSpawn', nil, 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 40)
-               
-            UiPop() end
-
...
-
-                ui.checkBox.create('Debug Mode', 'blob.debugMode')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Sound', 'blob.soundOn')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-            do UiPush()
-
-                local nameToLoad = regGetString('blob.loadSelected')
-                local savedBlobSkins = regListKeys('blob.saved')
-                local hasSavedBlobSkins = #savedBlobSkins > 0
-                -- Makes sure table is never empty (slider doesn't like empty tables)
-                if not hasSavedBlobSkins then
-                    savedBlobSkins = {"none"}
-                end
-                -- Makes sure currently selected name is always part of the list
-                if TableFind(savedBlobSkins, nameToLoad) == nil then
-                    regSetString('blob.loadSelected', savedBlobSkins[1])
-                end
-                ui.slider.create('Saved Blight Skins', 'blob.loadSelected', '', nil, nil, nil, nil, nil, nil, nil, savedBlobSkins)
-                nameToLoad = regGetString('blob.loadSelected')
-                ui.padding.create(0, 50)
-
-                do UiPush()
-
-                    local loadButton = ui.button.create('Load', whiteColor, whiteColor, nil, nil, nil, nil, not hasSavedBlobSkins)
-                    if loadButton then
-                        regSetString('blob.material', regGetString('blob.saved.'..nameToLoad..'.material'))
-                        regSetInt('blob.color.r', regGetInt('blob.saved.'..nameToLoad..'.color.r'))
-                        regSetInt('blob.color.g', regGetInt('blob.saved.'..nameToLoad..'.color.g'))
-                        regSetInt('blob.color.b', regGetInt('blob.saved.'..nameToLoad..'.color.b'))
-                        regSetString('blob.texture', regGetString('blob.saved.'..nameToLoad..'.texture'))
-                        regSetInt('blob.textureOpacity', regGetInt('blob.saved.'..nameToLoad..'.textureOpacity'))
-                        regSetInt('blob.reflective', regGetInt('blob.saved.'..nameToLoad..'.reflective'))
-                        regSetInt('blob.roughness', regGetInt('blob.saved.'..nameToLoad..'.roughness'))
-                        regSetInt('blob.metallic', regGetInt('blob.saved.'..nameToLoad..'.metallic'))
-                        regSetInt('blob.emissive', regGetInt('blob.saved.'..nameToLoad..'.emissive'))
-                    end
-                    ui.padding.create(ui.button.lastWidth + padW, 0)
-
-                    local deleteButton = ui.button.create('Delete', whiteColor, whiteColor, nil, nil, nil, nil, not hasSavedBlobSkins)
-                    if deleteButton then
-                        regClearKey('blob.saved.'..nameToLoad)
-                    end
-                    ui.padding.create(ui.button.lastWidth + padW, 0)
-
-                    local saveName = regGetString('blob.saveName')
-                    local saveButton = ui.button.create('Save as', whiteColor, whiteColor, nil, nil, nil, nil, string.len(saveName) == 0)
-                    if saveButton then
-                        regSetString('blob.saved.'..saveName..'.material', regGetString('blob.material'))
-                        regSetInt('blob.saved.'..saveName..'.color.r', regGetInt('blob.color.r'))
-                        regSetInt('blob.saved.'..saveName..'.color.g', regGetInt('blob.color.g'))
-                        regSetInt('blob.saved.'..saveName..'.color.b', regGetInt('blob.color.b'))
-                        regSetString('blob.saved.'..saveName..'.texture', regGetString('blob.texture'))
-                        regSetInt('blob.saved.'..saveName..'.textureOpacity', regGetInt('blob.textureOpacity'))
-                        regSetInt('blob.saved.'..saveName..'.reflective', regGetInt('blob.reflective'))
-                        regSetInt('blob.saved.'..saveName..'.roughness', regGetInt('blob.roughness'))
-                        regSetInt('blob.saved.'..saveName..'.metallic', regGetInt('blob.metallic'))
-                        regSetInt('blob.saved.'..saveName..'.emissive', regGetInt('blob.emissive'))
-                    end
-                    ui.padding.create(ui.button.lastWidth + padW, -20)
-
-                UiPop() end
-                ui.padding.create(0, 34)
-                ui.lineEdit.create('', 'blob.saveName', nil, nil, true)
-                ui.padding.create(0, 64)
-
-                ui.slider.create('Size', 'blob.size', 'Vox', 1, 16, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Movement delay', 'blob.movementDelay', 's', 0, 10)
-                ui.padding.create(0, 30)
-         
-                ui.slider.create('Corrosive on', 'blob.corrosive', 'materials', nil, nil, nil, nil, nil, nil, nil, corrosion)
-                ui.padding.create(0, 30)
-
-                do UiPush()
-                    ui.padding.create(200, 0)
-                    ui.checkBox.create('Solid  (no break on impact)', 'blob.solid')
-                UiPop() end
-                ui.checkBox.create('Explosive', 'blob.explosive')  
-                ui.padding.create(0, 60)
-
-                do UiPush()
-                    ui.padding.create(200, 0)
-                    ui.checkBox.create('Unbreakable', 'blob.unbreakable')
-                UiPop() end
-                ui.checkBox.create('Igniting', 'blob.igniting')  
-                ui.padding.create(0, 60)
-
-                -- Higher "qualities" send stuff flying way too much
-                --ui.slider.create('Physics quality', 'blob.physics', '', nil, nil, nil, nil, nil, nil, nil, quality)
-                --ui.padding.create(0, 30)
-
-            UiPop() end
-
...
-
-                ui.slider.create('Material (physics and sound)', 'blob.material', '', nil, nil, nil, nil, nil, nil, nil, materials)
-                ui.padding.create(0, 50)
-    
-                ui.colorPicker.create('Color', 'blob.color', nil, nil, nil, false)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Texture (visual and sound)', 'blob.texture', '', nil, nil, nil, nil, nil, nil, nil, textures)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Texture opacity', 'blob.textureOpacity', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Reflective', 'blob.reflective', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Roughness (reflection blur)', 'blob.roughness', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Metallic (color tint)', 'blob.metallic', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Emissive (glow)', 'blob.emissive', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-    
-            UiPop() end
-
-            do UiPush()
-
-                ui.padding.create(1300, 0)
-
-                UiFont('regular.ttf', font_normal)
-                UiAlign('left middle')
-                UiColor(0.6,0.6,0.6, 1)
-                UiText('EXPERIMENTAL: Try using', true)
-                UiText('with a single blight entity', true)
-    
-                ui.checkBox.create('Leave splash (lag lvl=10)', 'blob.leaveSplash')
-                ui.padding.create(0, 60)
-
-                ui.checkBox.create('Splash static objects (lag lvl=20)', 'blob.splashStatic')
-                ui.padding.create(0, 60)
-
-                ui.checkBox.create('Leave trail (lag lvl=5)', 'blob.leaveTrail')
-                ui.padding.create(0, 60)
-
-                ui.checkBox.create('Permanent trail (lag lvl=40)', 'blob.permanentTrail')
-                ui.padding.create(0, 60)
-
-                ui.checkBox.create('Self-replication (lag lvl=yes)', 'blob.selfClone')
-                ui.padding.create(0, 60)
-
-            UiPop() end
-
...
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Set Blight target', 'key.target', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Remove Blight at cursor', 'key.killOne', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Remove all Blight', 'key.killAll', true)
-            ui.padding.create(0, 50)
...
-            UiFont('regular.ttf', font_normal)
-            ui.padding.create(0, 50)
-            UiText('- Blight is composed of physical objects that can be destroyed', true)
-            UiText('', true)
-            UiText('- Blight will move to the closest loose object', true)
-            UiText('  or to the target you gave them', true)
-            UiText('', true)
-            UiText('- Blight will slowly eat matter around it', true)
-            UiText('', true)
-            UiText('- Blight parts may regroup into a larger blight entity', true)
-            UiText('', true)
-            UiText('- Blight may not always physics too well', true)
-            UiText('', true)
-
-        UiPop() end
-
...
-    function strToNumberTable(str, sep)
+
+function strToNumberTable(str, sep)
         sep = sep or ","
         local splittedStr = sSplit(str, sep)
         local out = {}
@@ -1566,7 +1584,8 @@
         end
         return out
