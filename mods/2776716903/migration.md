...
-    --dbp("sounds="..TableToStringRecursive(sounds))
-    if not sounds or not sounds.play or not sounds.play.thrust then
-        --dbp("initSounds")
-        initSounds()
...
+       toggleTimer = toggleTimer - GetTimeStep()
+    uiToggleKeyChange()
 
-function draw()
-
-    -- Draw new value after toggle key pressed
-    toggleTimer = toggleTimer - GetTimeStep()
-	uiToggleKeyChange()
-
-    uiManageGameOptions()
...
+#version 2
 function initDebug()
     db = regGetBool('jetpack.debugMode')
 end
...
# Migration Report: scripts\jetpack.lua
...
```diff
--- original/scripts\jetpack.lua
+++ patched/scripts\jetpack.lua
@@ -1,33 +1,10 @@
-Jetpack = {}
-Jetpack_info = {
-    {
...
-for k,v in pairs(particleTypes) do
-    Jetpack_info[1].particlesIndex[v]=k-1
-end
-
+#version 2
 function Jetpack.run()
     local jpi = Jetpack_info[1]
...
+            local playerTr = GetPlayerTransform(playerId)
             local localFront = Vec(0, 0, -1)
             local playerFrontVec = TransformToParentVec(playerTr, localFront) 
             jpi.playerFrontPos = VecAdd(playerCenter, playerFrontVec)
@@ -72,31 +49,31 @@
     end
 
     if InputPressed(regGetString('jetpack.toggleGimbalKey')) then
-        regSetBool('jetpack.gimbal', not regGetBool('jetpack.gimbal'))
+        regSetBool('jetpack.gimbal', not regGetBool('jetpack.gimbal'), true)
         toggleTimer = 2
         toggleText = "Gimbal thrust: "..(regGetBool('jetpack.gimbal') and "ON" or "OFF")
     end
 
     if InputPressed(regGetString('jetpack.toggleLateralKey')) then
-        regSetBool('jetpack.lateral', not regGetBool('jetpack.lateral'))
+        regSetBool('jetpack.lateral', not regGetBool('jetpack.lateral'), true)
         toggleTimer = 2
         toggleText = "Lateral thrust: "..(regGetBool('jetpack.lateral') and "ON" or "OFF")
     end
 
     if InputPressed(regGetString('jetpack.toggleHoverKey')) then
-        regSetBool('jetpack.hover', not regGetBool('jetpack.hover'))
+        regSetBool('jetpack.hover', not regGetBool('jetpack.hover'), true)
         toggleTimer = 2
         toggleText = "Hover: "..(regGetBool('jetpack.hover') and "ON" or "OFF")
     end
 
     if InputPressed(regGetString('jetpack.toggleFallDamagesKey')) then
-        regSetBool('jetpack.noFallDamages', not regGetBool('jetpack.noFallDamages'))
+        regSetBool('jetpack.noFallDamages', not regGetBool('jetpack.noFallDamages'), true)
         toggleTimer = 2
         toggleText = "No fall damages: "..(regGetBool('jetpack.noFallDamages') and "ON" or "OFF")
     end
 
     if InputPressed(regGetString('jetpack.toggleAirMomentumKey')) then
-        regSetBool('jetpack.keepAirMomentum', not regGetBool('jetpack.keepAirMomentum'))
+        regSetBool('jetpack.keepAirMomentum', not regGetBool('jetpack.keepAirMomentum'), true)
         toggleTimer = 2
         toggleText = "Keep air momentum: "..(regGetBool('jetpack.keepAirMomentum') and "ON" or "OFF")
     end
@@ -151,11 +128,11 @@
         thrustDir = TransformToParentVec(playerTr, thrustDir) 
         -- Apply thrust impulse
         local thrust = VecResize(thrustDir, regGetInt('jetpack.thrustPower') * GetTimeStep()) -- calculate thrust vector
-        local velocity = GetPlayerVelocity()
+        local velocity = GetPlayerVelocity(playerId)
         local correctionDir = Vec()
         -- If steer assist active, reduce the component of the velocity going against thrust direction
         local steerAssist = regGetInt('jetpack.steerAssist') / 100.0
-        if steerAssist > 0 then
+        if steerAssist ~= 0 then
             -- Correct side dir
             local projectedSpeed = VecProjToVecLen(velocity, thrustDir)
             local velocityAlongRotatedDir = VecResize(thrustDir, projectedSpeed)
@@ -170,7 +147,7 @@
             thrust = VecAdd(thrust, correctionDir)
         end
         velocity = VecAdd(velocity, thrust)
-        SetPlayerVelocity(velocity)
+        SetPlayerVelocity(playerId, velocity)
 
         if db then
             local pfp = jpi.playerFrontPos
@@ -191,51 +168,51 @@
 
 function Jetpack.updatePlayerPhysics()
     local jpi = Jetpack_info[1]
-    local vel = GetPlayerVelocity()
+    local vel = GetPlayerVelocity(playerId)
     -- Keep momentum in the air
     if regGetBool('jetpack.keepAirMomentum') then
-        local pos = GetPlayerTransform().pos
+        local pos = GetPlayerTransform(playerId).pos
         if not (IsPointInWater(pos) or jpi.playerWalking) then
             local h_vel = VecFlatten(vel)
             local dt = GetTimeStep()
             local max = (1/dt/100 / (1/dt/30))
             --dbp("max="..max..", len="..VecLength(h_vel))
             local correctionDir = VecResize(h_vel, math.min(max, VecLength(h_vel)))
-            SetPlayerVelocity(VecAdd(vel, correctionDir))
+            SetPlayerVelocity(playerId, VecAdd(vel, correctionDir))
             if db then
                 local pfp = jpi.playerFrontPos
                 dbl(pfp, VecAdd(pfp, correctionDir), 0, 0, 1, 1)
             end
         end
     end
-    local vel = GetPlayerVelocity()
+    local vel = GetPlayerVelocity(playerId)
     -- If air brake active, reduce velocity if not pressing any key
     if not jpi.thrusting then
         local airBrake = regGetInt('jetpack.airBrake') / 100.0
-        if airBrake > 0 then
+        if airBrake ~= 0 then
             local correctionDir = VecScale(vel, -airBrake * regGetFloat('jetpack.forceScale') * GetTimeStep())
-            SetPlayerVelocity(VecAdd(vel, correctionDir))
+            SetPlayerVelocity(playerId, VecAdd(vel, correctionDir))
             if db then
                 local pfp = jpi.playerFrontPos
                 dbl(pfp, VecAdd(pfp, correctionDir), 1, 0.5, 0.5, 1)
             end
         end
     end
-    local vel = GetPlayerVelocity()
+    local vel = GetPlayerVelocity(playerId)
     -- Hover when no key pressed, zero G when key pressed
     if regGetBool('jetpack.hover') then
         if jpi.enabled and (not jpi.thrustDown) then
             local antigrav = 0.16663932800293 -- 7.5 * GetTimeStep()
             local target = Vec(vel[1], math.max(0, antigrav, vel[2]), vel[3])
             local correctionDir = VecSub(target, vel)
-            SetPlayerVelocity(VecAdd(vel, correctionDir))
+            SetPlayerVelocity(playerId, VecAdd(vel, correctionDir))
             if db then
                 local pfp = jpi.playerFrontPos
                 dbl(pfp, VecAdd(pfp, correctionDir), 1, 0.5, 1, 1)
             end
         end
     end
-    local vel = GetPlayerVelocity()
+    local vel = GetPlayerVelocity(playerId)
     ...
     if regGetBool('jetpack.noFallDamages') then
@@ -260,7 +237,7 @@
     local hovering = regGetBool('jetpack.hover')
     if jpi.enabled and (jpi.thrusting or hovering) then
         local playerCenter = AabbGetPlayerCenter()
-        local playerTr = GetPlayerTransform()
+        local playerTr = GetPlayerTransform(playerId)
         local localBack = Vec(0, 0.4, 0.3)
         local playerBackVec = TransformToParentVec(playerTr, localBack) 
         local playerBackPos = VecAdd(playerCenter, playerBackVec)
@@ -269,7 +246,7 @@
         end
 
         local a = regGetInt('jetpack.smokeColor.a') / 255.0
-        if a > 0 then
+        if a ~= 0 then
             local r = regGetInt('jetpack.smokeColor.r') / 255.0
             local g = regGetInt('jetpack.smokeColor.g') / 255.0
             local b = regGetInt('jetpack.smokeColor.b') / 255.0
@@ -472,8 +449,9 @@
     else
         -- Jetpack model not required, delete it
         local jetpackShape = Jetpack.getJetpack(animator)
-        if jetpackShape > 0 then
+        if jetpackShape ~= 0 then
             Delete(jetpackShape)
         end
     end
...
+#version 2
 function modReset()
     ...
     -- Jetpack
-    regSetBool('jetpack.gimbal'          , true)
-    regSetBool('jetpack.lateral'         , true)
-    regSetBool('jetpack.hover'           , false)
-    regSetBool('jetpack.silentHover'     , false)
-    regSetBool('jetpack.enableInAir'     , false)
-    regSetInt('jetpack.thrustPower'      , 22)
-    regSetBool('jetpack.noFallDamages'   , false)
-    regSetInt('jetpack.steerAssist'      , 30)
-    regSetInt('jetpack.airBrake'         , 0)
-    regSetBool('jetpack.keepAirMomentum' , false)
-    regSetBool('jetpack.debugMode'       , false)
-    regSetInt('jetpack.soundVolume'      , 50)
-    regSetInt('jetpack.smokeColor.r'     , 255)
-    regSetInt('jetpack.smokeColor.g'     , 255)
-    regSetInt('jetpack.smokeColor.b'     , 255)
-    regSetInt('jetpack.smokeColor.a'     , 255)
-    regSetInt('jetpack.smokeGlow'        , 0)
-    regSetString('jetpack.smokeParticle' , 'smoke')
-    regSetBool('jetpack.rainbowSmoke'    , false)
-    regSetFloat('jetpack.forceScale'     , 3.0)
-    regSetBool('jetpack.showJetpack'     , true)
+    regSetBool('jetpack.gimbal'          , true, true)
+    regSetBool('jetpack.lateral'         , true, true)
+    regSetBool('jetpack.hover'           , false, true)
+    regSetBool('jetpack.silentHover'     , false, true)
+    regSetBool('jetpack.enableInAir'     , false, true)
+    regSetInt('jetpack.thrustPower'      , 22, true)
+    regSetBool('jetpack.noFallDamages'   , false, true)
+    regSetInt('jetpack.steerAssist'      , 30, true)
+    regSetInt('jetpack.airBrake'         , 0, true)
+    regSetBool('jetpack.keepAirMomentum' , false, true)
+    regSetBool('jetpack.debugMode'       , false, true)
+    regSetInt('jetpack.soundVolume'      , 50, true)
+    regSetInt('jetpack.smokeColor.r'     , 255, true)
+    regSetInt('jetpack.smokeColor.g'     , 255, true)
+    regSetInt('jetpack.smokeColor.b'     , 255, true)
+    regSetInt('jetpack.smokeColor.a'     , 255, true)
+    regSetInt('jetpack.smokeGlow'        , 0, true)
+    regSetString('jetpack.smokeParticle' , 'smoke', true)
+    regSetBool('jetpack.rainbowSmoke'    , false, true)
+    regSetFloat('jetpack.forceScale'     , 3.0, true)
+    regSetBool('jetpack.showJetpack'     , true, true)
 
     -- Key bindings
     regInitKey('jetpack.upKey'               , GetString("options.input.keymap.jump"))
@@ -63,21 +64,22 @@
...
@@ -10,7 +11,7 @@
         ...
         thrust = function(pos)
             local vol = regGetInt('jetpack.soundVolume') / 100
-            if vol > 0 then
+            if vol ~= 0 then
                 PlayLoop(sounds.thrust, pos, vol)
             end
         end,
@@ -18,3 +19,4 @@
     }
...
-particleTypes = {'smoke', 'thrunder', 'bubbles', 'dust', 'ball', 'fire', 'square', 'none', 'snow', 'none2', 'none3', 'none4', 'hay', 'water', 'leaf', 'wave'}
-
-options_tabs_render = {
-
-    Jetpack = function()
...
-            do UiPush()
-
-                ui.checkBox.create('Gimbal thrust (ignore camera pitch angle)', 'jetpack.gimbal')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Lateral thrust (allow thrust forward/back/left/right)', 'jetpack.lateral')
-                ui.padding.create(0, 64)
-
-                ui.slider.create('Steer assist (momentum aligns with key pressed)', 'jetpack.steerAssist', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Air brake (reduce speed while not pressing anything)', 'jetpack.airBrake', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                do UiPush()
-
-                    ui.padding.create(300, 0)
-                    ui.checkBox.create('Silent hover', 'jetpack.silentHover')
-                    ui.padding.create(0, 64)
-            
-                UiPop() end
-
-                ui.checkBox.create('Hover (stop falling)', 'jetpack.hover')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Keep momentum in the air (no air drag)', 'jetpack.keepAirMomentum')
-                ui.padding.create(0, 64)
-
-                ui.slider.create('Thrust power', 'jetpack.thrustPower', '', 10, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-            UiPop() end
-
...
-
-                ui.checkBox.create('Double press to activate (wont activate on the ground)', 'jetpack.enableInAir')
-                ui.padding.create(0, 64)
-
-                ui.slider.create('Sound volume', 'jetpack.soundVolume', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Smoke particle', 'jetpack.smokeParticle', '', nil, nil, nil, nil, nil, nil, nil, particleTypes)
-                ui.padding.create(0, 50)
-                
-                ui.colorPicker.create('Smoke color', 'jetpack.smokeColor', nil, nil, nil, true)
-                ui.padding.create(0, 50)
-
-                ui.slider.create('Smoke glow', 'jetpack.smokeGlow', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                ui.checkBox.create('Rainbow smoke', 'jetpack.rainbowSmoke')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-
-                ui.checkBox.create('Debug Mode', 'jetpack.debugMode')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('No fall damages', 'jetpack.noFallDamages')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Show Jetpack', 'jetpack.showJetpack')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-        do UiPush()
-
-            ui.keyBinding.create('Up', 'jetpack.upKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Down', 'jetpack.downKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Forward', 'jetpack.forwardKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Back', 'jetpack.backKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Left', 'jetpack.leftKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Right', 'jetpack.rightKey', true)
-            ui.padding.create(0, 50)
-
...
-        do UiPush()
-
-            ui.padding.create(500, 0)
-
-            ui.keyBinding.create('Toggle gimbal thrust', 'jetpack.toggleGimbalKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Toggle lateral thrust', 'jetpack.toggleLateralKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Toggle hover', 'jetpack.toggleHoverKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Toggle fall damages', 'jetpack.toggleFallDamagesKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Toggle air momentum', 'jetpack.toggleAirMomentumKey', true)
-            ui.padding.create(0, 50)
-           
-        UiPop() end
-
-        do UiPush()
-
-            ui.padding.create(1000, 0)
-
-            ui.keyBinding.create('Enable jetpack', 'jetpack.enableJetpackKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Disable jetpack', 'jetpack.disableJetpackKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Options (to open this menu ingame)', 'jetpack.optionsKey', true)
-            ui.padding.create(0, 50)
-           
-        UiPop() end
-        
...
---- Manage when to open and close the options menu.
 function uiManageGameOptions()
     ...
     if InputPressed(regGetString('jetpack.optionsKey')) then UI_GAME = not UI_GAME end
@@ -370,22 +203,6 @@
     end
