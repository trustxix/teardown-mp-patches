...
| `python -m tools.deploy_framework --mod "X"` | **Deploy Trust Realism framework** to a mod (LF + ASCII safe) |
| `python -m tools.deploy_framework --all` | Redeploy framework to ALL mods that have it |
| `python -m tools.deploy_framework --fix-lua` | Fix CRLF/non-ASCII in ALL .lua files under Documents/Teardown/mods/ |

**MANDATORY: After writing ANY .lua file to Documents/Teardown/mods/, run `python -m tools.deploy_framework --fix-lua` or manually ensure LF-only line endings and ASCII-only content. CRLF and non-ASCII characters cause silent compile failures in Teardown's preprocessor.**
...
41. `#version 2` can appear on ANY line in a script — the preprocessor scans the whole file. Do NOT enforce line-1 placement. However, ensure LF-only line endings in Lua scripts — CRLF can cause compile errors with the preprocessor. **Also: ASCII only in all .lua files.** No em dashes, degree symbols, or any non-ASCII characters — even in comments. The preprocessor silently fails to compile files with UTF-8 multibyte characters. Use `--` instead of em dash, `deg` instead of degree symbol.
