...
-	
-	reticlesprite = LoadSprite("MOD/sprites/yma.png")
...
-RECOIL_BASE_HIP   = 0.9   -- per-shot pitch in hip
-RECOIL_BASE_ADS   = 0.8   -- per-shot pitch in ADS
...
-	RegisterTool("rmwqbz95", "QBZ-95", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwqbz95.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwqbz95", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
@@ -251,7 +159,7 @@
 			if GetString("game.player.tool") == "rmwqbz95" then --- When Zero then its empty
 			
 			local optic = opticssight[10]
-                if charginghandletime > 0 then ---Charginghandle
+                if charginghandletime ~= 0 then ---Charginghandle
 					SetShapeLocalTransform(optic, Transform(Vec(-0.01, 0.12, -0.22), QuatEuler(-90, 0, 0)))
 				else
 					SetShapeLocalTransform(optic, Transform(Vec(-0.01, 0.12, -0.32), QuatEuler(-90, 0, 0)))
@@ -479,7 +387,7 @@
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwqbz95" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwqbz95", "QBZ-95", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwqbz95.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
