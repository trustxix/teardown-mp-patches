...
# Migration Report: scripts\labyrinth.lua
...
```diff
--- original/scripts\labyrinth.lua
+++ patched/scripts\labyrinth.lua
@@ -1,89 +1,4 @@
-
-Labyrinth = {}
--- Labyrinth segments are the map of the labyrinth in a given area 
--- The areas start at 3x3 at the center of the world then grow in powers of 3.
--- Index in this list is level+1, each segment is list of wall data for one chunk.
--- Wall data for one chunk is a set of direction str.
--- eg: Labyrinth.segments = {
---                              { -- Level 0 (3x3)
---                                  {north=true},           -- Walls for chunk [0, 0]
---                                  {},                     -- Walls for chunk [1, 0]
---                                  {north=true,east=true}, -- Walls for chunk [2, 0]
---                                  {north=true,east=true}, -- Walls for chunk [0, 1]
---                                  ...
---                              },
---                              { -- Level 1 (3x3)
---                                  {south=true,east=true,north=true}, -- Walls for chunk [0, 0]
---                                  {north=true,east=true},            -- Walls for chunk [1, 0]
---                                  {north=true,east=true},            -- Walls for chunk [2, 0]
---                                  {north=true,east=true},            -- Walls for chunk [0, 1]
---                                  ...
---                              },
...
---                                  {north=true,east=true},            -- Walls for chunk [1, 0]
---                                  {north=true,east=true},            -- Walls for chunk [2, 0]
---                                  {north=true,east=true},            -- Walls for chunk [3, 0]
---                                  {north=true,east=true},            -- Walls for chunk [4, 0]
---                                  {north=true,east=true},            -- Walls for chunk [5, 0]
---                                  ...
---                              },
---                              { -- Level 3 (27x27)
---                                  ...
...
---                          }
-Labyrinth.segments = {}
--- Holds the information needed to generate one segment procedurally
...
-Labyrinth.seed = 0
--- Number of labyrinth generation algorithm steps to execute each tick
-Labyrinth.stepsPerTick = 1
--- These are the persistent flags allowed to be added to a given chunk
-Labyrinth.flags = {"north", "east", "south", "west", "entrance", "exit"}
--- Enum used to compress/uncompress wall data
-Labyrinth.wallFlags = {
-    north = 1,
-    east = 2,
-    south = 4,
...
-    exit = 32
-}
--- Direction abbreviation to full name
-Labyrinth.dirFullName = {
-    N = "north",
-    NE = "north-east",
-    E = "east",
-    SE = "south-east",
...
-    C = "center",
-}
--- Reverse map of WorldGen.directions
-Labyrinth.dirIndex = {
-    north = 1,
-    east = 2,
-    south = 3,
...
+#version 2
 function Labyrinth.Init()
     ...
     -- Load data segments from registry
@@ -93,15 +8,12 @@
 
 end
 
-
--- Reset all segments in the registry
 function Labyrinth.ResetSegments()
 
     regClearKey("segments")
 
 end
 
--- Load segments from the registry
 function Labyrinth.LoadSegments()
 
     Labyrinth.segments = {}
@@ -109,7 +21,7 @@
     local prevSeed = regGetInt('world.prevSeed')
     if prevSeed ~= Labyrinth.seed then
         Labyrinth.ResetSegments()
-        regSetInt('world.prevSeed', Labyrinth.seed)
+        regSetInt('world.prevSeed', Labyrinth.seed, true)
         return
     end
     -- Load all segments found in the registry
@@ -129,29 +41,27 @@
 
 end
 
--- Save a segment for the given level in the registry
 function Labyrinth.SaveSegment(level, segment)
 
     local segmentStr = ""
     for i=1, #segment do
-        if #segmentStr > 0 then
+        if #segmentStr ~= 0 then
             segmentStr = segmentStr..","
         end
         segmentStr = segmentStr..Labyrinth.CompressWalls(segment[i])
     end
-    regSetString("segments.segment_"..level, segmentStr)
-
-end
-
--- Save a segment for the given level in the registry
+    regSetString("segments.segment_"..level, segmentStr, true)
+
+end
+
 function Labyrinth.SaveSegmentByStep()
 
-    if #Labyrinth.generation.segmentStr > 0 then
+    if #Labyrinth.generation.segmentStr ~= 0 then
         Labyrinth.generation.segmentStr = Labyrinth.generation.segmentStr..","
     end
     Labyrinth.generation.segmentStr = Labyrinth.generation.segmentStr..Labyrinth.CompressWalls(Labyrinth.generation.segment[Labyrinth.generation.index])
     if Labyrinth.generation.index >= #Labyrinth.generation.segment then
-        regSetString("segments.segment_"..Labyrinth.generation.segmentLevel, Labyrinth.generation.segmentStr)
+        regSetString("segments.segment_"..Labyrinth.generation.segmentLevel, Labyrinth.generation.segmentStr, true)
         --dbp("Segment saved="..Labyrinth.generation.segmentStr)
         return true
     end
@@ -160,7 +70,6 @@
...
--- Get the size in number of chunks for a given segment level
 function Labyrinth.GetSegmentSize(level)
...
--- Get the minimum absolute coordinate to be in the given level
 function Labyrinth.GetLevelMinAbsCoord(level)
 
     -- This formula gives the min abs coord to be in level x
@@ -180,12 +88,10 @@
 
 end
 
--- Get the maximum (in absolute value) of the X,Y coordinate provided
 function Labyrinth.GetMaxAbsCoord(chunkPos)
     return math.max(math.abs(chunkPos[1]), math.abs(chunkPos[3]))
 end
 
--- Get the segment level for the given chunk
 function Labyrinth.GetSegmentLevel(chunkPos)
 
     local maxAbsCoord = Labyrinth.GetMaxAbsCoord(chunkPos)
@@ -199,7 +105,6 @@
 
 end
 
--- Get the wall data for the given chunk
 function Labyrinth.GetWallData(chunkPos)
 
     local level = Labyrinth.GetSegmentLevel(chunkPos)
@@ -260,7 +165,6 @@
 
 end
 
--- Get the list of walls for the given wallData
 function Labyrinth.GetWallList(wallData)
...
--- Get the number of walls for the given wallData
 function Labyrinth.GetWallCount(wallData)
...
--- Gets a random number depending on the seed and the chunk position
--- (always the same number if called multiple times with the same inputs and seed)
 function Labyrinth.GetProceduralRand(chunkPos, min, max)
...
--- Remove the given walls for a wallData
 function Labyrinth.RemoveWalls(wallData, wallList)
     for i=1, #wallList do
         wallData[wallList[i]] = nil
@@ -315,14 +215,12 @@
     return wallData
 end
 
--- Get the segment index value from local coordinates and size
 function Labyrinth.GetSegmentIndex(localX, localY, size)
     local index = localX + (localY * size) + 1
     -- dbp("GetSegmentIndex("..localX..", "..localY..", "..size..")="..index)
     return index
 end
 
--- Rotate a wall in the given newDir so that a wall placed north will now be placed at newDir
 function Labyrinth.GetRotatedWall(wallDir, newDir)
     
     local index = Labyrinth.dirIndex[wallDir] - 1
@@ -337,8 +235,6 @@
 
 end
 
--- Get the section containing the given chunkPos
--- A section is the segment position relative to the labyrinth center
 function Labyrinth.GetSection(chunkPos, level)
 
     -- Special case for level 0 in the center
@@ -386,7 +282,6 @@
 
 end
 
--- Get the center chunk of the given section at the given level
 function Labyrinth.GetSectionCenter(section, absOffset)
...
--- Get the next segment level that is not yet generated
 function Labyrinth.GetNextSegmentLevel()
     return #Labyrinth.segments
 end
 
--- Starts generation of a new segment at the given level
 function Labyrinth.StartGeneration(level)
 
     dbp("Start generation for level "..level)
@@ -431,9 +324,6 @@
 
 end
 
--- The central 3x3 segment always has the same shape:
--- walls all around with opening in the middle of each wall.
--- The north-west corner is the first described, then increasing from west (i-1) to east (i+1), then from north (j-1) to south (j+1)
 function Labyrinth.GenerateStartSegment()
 
     Labyrinth.generation.segment = {
@@ -444,7 +334,6 @@
 
 end
 
--- Compress a set of walls into an integer
 function Labyrinth.CompressWalls(wallsSet)
...
--- Tells if a given walls set contains a wall for the given direction
 function Labyrinth.HasWall(wallsSet, direction)
     return wallsSet[direction]
 end
 
--- Tells if a given walls integer contains a flag
 function Labyrinth.HasFlag(wallsInt, flag)
     return bitoper(wallsInt, Labyrinth.wallFlags[flag], "AND") > 0
 end
 
--- Uncompress an integer into a list of walls
 function Labyrinth.UncompressWalls(wallsInt)
...
--- Generate a new labyrinth segment if needed, X steps in the generation algorithm
 function Labyrinth.Generate()
 
     -- If player is close to a non generated segment and we are not already generating, start generating next segment
@@ -503,7 +388,6 @@
 
 end
 
--- Generate a new labyrinth segment, one step in the generation algorithm
 function Labyrinth.GenerateOneStep()
 
     if Labyrinth.generation.phase == "init" then
@@ -534,7 +418,7 @@
     elseif Labyrinth.generation.phase == "generate" then
 
         -- Run the recursive backtracking algo to generate the labyrinth, this can take time so do it in steps
-        if #Labyrinth.generation.stack > 0 then
+        if #Labyrinth.generation.stack ~= 0 then
             -- Mark current location as visited
             local currentPos = Labyrinth.generation.stack[#Labyrinth.generation.stack]
             local segmentIndex = Labyrinth.GetSegmentIndex(currentPos[1], currentPos[3], Labyrinth.generation.size)
@@ -623,8 +507,6 @@
...
--- Initialize and store a custom sequence of seeded random numbers
 function Labyrinth.InitRandomSequence(seed, length)
 
     Labyrinth.generation.randSeq = {}
@@ -643,7 +525,6 @@
 
 end
 
--- Get the next stored random number
 function Labyrinth.GetNextRand(min, max)
 
     Labyrinth.generation.randIndex = Labyrinth.generation.randIndex + 1
@@ -657,7 +538,6 @@
 
 end
 
--- Get the next stored random number as an integer within range
 function Labyrinth.GetNextRandInt(min, max)
 
     -- local rnd = math.floor(Labyrinth.GetNextRand(min, max+1))
@@ -667,7 +547,6 @@
 
 end
 
--- Find neighbor cases of the given pos in the maze that are unvisited by the generation algo
 function Labyrinth.FindUnvisitedNeighbors(currentPos)
...
--- Tells if a given position in the maze is visited by the generation algorithm
 function Labyrinth.IsVisited(pos)
     local segmentIndex = Labyrinth.GetSegmentIndex(pos[1], pos[3], Labyrinth.generation.size)
     return Labyrinth.generation.segment[segmentIndex]["visited"]
 end
 
--- Breaks the wall between the given chunks positions (in relative coordinates in current segment)
--- (0, 0) at north-west
 function Labyrinth.BreakWall(pos1, pos2)
 
     local segmentIndex1 = pos1[1] + (pos1[3] * Labyrinth.generation.size) + 1
@@ -729,7 +605,6 @@
 
 end
 
--- Get the direction to go from pos1 to pos2
 function Labyrinth.GetDirFromTo(pos1, pos2)
...
# Migration Report: scripts\objectives.lua
...
```diff
--- original/scripts\objectives.lua
+++ patched/scripts\objectives.lua
@@ -1,10 +1,4 @@
-
-Objectives = {}
-Objectives.distance = {}
-Objectives.level = {}
-Objectives.treasures = {}
-Objectives.survive = {}
-
+#version 2
...
-
 ...
 function Objectives.Run()
 
@@ -89,7 +82,7 @@
     if Objectives.treasures.active then
         -- Detect treasure on hover
         local maxDist = 3
...
+        local playerTr = GetPlayerCameraTransform(playerId)
         ...
         if GetBool("game.thirdperson") then
             local headTr = GetPlayerHeadTransform()
@@ -131,7 +124,7 @@
             end
         end)
         -- Chek if player is dead
-        local playerDead = (GetPlayerHealth() == 0)
+        local playerDead = (GetPlayerHealth(playerId) == 0)
         -- Update time
         if not playerDead then
             Objectives.survive.time = Objectives.survive.time + GetTimeStep()
@@ -153,7 +146,7 @@
 end
 
 function Objectives.TreasureHovered()
-    return (GetPlayerVehicle() == 0) and (Objectives.treasures.hovered ~= nil)
+    return (GetPlayerVehicle(playerId) == 0) and (Objectives.treasures.hovered ~= nil)
 end
 
 function Objectives.GetObjectId(prefix, chunkPos)
@@ -188,7 +181,6 @@
     return Labyrinth.GetProceduralRand(chunkPos, 0, 1) == 1
 
 end
-
 
 function Objectives.SpawnTreasure(chunkPos)
...
-
 function Objectives.CollectTreasure(treasureShape)
 
     local treasureId = GetTagValue(treasureShape, "treasureId")
@@ -220,7 +211,6 @@
 
 end
 
-
 function Objectives.ShouldSpawnRobot(chunkPos)
 
     if not Objectives.survive.active then
@@ -253,7 +243,6 @@
     end
...
-
 ...
 function Objectives.SpawnRobot(chunkPos)
...
-
 function Objectives.GetRobotTypesAllowed()
 
         -- Find existing robot types
@@ -323,4 +311,5 @@
         end
...
+#version 2
 function modReset()
     ...
     -- World
-    regSetBool('world.debugMode'    , false)
-    regSetInt('world.chunkSize'     , 64)
-    regSetInt('world.renderDistance', 8)
-    regSetBool('world.despawnChunks', false)
-    regSetString('world.generate'   , "floor + walls + ceiling")
-    regSetInt('world.seed'          , 0)
-    regSetInt('world.prevSeed'      , 0)
-    regSetInt('world.stepsPerTick'  , 50)
-    regSetBool('world.lightBeams'   , false)
+    regSetBool('world.debugMode'    , false, true)
+    regSetInt('world.chunkSize'     , 64, true)
+    regSetInt('world.renderDistance', 8, true)
+    regSetBool('world.despawnChunks', false, true)
+    regSetString('world.generate'   , "floor + walls + ceiling", true)
+    regSetInt('world.seed'          , 0, true)
+    regSetInt('world.prevSeed'      , 0, true)
+    regSetInt('world.stepsPerTick'  , 50, true)
+    regSetBool('world.lightBeams'   , false, true)
 
     -- Floor
-    regSetBool('floor.unbreakable'  , true)
-    regSetString('floor.material'   , "masonry")
-    regSetInt('floor.color.r'       , 220)
-    regSetInt('floor.color.g'       , 170)
-    regSetInt('floor.color.b'       , 116)
-    regSetInt('floor.color.a'       , 255)
-    regSetString('floor.texture'    , "soaked")
-    regSetInt('floor.textureOpacity', 100)
-    regSetInt('floor.reflective'    , 0)
-    regSetInt('floor.roughness'     , 0)
-    regSetInt('floor.metallic'      , 0)
-    regSetInt('floor.emissive'      , 0)
-    regSetBool('floor.transparent'  , false)
-    regSetInt('floor.thickness'     , 4)
+    regSetBool('floor.unbreakable'  , true, true)
+    regSetString('floor.material'   , "masonry", true)
+    regSetInt('floor.color.r'       , 220, true)
+    regSetInt('floor.color.g'       , 170, true)
+    regSetInt('floor.color.b'       , 116, true)
+    regSetInt('floor.color.a'       , 255, true)
+    regSetString('floor.texture'    , "soaked", true)
+    regSetInt('floor.textureOpacity', 100, true)
+    regSetInt('floor.reflective'    , 0, true)
+    regSetInt('floor.roughness'     , 0, true)
+    regSetInt('floor.metallic'      , 0, true)
+    regSetInt('floor.emissive'      , 0, true)
+    regSetBool('floor.transparent'  , false, true)
+    regSetInt('floor.thickness'     , 4, true)
 
     -- Walls
-    regSetBool('walls.unbreakable'  , true)
-    regSetString('walls.material'   , "masonry")
-    regSetInt('walls.color.r'       , 220)
-    regSetInt('walls.color.g'       , 170)
-    regSetInt('walls.color.b'       , 116)
-    regSetInt('walls.color.a'       , 255)
-    regSetString('walls.texture'    , "soaked")
-    regSetInt('walls.textureOpacity', 100)
-    regSetInt('walls.reflective'    , 0)
-    regSetInt('walls.roughness'     , 0)
-    regSetInt('walls.metallic'      , 0)
-    regSetInt('walls.emissive'      , 0)
-    regSetBool('walls.transparent'  , false)
-    regSetInt('walls.thickness'     , 4)
-    regSetInt('walls.height'        , 40)
+    regSetBool('walls.unbreakable'  , true, true)
+    regSetString('walls.material'   , "masonry", true)
+    regSetInt('walls.color.r'       , 220, true)
+    regSetInt('walls.color.g'       , 170, true)
+    regSetInt('walls.color.b'       , 116, true)
+    regSetInt('walls.color.a'       , 255, true)
+    regSetString('walls.texture'    , "soaked", true)
+    regSetInt('walls.textureOpacity', 100, true)
+    regSetInt('walls.reflective'    , 0, true)
+    regSetInt('walls.roughness'     , 0, true)
+    regSetInt('walls.metallic'      , 0, true)
+    regSetInt('walls.emissive'      , 0, true)
+    regSetBool('walls.transparent'  , false, true)
+    regSetInt('walls.thickness'     , 4, true)
+    regSetInt('walls.height'        , 40, true)
 
     -- Ceiling
-    regSetBool('ceiling.unbreakable'  , true)
-    regSetString('ceiling.material'   , "glass")
-    regSetInt('ceiling.color.r'       , 0)
-    regSetInt('ceiling.color.g'       , 0)
-    regSetInt('ceiling.color.b'       , 0)
-    regSetInt('ceiling.color.a'       , 255)
-    regSetString('ceiling.texture'    , "none")
-    regSetInt('ceiling.textureOpacity', 100)
-    regSetInt('ceiling.reflective'    , 0)
-    regSetInt('ceiling.roughness'     , 0)
-    regSetInt('ceiling.metallic'      , 0)
-    regSetInt('ceiling.emissive'      , 0)
-    regSetBool('ceiling.transparent'  , true)
-    regSetInt('ceiling.thickness'     , 4)
+    regSetBool('ceiling.unbreakable'  , true, true)
+    regSetString('ceiling.material'   , "glass", true)
+    regSetInt('ceiling.color.r'       , 0, true)
+    regSetInt('ceiling.color.g'       , 0, true)
+    regSetInt('ceiling.color.b'       , 0, true)
+    regSetInt('ceiling.color.a'       , 255, true)
+    regSetString('ceiling.texture'    , "none", true)
+    regSetInt('ceiling.textureOpacity', 100, true)
+    regSetInt('ceiling.reflective'    , 0, true)
+    regSetInt('ceiling.roughness'     , 0, true)
+    regSetInt('ceiling.metallic'      , 0, true)
+    regSetInt('ceiling.emissive'      , 0, true)
+    regSetBool('ceiling.transparent'  , true, true)
+    regSetInt('ceiling.thickness'     , 4, true)
 
     -- Objectives
-    regSetBool('objective.distance'       , false)
-    regSetBool('objective.level'          , false)
-    regSetBool('objective.treasures'      , false)
-    regSetBool('objective.survive'        , false)
-    regSetBool('objective.survive.fire'   , true)
-    regSetBool('objective.survive.guns'   , true)
-    regSetBool('objective.survive.rockets', true)
-    regSetInt('objective.survive.popMult' , 1)
+    regSetBool('objective.distance'       , false, true)
+    regSetBool('objective.level'          , false, true)
+    regSetBool('objective.treasures'      , false, true)
+    regSetBool('objective.survive'        , false, true)
+    regSetBool('objective.survive.fire'   , true, true)
+    regSetBool('objective.survive.guns'   , true, true)
+    regSetBool('objective.survive.rockets', true, true)
+    regSetInt('objective.survive.popMult' , 1, true)
...
# Migration Report: scripts\sound.lua
...
```diff
--- original/scripts\sound.lua
+++ patched/scripts\sound.lua
@@ -1,3 +1,4 @@
+#version 2
 function initSounds()
...
# Migration Report: scripts\spawnchunks.lua
...
```diff
--- original/scripts\spawnchunks.lua
+++ patched/scripts\spawnchunks.lua
@@ -1,90 +1,9 @@
-#include "debug.lua"
...
-
-WorldGen = {}
-WorldGen.directions = {"north", "east", "south", "west"}
--- List of chunk positions where light beams should be drawn
-WorldGen.lightBeams = {}
-
-function init()
-    checkRegInitialized()
...
-    initDebug()
-    initSounds()
-    -- Speed of chunks generation
-    WorldGen.bufferSpeed = 0
-    WorldGen.maxBufferSpeed = 0.02
-    -- Chunks are squares of the given chunkSize with unlimited vertical size
-    WorldGen.chunkSizeVoxels = regGetInt('world.chunkSize')
-    -- Registry value is in number of voxels, convert it to world length unit
-    WorldGen.chunkSize = WorldGen.chunkSizeVoxels * 0.1
-    -- Number of cnunks to render around the player
-    WorldGen.renderDistance = regGetInt('world.renderDistance')
-    -- Current chunk containing the player
-    WorldGen.chunkPos = Vec()
-    -- List of chunks discovered
-    WorldGen.chunkIndexes = {}
-    for i=-10, 10 do
-        WorldGen.chunkIndexes[i] = {}
-        for j=-10, 10 do
-            WorldGen.chunkIndexes[i][j] = false
-        end
-    end
-    -- Reverse mapping for texture indexes (index starts at 0)
-    WorldGen.texturesIndex = {}
-    for k,v in pairs(textures) do
-        WorldGen.texturesIndex[v]=k-1
-    end
-    -- Light beams on each level entrance
-    WorldGen.showLightBeams = regGetBool('world.lightBeams')
-    WorldGen.lightBeamSprite = LoadSprite("MOD/img/line.png")
-    -- Initialize the labyrinth generator
-    Labyrinth.Init()
...
-function tick()
-
-    local pos = AabbGetPlayerCenter()
-    WorldGen.chunkPos = GetChunckPos(pos)
-    -- Display debug infos
-    debugMod()
-    -- Prepare future labyrinth segments
-    Labyrinth.Generate()
-    -- Manage objectives
-    Objectives.Run()
-    -- Render chunks inside render distance
-    RenderWorld()
-    -- Stop rendering chunck outside render distance
-    CleanupWorld()
-    -- Draw light beams if requested
-    DrawLightBeams()
-
-end
-
...
-    if WorldGen.bufferSpeed > 0 then
+    if WorldGen.bufferSpeed ~= 0 then
         WorldGen.bufferSpeed = WorldGen.bufferSpeed - 1 * dt
     else
         WorldGen.bufferSpeed = WorldGen.maxBufferSpeed
@@ -95,7 +14,6 @@
...
-
     ...
     --A very complicated way of making a spiral
     for i=1, spiralLimit + 1 do
@@ -137,7 +55,6 @@
     end
 end
 
--- Return the chunk index coordinates of a given world coordinate
 function GetChunckPos(worldPos)
     return Vec(floatToInt(worldPos[1] / WorldGen.chunkSize), 0, floatToInt(worldPos[3] / WorldGen.chunkSize))
 end
@@ -199,7 +116,7 @@
 
 function RenderFloor(inputX, inputY)
     local material = regGetString('floor.material')
-    regSetBool('floor.transparent', material == "glass")
+    regSetBool('floor.transparent', material == "glass", true)
     local r = regGetFloat('floor.color.r')/255
     local g = regGetFloat('floor.color.g')/255
     local b = regGetFloat('floor.color.b')/255
@@ -230,7 +147,7 @@
 
 function RenderWalls(inputX, inputY, wallData)
     local material = regGetString('walls.material')
-    regSetBool('walls.transparent', material == "glass")
+    regSetBool('walls.transparent', material == "glass", true)
     local r = regGetFloat('walls.color.r')/255
     local g = regGetFloat('walls.color.g')/255
     local b = regGetFloat('walls.color.b')/255
@@ -296,7 +213,7 @@
 
 function RenderCeiling(inputX, inputY)
     local material = regGetString('ceiling.material')
-    regSetBool('ceiling.transparent', material == "glass")
+    regSetBool('ceiling.transparent', material == "glass", true)
     local r = regGetFloat('ceiling.color.r')/255
     local g = regGetFloat('ceiling.color.g')/255
     local b = regGetFloat('ceiling.color.b')/255
@@ -345,7 +262,6 @@
     end
...
@@ -410,7 +326,7 @@
     ...
     -- For vehicles, delete the body instead to avoid hanging wheels
-    if vehicle > 0 then
+    if vehicle ~= 0 then
         if shapeBody == vehicleMainBody then
             Delete(shapeBody)
             Delete(shape)
@@ -444,4 +360,65 @@
         DrawSprite(WorldGen.lightBeamSprite, spriteTransform, WorldGen.chunkSize * 5, beamHeight, 1, 1, 1, alpha, true, false)
     end
...
-        do UiPush()
-
-            ui.padding.create(0, 20)
-
-            do UiPush()
-
-                ui.slider.create('Chunk size', 'world.chunkSize', 'Voxels', 10, 512, nil, nil, nil, nil, true)
-                ui.padding.create(0, 90)
-
-                ui.slider.create('Render distance', 'world.renderDistance', 'Chunks', 1, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 90)
-
-                ui.checkBox.create('Despwan chunks out of render distance', 'world.despawnChunks')
-                ui.padding.create(0, 90)
-
-                ui.slider.create('Generate', 'world.generate', '', nil, nil, nil, nil, nil, nil, nil, {"floor + walls", "floor + walls + ceiling"})
-                ui.padding.create(0, 90)
-
-            UiPop() end
-
...
-
-                ui.checkBox.create('Debug Mode', 'world.debugMode')
-                ui.padding.create(0, 90)
-
-                ui.textInput.create('Seed', 'world.seed', '', nil, nil, nil, nil, "int")
-                ui.padding.create(0, 60)
-                UiFont('regular.ttf', font_normal)
-                UiColor(1, 0.6, 0.6, 1)
-                UiAlign('left middle')
-                UiText("CAUTION: Changing seed will clear cache and restart generation from scratch")
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Generation steps per tick', 'world.stepsPerTick', '', 1, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 40)
-                UiFont('regular.ttf', font_normal)
-                UiColor(0.6, 0.6, 0.6, 1)
-                UiAlign('left middle')
-                UiText("Note: Reduce this value if you notice frame drops")
-                ui.padding.create(0, 50)
-
-                ui.checkBox.create('Show light beams (at entrance of each level)', 'world.lightBeams')
-                ui.padding.create(0, 90)
-
-            UiPop() end
-
...
-        do UiPush()
-
-            ui.slider.create('Material (physics and sound)', 'floor.material', '', nil, nil, nil, nil, nil, nil, nil, materials)
-            ui.padding.create(0, 50)
-
-            ui.colorPicker.create('Color', 'floor.color', nil, nil, nil, false)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Texture (visual and sound)', 'floor.texture', '', nil, nil, nil, nil, nil, nil, nil, textures)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Texture opacity', 'floor.textureOpacity', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Reflective', 'floor.reflective', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Roughness (reflection blur)', 'floor.roughness', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Metallic (color tint)', 'floor.metallic', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Emissive (glow)', 'floor.emissive', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-
-        UiPop() end
-
...
-            ui.padding.create(800, 0)
-
-            ui.checkBox.create('Unbreakable', 'floor.unbreakable')
-            ui.padding.create(0, 70)
-
-            ui.slider.create('Thickness', 'floor.thickness', 'Voxels', 1, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 50)
-           
-        UiPop() end
-        
...
-        do UiPush()
-
-            ui.slider.create('Material (physics and sound)', 'walls.material', '', nil, nil, nil, nil, nil, nil, nil, materials)
-            ui.padding.create(0, 50)
-
-            ui.colorPicker.create('Color', 'walls.color', nil, nil, nil, false)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Texture (visual and sound)', 'walls.texture', '', nil, nil, nil, nil, nil, nil, nil, textures)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Texture opacity', 'walls.textureOpacity', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Reflective', 'walls.reflective', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Roughness (reflection blur)', 'walls.roughness', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Metallic (color tint)', 'walls.metallic', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Emissive (glow)', 'walls.emissive', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-
-        UiPop() end
-
...
-            ui.padding.create(800, 0)
-
-            ui.checkBox.create('Unbreakable', 'walls.unbreakable')
-            ui.padding.create(0, 70)
-
-            ui.slider.create('Thickness', 'walls.thickness', 'Voxels', 1, 10, nil, nil, nil, nil, true)
-            ui.padding.create(0, 50)
-
-            ui.slider.create('Height', 'walls.height', 'Voxels', 1, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 50)
-
-        UiPop() end
-        
...
-        do UiPush()
-
-            ui.slider.create('Material (physics and sound)', 'ceiling.material', '', nil, nil, nil, nil, nil, nil, nil, materials)
-            ui.padding.create(0, 50)
-
-            ui.colorPicker.create('Color', 'ceiling.color', nil, nil, nil, false)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Texture (visual and sound)', 'ceiling.texture', '', nil, nil, nil, nil, nil, nil, nil, textures)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Texture opacity', 'ceiling.textureOpacity', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Reflective', 'ceiling.reflective', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Roughness (reflection blur)', 'ceiling.roughness', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Metallic (color tint)', 'ceiling.metallic', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Emissive (glow)', 'ceiling.emissive', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-
-        UiPop() end
-
...
-            ui.padding.create(800, 0)
-
-            ui.checkBox.create('Unbreakable', 'ceiling.unbreakable')
-            ui.padding.create(0, 70)
-
-            ui.slider.create('Thickness', 'ceiling.thickness', 'Voxels', 1, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 50)
-           
-        UiPop() end
-        
...
-            UiAlign('left middle')
-
-            ui.checkBox.create('Escape: Walk as far away from the center as you can!', 'objective.distance')
-            ui.padding.create(0, 50)
-            UiFont('regular.ttf', font_normal)
-            UiColor(0.6, 0.6, 0.6, 1)
-            UiText("Note: Distance explored only count if you are inside the labyrinth (not flying over)")
-            ui.padding.create(0, 50)
-
-            ui.checkBox.create('Complete: Complete the maximum possible maze levels!', 'objective.level')
-            ui.padding.create(0, 50)
-            UiFont('regular.ttf', font_normal)
-            UiColor(0.6, 0.6, 0.6, 1)
-            UiText("Note: Each level is 9 times larger than the previous one")
-            ui.padding.create(0, 50)
-
-            ui.checkBox.create('Explore: Collect as many treasures as you can!', 'objective.treasures')
-            ui.padding.create(0, 50)
-            UiFont('regular.ttf', font_normal)
-            UiColor(0.6, 0.6, 0.6, 1)
-            UiText("Note: Treasures will randomly spawn in dead-ends of the labyrinth")
-            ui.padding.create(0, 50)
-
-            ui.checkBox.create('Survive: Stay alive as long as you can!', 'objective.survive')
-            ui.padding.create(0, 50)
-            UiFont('regular.ttf', font_normal)
-            UiColor(0.6, 0.6, 0.6, 1)
-            UiText("Note: Robots will spawn in the labyrinth and attack you")
-            ui.padding.create(0, 50)
-
-        UiPop() end
-
-        do UiPush()
-
-            ui.padding.create(800, 380)
-
-            do UiPush()
-                do UiPush()
-                    ui.checkBox.create('Flamethrower robots', 'objective.survive.fire')
-                UiPop() end
-                ui.padding.create(300, 0)
-                do UiPush()
-                    ui.checkBox.create('Minigun robots', 'objective.survive.guns')
-                UiPop() end
-                ui.padding.create(300, 0)
-                do UiPush()
-                    ui.checkBox.create('Rocket robots', 'objective.survive.rockets')
-                UiPop() end
-            UiPop() end
-            ui.padding.create(0, 100)
-            
-            ui.slider.create('Max robots population', 'objective.survive.popMult', 'x 6', 1, 10, nil, nil, nil, nil, true)
-            ui.padding.create(0, 40)
-            UiAlign('left middle')
-            UiFont('regular.ttf', font_normal)
-            UiColor(1, 0.6, 0.6, 1)
-            UiText("CAUTION: Too many robots will lag your game")
-            ui.padding.create(0, 60)
-
-        UiPop() end
-        
...
-}
-
--- Draw key hint on currently hovered treasure
 function uiDrawTreasureHint()
     if Objectives.TreasureHovered() then
         UiPush()
@@ -491,40 +216,15 @@
     end
 end
...
-
---[[TRANSFORM]]
-    function TransformRotateAround(tr, center, rot)
+
+function VecResize(vec, size) return VecScale(VecNormalize(vec), size) end
+
+function VecProjToVecLen(vec1, vec2) return VecDot(vec1, vec2) / VecLength(vec2) end
...
+
+function TransformRotateAround(tr, center, rot)
         local centerTrBefore = Transform(center, tr.rot)
         local centerTrAfter = Transform(center, rot)
         local localTr = TransformToLocalTransform(centerTrBefore, tr)
         return TransformToParentTransform(centerTrAfter, localTr)
     end
-    function TransformMoveCenterTo(tr, center, pos)
+
+function TransformMoveCenterTo(tr, center, pos)
         local centerTrBefore = Transform(center, tr.rot)
         local centerTrAfter = Transform(pos, tr.rot)
         local localTr = TransformToLocalTransform(centerTrBefore, tr)
@@ -66,9 +70,7 @@
         return newTr
     end
