...
-	RegisterTool("rmw92fs", "92FS", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmw92fs.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmw92fs", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
@@ -236,7 +145,7 @@
 			if GetString("game.player.tool") == "rmw92fs" then --- When Zero then its empty
 			
 			local optic = opticssight[10]
-                if charginghandletime > 0 then ---Charginghandle
+                if charginghandletime ~= 0 then ---Charginghandle
 					SetShapeLocalTransform(optic, Transform(Vec(-0.025, 0.0905, -0.18), QuatEuler(-90, 0, 0)))
 				else
 					SetShapeLocalTransform(optic, Transform(Vec(-0.025, 0.0905, -0.28), QuatEuler(-90, 0, 0)))
@@ -461,7 +370,7 @@
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmw92fs" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmw92fs", "92FS", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmw92fs.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
