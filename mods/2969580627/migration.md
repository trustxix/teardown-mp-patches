# Migration Report: corrupted_crystal\scripts\corrupted_crystal.lua
...
```diff
--- original/corrupted_crystal\scripts\corrupted_crystal.lua
+++ patched/corrupted_crystal\scripts\corrupted_crystal.lua
@@ -1,150 +1,4 @@
...
-
-function init()
-    local cci = CorruptedCrystal_info[1]
-    checkRegInitialized()
-    -- Gather information required for the script to work from body tags
-    cci.body = FindBody("corruptedCrystalBody")
-    cci.shape = FindShape("corruptedCrystalShape")
-    -- We know the script is the parent of the body in prefab
-    cci.script = cci.body-1
-    if not CorruptedCrystal.isManagerStarted() then
-        CorruptedCrystal.startManager()
-    end
-    if cci.body == 0 then
-        CorruptedCrystal.abortInitWithError("corruptedCrystalBody")
-        return
-    end
-    cci.voxSize = tonumber(GetTagValue(cci.body, "size"))
-    if cci.voxSize == nil then
-        CorruptedCrystal.abortInitWithError("size")
-        return
-    end
-    cci.material = GetTagValue(cci.body, "material")
-    if cci.material == "" then
-        CorruptedCrystal.abortInitWithError("material")
-        return
-    end
-    cci.size = cci.voxSize*0.1
-    cci.diagonal2D = cci.size * math.sqrt(2)
-    cci.diagonal3D = cci.size * math.sqrt(3)
-    cci.color = strToNumberTable(GetTagValue(cci.body, "color"))
-    if cci.color == nil or #cci.color ~= 4 then
-        CorruptedCrystal.abortInitWithError("color")
-        return
-    end
-    local textureAndOpacity = strToNumberTable(GetTagValue(cci.body, "texture"))
-    if textureAndOpacity == nil or #textureAndOpacity ~= 2 then
-        CorruptedCrystal.abortInitWithError("texture")
-        return
-    end
-    cci.textureIndex = textureAndOpacity[1]
-    cci.textureOpacity = textureAndOpacity[2]
-    local pbr = strToNumberTable(GetTagValue(cci.body, "pbr"))
-    if pbr == nil or #pbr ~= 4 then
-        CorruptedCrystal.abortInitWithError("pbr")
-        return
-    end
-    cci.reflective = pbr[1]
-    cci.roughness = pbr[2]
-    cci.metallic = pbr[3]
-    cci.emissive = pbr[4]
-    if HasTag(cci.body, "coreDistance") then
-        cci.coreDistance = tonumber(GetTagValue(cci.body, "coreDistance"))
-        if cci.coreDistance == nil then
-            CorruptedCrystal.abortInitWithError("coreDistance")
-            return
-        end
-    end
-    if HasTag(cci.body, "firstShape") then
-        cci.firstShape = tonumber(GetTagValue(cci.body, "firstShape"))
-        if cci.firstShape == nil then
-            CorruptedCrystal.abortInitWithError("firstShape")
-            return
-        end
-    end
-    cci.unbreakable = HasTag(cci.body, "unbreakable")
-    cci.explosive = HasTag(cci.body, "explosive")
-    -- For core crystal, override some settings with those from the options
-    if cci.coreDistance == 0 then
-        cci.unbreakable = regGetBool('crystal.unbreakable')
-        if cci.unbreakable then
-            SetTag(cci.body, "unbreakable")
-        else
-            RemoveTag(cci.body, "unbreakable")
-        end
-        cci.explosive = regGetBool('crystal.explosive')
-        if cci.explosive then
-            local explosionRadius = cci.size/2
-            SetTag(cci.body, "explosive", ""..explosionRadius)
-        else
-            RemoveTag(cci.body, "explosive")
-        end
-    end
-    cci.center = AabbGetBodyCenterPos(cci.body)
-    -- Init registry-based values
-    CorruptedCrystal.startGrowTimer()
-    cci.airChances = regGetInt('crystal.grow.airChances')
-    -- If crstal spawned with growComplete then it came from a fustion, so check if it should merge again
-    if CorruptedCrystal.isGrowthComplete() then
-        cci.wasMerged = true
-        CorruptedCrystal.setGrowthComplete(false)
-    end
-    -- Check for conversion
-    if regGetBool('crystal.convertCovered') or regGetBool('crystal.convertTouched') then
-        -- Check if any of the shapes inside the crystal is fully covered / touching
-        CorruptedCrystal.tryConvert()
-    end
-    -- Ignite nearby flammable objects
-    CorruptedCrystal.ignite()
-end
-
+#version 2
 function CorruptedCrystal.abortInitWithError(tagMissing)
     local cci = CorruptedCrystal_info[1]
     local oldDb = db
@@ -153,29 +7,6 @@
     db = oldDb
     cci.broken = true
     CorruptedCrystal.onCrystalBroken()
-end
-
-function tick()
-    debugMod()
-    CorruptedCrystal.checkIntegrity()
-    local ret = CorruptedCrystal.grow()
-    -- If we returned because grow end condition reached
-    if ret == 1 then
-        CorruptedCrystal.onGrowthComplete()
-    -- If we returned because spawn position was refused
-    elseif ret == 2 then
-        CorruptedCrystal.speedUpGrowth()
-    end
-end
-
-function update()
...
-function draw()
-    if db then
-        uiDrawCrystalDebugInfo()
-    end
-    uiDrawCrystalHint()
 end
 
 function CorruptedCrystal.checkIntegrity()
@@ -256,7 +87,7 @@
     end
     -- Only grow when timer is elapsed
     cci.growTimer = cci.growTimer - GetTimeStep()
-    if cci.growTimer > 0 then
+    if cci.growTimer ~= 0 then
         return 0
     end
     CorruptedCrystal.startGrowTimer()
@@ -284,7 +115,7 @@
     local growDir = cci.growDirs[rdm(1,#cci.growDirs)]
     -- Apply angle to grow direction if needed
     local directionOffset = rdmf(regGetFloat("crystal.grow.dirMin")/2, regGetFloat("crystal.grow.dirMax")/2)
-    if directionOffset > 0 then
+    if directionOffset ~= 0 then
         -- Take any vector orthogonal to growDir
         local tmp = Vec(1, 1, 1)
         local aim = VecCross(growDir, tmp)
@@ -467,7 +298,7 @@
     -- so we use tool.lua for that
     local cci = CorruptedCrystal_info[1]
     local scriptId = "crystal.scripts."..cci.script
-    regSetBool(scriptId, true)
+    regSetBool(scriptId, true, true)
 end
 
 function CorruptedCrystal.dissolve(pos, radius, protectSelf)
@@ -535,7 +366,7 @@
     local cci = CorruptedCrystal_info[1]
     if regGetBool('crystal.grow.fastRetry') then
         local maxAttempts = regGetInt("crystal.grow.stopAttempts")
...
@@ -784,7 +615,7 @@
     -- Mark current shape for crystal converstion
     -- This is all centralized into a manager script to avoid freezes on very large shapes
     local shapeId = "crystal.shapes."..shape
-    regSetBool(shapeId, true)
+    regSetBool(shapeId, true, true)
     -- Save conversion data in shape tags
     SetTag(shape, "next_material", cci.material)
     SetTag(shape, "next_color", cci.color[1]..','..cci.color[2]..','..cci.color[3]..','..cci.color[4])
@@ -822,8 +653,127 @@
                             </group> \
                         </prefab>'
     local entities = Spawn(xmlString, Transform())
-    regSetInt("crystal.scriptManager", entities[1])
-end
-
-UpdateQuickloadPatch()
-
+    regSetInt("crystal.scriptManager", entities[1], true)
+end
+
...
+#version 2
 function initDebug()
     db = regGetBool('crystal.debugMode')
 end
...
-manager_info = {
-    {
-        maxTickOperations = 10000, --1000000,
-        tickOperations = 0,
-        shape = 0,
-        material = nil,
-        color = {},
...
-    -- Scripts list
-    regClearKey("crystal.scripts")
-    -- Shapes to convert list
-    regClearKey("crystal.shapes")
-end
-
-function tick()
-    manager.cleanupScripts()
-    manager.convertShapes()
-end
-
-
+#version 2
 function manager.cleanupScripts()
     local keys = regListKeys("crystal.scripts")
     for i=1,#keys do
@@ -56,7 +16,7 @@
         manager.clearShape()
     end
     mai.tickOperations = 0
-    if mai.shape > 0 then
+    if mai.shape ~= 0 then
         -- Keep converting shape in progress if any
         local res = manager.convert()
         if not res then
@@ -211,4 +171,16 @@
     return material, color, textureIndex, textureOpacity, reflective, roughness, metallic, emissive, unbreakable, explosive
 end
...
+#version 2
 function modReset()
     ...
     -- Global parameters
-    regSetBool('crystal.debugMode'    , false)
-    regSetBool('crystal.soundOn'      , true)
+    regSetBool('crystal.debugMode'    , false, true)
+    regSetBool('crystal.soundOn'      , true, true)
 
     -- Physical characteristics and appearance
-    regSetBool('crystal.unbreakable'  , false)
-    regSetBool('crystal.explosive'    , false)
-    regSetBool('crystal.igniting'     , false)
-    regSetBool('crystal.mimetic'      , false)
-    regSetInt('crystal.size'          , 8)
-    regSetString('crystal.material'   , "glass")
-    regSetInt('crystal.color.r'       , 87)
-    regSetInt('crystal.color.g'       , 24)
-    regSetInt('crystal.color.b'       , 134)
-    regSetString('crystal.texture'    , "none")
-    regSetInt('crystal.textureOpacity', 0)
-    regSetInt('crystal.reflective'    , 0)
-    regSetInt('crystal.roughness'     , 0)
-    regSetInt('crystal.metallic'      , 0)
-    regSetInt('crystal.emissive'      , 5)
-    regSetInt('crystal.light.brightness'      , 0)
-    regSetFloat('crystal.light.sizeMultiplier', 2.0)
-    regSetString('crystal.loadSelected'    , "none")
+    regSetBool('crystal.unbreakable'  , false, true)
+    regSetBool('crystal.explosive'    , false, true)
+    regSetBool('crystal.igniting'     , false, true)
+    regSetBool('crystal.mimetic'      , false, true)
+    regSetInt('crystal.size'          , 8, true)
+    regSetString('crystal.material'   , "glass", true)
+    regSetInt('crystal.color.r'       , 87, true)
+    regSetInt('crystal.color.g'       , 24, true)
+    regSetInt('crystal.color.b'       , 134, true)
+    regSetString('crystal.texture'    , "none", true)
+    regSetInt('crystal.textureOpacity', 0, true)
+    regSetInt('crystal.reflective'    , 0, true)
+    regSetInt('crystal.roughness'     , 0, true)
+    regSetInt('crystal.metallic'      , 0, true)
+    regSetInt('crystal.emissive'      , 5, true)
+    regSetInt('crystal.light.brightness'      , 0, true)
+    regSetFloat('crystal.light.sizeMultiplier', 2.0, true)
+    regSetString('crystal.loadSelected'    , "none", true)
     local savedCrystalSkins = regListKeys('crystal.saved')
-    if #savedCrystalSkins > 0 then
-        regSetString('crystal.loadSelected', savedCrystalSkins[1])
+    if #savedCrystalSkins ~= 0 then
+        regSetString('crystal.loadSelected', savedCrystalSkins[1], true)
     end
-    regSetString('crystal.saveName'        , "")
+    regSetString('crystal.saveName'        , "", true)
 
     -- Growth parameters
-    regSetFloat('crystal.grow.delayMin'  , 0.1)
-    regSetFloat('crystal.grow.delayMax'  , 0.3)
-    regSetInt('crystal.grow.dirMin'      , 0)
-    regSetInt('crystal.grow.dirMax'      , 90)
-    regSetInt('crystal.grow.angleMin'    , 0)
-    regSetInt('crystal.grow.angleMax'    , 90)
-    regSetInt('crystal.grow.stopSuccess' , 2)
-    regSetInt('crystal.grow.stopAttempts', 30)
-    regSetInt('crystal.grow.airChances'  , 0)
-    regSetInt('crystal.grow.airReduction', 0)
-    regSetInt('crystal.grow.coreDistance', 100)
-    regSetBool('crystal.grow.fastRetry'  , false) 
-    regSetBool('crystal.grow.exponential', false) 
-    regSetBool('crystal.grow.corral'     , false)
-    regSetBool('crystal.grow.firstShape' , false) 
-    regSetBool('crystal.dissolveOnGrow'  , false)
-    regSetBool('crystal.breakOnStop'     , false)
-    regSetBool('crystal.dissolveOnBreak' , false)
-    regSetBool('crystal.dissolveProtect' , true)
-    regSetBool('crystal.fusion'          , false)
-    regSetInt('crystal.fusionCount'      , 8)
-    regSetBool('crystal.convertCovered'  , false)
-    regSetBool('crystal.convertTouched'  , false)
+    regSetFloat('crystal.grow.delayMin'  , 0.1, true)
+    regSetFloat('crystal.grow.delayMax'  , 0.3, true)
+    regSetInt('crystal.grow.dirMin'      , 0, true)
+    regSetInt('crystal.grow.dirMax'      , 90, true)
+    regSetInt('crystal.grow.angleMin'    , 0, true)
+    regSetInt('crystal.grow.angleMax'    , 90, true)
+    regSetInt('crystal.grow.stopSuccess' , 2, true)
+    regSetInt('crystal.grow.stopAttempts', 30, true)
+    regSetInt('crystal.grow.airChances'  , 0, true)
+    regSetInt('crystal.grow.airReduction', 0, true)
+    regSetInt('crystal.grow.coreDistance', 100, true)
+    regSetBool('crystal.grow.fastRetry'  , false, true) 
+    regSetBool('crystal.grow.exponential', false, true) 
+    regSetBool('crystal.grow.corral'     , false, true)
+    regSetBool('crystal.grow.firstShape' , false, true) 
+    regSetBool('crystal.dissolveOnGrow'  , false, true)
+    regSetBool('crystal.breakOnStop'     , false, true)
+    regSetBool('crystal.dissolveOnBreak' , false, true)
+    regSetBool('crystal.dissolveProtect' , true, true)
+    regSetBool('crystal.fusion'          , false, true)
+    regSetInt('crystal.fusionCount'      , 8, true)
+    regSetBool('crystal.convertCovered'  , false, true)
+    regSetBool('crystal.convertTouched'  , false, true)
 
     -- Key bindings
     regInitKey('key.place'  , "lmb")
@@ -60,7 +61,7 @@
     regInitKey('key.resume' , "n")
     regInitKey('key.remove' , "k")
     regInitKey('key.options', "o")
-    regSetBool('crystal.globalKeys', false) 
+    regSetBool('crystal.globalKeys', false, true) 
...
-        spawnTr = Transform(),
-        crystalShapes = {},
-        crystalShape2CenterTr = nil,
-        spawnedCrystalBody1 = nil,
-        spawnedCrystalBody2 = nil,
-        angle = 0,
-        angleVel = 100,
-        toolSize = 4,
-        placeSound = LoadSound("glass/land0"),
-        texturesIndex = {},
-        setup = {
-            name = 'corrupted_crystal',
-            title = 'Corrupted Crystal',
-            voxPath = 'MOD/vox/crystal_tool.vox',
-        },
-        updateColor = true,
-    }
-}
...
-tool.input = {}
+#version 2
 function tool.input.didPlace()
     return InputPressed(regGetString('key.place'))
 end
+
 function tool.input.didStop()
     return InputPressed(regGetString('key.stop'))
 end
+
 function tool.input.didResume()
     return InputPressed(regGetString('key.resume'))
 end
+
...
+    SetBool('game.tool.'..ti.setup.name..'.enabled', true, true)
 ...
 function tool.run()
@@ -210,7 +187,7 @@
     -- Makes sure spawned crustals stick to the central ones
     if ti.spawnedCrystalBody1 and ti.spawnedCrystalBody2 then
         local radius = ((ti.toolSize * 0.1) / 2) - 0.025
-        local vel = VecScale(GetPlayerVelocity(), GetTimeStep() * 2)
+        local vel = VecScale(GetPlayerVelocity(playerId), GetTimeStep() * 2)
         local tr1 = TransformToParentTransform(GetBodyTransform(GetShapeBody(ti.crystalShapes[1])), GetShapeLocalTransform(ti.crystalShapes[1]))
         tr1.pos = VecAdd(TransformToParentPoint(tr1, Vec(-radius, -radius, -radius)), vel)
         local tr2 = TransformToParentTransform(GetBodyTransform(GetShapeBody(ti.crystalShapes[2])), GetShapeLocalTransform(ti.crystalShapes[2]))
@@ -219,7 +196,6 @@
         SetBodyTransform(ti.spawnedCrystalBody2, tr2)
     end
 end
-
 
 function tool.placeCrystal()
     local ti = tool_info[1]
@@ -291,11 +267,10 @@
     </prefab>'
     local entities = Spawn(xmlString, spawnTr, true, true)
     -- Play sound
-    if #entities > 0 then
+    if #entities ~= 0 then
         PlaySound(ti.placeSound, GetCameraTransform().pos, 0.3)
     end
 end
-
 
 function tool.stopGrowth()
     local bodies = FindBodies("corruptedCrystalBody", true)
@@ -373,4 +348,5 @@
         end
     end
     return body
...
---1, 3, 4, 8, 9, 14
---none / rock / concrete / brick / heavymetal
...
-            do UiPush()
-
-                local nameToLoad = regGetString('crystal.loadSelected')
-                local savedCrystalSkins = regListKeys('crystal.saved')
-                local hasSavedCrystalSkins = #savedCrystalSkins > 0
-                -- Makes sure table is never empty (slider doesn't like empty tables)
-                if not hasSavedCrystalSkins then
-                    savedCrystalSkins = {"none"}
-                end
-                -- Makes sure currently selected name is always part of the list
-                if TableFind(savedCrystalSkins, nameToLoad) == nil then
-                    regSetString('crystal.loadSelected', savedCrystalSkins[1])
-                end
-                ui.slider.create('Saved Crystal Skins', 'crystal.loadSelected', '', nil, nil, nil, nil, nil, nil, nil, savedCrystalSkins)
-                nameToLoad = regGetString('crystal.loadSelected')
-                ui.padding.create(0, 50)
-
-                do UiPush()
-
-                    local loadButton = ui.button.create('Load', whiteColor, whiteColor, nil, nil, nil, nil, not hasSavedCrystalSkins)
-                    if loadButton then
-                        regSetString('crystal.material', regGetString('crystal.saved.'..nameToLoad..'.material'))
-                        regSetInt('crystal.color.r', regGetInt('crystal.saved.'..nameToLoad..'.color.r'))
-                        regSetInt('crystal.color.g', regGetInt('crystal.saved.'..nameToLoad..'.color.g'))
-                        regSetInt('crystal.color.b', regGetInt('crystal.saved.'..nameToLoad..'.color.b'))
-                        regSetString('crystal.texture', regGetString('crystal.saved.'..nameToLoad..'.texture'))
-                        regSetInt('crystal.textureOpacity', regGetInt('crystal.saved.'..nameToLoad..'.textureOpacity'))
-                        regSetInt('crystal.reflective', regGetInt('crystal.saved.'..nameToLoad..'.reflective'))
-                        regSetInt('crystal.roughness', regGetInt('crystal.saved.'..nameToLoad..'.roughness'))
-                        regSetInt('crystal.metallic', regGetInt('crystal.saved.'..nameToLoad..'.metallic'))
-                        regSetInt('crystal.emissive', regGetInt('crystal.saved.'..nameToLoad..'.emissive'))
-                    end
-                    ui.padding.create(ui.button.lastWidth + padW, 0)
-
-                    local deleteButton = ui.button.create('Delete', whiteColor, whiteColor, nil, nil, nil, nil, not hasSavedCrystalSkins)
-                    if deleteButton then
-                        regClearKey('crystal.saved.'..nameToLoad)
-                    end
-                    ui.padding.create(ui.button.lastWidth + padW, 0)
-
-                    local saveName = regGetString('crystal.saveName')
-                    local saveButton = ui.button.create('Save as', whiteColor, whiteColor, nil, nil, nil, nil, string.len(saveName) == 0)
-                    if saveButton then
-                        regSetString('crystal.saved.'..saveName..'.material', regGetString('crystal.material'))
-                        regSetInt('crystal.saved.'..saveName..'.color.r', regGetInt('crystal.color.r'))
-                        regSetInt('crystal.saved.'..saveName..'.color.g', regGetInt('crystal.color.g'))
-                        regSetInt('crystal.saved.'..saveName..'.color.b', regGetInt('crystal.color.b'))
-                        regSetString('crystal.saved.'..saveName..'.texture', regGetString('crystal.texture'))
-                        regSetInt('crystal.saved.'..saveName..'.textureOpacity', regGetInt('crystal.textureOpacity'))
-                        regSetInt('crystal.saved.'..saveName..'.reflective', regGetInt('crystal.reflective'))
-                        regSetInt('crystal.saved.'..saveName..'.roughness', regGetInt('crystal.roughness'))
-                        regSetInt('crystal.saved.'..saveName..'.metallic', regGetInt('crystal.metallic'))
-                        regSetInt('crystal.saved.'..saveName..'.emissive', regGetInt('crystal.emissive'))
-                    end
-                    ui.padding.create(ui.button.lastWidth + padW, -20)
-
-                UiPop() end
-                ui.padding.create(0, 34)
-                ui.lineEdit.create('', 'crystal.saveName', nil, nil, true)
-                ui.padding.create(0, 64)
-
-                ui.slider.create('Size', 'crystal.size', 'Vox', 1, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                UiFont('regular.ttf', font_normal)
-                UiAlign('left middle')
-                UiColor(1,0.6,0.6, 1)
-                UiText('CAUTION: Crystals larger than 50 will not work well', true)
-                UiText('with dissolve or explosive, and will lag when broken', true)
-                UiText('', true)
-                ui.slider.create('Core light size multiplier', 'crystal.light.sizeMultiplier', 'x', 0, 10)
-                ui.padding.create(0, 30)
-                ui.slider.create('Core light brightness (may lag)', 'crystal.light.brightness', '', 0, 400, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                do UiPush()
-                    ui.padding.create(400, 0)
-                    ui.checkBox.create('Explosive', 'crystal.explosive')  
-                UiPop() end
-                do UiPush()
-                    ui.padding.create(200, 0)
-                    ui.checkBox.create('Igniting', 'crystal.igniting')  
-                UiPop() end
-                ui.checkBox.create('Unbreakable', 'crystal.unbreakable')
-                ui.padding.create(0, 40)
-                UiColor(1,0.6,0.6, 1)
-                UiText('CAUTION: Detonating large explosive Crystals', true)
-                UiText('will probably kill your game', true)
-                UiText('', true)
-
-            UiPop() end
-
...
-
-                ui.slider.create('Material (physics and sound)', 'crystal.material', '', nil, nil, nil, nil, nil, nil, nil, materials)
-                ui.padding.create(0, 50)
-    
-                ui.colorPicker.create('Color', 'crystal.color', nil, nil, nil, false)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Texture (visual and sound)', 'crystal.texture', '', nil, nil, nil, nil, nil, nil, nil, textures)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Texture opacity', 'crystal.textureOpacity', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Reflective', 'crystal.reflective', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Roughness (reflection blur)', 'crystal.roughness', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Metallic (color tint)', 'crystal.metallic', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Emissive (glow)', 'crystal.emissive', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-    
-            UiPop() end
-
...
-
-                ui.checkBox.create('Debug Mode', 'crystal.debugMode')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Sound', 'crystal.soundOn')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Mimetic Crystals', 'crystal.mimetic')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-            do UiPush()
-
-                ui.range_slider.create('Grow delay range', 'crystal.grow.delayMin', 'crystal.grow.delayMax', 's', 0, 5)
-                ui.padding.create(0, 40)
-                ui.range_slider.create('Grow direction range', 'crystal.grow.dirMin', 'crystal.grow.dirMax', 'deg', 0, 90, nil, nil, nil, nil, true)
-                ui.padding.create(0, 40)
-                ui.range_slider.create('Grow rotation range', 'crystal.grow.angleMin', 'crystal.grow.angleMax', 'deg', 0, 90, nil, nil, nil, nil, true)
-                ui.padding.create(0, 40)
-                ui.slider.create('Maximum grow success (100 = infinite)', 'crystal.grow.stopSuccess', 'times', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                ui.slider.create('Maximum grow attempts (100 = infinite)', 'crystal.grow.stopAttempts', 'times', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                ui.slider.create('Chances to grow away from the ground', 'crystal.grow.airChances', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                ui.slider.create('Chances reduction per succesful grow', 'crystal.grow.airReduction', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                ui.slider.create('Maximum grow distance away from core (100 = infinite)', 'crystal.grow.coreDistance', 'crystals', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-            UiPop() end
-
...
-                ui.padding.create(600, 0)
-
-                ui.checkBox.create('Retry faster when grow attempt failed', 'crystal.grow.fastRetry')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Exponential grow speed (and lag)', 'crystal.grow.exponential')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Merge groups of Crystals into a larger one', 'crystal.fusion')
-                ui.padding.create(0, 64)
-                ui.slider.create('Min amount of Crystals for merge', 'crystal.fusionCount', 'crystals', 2, 8, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                ui.checkBox.create('Turn objects covered with Crystals into crystal', 'crystal.convertCovered')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Turn objects touched by Crystals into crystal', 'crystal.convertTouched')
-                ui.padding.create(0, 40)
-                UiColor(1,0.6,0.6, 1)
-                UiText('CAUTION: Conversion of large objects into crystal', true)
-                UiText('will cause huge lag spikes and may crash', true)
-                UiText('', true)
-
-            UiPop() end
-
...
-                ui.padding.create(1200, 0)
-
-                ui.checkBox.create('Dissolve matter when growing', 'crystal.dissolveOnGrow')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Shatter when grow complete', 'crystal.breakOnStop')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Dissolve matter when shattered', 'crystal.dissolveOnBreak')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Dissolve does not affect other Crystals', 'crystal.dissolveProtect')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Grow in corral pattern', 'crystal.grow.corral')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Only grow around first object touched', 'crystal.grow.firstShape')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-            ui.padding.create(0, 50)
-            UiFont('regular.ttf', font_normal)
-            ui.keyBinding.create('Place a Crystal', 'key.place', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Stop propagation', 'key.stop', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Resume propagation', 'key.resume', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Remove all Crystals', 'key.remove', true)
-            ui.padding.create(0, 50)
...
-            UiText('', true)
-            ui.checkBox.create('Make stop/resume/remove keys always available', 'crystal.globalKeys')
-            ui.padding.create(0, 40)
-
-        UiPop() end
-
...
-            UiFont('regular.ttf', font_normal)
-            ui.padding.create(0, 50)
-            UiText('- Corrupted Crystals placed with the tool are static', true)
-            UiText('  (they are frozen in space and cannot move no matter what)', true)
-            UiText('', true)
-            UiText('- Corrupted Crystals created from Spawn menu become static when', true)
-            UiText('  they stop moving and you look away from them', true)
-            UiText('', true)
-            UiText('- Only static Corrupted Crystals will grow', true)
-            UiText('', true)
-            UiText('- Corrupted Crystals grow near the surface of objects', true)
-            UiText('', true)
-            UiText('- Corrupted Crystals stop growing when broken', true)
-            UiText('', true)
-
-        UiPop() end
-
...
--- Draw crystal debug infos
 function uiDrawCrystalDebugInfo()
     local cci = CorruptedCrystal_info[1]
     UiPush()
@@ -525,7 +225,6 @@
     UiPop()
...
--- Draw crystal debug infos
 function uiDrawCrystalHint()
     local cci = CorruptedCrystal_info[1]
     if not IsBodyDynamic(cci.body) then
@@ -553,7 +252,6 @@
     UiPop()
 end
...
+
 function ObbDraw(c, r, g, b, a, zCheck)
     ...
     g = g or 1
@@ -439,12 +464,14 @@
     drawFn(c[3], c[7], r, g, b, a)
     drawFn(c[4], c[8], r, g, b, a)
 end
+
