...
-	RegisterTool("rmwg36c", "G36C", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwg36c.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwg36c", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
@@ -246,7 +155,7 @@
 			if GetString("game.player.tool") == "rmwg36c" then --- When Zero then its empty
 			
 			local optic = opticssight[10]
-                if charginghandletime > 0 then ---Charginghandle
+                if charginghandletime ~= 0 then ---Charginghandle
 					SetShapeLocalTransform(optic, Transform(Vec(0, 0.118, -0.13), QuatEuler(-90, 0, 0)))
 				else
 					SetShapeLocalTransform(optic, Transform(Vec(0, 0.118, -0.248), QuatEuler(-90, 0, 0)))
@@ -473,7 +382,7 @@
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwg36c" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwg36c", "G36C", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwg36c.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
