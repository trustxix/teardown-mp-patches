...
-	RegisterTool("rmwauga3", "AUG A3", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwauga3.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwauga3", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwauga3" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwauga3", "AUG A3", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwauga3.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
...
-
-	--Check if rmwauga3 is selected
-	if GetString("game.player.tool") == "rmwauga3" then
...
-			hands = hands + 1
-			end
-			if GetString("game.player.tool") == "rmwauga3" then --- When Zero then its empty
-			
...
+                        RemoveTag(optic, "invisible")
+
+    					SetShapeLocalTransform(optic, Transform(Vec(-0.05, 0.15, -0.2), QuatEuler(-90, 0, 0)))
+    				else
+    				SetTag(optic, "invisible")
+
+                    end
...
+                        RemoveTag(optic, "invisible")
+
+    					SetShapeLocalTransform(optic, Transform(Vec(-0.025, 0.165, -0.2), QuatEuler(-90, 0, 0)))
+    				else
+    				SetTag(optic, "invisible")
+
+                    end
...
+                    if grips == 2 then
+                        RemoveTag(grip, "invisible")
+    					SetShapeLocalTransform(grip, Transform(Vec(-0.025, -0.03, -0.5), QuatEuler(-90, 0, 0)))
+    				else
+    					SetTag(grip, "invisible")
+
+                    end
...
-    end
-	
-if GetString("game.player.tool") == "rmwauga3" and not fireswitch and not reloading and not attachmentmenu then
-		UiPush()
...
-			end
-
-if GetString("game.player.tool") == "rmwauga3" and fireswitch and not reloading and not attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(1, 1, 1)
...
-			end
-			
-if GetString("game.player.tool") == "rmwauga3" and reloading and not attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(1, 1, 1)
...
-				
-			end
-if GetString("game.player.tool") == "rmwauga3" and not reloading and attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(0.8, 0.8, 0.8)
