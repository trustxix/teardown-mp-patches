...
+++ patched/main.lua
@@ -1,50 +1,9 @@
---rmwpkppecheneg custom tool example
-
...
+#version 2
 function SpentCasing()
 	...
 	local casingpos = TransformToParentPoint(gt, Vec(-0.18, 0.09, -0.45))
...
@@ -53,56 +12,11 @@
 	local gt = GetBodyTransform(GetToolBody())
 	local linkpos = TransformToParentPoint(gt, Vec(-0.18, 0.09, -0.45))
 	local fwdpos = TransformToParentPoint(gt, Vec(-6+math.random()*4, 0.5+math.random()*4, -0.65+math.random()*4))
...
-	RegisterTool("rmwpkppecheneg", "PKP Penecheg", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwpkppecheneg.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwpkppecheneg", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
@@ -172,7 +83,9 @@
         return muzzleTransform.pos, VecAdd(d, rndVec(spreadValue))
     end
 end
-function tick(dt)
+fun
...
+tion tick(dt)
 	...
 	--Check if rmwpkppecheneg is selected
@@ -189,7 +102,7 @@
 		local d = GetToolBody()
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwpkppecheneg" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwpkppecheneg", "PKP Penecheg", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwpkppecheneg.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
