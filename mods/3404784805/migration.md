...
+++ patched/main.lua
@@ -1,101 +1,13 @@
---rmwcoltkingcobra custom tool example
-
...
+#version 2
 function SpentCasing()
 	...
 	local fwdpos = TransformToParentPoint(gt, Vec(-3+math.random()*4, 0.5+math.random()*4, -0.65+math.random()*4))
...
-	RegisterTool("rmwcoltkingcobra", "Colt King Cobra", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwcoltkingcobra.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwcoltkingcobra", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
+tion tick(dt)
 	...
 	--Check if rmwcoltkingcobra is selected
@@ -184,7 +94,7 @@
 		local d = GetToolBody()
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwcoltkingcobra" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwcoltkingcobra", "Colt King Cobra", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwcoltkingcobra.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
