...
-RECOIL_BASE_HIP   = 1.3   -- per-shot pitch in hip
...
-	RegisterTool("rmwusp45", "USP45", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwusp45.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwusp45", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
@@ -229,7 +142,7 @@
 			if GetString("game.player.tool") == "rmwusp45" then --- When Zero then its empty
 			
 			local optic = opticssight[10]
-                if charginghandletime > 0 then ---Charginghandle
+                if charginghandletime ~= 0 then ---Charginghandle
 					SetShapeLocalTransform(optic, Transform(Vec(-0.027, 0.0912, -0.21), QuatEuler(-90, 0, 0)))
 				else
 					SetShapeLocalTransform(optic, Transform(Vec(-0.027, 0.0912, -0.31), QuatEuler(-90, 0, 0)))
@@ -458,7 +371,7 @@
...
+
+tion draw()
 if GetString("game.player.tool") == "rmwusp45" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwusp45", "USP45", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwusp45.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
