...
# Migration Report: scripts\control.lua
...
```diff
--- original/scripts\control.lua
+++ patched/scripts\control.lua
@@ -1,64 +1,13 @@
-Control = {}
-Control.info = {
-    throwableBody = nil,
...
-    shieldPieceRadius = 0,
-    nbShieldPieces = 7,
-    shieldSound = LoadSound("MOD/snd/shield.ogg"),
-
...
-    slamDuration = 0,
-    slamDir = nil,
-    slamSound = LoadSound("MOD/snd/slam0.ogg"),
-    
-    playerWalking = false,
-    playerFrontPos = Vec(),
-
...
+            local playerTr = GetPlayerTransform(playerId)
             ...
             ci.playerFrontPos = VecAdd(playerCenter, playerFrontVec)
@@ -96,7 +45,7 @@
     if not Control.isEnabled() then
         return false
     end
-    if GetPlayerVehicle() == 0 then
+    if GetPlayerVehicle(playerId) == 0 then
         -- Update player physics (hover, fall damage, momentum conservation)
         return Control.updatePlayerPhysics()
     end
@@ -232,14 +181,14 @@
 
 function Control.applyMovement()
     local ci = Control.info
-    --dbp("playerVel="..VecStr(GetPlayerVelocity()))
+    --dbp("playerVel="..VecStr(GetPlayerVelocity(playerId)))
     if db then
         local alpha = ci.playerWalking and 0.5 or 1
         dbpt(ci.playerFrontPos, 1, 1, 1, alpha)
     end
     local vol = regGetInt('control.soundVolume') / 100
...
+    local vel = GetPlayerVelocity(playerId)
     if Control.isKeyEnabled('slamKey') then
         local dt = GetTimeStep()
         local slamSpeed = regGetInt('control.slamSpeed')
@@ -257,7 +206,7 @@
             ci.slamDuration = dist / slamSpeed 
         end
         -- If slam is in progress
-        if ci.slamDuration > 0 then
+        if ci.slamDuration ~= 0 then
             ci.slamDuration = ci.slamDuration - dt
             Control.rejectControlBodies()
             QueryRequire("physical")
@@ -267,9 +216,9 @@
                 local slamCenter = AabbGetPlayerCenter()
                 MakeHole(slamCenter, ci.slamRadius, ci.slamRadius, ci.slamRadius, vol == 0)
                 ci.slamDuration = 0
...
-                if vol > 0 then
+                if vol ~= 0 then
                     PlaySound(ci.slamSound, playerPos, vol)
                 end
                 -- Add particle effect
@@ -277,13 +226,13 @@
             else
                 -- Else maintain speed
                 local slamForce = VecResize(ci.slamDir, slamSpeed) -- calculate slam vector
-                SetPlayerVelocity(slamForce)
-            end
-        end
-    end
-    local vel = GetPlayerVelocity()
+                SetPlayerVelocity(playerId, slamForce)
+            end
+        end
+    end
+    local vel = GetPlayerVelocity(playerId)
     if Control.isKeyEnabled('jumpKey') then
-        local playerPos = GetPlayerTransform().pos
+        local playerPos = GetPlayerTransform(playerId).pos
         -- Perform higher jump
         if ci.startJump and ci.playerWalking then
             -- Only jump when on floor
@@ -293,7 +242,7 @@
         local jumping = false
         if ci.pressedJump and ci.jumpStartPos then
             if playerPos[2] - ci.jumpStartPos[2] < regGetFloat('control.jumpHeight') then
-                SetPlayerVelocity(Vec(vel[1], 4, vel[3]))
+                SetPlayerVelocity(playerId, Vec(vel[1], 4, vel[3]))
                 jumping = true
             end
         end
@@ -301,7 +250,7 @@
...
-    local vel = GetPlayerVelocity()
+    local vel = GetPlayerVelocity(playerId)
     if Control.isKeyEnabled('dashKey') and ci.pressedDash then
         -- Get pressed direction based in input pressed
         local movementDir = Vec(0, 0, 0)
@@ -334,7 +283,7 @@
         end
         -- Apply dash impulse
         local dashForce = VecResize(movementDir, regGetInt('control.dashSpeed')) -- calculate dash vector
-        SetPlayerVelocity(VecAdd(vel, dashForce))
+        SetPlayerVelocity(playerId, VecAdd(vel, dashForce))
 
         if db then
             local pfp = ci.playerFrontPos
@@ -348,7 +297,7 @@
 
 function Control.updatePlayerPhysics()
     local ci = Control.info
-    local vel = GetPlayerVelocity()
+    local vel = GetPlayerVelocity(playerId)
     local dt = GetTimeStep()
     -- Hover when no key pressed, zero G when key pressed
     if Control.isKeyEnabled('hoverKey') then
@@ -363,14 +312,14 @@
                 local antigrav = 0.16663932800293 -- 7.5 * dt
...
+                SetPlayerVelocity(playerId, VecAdd(vel, correctionDir))
                 ...
                 -- This is bad hover mode, which slowly falls down
-                SetPlayerVelocity(Vec(vel[1], math.max(vel[2], hoverDuration-ci.hoverTime), vel[3]))
+                SetPlayerVelocity(playerId, Vec(vel[1], math.max(vel[2], hoverDuration-ci.hoverTime), vel[3]))
             end
         else
             if ci.playerWalking then
@@ -378,7 +327,7 @@
             end
         end
     end
...
+    local vel = GetPlayerVelocity(playerId)
     -- Negate fall damages
     if regGetBool('control.noFallDamages') then
         if vel[2] < -5 then
@@ -408,7 +357,7 @@
     end
...
-    local playerTr = GetPlayerTransform()
+    local playerTr = GetPlayerTransform(playerId)
     local bodyTr = Transform()
     local center = Vec()
     if ci.throwableBody then
@@ -434,7 +383,7 @@
             Control.makeHeldUnbreakable(true)
         end
         -- Play sound if needed
-        if vol > 0 then
+        if vol ~= 0 then
             PlaySound(ci.grabSound, holdPos, vol)
         end
     end
@@ -457,7 +406,7 @@
         end
         dbp("throw velocity="..VecStr(velocity))
         -- Play sound if needed
-        if vol > 0 then
+        if vol ~= 0 then
             PlaySound(ci.throwSound, center, vol)
         end
     end
@@ -513,7 +462,7 @@
     end
...
-    local playerTr = GetPlayerTransform()
+    local playerTr = GetPlayerTransform(playerId)
     local flatFrontDir = VecFlatten(TransformToParentVec(playerTr, Vec(0, 0, -1)))
     
     -- When shield start, find shield pieces
@@ -558,7 +507,7 @@
         -- Add particle effect
         Control.buildShieldEffect()
         -- Play sound if needed
-        if vol > 0 then
+        if vol ~= 0 then
             local center = TransformToParentPoint(playerTr, ci.localShieldPos[1])
             PlaySound(ci.shieldSound, center, vol)
         end
@@ -620,7 +569,7 @@
 function Control.makeShieldPieces(holeDir)
     local ci = Control.info
     local vol = regGetInt('control.soundVolume') / 100
-    local playerTr = GetPlayerTransform()
+    local playerTr = GetPlayerTransform(playerId)
     local breakRadius = ci.shieldPieceRadius * 2
     local startPos = VecAdd(playerTr.pos, VecResize(holeDir, breakRadius * 2))
...
+    local playerTr = GetPlayerTransform(playerId)
     local flatFrontDir = VecFlatten(TransformToParentVec(playerTr, Vec(0, 0, -1)))
     local flatRightDir = VecFlatten(TransformToParentVec(playerTr, Vec(1, 0, 0)))
     local distToPlayer = 2
-    local fixedEyeHeight = 1.7 --GetPlayerCameraTransform().pos[2] - playerTr.pos[2]
+    local fixedEyeHeight = 1.7 --GetPlayerCameraTransform(playerId).pos[2] - playerTr.pos[2]
     ci.shieldPieceRadius = fixedEyeHeight / (1 + math.sqrt(3))
     local height = ci.shieldPieceRadius * math.sqrt(3)
     -- Compute center piece position
@@ -941,7 +890,7 @@
     local lifetime = 2
     local scanRadius = 1
     local nbParticles = 20
-    local playerTr = GetPlayerTransform()
+    local playerTr = GetPlayerTransform(playerId)
 
     for i=1,#ci.shieldPieces do
         -- Find a voxel color from the shield piece center
@@ -967,4 +916,5 @@
             end
         end
     end
...
+#version 2
 function initDebug()
     db = regGetBool('control.debugMode')
 end
...
+#version 2
 function modReset()
     ...
     -- Control
-    regSetInt('control.dashSpeed'            , 22)
-    regSetBool('control.noFallDamages'       , false)
-    regSetBool('control.debugMode'           , false)
-    regSetInt('control.soundVolume'          , 50)
-    regSetFloat('control.forceScale'         , 3.0)
-    regSetBool('control.drawThrowableOutline', true)
-    regSetFloat('control.throwVelocity'      , 30.0)
-    regSetFloat('control.hoverDuration'      , 10.0)
-    regSetFloat('control.jumpHeight'         , 3.0)
-    regSetBool('control.drawSlamTarget'      , true)
-    regSetInt('control.slamSpeed'            , 100)
-    regSetBool('control.unbreakableShield'   , false)
-    regSetBool('control.unbreakableHold'     , false)
-    regSetBool('control.telekinesisOutline'  , false)
+    regSetInt('control.dashSpeed'            , 22, true)
+    regSetBool('control.noFallDamages'       , false, true)
+    regSetBool('control.debugMode'           , false, true)
+    regSetInt('control.soundVolume'          , 50, true)
+    regSetFloat('control.forceScale'         , 3.0, true)
+    regSetBool('control.drawThrowableOutline', true, true)
+    regSetFloat('control.throwVelocity'      , 30.0, true)
+    regSetFloat('control.hoverDuration'      , 10.0, true)
+    regSetFloat('control.jumpHeight'         , 3.0, true)
+    regSetBool('control.drawSlamTarget'      , true, true)
+    regSetInt('control.slamSpeed'            , 100, true)
+    regSetBool('control.unbreakableShield'   , false, true)
+    regSetBool('control.unbreakableHold'     , false, true)
+    regSetBool('control.telekinesisOutline'  , false, true)
 
     -- Key modes
-    regSetBool('control.throwToggle' , false)
-    regSetBool('control.shieldToggle', false)
-    regSetBool('control.toolOnly'    , false)
+    regSetBool('control.throwToggle' , false, true)
+    regSetBool('control.shieldToggle', false, true)
+    regSetBool('control.toolOnly'    , false, true)
 
     -- Key bindings
     regInitKey('control.forwardKey', GetString("options.input.keymap.up"))
@@ -58,21 +59,22 @@
...
-            do UiPush()
-
-                ui.checkBox.create('Draw outline on telekinesis target', 'control.drawThrowableOutline')
-                ui.padding.create(0, 64)
-                
-                ui.slider.create('Telekinesis throw speed', 'control.throwVelocity', 'm/s', 1, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                local oldUnbreak = regGetBool('control.unbreakableHold')
-                ui.checkBox.create('Objects held by telekinesis are unbreakable', 'control.unbreakableHold')
-                local newUnbreak = regGetBool('control.unbreakableHold')
-                ui.padding.create(0, 64)
-                if Control and oldUnbreak ~= newUnbreak then
-                    Control.makeHeldUnbreakable(newUnbreak)
-                end
-
-                ui.slider.create('Extra jump height', 'control.jumpHeight', 'm', 0, 10)
-                ui.padding.create(0, 30)
-
-                ui.checkBox.create('Draw ground slam target', 'control.drawSlamTarget')
-                ui.padding.create(0, 64)
-
-                ui.slider.create('Dash speed', 'control.dashSpeed', '', 10, 60, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Hover is less effective after (100 = never)', 'control.hoverDuration', 's', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-            UiPop() end
-
...
-
-                ui.checkBox.create('Debug Mode', 'control.debugMode')
-                ui.padding.create(0, 64)
-
-                ui.slider.create('Sound volume', 'control.soundVolume', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                ui.checkBox.create('No fall damages', 'control.noFallDamages')
-                ui.padding.create(0, 64)
-
-                local oldUnbreak = regGetBool('control.unbreakableShield')
-                ui.checkBox.create('Unbreakable shield', 'control.unbreakableShield')
-                local newUnbreak = regGetBool('control.unbreakableShield')
-                ui.padding.create(0, 64)
-                if Control and oldUnbreak ~= newUnbreak then
-                    Control.makeShieldUnbreakable(newUnbreak)
-                end
-
-                ui.checkBox.create('Draw outline during telekinesis', 'control.telekinesisOutline')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-        do UiPush()
-
-            ui.keyBinding.create('Forward', 'control.forwardKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Back', 'control.backKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Left', 'control.leftKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Right', 'control.rightKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Jump', 'control.jumpKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Hover', 'control.hoverKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Dash', 'control.dashKey', true)
-            ui.padding.create(0, 50)
-
-            ui.checkBox.create('Powers active only when Control tool in hand', 'control.toolOnly', true)
-            ui.padding.create(0, 50)
-
...
-
-            ui.keyBinding.create('Telekinetic Grab/Throw', 'control.throwKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Shield', 'control.shieldKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Slam', 'control.slamKey', true)
-            ui.padding.create(0, 50)
-
-            ui.keyBinding.create('Options (to open this menu ingame)', 'control.optionsKey', true)
-            ui.padding.create(0, 50)
-           
-        UiPop() end
-
...
-
-            ui.checkBox.create('Throw on key press (not hold)', 'control.throwToggle', true)
-            ui.padding.create(0, 50)
-
-            ui.checkBox.create('Shield on key press (not hold)', 'control.shieldToggle', true)
-            ui.padding.create(0, 50)
-           
-        UiPop() end
-        
...
---- Manage when to open and close the options menu.
 function uiManageGameOptions()
     ...
     if InputPressed(regGetString('control.optionsKey')) then UI_GAME = not UI_GAME end
@@ -366,22 +217,6 @@
     end
