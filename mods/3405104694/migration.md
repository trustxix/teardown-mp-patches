...
-	pumpsnd = LoadSound("MOD/snd/pump.ogg")
-	
...
-	RegisterTool("rmw870", "M870", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmw870.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmw870", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
@@ -235,7 +143,7 @@
 			if GetString("game.player.tool") == "rmw870" then --- When Zero then its empty
 			
 			local optic = opticssight[9]
-                if pumptime > 0 then ---Magazine
+                if pumptime ~= 0 then ---Magazine
 					SetShapeLocalTransform(optic, Transform(Vec(-0.025, 0.065, -0.4), QuatEuler(-90, 0, 0)))
 				else
 					SetShapeLocalTransform(optic, Transform(Vec(-0.025, 0.065, -0.55), QuatEuler(-90, 0, 0)))
@@ -422,7 +330,7 @@
 					coolDown = 0.4
 					pumptime = 0.25
 					PlaySound(pumpsnd)
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmw870" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmw870", "M870", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmw870.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
