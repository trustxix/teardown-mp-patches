...
-#include "scripts/body_controller.lua"
-#include "scripts/training.lua"
-#include "scripts/neural_network.lua"
-#include "scripts/genetic_algo.lua"
...
# Migration Report: scripts\body_controller.lua
...
```diff
--- original/scripts\body_controller.lua
+++ patched/scripts\body_controller.lua
@@ -1,18 +1,4 @@
-BodyController = {}
--- Nested object to make sure quick save will keep state
-BodyController_info = {
-    {
-        -- list of all body controllers ingame
-        bodyControllersList = {},
-        -- Stores the index of the body controller with the best fitness
-        bestFitnessIndex = nil,
-        -- Stores the average fitness of all body controllers
-        averageFitness = 0,
-        -- Number of samples for the sliding fitness average
-        averageMaxSamples = 100,
-    }
-}
-
+#version 2
 function BodyController.run()
     local bcInfo = BodyController_info[1]
     --> Process instances.
@@ -24,7 +10,7 @@
         bodyController.lastBodyCenter = AabbGetBodyCenterPos(bodyController.body)
         bcInfo.averageFitness = bcInfo.averageFitness + BodyController.fitness(bodyController)
     end
-    if #bcInfo.bodyControllersList > 0 then
+    if #bcInfo.bodyControllersList ~= 0 then
         bcInfo.averageFitness = bcInfo.averageFitness / #bcInfo.bodyControllersList
     end
...
--- The fitness of a body controller is the average of each instant fitness recorded
 function BodyController.fitness(bodyController)
     local fitnessCount = #bodyController.fitnessSamples
     if fitnessCount == 0 then
@@ -96,7 +81,6 @@
     end
 end
 
--- Gives a new neural network to an existing body controller
 function BodyController.setNeuralNetwork(bodyController, neuralNetwork)
     if not ai.neural_network.IsCompatible(neuralNetwork, bodyController.objective) then
         dbm("Cannot update neural network of body controller "..bodyController.id..": Incopatible objective")
@@ -108,7 +92,6 @@
     BodyController.activate(bodyController, regGetBool('tool.createActiveAI'))
 end
 
--- Gives a new objective and neural network to an existing body controller
 function BodyController.convert(bodyController, objective, neuralNetwork)
     local gaInfo = genetic_algo_info[1]
     bodyController.objective = objective
@@ -116,7 +99,6 @@
     BodyController.initRunnigAlgosData(bodyController)
 end
 
--- Gets the fitness of the best body controller
 function BodyController.bestFitness()
     local bcInfo = BodyController_info[1]
     if bcInfo.bestFitnessIndex == nil or bcInfo.bestFitnessIndex > #bcInfo.bodyControllersList then
@@ -126,13 +108,11 @@
     end
 end
 
--- Gets the average fitness of all body controllers
 function BodyController.averageFitness()
     local bcInfo = BodyController_info[1]
     return bcInfo.averageFitness
 end
 
--- Gets the indexes of body controllers matching the current objective
 function BodyController.getEligiblePopulationIndexes(objective)
     local bcInfo = BodyController_info[1]
     local indexes = {}
@@ -145,7 +125,6 @@
     return indexes
...
--- Randomize the neural network of this controller (used when training starts)
 function BodyController.randomize(bodyController, deactivate)
     deactivate = deactivate or false
     ai.neural_network.Init(bodyController.neuralNetwork)
@@ -153,29 +132,24 @@
     BodyController.activate(bodyController, not deactivate)
 end
 
--- Returns a flat list representing the genes of this controller
 function BodyController.getGenes(bodyController)
     return ai.neural_network.GetFlatParams(bodyController.neuralNetwork)
 end
 
--- Set the internal values of this controller based on the given gene list
 function BodyController.setGenes(bodyController, genes)
     bodyController.success = ai.neural_network.SetFlatParams(bodyController.neuralNetwork, genes)
     BodyController.resetFitness(bodyController)
     BodyController.activate(bodyController, true)
 end
 
--- Activate or deactivate a body controller
 function BodyController.activate(bodyController, active)
     bodyController.active = active
 end
 
--- Toggle active state of a body controller
 function BodyController.toggleActivation(bodyController)
     bodyController.active = not bodyController.active
 end
 
--- Int the parameters involved in Deep Q-Learning algorithm
 function BodyController.initDeepQLearningParams(bodyController)
     bodyController.targetNeuralNetwork = DeepCopy(bodyController.neuralNetwork)
     bodyController.bufferNeuralNetwork = DeepCopy(bodyController.neuralNetwork)
@@ -183,7 +157,6 @@
     BodyController.bufferClear(bodyController.buffer)
 end
 
--- If genetic algo is running, randomize the newly created neural network
 function BodyController.initRunnigAlgosData(bodyController)
     local gaInfo = genetic_algo_info[1]
      -- If genetic algo is running, reandomize the newly created neural network
@@ -199,7 +172,6 @@
     end
 end
 
--- Push a record at the end of the buffer
 function BodyController.bufferPush(buffer, state_in, action, next_state, reward, done)
     TableAppend(buffer.states, DeepCopy(state_in))
     TableAppend(buffer.actions, DeepCopy(action))
@@ -208,7 +180,6 @@
     TableAppend(buffer.dones, DeepCopy(done))
 end
 
--- Clear the records buffer
 function BodyController.bufferClear(buffer)
     buffer.states = {}
     buffer.actions = {}
@@ -217,19 +188,10 @@
     buffer.dones = {}
...
--- Return the curret size of the buffer
 function BodyController.bufferCount(buffer)
...
-
-
-BodyController.instances = {}
-
---- Create a body controller object and store it in the list of instances.
 function BodyController.instances.attach(body)
     local bcInfo = BodyController_info[1]
...
-
---- Marks for removal the body controller associated with the specified body, return true if the controller existed and was removed
 function BodyController.instances.detach(body)
 
     local bodyController = BodyController.instances.getBodyController(body)
@@ -302,7 +262,6 @@
 
 end
 
---- Attach to the body if it has no controller yet, else etach from it
 function BodyController.instances.attachDetach(body)
 
     if not BodyController.instances.detach(body) then
@@ -311,7 +270,6 @@
 
 end
 
---- Returns the body controller attached to the given body, or nil if not found
 function BodyController.instances.getBodyController(body)
...
---- Mark a body controller for removal to safely remove it near the end of the frame.
 function BodyController.instances.markBodyControllerForRemoval(body_controller)
     body_controller.removeBodyController = true
 end
 
---- Safely delete all body controllers that are marked for removal.
 function BodyController.instances.removeMarkedBodyControllers()
...
-
-
-BodyController.brain = {}
--- Map of actions to execute if the corresponding output of the neural network is the highest
 function BodyController.brain.buildActionsMap()
     local map = {}
     -- Builds a map of every possible combination of [-1, 0, 1] for a 3D vector
@@ -363,9 +312,7 @@
     end
     return map
 end
-BodyController.brain.actionsMap = BodyController.brain.buildActionsMap()
-
--- Read the inputs, feed them to the neural network, apply the output actions, update fitness
+
 function BodyController.brain.process(bodyController)
 
     local alive = BodyController.brain.checkAlive(bodyController)
@@ -397,7 +344,6 @@
     return true
 end
 
--- Check if the body is "alive" = if it's not fully broken, and if it is, remove its controller
 function BodyController.brain.checkAlive(bodyController)
 
     if not IsBodyValid(bodyController.body) then
@@ -409,7 +355,6 @@
 
 end
 
--- Check if the AI should remain active or become inactive
 function BodyController.brain.checkActivation(bodyController)
 
     -- Inactive AIs remain inactive
@@ -436,7 +381,6 @@
 
 end
 
--- Read the list of inputs based on objective
 function BodyController.brain.sense(bodyController)
...
--- Scan the surroundings and return a list of distances to objects hit
 function BodyController.brain.scanSurroundings(bodyController)
...
--- Control the body with the given action values
 function BodyController.brain.control(bodyController, action)
     -- Gets the action vector from the action index
     local actionVec = BodyController.brain.actionsMap[action]
@@ -531,8 +473,6 @@
     return true
 end
 
--- Compute the fitness for the current time step and record its value
--- The fitness is stored as a percentage of how good is the object doing to reach the objective
 function BodyController.brain.updateFitness(bodyController)
...
--- Returns the result of the last action with the associated reward (fitness)
 function BodyController.brain.getLastStep(bodyController)
     -- State of previous turn produced action of previous turn
     local state_in = ternary(#bodyController.lastSenseData[2] > 0, bodyController.lastSenseData[2], BodyController.brain.getDummyInput(bodyController))
@@ -658,7 +597,6 @@
     return state_in, action, next_state, reward, done
 end
 
--- Returns an empty set of inputs
 function BodyController.brain.getDummyInput(bodyController)
     local inputs = {}
     local actualInputs, actualOutputs = ai.neural_network.GetInOut(bodyController.neuralNetwork)
@@ -666,4 +604,5 @@
         TableAppend(inputs, 0)
     end
     return inputs
-end+end
+
...
+
 function dds(transform, radius, sides, info)
     if db then
         local tr1 = TransformCopy(transform)
@@ -36,11 +42,8 @@
     debugBodyControllers()
 end
 
--- Custom debug messages management to avoid flooding the screen
-maxDebugMessages = 10
-lastDebugMessages = {}
-lastDebugTimer = 6
 function dbm(str) addDebugMessage(str) print(str) end
+
 function addDebugMessage(str)
     lastDebugMessages[#lastDebugMessages+1] = str
     if #lastDebugMessages > maxDebugMessages then
@@ -51,7 +54,7 @@
 
 function debugBodyControllers()
 
-    if lastDebugTimer > 0 then
+    if lastDebugTimer ~= 0 then
         lastDebugTimer = lastDebugTimer - GetTimeStep()
     end
     -- local bcInfo = BodyController_info[1]
@@ -64,3 +67,4 @@
     -- end
...
# Migration Report: scripts\genetic_algo.lua
...
```diff
--- original/scripts\genetic_algo.lua
+++ patched/scripts\genetic_algo.lua
@@ -1,17 +1,4 @@
-
-
-genetic_algo = {}
...
-
 ...
 function genetic_algo.resume()
     local bcInfo = BodyController_info[1]
@@ -159,7 +145,7 @@
     training.DeepQLearning.UpdateGlobal()
 
     -- Let the population evolve freely for the intended duration
-    if gaInfo.timer > 0 then
+    if gaInfo.timer ~= 0 then
         gaInfo.timer = gaInfo.timer - GetTimeStep()
         return
     end
@@ -201,7 +187,6 @@
     gaInfo.timer = regGetInt('genetic_algo.generationDuration')
 end
 
--- Select the population members that "survived" for the next generation
 function genetic_algo.selection(genomes)
     -- Keep only the top X% of the population to pick parents for the next generation
     -- Also compute total fitness for the remaining genomes
@@ -254,7 +239,6 @@
     return newGenomes
 end
 
--- Make crossover between some memebers of the population and produce childs
 function genetic_algo.crossover(newGenomes)
     local crossoverGenomes = {}
     local newGenomesCount = #newGenomes;
@@ -312,7 +296,6 @@
     return newGenomes
 end
 
--- Mutate one gene for some members of the population
 function genetic_algo.mutation(newGenomes)
     local mutationRate = regGetInt('genetic_algo.mutationRate')
     local mutationMin = regGetInt('genetic_algo.mutationMin')
@@ -334,7 +317,6 @@
     return newGenomes
 end
 
--- Update the unbreakable state of bodies based on setting and current training state
 function genetic_algo.updateUnbreakableStates()
     local bcInfo = BodyController_info[1]
     local gaInfo = genetic_algo_info[1]
@@ -348,4 +330,5 @@
             RemoveTag(bodyController.body, "unbreakable")
         end
     end
-end+end
+
...
+++ patched/scripts\neural_network.lua
@@ -1,90 +1,4 @@
--- Script inspired by the following examples, adapted and completed with features needed for the AI Trainer mod:
--- https://github.com/Dragonrick123/Neural-Network-in-lua-Module/blob/general/MainModule.lua (lots of errors in this one but I kept the main code structure)
--- https://gist.github.com/cassiozen/de0dff87eb7ed599b5d0 (probably a bit more functional, but harder to read)
--- NB: This might not be the most optimized way to store and process a neural network, so don't expect crazy performance out of it.
---     The primary goal here whas to get something easy enough to read so I can understand a little bit what I'm doing :p
-
-ai = {
-    neural_network = {}
-}
-
--- Neural network structure example (as generated through the ai.neural_network.AddLayer() function)
-ai.neural_network.example = {
-    -- Number of inputs of the network (Neurons in the first layer)
-    nbInputs = 3,
-    -- Array of Layers
-    layers = {
-        -- Layer 0 (input) is not stored as it has no weight or bias
-        -- Layer 1
-        {
-            -- Activation function of this layer, must match with an existing function name in ai.activations
-            activation = "relu",
-            -- Array of Neurons
-            neurons = {
-                -- Neuron 1
-                {
-                    -- One weight per Neuron in Layer 0
-                    weights = {0, 0, 0},
-                    -- One bias for the current Neuron
-                    bias = 0,
-                },
-                -- Neuron 2
-                {
...
-        },
-        -- ...
-        -- As many layers as needed can be added here, with a custom number of neurons and activation function each
-        -- ...
-        -- Layer 2 (output)
-        {
-            activation = "sigmoid",
...
-                -- Neuron 1
-                {
-                    -- One weight per neuron in Layer 1
-                    weights = {0, 0},
-                    bias = 0,
-                },
-                -- Neuron 2
...
-}
-
--- Resets the Adam optimization algorithm
+#version 2
 function ai.optim.Adam.Reset()
     local self = ai_optim_Adam[1]
 	self.m_dparams = {}
@@ -96,8 +10,6 @@
     self.eta =  0.01
 end
 
--- Runs one step of the Adam optimization algorithm for the given parameter value
--- Use paramName to keep the previous information of each parameter to optimize
 function ai.optim.Adam.Update(paramName, value, gradient)
 	local self = ai_optim_Adam[1]
     if self.m_dparams[paramName] == nil then
@@ -117,16 +29,10 @@
     return value - self.eta*(m_dparam_corr/(math.sqrt(v_dparam_corr)+self.epsilon))
 end
...
-    derivate = {}
-}
--- I found some useful hints on what type of activation function to use for what type of problem here:
--- https://towardsdatascience.com/how-to-choose-the-right-activation-function-for-neural-networks-3941ff0e6f9c
--- limits: [-inf, +inf], curve: /
 function ai.activations.identity(array)
 	return array
 end
+
 function ai.activations.derivate.identity(array)
     local out = {}
     for i=1,#array do
@@ -135,15 +41,16 @@
 	return out
...
--- limits: [-1, 1], curve: _/‾
-function ai.activations.tanh(array)
+nction ai.activations.tanh(array)
     local out = {}
     for i=1,#array do
         out[i] = math.tanh(array[i])
     end
 	return out
 end
-function ai.activations.derivate.tanh(array)
+f
+
+nction ai.activations.derivate.tanh(array)
     local out = {}
     for i=1,#array do
         out[i] = 1 - (math.tanh(array[i])^2)
@@ -151,15 +58,16 @@
 	return out
 end
 
--- limits: [0, 1], curve: _/‾
-function ai.activations.sigmoid(array)
+tion ai.activations.sigmoid(array)
     local out = {}
     for i=1,#array do
         out[i] = 1/(1 + math.exp(-array[i]))
     end
 	return out
 end
-function ai.activations.derivate.sigmoid(array)
+fun
+
+tion ai.activations.derivate.sigmoid(array)
     local sigmoids = ai.activations.sigmoid(array)
     local out = {}
     for i=1,#array do
@@ -168,15 +76,18 @@
 	return out
...
--- limits: [0, +inf], curve: _/
-function ai.activations.relu(array)
+--
+
+tion ai.activations.relu(array)
     local out = {}
     for i=1,#array do
...
-function ai.activations.derivate.relu(array)
+fun
+
+tion ai.activations.derivate.relu(array)
     local out = {}
     for i=1,#array do
         out[i] = ternary(array[i] > 0, 1, 0)
@@ -184,8 +95,9 @@
 	return out
 end
 
--- limits: [-inf, +inf], curve: _/
-function ai.activations.leaky_relu(array)
+--
+
+tion ai.activations.leaky_relu(array)
     local out = {}
     local alpha = 0.01
     for i=1,#array do
@@ -193,7 +105,9 @@
...
-function ai.activations.derivate.leaky_relu(array)
+fun
+
+tion ai.activations.derivate.leaky_relu(array)
     local out = {}
     for i=1,#array do
         -- Normally derivate is undefined at point 0, but keep it at 0.01 anyway
@@ -202,15 +116,18 @@
 	return out
 end
 
--- limits: [0, 6], curve: _/‾
-function ai.activations.relu6(array)
+--
+
+on ai.activations.relu6(array)
     local out = {}
     for i=1,#array do
         out[i] = math.min(math.max(0, array[i]), 6)
     end
 	return out
 end
-function ai.activations.derivate.relu6(array)
+funct
+
+on ai.activations.derivate.relu6(array)
     local out = {}
     for i=1,#array do
         out[i] = ternary(array[i] > 0, ternary(array[i] < 6, 1, 0), 0)
@@ -218,8 +135,9 @@
 	return out
 end
 
--- limits: [0, 1], curve: /\/\/
-function ai.activations.softmax(array)
+-- l
+
+on ai.activations.softmax(array)
     local sum = 0
     local exps = {}
     for i=1,#array do
@@ -232,7 +150,9 @@
...
-function ai.activations.derivate.softmax(array)
+funct
+
+on ai.activations.derivate.softmax(array)
     local out = {}
     for i=1,#array do
         out[i] = 1
@@ -240,8 +160,9 @@
...
--- limits: [0, +inf], curve: _/
-function ai.activations.swish(array)
+-- l
+
+on ai.activations.swish(array)
 	local sigmoids = ai.activations.sigmoid(array)
     local out = {}
     for i=1,#array do
@@ -249,7 +170,9 @@
...
-function ai.activations.derivate.swish(array)
+funct
+
+on ai.activations.derivate.swish(array)
 	local sigmoids = ai.activations.sigmoid(array)
     local out = {}
     for i=1,#array do
@@ -258,15 +181,18 @@
...
--- limits: [0, +inf], curve: _/
-function ai.activations.hard_swish(array)
+-- l
+
+on ai.activations.hard_swish(array)
     local out = {}
     for i=1,#array do
         out[i] = array[i] * (math.min(math.max(0, array[i] + 3), 6) / 6 )
     end
 	return out
 end
-function ai.activations.derivate.hard_swish(array)
+funct
+
+on ai.activations.derivate.hard_swish(array)
 	local sigmoids = ai.activations.sigmoid(array)
     local out = {}
     for i=1,#array do
@@ -275,8 +201,9 @@
...
--- Returns the activation function to use based on its name
-function ai.activations.GetFunction(Activation)
+-- R
+
+on ai.activations.GetFunction(Activation)
 	if type(Activation) == "function" then
 	    return Activation
     elseif ai.activations[Activation] then
@@ -286,8 +213,9 @@
 	end
 end
 
--- Returns the derivate of the activation function to use based on its name
-function ai.activations.GetDerivateFunction(Activation)
+-- R
+
+on ai.activations.GetDerivateFunction(Activation)
     if ai.activations.derivate[Activation] then
         return ai.activations.derivate[Activation]
     else
@@ -295,8 +223,9 @@
     end
 end
 
--- Creates an empty Neural Network with the given number of inputs
-function ai.neural_network.Create(nbInputs)
+-- C
+
+on ai.neural_network.Create(nbInputs)
 	local newNeuralNetwork = {
         nbInputs = nbInputs,
         layers = {}
@@ -304,13 +233,15 @@
...
--- Copy an existing Neural Network
-function ai.neural_network.Copy(neuralNetwork)
+-- C
+
+on ai.neural_network.Copy(neuralNetwork)
 	return DeepCopy(neuralNetwork)
 end
 
--- Export a neural network to a json string
-function ai.neural_network.ToJson(neuralNetwork)
+-- E
+
+on ai.neural_network.ToJson(neuralNetwork)
     local status, retval = pcall(json.encode, neuralNetwork);
     if not status then
         dbm("Failed to export neural network to json: "..retval)
@@ -319,8 +250,9 @@
 	return retval
 end
 
--- Import a neural network from a json string
-function ai.neural_network.FromJson(jsonStr)
+-- I
+
+on ai.neural_network.FromJson(jsonStr)
     local status, retval = pcall(json.decode, jsonStr);
     if not status then
         dbm("Failed to import neural network from json: "..retval)
@@ -329,8 +261,9 @@
 	return retval
 end
 
--- Adds a layer at the end of an existing Neural Network
-function ai.neural_network.AddLayer(neuralNetwork, nbNeurons, activation)
+-- A
+
+on ai.neural_network.AddLayer(neuralNetwork, nbNeurons, activation)
 	local newLayer = {
         activation = activation,
         neurons = {}
@@ -342,7 +275,7 @@
     -- Creates the weights array with the correct number of values bases on the number of neurons in the previous layer
     local neuronsInPreviousLayer = neuralNetwork.nbInputs
     local lastLayer = #neuralNetwork.layers
-    if lastLayer > 0 then
+    if lastLayer ~= 0 then
         neuronsInPreviousLayer = #neuralNetwork.layers[lastLayer].neurons
     end
     for i=1,neuronsInPreviousLayer do
@@ -354,14 +287,9 @@
     TableAppend(neuralNetwork.layers, newLayer)
 end
 
-
-
-------------------------------------------------------------------------------------------------------
--- START MOD-SPECIFIC (functions in this block are specific to this mod, outside of it are generic) --
-------------------------------------------------------------------------------------------------------
-
--- Returns the default neural network to use for each training objective
-function ai.neural_network.GetDefaultNN(objective)
+--
+
+on ai.neural_network.GetDefaultNN(objective)
     if objective == "balance" then
         -- 9 inputs: Body rotation on each axis (3) + Body rotation speed on each axis (3) + Body dimensions (3)
         local newNN = ai.neural_network.Create(9)
@@ -392,15 +320,17 @@
     end
 end
 
--- Returns if the given neural network if compatible with the objective
-function ai.neural_network.IsCompatible(neuralNetwork, objective)
+-- R
+
+on ai.neural_network.IsCompatible(neuralNetwork, objective)
     local expectedInputs, expectedOutputs = ai.neural_network.GetExpectedInOut(objective)
     local actualInputs, actualOutputs = ai.neural_network.GetInOut(neuralNetwork)
     return actualInputs == expectedInputs and actualOutputs == expectedOutputs
 end
 
--- Returns the expected number of inputs and outputs for this objective
-function ai.neural_network.GetExpectedInOut(objective)
+-- R
+
+on ai.neural_network.GetExpectedInOut(objective)
     local expectedInputs = -1
     local expectedOutputs = -1
     if objective == "balance" then
@@ -418,27 +348,23 @@
     return expectedInputs, expectedOutputs
 end
 
--- Returns the number of inputs and outputs for this neural network
-function ai.neural_network.GetInOut(neuralNetwork)
+-- R
+
+on ai.neural_network.GetInOut(neuralNetwork)
     if not neuralNetwork then
         return nil, nil
     end
     local inputs = neuralNetwork.nbInputs
     local outputs = 0
-    if #neuralNetwork.layers > 0 then
+    if #neuralNetwork.layers ~= 0 then
         outputs = #neuralNetwork.layers[#neuralNetwork.layers].neurons
     end
...
-
-
--- Computes the value returned by the given neuron of the given layer for the provided inputs
-function ai.neural_network.NeuronThink(neuralNetwork, layerIndex, neuronIndex, inputs)
+----
+
+on ai.neural_network.NeuronThink(neuralNetwork, layerIndex, neuronIndex, inputs)
     if layerIndex < 1 or layerIndex > #neuralNetwork.layers then
         dbm("NeuronThink: Layer index out of bounds: "..tostring(layerIndex))
         return nil
@@ -462,8 +388,9 @@
     return output
 end
 
--- Apply the activation function of the layer (restrain the input to a predefined range)
-function ai.neural_network.ActivateLayer(neuralNetwork, layerIndex, inputs)
+-- A
+
+on ai.neural_network.ActivateLayer(neuralNetwork, layerIndex, inputs)
     if layerIndex < 1 or layerIndex > #neuralNetwork.layers then
         dbm("ActivateLayer: Layer index out of bounds: "..tostring(layerIndex))
         return nil
@@ -477,8 +404,9 @@
     return activationFn(inputs)
 end
 
--- Propagates the given inputs through the whole network and returns the output values
-function ai.neural_network.ForwardPropagate(neuralNetwork, inputs, returnAll)
+-- P
+
+on ai.neural_network.ForwardPropagate(neuralNetwork, inputs, returnAll)
     if #inputs ~= neuralNetwork.nbInputs then
         dbm("ForwardPropagate: Mismatching sizes nbInputs="..neuralNetwork.nbInputs..", #inputs="..#inputs)
         return nil
@@ -513,8 +441,9 @@
     end
 end
 
--- Propagates the outputs through the whole network and update weights and bias to get closer to the provided inputs
-function ai.neural_network.BackwardPropagate(neuralNetwork, inputs, desiredOutputs)
+-- P
+
+on ai.neural_network.BackwardPropagate(neuralNetwork, inputs, desiredOutputs)
 
     local actualInputs, actualOutputs = ai.neural_network.GetInOut(neuralNetwork)
     if #inputs ~= actualInputs then
@@ -554,9 +483,9 @@
 
 end
 
--- Define the backward propagation step as a method
--- Inspired from https://blog.zhaytam.com/2018/08/15/implement-neural-network-backpropagation/
-function ai.neural_network.backward_step(neuralNetwork, inputs, activations, targets)
+-- D
+
+on ai.neural_network.backward_step(neuralNetwork, inputs, activations, targets)
     local param_grads = {}  -- List of parameter gradients for each layer
     local output_grad = {} -- The error gradient at the output of the current layer
     -- Initialize gradient form the network expected and actual outputs
@@ -598,9 +527,9 @@
     return param_grads  -- Return the parameter gradients
 end
 
--- Return a list of gradients over the parameters
--- This builds a flat list list in the same order taken by ai.neural_network.SetFlatParams()
-function ai.neural_network.get_params_grad(neuralNetwork, inputs, output_grad)
+-- R
+
+on ai.neural_network.get_params_grad(neuralNetwork, inputs, output_grad)
     local param_grads = {}
     for outputIndex=1,#output_grad do
         local bias_grad = 0
@@ -614,8 +543,9 @@
     return param_grads
 end
...
-function ai.neural_network.get_input_grad(neuralNetwork, layerIndex, outputs, output_grad)
+-- R
+
+on ai.neural_network.get_input_grad(neuralNetwork, layerIndex, outputs, output_grad)
     local input_grads = {}
     local layer = neuralNetwork.layers[layerIndex]
     local activationDerivateFn = ai.activations.GetDerivateFunction(layer.activation)
@@ -644,9 +574,9 @@
     return input_grads
 end
 
--- Randomly initializes a neural network using the "Xavier and He" initialisation method
--- https://prateekvishnu.medium.com/xavier-and-he-normal-he-et-al-initialization-8e3d7a087528
-function ai.neural_network.Init(neuralNetwork)
+-- R
+
+on ai.neural_network.Init(neuralNetwork)
 	for layerIndex=1,#neuralNetwork.layers do
         local layer = neuralNetwork.layers[layerIndex]
...
--- Returns a flat list of all weights of the neural network
-function ai.neural_network.GetFlatWeights(neuralNetwork)
+-- R
+
+on ai.neural_network.GetFlatWeights(neuralNetwork)
     local weights = {}
     for layerIndex=1,#neuralNetwork.layers do
         local layer = neuralNetwork.layers[layerIndex]
@@ -675,8 +606,9 @@
...
--- Returns a flat list of all biases of the neural network
-function ai.neural_network.GetFlatBiases(neuralNetwork)
+-- R
+
+on ai.neural_network.GetFlatBiases(neuralNetwork)
     local biases = {}
     for layerIndex=1,#neuralNetwork.layers do
         local layer = neuralNetwork.layers[layerIndex]
@@ -687,8 +619,9 @@
...
--- Returns a flat list of all parameters of the neural network
-function ai.neural_network.GetFlatParams(neuralNetwork)
+-- R
+
+on ai.neural_network.GetFlatParams(neuralNetwork)
     local params = {}
     -- This loop must match exactly with the one in ai.neural_network.SetFlatParams(neuralNetwork)
     for layerIndex=1,#neuralNetwork.layers do
@@ -704,8 +637,9 @@
     return params
 end
 
--- Returns a flat list of uniaue param names
-function ai.neural_network.GetUniqueParamNames(neuralNetwork)
+-- R
+
+on ai.neural_network.GetUniqueParamNames(neuralNetwork)
     local paramNames = {}
     -- This loop must match exactly with the one in ai.neural_network.SetFlatParams(neuralNetwork)
     for layerIndex=1,#neuralNetwork.layers do
@@ -721,8 +655,9 @@
...
--- Loads a flat list of parameters in this neural network
-function ai.neural_network.SetFlatParams(neuralNetwork, params)
+-- L
+
+on ai.neural_network.SetFlatParams(neuralNetwork, params)
     local tmpParams = ai.neural_network.GetFlatParams(neuralNetwork)
     if #tmpParams ~= #params then
         dbm("Cannot update neural network from flat parameters, expected "..#tmpParams.." parameters but got "..#params)
@@ -747,4 +682,5 @@
         end
     end
     return true
...
+#version 2
 function modReset()
     ...
     -- Tool
-    regSetBool('tool.highlightControllers', true)
-    regSetBool('tool.showTrainingInfo'    , true)
-    regSetBool('tool.showDebugMessage'    , true)
-    regSetBool('tool.debugMode'           , false)
-    regSetBool('tool.showAIInfo'          , true)
-    regSetBool('tool.createActiveAI'      , false)
-    regSetBool('tool.randomAddDuringTrain', true)
+    regSetBool('tool.highlightControllers', true, true)
+    regSetBool('tool.showTrainingInfo'    , true, true)
+    regSetBool('tool.showDebugMessage'    , true, true)
+    regSetBool('tool.debugMode'           , false, true)
+    regSetBool('tool.showAIInfo'          , true, true)
+    regSetBool('tool.createActiveAI'      , false, true)
+    regSetBool('tool.randomAddDuringTrain', true, true)
 
     -- Neural Network
-    regSetString('neural_network.objective'   , "balance")
+    regSetString('neural_network.objective'   , "balance", true)
     local defaultNN = ai.neural_network.GetDefaultNN("balance")
     local defaultNNStr = ai.neural_network.ToJson(defaultNN)
-    regSetString('neural_network.default'     , defaultNNStr)
-    regSetString('neural_network.current'     , defaultNNStr)
-    regSetString('neural_network.loadSelected', "none")
+    regSetString('neural_network.default'     , defaultNNStr, true)
+    regSetString('neural_network.current'     , defaultNNStr, true)
+    regSetString('neural_network.loadSelected', "none", true)
     local savedNeuralNetworks = regListKeys('neural_network.saved')
-    if #savedNeuralNetworks > 0 then
-        regSetString('neural_network.loadSelected', savedNeuralNetworks[1])
-    end
-    regSetString('neural_network.saveName'     , "")
-    regSetInt('neural_network.accelRate'       , 1)
-    regSetInt('neural_network.distToPlayer'    , 3)
-    regSetInt('neural_network.scanRange'       , 10)
-    regSetBool('neural_network.inactiveOnError', true)
-    regSetBool('neural_network.inactiveOnBreak', true)
+    if #savedNeuralNetworks ~= 0 then
+        regSetString('neural_network.loadSelected', savedNeuralNetworks[1], true)
+    end
+    regSetString('neural_network.saveName'     , "", true)
+    regSetInt('neural_network.accelRate'       , 1, true)
+    regSetInt('neural_network.distToPlayer'    , 3, true)
+    regSetInt('neural_network.scanRange'       , 10, true)
+    regSetBool('neural_network.inactiveOnError', true, true)
+    regSetBool('neural_network.inactiveOnBreak', true, true)
 
     -- Training
-    regSetInt('training.batch_size'     , 32)
-    regSetInt('training.epsilonDecrease', 8)
-    regSetBool('training.batchMode'     , false)
+    regSetInt('training.batch_size'     , 32, true)
+    regSetInt('training.epsilonDecrease', 8, true)
+    regSetBool('training.batchMode'     , false, true)
  
     -- Genetic Algorithm
-    regSetInt('genetic_algo.generationDuration', 5)
-    regSetInt('genetic_algo.keepTopPercent'    , 50)
-    regSetString('genetic_algo.selectionType'  , "linear")
-    regSetInt('genetic_algo.crossoverPercent'  , 100)
-    regSetString('genetic_algo.crossoverType'  , "slice-1")
-    regSetInt('genetic_algo.mutationRate'      , 5)
-    regSetInt('genetic_algo.mutationMin'       , -2)
-    regSetInt('genetic_algo.mutationMax'       , 2)
-    regSetBool('genetic_algo.trainDuringGen'   , true)
-    regSetBool('genetic_algo.mutationMultiply' , true)
-    regSetBool('genetic_algo.unbreakableTrain' , false)
+    regSetInt('genetic_algo.generationDuration', 5, true)
+    regSetInt('genetic_algo.keepTopPercent'    , 50, true)
+    regSetString('genetic_algo.selectionType'  , "linear", true)
+    regSetInt('genetic_algo.crossoverPercent'  , 100, true)
+    regSetString('genetic_algo.crossoverType'  , "slice-1", true)
+    regSetInt('genetic_algo.mutationRate'      , 5, true)
+    regSetInt('genetic_algo.mutationMin'       , -2, true)
+    regSetInt('genetic_algo.mutationMax'       , 2, true)
+    regSetBool('genetic_algo.trainDuringGen'   , true, true)
+    regSetBool('genetic_algo.mutationMultiply' , true, true)
+    regSetBool('genetic_algo.unbreakableTrain' , false, true)
 
     -- Key bindings
     regInitKey('key.attach'  , "lmb")
@@ -79,21 +80,22 @@
...
-    end
-end+        regSetString(path, value, true)
+    end
+end
+
...
-
-        input = {
-            didAttachDetachAI = function() return tool.tool.active() and InputPressed(regGetString('key.attach')) end,
-            didCopyAI = function() return tool.tool.active() and InputPressed(regGetString('key.copy')) end,
-            didActivateAI = function() return tool.tool.active() and InputPressed(regGetString('key.activate')) end,
-            didConvert = function() return tool.tool.active() and InputPressed(regGetString('key.convert')) end,
-            didStartStopTraining = function() return tool.tool.active() and InputPressed(regGetString('key.start')) end,
-            didPauseResumeTraining = function() return tool.tool.active() and InputPressed(regGetString('key.pause')) end,
-            didRemove = function() return tool.tool.active() and InputPressed(regGetString('key.remove')) end,
-        },
-
-        setup = {
-            name = 'ai_trainer',
-            title = 'AI Trainer',
-            voxPath = 'MOD/vox/remote.vox',
-        },
-
-        active = function()
...
+#version 2
 function tool.run()
     ...
     -- Get cursor position at all times even if tool is not selected
-    local playerTr = GetPlayerCameraTransform()
+    local playerTr = GetPlayerCameraTransform(playerId)
     local direction = TransformToParentVec(playerTr, Vec(0, 0, -1))
     local hit, dist, normal, shape = QueryRaycast(playerTr.pos, direction, 1000000)
     if hit then
@@ -57,7 +25,7 @@
                 if tool.tool.aimBody then
                     local bodyController = BodyController.instances.getBodyController(tool.tool.aimBody)
                     if bodyController then
-                        regSetString('neural_network.current', ai.neural_network.ToJson(bodyController.neuralNetwork))
+                        regSetString('neural_network.current', ai.neural_network.ToJson(bodyController.neuralNetwork), true)
                     end
                 end
             end
@@ -107,4 +75,5 @@
...
# Migration Report: scripts\training.lua
...
```diff
--- original/scripts\training.lua
+++ patched/scripts\training.lua
@@ -1,16 +1,4 @@
-training = {
-    DeepQLearning = {}
-}
--- Nested object to make sure quick save will keep state
-training_DeepQLearning = {
-    {
-        epsilon = 0.0,
-        discount = 0.99,
-    }
...
--- Algo bsed on this post: https://stackoverflow.com/questions/39932611/how-to-implement-deep-q-learning-gradient-descent
+#version 2
 function training.DeepQLearning.TrainStep(main_nn, target_nn, states, actions, rewards, next_states, dones)
     local self = training_DeepQLearning[1]
     local buffer_size = #states
@@ -63,15 +51,17 @@
     return true
 end
 
--- Resets the Deep Q-Learning optimization algorithm
-function training.DeepQLearning.Reset()
+-- Resets the
+
+ng.DeepQLearning.Reset()
     local self = training_DeepQLearning[1]
 	self.epsilon = 1.0
 	self.discount = 0.99
 end
 
--- Runs one step of the Deep Q-Learning optimization algorithm
-function training.DeepQLearning.Update(main_nn, target_nn, buffer_nn, buffer, state_in, action, next_state, reward, done)
+-- Runs one s
+
+ng.DeepQLearning.Update(main_nn, target_nn, buffer_nn, buffer, state_in, action, next_state, reward, done)
     local batch_size = regGetInt('training.batch_size')
     local batchMode = regGetBool('training.batchMode')
     -- Save to experience replay buffer.
@@ -109,8 +99,9 @@
     return false
 end
 
--- Runs one global step of the Deep Q-Learning optimization algorithm (once after each body controller have been processed)
-function training.DeepQLearning.UpdateGlobal()
+-- Runs one g
+
+ng.DeepQLearning.UpdateGlobal()
     local self = training_DeepQLearning[1]
     -- Reduce epsilon value over time
     if self.epsilon > 0.05 then
@@ -119,8 +110,9 @@
     end
 end
 
--- Shuffle the position of each element in the buffer (keeping coherence across all buffers)
-function training.DeepQLearning.ShuffleSamples(buffer)
+-- Shuffle th
+
+ng.DeepQLearning.ShuffleSamples(buffer)
     local size = BodyController.bufferCount(buffer)
     for i=1,size do
         local j = rdm(i,size)
@@ -132,8 +124,9 @@
     end
...
-function training.DeepQLearning.SelectEpsilonGreedyAction(outputs)
+-- Take rando
+
+ng.DeepQLearning.SelectEpsilonGreedyAction(outputs)
     local self = training_DeepQLearning[1]
     -- Roll random value in [0, 1]
     if rdmf() < training.DeepQLearning.GetExplorationRatio() then
@@ -143,19 +136,21 @@
     end
 end
 
--- Completes the Deep Q-Learning optimization algorithm
-function training.DeepQLearning.Terminate()
+-- Completes 
+
+ng.DeepQLearning.Terminate()
 	local self = training_DeepQLearning[1]
 	self.epsilon = 0.0
 end
 
--- Returns the current exploration vs execution ratio
--- Exploration is when the AI makes decisions randomly, execution is whne it follows the best choice from the neural network
-function training.DeepQLearning.GetExplorationRatio()
+-- Returns th
+
+ng.DeepQLearning.GetExplorationRatio()
     local self = training_DeepQLearning[1]
     if regGetBool('genetic_algo.trainDuringGen') then
         return self.epsilon
     else
         return 0
     end
...
-objectives = {"balance", "follow", "avoid"}
-
-selectionTypes = {"linear", "roulette"}
-
-crossoverTypes = {"slice-1", "slice-2", "random"}
-
-options_tabs_render = {
-
-    Tool = function()
...
-            do UiPush()
-
-                ui.checkBox.create('Highlight AI-controlled objects', 'tool.highlightControllers')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Show training info text', 'tool.showTrainingInfo')
-                ui.padding.create(0, 64)
-                UiColor(1,1,1, 1)
-                UiFont('bold.ttf', 32)
-                UiText('Log:')
...
-                if lastDebugMessages then
-                    for i=1, #lastDebugMessages do
-                        UiText(lastDebugMessages[i])
-                        ui.padding.create(0, 28)
-                    end
-                end
-
-            UiPop() end
...
-
-                ui.checkBox.create('Show AI info on hover', 'tool.showAIInfo')
-                ui.padding.create(0, 64)
-                
-                ui.checkBox.create('Show last log message ingame', 'tool.showDebugMessage')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-
-                ui.checkBox.create('Attached AIs start active', 'tool.createActiveAI')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Randomize AIs added during training', 'tool.randomAddDuringTrain')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-
-            do UiPush()
-                local objectiveBefore = regGetString('neural_network.objective')
-                do UiPush()
-                    -- Objective description
-                    UiFont('regular.ttf', font_normal)
-                    UiAlign('left middle')
-                    ui.padding.create(400 + padW2, font_normal * 2)
-                    if objectiveBefore == "balance" then
-                        UiText("(Try to balance on one corner)")
-                    elseif objectiveBefore == "follow" then
-                        UiText("(Try to follow the player)")
-                    elseif objectiveBefore == "avoid" then
-                        UiText("(Try to move away from objects)")
-                    end
-                UiPop() end
-
-                ui.slider.create('Training objective', 'neural_network.objective', '', nil, nil, nil, nil, nil, nil, nil, objectives)
-                local objectiveAfter = regGetString('neural_network.objective')
-                -- If objective changed, update default neural network
-                if objectiveAfter ~= objectiveBefore then
-                    local defaultNN = ai.neural_network.GetDefaultNN(objectiveAfter)
-                    regSetString('neural_network.default', ai.neural_network.ToJson(defaultNN))
-                    if genetic_algo and genetic_algo.getState() ~= "stopped" then
-                        dbm("Training stopped: Objective changed")
-                        genetic_algo.stop()
-                    end
-                end
-                ui.padding.create(0, 50)
-
-                local loadDefaultButton = ui.button.create('Load Default', whiteColor, whiteColor)
-                if loadDefaultButton then
-                    regSetString('neural_network.current', regGetString('neural_network.default'))
-                end
-                ui.padding.create(0, 50)
-
-                ui.lineEdit.create('Current neural network', 'neural_network.current', 1600)
-                ui.padding.create(0, 80)
-
-                local currentNN = ai.neural_network.FromJson(regGetString('neural_network.current'))
-                if not currentNN then
-                    do UiPush()
-                        UiColor(1, 0.6, 0.6, 1)
-                        UiAlign('left middle')
-                        UiText('ERROR: Invalid syntax in current network, see log in "Tool" tab for more details.\n'
-                        ..'FIX: Resolve the error or use the "Load Default" button.')
-                    UiPop() end
-                else
-                    local expectedInputs, expectedOutputs = ai.neural_network.GetExpectedInOut(regGetString('neural_network.objective'))
-                    local actualInputs, actualOutputs = ai.neural_network.GetInOut(currentNN)
-                    if not (actualInputs == expectedInputs and actualOutputs == expectedOutputs) then
-                        do UiPush()
-                            UiColor(1, 0.6, 0.6, 1)
-                            UiAlign('left middle')
-                            UiText('ERROR: Current network has '..tostring(actualInputs)..' inputs and '..tostring(actualOutputs)
-                                    ..' outputs, but selected objective requires '..tostring(expectedInputs)..' inputs and '..tostring(expectedOutputs)..' outputs.\n'
-                                    ..'FIX: Load a copatible network or use the "Load Default" button.')
-                        UiPop() end
-                    end
-                end
-                ui.padding.create(0, 50)
-                
-                ui.slider.create('Rotation acceleration rate of the object controlled', 'neural_network.accelRate', 'Unit/Second', 1, 10, nil, nil, nil, nil, true)
-                ui.padding.create(0, 50)
-
-                ui.slider.create('Distance to player when following', 'neural_network.distToPlayer', 'Meters', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 50)
-
-                ui.slider.create('Scan range when avoiding', 'neural_network.scanRange', 'Meters', 1, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 50)
-
-            UiPop() end
-
...
-                ui.padding.create(800, 0)
-
-                local nameToLoad = regGetString('neural_network.loadSelected')
-                local savedNeuralNetworks = regListKeys('neural_network.saved')
-                local hasSavedNetworks = #savedNeuralNetworks > 0
-                -- Makes sure table is never empty (slider doesn't like empty tables)
-                if not hasSavedNetworks then
-                    savedNeuralNetworks = {"none"}
-                end
-                -- Makes sure currently selected name is always part of the list
-                if TableFind(savedNeuralNetworks, nameToLoad) == nil then
-                    regSetString('neural_network.loadSelected', savedNeuralNetworks[1])
-                end
-                ui.slider.create('Saved neural networks', 'neural_network.loadSelected', '', nil, nil, nil, nil, nil, nil, nil, savedNeuralNetworks)
-                nameToLoad = regGetString('neural_network.loadSelected')
-                ui.padding.create(0, 50)
-
-                do UiPush()
-
-                    local loadButton = ui.button.create('Load', whiteColor, whiteColor, nil, nil, nil, nil, not hasSavedNetworks)
-                    if loadButton then
-                        regSetString('neural_network.current', regGetString('neural_network.saved.'..nameToLoad))
-                    end
-                    ui.padding.create(ui.button.lastWidth + padW, 0)
-
-                    local deleteButton = ui.button.create('Delete', whiteColor, whiteColor, nil, nil, nil, nil, not hasSavedNetworks)
-                    if deleteButton then
-                        regClearKey('neural_network.saved.'..nameToLoad)
-                    end
-                    ui.padding.create(ui.button.lastWidth + padW, 0)
-
-                    local saveName = regGetString('neural_network.saveName')
-                    local saveButton = ui.button.create('Save as', whiteColor, whiteColor, nil, nil, nil, nil, string.len(saveName) == 0)
-                    if saveButton then
-                        regSetString('neural_network.saved.'..saveName, regGetString('neural_network.current'))
-                    end
-                    ui.padding.create(ui.button.lastWidth + padW, -20)
-
-                    ui.lineEdit.create('', 'neural_network.saveName', nil, nil, true)
-
-                UiPop() end
-                ui.padding.create(0, 200)
-
-                ui.checkBox.create('Deactivate AI if it encountered an error', 'neural_network.inactiveOnError')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Deactivate AI if it broke the object it controls', 'neural_network.inactiveOnBreak')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-            UiAlign('left middle')
-            -- Title
-            UiText('Genetic Algorithm')
-            -- Group box
-            do UiPush()
-                ui.padding.create(-20, 20)
-                local w = 1040
-                local h = 530
-                local border = 2
-                UiAlign('left top')
-                UiColor(ui.colors.g3[1],ui.colors.g3[2],ui.colors.g3[3],1)
-                UiRect(w, h)
-                ui.padding.create((w / 2), (h / 2))
-                UiAlign('center middle')
-                UiColor(ui.colors.g1[1],ui.colors.g1[2],ui.colors.g1[3],1)
-                UiRect(w- (border * 2), h - (border * 2))
-            UiPop() end 
-            
-            ui.padding.create(1050, 0)
-
-            -- Title
-            UiText('Gradient Descent')
-            -- Group box
-            do UiPush()
-                ui.padding.create(-20, 20)
-                local w = 580
...
-        do UiPush()
-
-            ui.slider.create('Generation duration', 'genetic_algo.generationDuration', 'Seconds', 0, 300, nil, nil, nil, nil, true)
-            ui.padding.create(0, 50)
-
-            ui.slider.create('Selection keeps only the top X% fitness', 'genetic_algo.keepTopPercent', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 50)
-
-            ui.slider.create('Selection method', 'genetic_algo.selectionType', '', nil, nil, nil, nil, nil, nil, nil, selectionTypes)
-            ui.padding.create(0, 50)
-
-            ui.slider.create('Proportion of population doing crossover', 'genetic_algo.crossoverPercent', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 50)
-
-            ui.slider.create('Crossover method', 'genetic_algo.crossoverType', '', nil, nil, nil, nil, nil, nil, nil, crossoverTypes)
-            ui.padding.create(0, 50)
-
-        UiPop() end
-
...
-        do UiPush()
-
-            ui.padding.create(550, 0)
-
-            ui.slider.create('Mutation chance', 'genetic_algo.mutationRate', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 50)
-
-            ui.range_slider.create('Value range for mutation', 'genetic_algo.mutationMin', 'genetic_algo.mutationMax', nil, -100, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 50)
-
-            ui.checkBox.create('Mutation multiply exising gene value', 'genetic_algo.mutationMultiply')
-            ui.padding.create(0, 64)
-
-            local oldUnbreak = regGetBool('genetic_algo.unbreakableTrain')
-            ui.checkBox.create('Unbreakable objects during training', 'genetic_algo.unbreakableTrain')
-            local newUnbreak = regGetBool('genetic_algo.unbreakableTrain')
-            if genetic_algo and oldUnbreak ~= newUnbreak then
-                genetic_algo.updateUnbreakableStates()
-            end
-            
-            ui.padding.create(0, 64)
-
...
-            ui.padding.create(1050, 0)
-
-            ui.checkBox.create('Also train neural network during each generation',  'genetic_algo.trainDuringGen')
-            ui.padding.create(0, 64)
-
-            ui.slider.create('Number of data samples for training', 'training.batch_size', 'Samples', 1, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 50)
-
-            ui.slider.create('Exploration decrease rate slowness', 'training.epsilonDecrease', '', 1, 64, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-            UiAlign('left middle')
-            UiColor(0.6,0.6,0.6, 1)
-            UiText('Higher values = slower decrease rate')
-            ui.padding.create(0, 40)
-
-            ui.checkBox.create('Process all samples at once', 'training.batchMode')
-            ui.padding.create(0, 64)
-            UiAlign('left middle')
-            UiColor(0.6,0.6,0.6, 1)
-            UiText('CAUTION: Will cause huge lag spikes with many AIs')
-
-        UiPop() end
-
-
...
-            ui.padding.create(0, 50)
-            UiFont('regular.ttf', font_normal)
-            ui.keyBinding.create('Attach/Detach AI to object', 'key.attach', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Copy AI from object', 'key.copy', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Activate/Deactivate AI on object', 'key.activate', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Apply current AI and objective to all existing AIs', 'key.convert', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Start/Stop training', 'key.start', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Pause/Resume training', 'key.pause', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Remove all AIs', 'key.remove', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Options', 'key.options', true)
-            ui.padding.create(0, 50)
-            UiColor(0.6,0.6,0.6, 1)
-            UiText('To bind a mouse button, when the binding is waiting\nfor an input, click with the button you want\nanywhere outside the binding box.')
-            -- ui.padding.create(0, 100)
-            -- UiText('To clear a binding, when the binding is waiting\nfor an input, left click inside the binding box.')
-
-        UiPop() end
-
-        do UiPush()
...
-            UiFont('regular.ttf', font_normal)
-            ui.padding.create(0, 50)
-            UiText('- This mod is my attempt at bringing some machine learning into Teardown,', true)
-            UiText('    it is mostly there to demonstrate it is possible,', true)
-            UiText('    and to have fun looking at some basic AIs struggling to achieve simple tasks :p', true)
-            UiText('', true)
-            UiText('- The "AI" used in this mod is a neural network that you can train to perform various tasks', true)
-            UiText('    (only one task at a time though)', true)
-            UiText('', true)
-            UiText('- The training method used is a Genetic Algorithm, which reproduces the rules of', true)
-            UiText('    natural selection to find the best specimens in a group', true)
-            UiText('', true)
-            UiText('- A starting population of 20 should be good (or down to 10 if you get low FPS) then', true)
-            UiText('    with the default settings it can take up to 400 generations to start getting good results', true)
-            UiText('', true)
-            UiText('- You may want to make some free space around the learning objects and make sure', true)
-            UiText('    they are not stuck (objects in water will train badly except for balance objective)', true)
-            UiText('', true)
-            UiText('- You may want to train the objects on flat ground to avoid learning weird behavor', true)
-            UiText('    due to the terrain slopes or being stopped by walls (except for avoid objective)', true)
-            UiText('', true)
-
-        UiPop() end
-
...
---- Draw training info
 function uiDrawTrainingInfo()
 
     if not regGetBool('tool.showTrainingInfo') then
@@ -665,7 +254,6 @@
 
 end
 
---- Draw last debug message
 function uiDrawDebugMessage()
 
     if not regGetBool('tool.showDebugMessage') or not lastDebugMessages then
@@ -687,7 +275,6 @@
 
 end
 
---- Draw informarion of the body controller
 function uiDrawBodyControllerInfo()
 
     if not regGetBool('tool.showAIInfo') or not tool.tool.aimBody then
@@ -732,7 +319,6 @@
...
-ui.range_slider = {}
-
 function ui.range_slider.create(title, registryPathMin, registryPathMax, valueText, min, max, w, h, fontSize, inline, intSlider)
