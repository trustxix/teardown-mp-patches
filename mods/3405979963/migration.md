...
+++ patched/main.lua
@@ -1,101 +1,13 @@
---rmwdeserteagle custom tool example
-
...
-RECOIL_BASE_HIP   = 3   -- per-shot pitch in hip
...
-VISUAL_SCALE_HIP  = 4   -- viewmodel tilt multiplier (hip)
-VISUAL_SCALE_ADS  = 3   -- viewmodel tilt multiplier (ADS)
-CAMERA_SCALE_HIP  = 2   -- camera tilt multiplier (hip)
...
-	RegisterTool("rmwdeserteagle", "Desert Eagle", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwdeserteagle.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwdeserteagle", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
+tion tick(dt)
 	...
 	--Check if rmwdeserteagle is selected
@@ -184,7 +94,7 @@
 		local d = GetToolBody()
...
@@ -233,7 +143,7 @@
 			if GetString("game.player.tool") == "rmwdeserteagle" then --- When Zero then its empty
 			
 			local optic = opticssight[10]
-                if charginghandletime > 0 then ---Charginghandle
+                if charginghandletime ~= 0 then ---Charginghandle
 					SetShapeLocalTransform(optic, Transform(Vec(-0.04, 0.077, -0.17), QuatEuler(-90, 0, 0)))
 				else
 					SetShapeLocalTransform(optic, Transform(Vec(-0.04, 0.077, -0.27), QuatEuler(-90, 0, 0)))
@@ -478,7 +388,7 @@
 					smoke = math.min(1.0, smoke + 0.1)
 					coolDown = 0.08
 					charginghandletime = 0.078
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwdeserteagle" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwdeserteagle", "Desert Eagle", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwdeserteagle.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
