...
-	RegisterTool("rmwrpg7", "RPG-7", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwrpg7.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwrpg7", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
-	--Check if rmwrpg7 is selected
-	if GetString("game.player.tool") == "rmwrpg7" then
...
-			
-			
-			if GetString("game.player.tool") == "rmwrpg7" then --- When Zero then its empty
-			
-			local optic = opticssight[9]
-                if reloadtimer > 1 or magazine == 0 then ---Magazine
...
-                    RemoveTag(optic, "invisible")
-					SetShapeLocalTransform(optic, Transform(Vec(-0.1, 0.15, -0.1), QuatEuler(-90, 0, 0)))
-				else
-				SetTag(optic, "invisible")
-					
-                end
...
-					SetShapeLocalTransform(hand2, Transform(Vec(0.0765, -0.03, -0.1), QuatEuler(-90, 20, 0)))
-				else
...
-		
-		if reloading then
-		toolAnimator.offsetTransform = (Transform(Vec(-0.2, -0.2, 0), QuatEuler(45, 20, -35)))
-		tickToolAnimator(toolAnimator, dt)
-		end
-		
...
-function draw()
-
-if GetString("game.player.tool") == "rmwrpg7" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwrpg7", "RPG-7", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwrpg7.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
...
-
-if GetString("game.player.tool") == "rmwrpg7" and not fireswitch and not reloading and not attachmentmenu then
-		UiPush()
...
-			UiText(magazine.." (40 mm) Auto")
-				
-				
-			end
-
-if GetString("game.player.tool") == "rmwrpg7" and fireswitch and not reloading and not attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(1, 1, 1)
-			UiFont("bold.ttf", 32)
-			UiTextOutline(0,0,0,1,0.1)
-			UiText(magazine.." (40 mm) Semi")
-			
-			end
-			
-if GetString("game.player.tool") == "rmwrpg7" and reloading and not attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(1, 1, 1)
...
-				
-			end
-if GetString("game.player.tool") == "rmwrpg7" and not reloading and attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(0.8, 0.8, 0.8)
