...
+++ patched/main.lua
@@ -1,103 +1,13 @@
---rmwbarrett50cal custom tool example
-
...
-RECOIL_BASE_HIP   = 4   -- per-shot pitch in hip
-RECOIL_BASE_ADS   = 3   -- per-shot pitch in ADS
...
-VISUAL_SCALE_HIP  = 3   -- viewmodel tilt multiplier (hip)
-VISUAL_SCALE_ADS  = 2   -- viewmodel tilt multiplier (ADS)
-CAMERA_SCALE_HIP  = 2   -- camera tilt multiplier (hip)
-CAMERA_SCALE_ADS  = 2   -- camera tilt multiplier (ADS)
-
...
-	RegisterTool("rmwbarrett50cal", "Barrett M82", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwbarrett50cal.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwbarrett50cal", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
--- Add recoil when a shot happens
 function AddRecoil(aiming, grips, muzzlec)
     ...
     if grips == 1 or grips == 2 or grips == 3 then base = base * GRIP_MULT end
@@ -127,7 +36,6 @@
...
+tion tick(dt)
 	...
 	--Check if rmwbarrett50cal is selected
@@ -186,7 +94,7 @@
 		local d = GetToolBody()
...
-                if barellmove > 0 then ---Barell
+                if barellmove ~= 0 then ---Barell
                     
 					
 					SetShapeLocalTransform(optic, Transform(Vec(-0.026, 0.12, -0.08), QuatEuler(-90, 0, 0)))
@@ -468,7 +376,7 @@
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwbarrett50cal" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwbarrett50cal", "Barrett M82", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwbarrett50cal.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
