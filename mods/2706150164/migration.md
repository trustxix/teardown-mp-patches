...
-    if not sounds or not sounds.play or not sounds.play.bounce then
-        initSounds()
-    end
-
-    tool.run()
...
+
 function dds(transform, radius, sides, info)
     if db then
         local tr1 = TransformCopy(transform)
@@ -59,3 +65,4 @@
     dbw('TOOL rot', Gun.padPlaceholder.tr.rot)
     dbw('TOOL pos', Gun.padPlaceholder.getPos())
 end
+
...
-Gun.editPrecision = 5
-Gun.editRotationPitch = false
-Gun.editRotationRoll = false
-Gun.editRotationYaw = false
-
-Gun.padPlaceholder = {
...
-        local padPos = VecAdd(playerTr.pos, VecScale(Gun.padPlaceholder.tr.pos, Gun.padPlaceholder.distFromPlayer))
-        -- If pos should be aligned on grid, only accept integer pos
-        if regGetBool('tool.useGrid') then
-            padPos = Vec(floatToInt(padPos[1]), floatToInt(padPos[2]), floatToInt(padPos[3]))
-        end
-        return padPos
-    end,
-
...
-        local roll, yaw, pitch = GetQuatEuler(Gun.padPlaceholder.lastPlayerRot)
-        local padRot = QuatRotateQuat(QuatEuler(0, yaw, pitch), Gun.padPlaceholder.tr.rot)
-        -- If rot should be aligned on grid, ignore player rot
-        if regGetBool('tool.useGrid') then
-            padRot = Gun.padPlaceholder.tr.rot
-        end
-        return padRot
-    end,
-
-    getPosTransform = function()
-        return Transform(Gun.padPlaceholder.getPos(), Gun.padPlaceholder.getRot())
-    end,
-
-    getSize = function()
-        return regGetFloat('pad.size')
-    end,
-
-    getColor = function()
-        local color = Pad.getColor()
-        color.a = Gun.editionMode and 1 or regGetFloat('tool.padPlaceholder.alpha')
-        return color
-    end,
-
-    getStep = function()
...
-        Gun.padPlaceholder.tr.pos = offsetVector
-        Gun.padPlaceholder.lastPlayerRot = playerTr.rot
-    end,
-
-    draw = function()
-        local size = Gun.padPlaceholder.getSize()
-        local color = Gun.padPlaceholder.getColor()
-        DrawOutlineRectCentered(Gun.padPlaceholder.getPosTransform(), size, size, color)
-    end,
-}
-
+#version 2
 function Gun.getPadType()
     return Pad.ability.list[Pad.ability.selected].name
-end
-
-Gun.actions = {
-
-    placePad = function()
-        if not UI_GAME then
-            Pad.pads.instances.spawn(Gun.padPlaceholder.getPosTransform(), Gun.getPadType())
-        end
-    end,
-
-    replacePad = function()
-        if not UI_GAME then
-            local removedPadsTransform = Pad.pads.instances.removeClosestPads()
-            for i=1,#removedPadsTransform do
-                Pad.pads.instances.spawn(removedPadsTransform[i], Gun.getPadType())
-            end
-        end
-    end,
-
-    reset = function()
-        Pad_padsList = {}
-        dbp('Pads have been reset.' .. sfnTime(), true)
-        beep()
-    end,
-
-    deletePad = function()
-        local removedPadsTransform = Pad.pads.instances.removeClosestPads()
-        if #removedPadsTransform > 0 then
-            beep()
-        end
-    end,
-
-    reinitPadPlaceholder = function()
-        Gun.padPlaceholder.tr = Transform(Vec(1, 0, 0), QuatEuler(90, 0, 0))
-        Gun.padPlaceholder.distFromPlayer = 2
-    end,
-
...
-    end,
-
-    enableRotationPitch = function(state)
-        Gun.editRotationPitch = state
-    end,
-
-    enableRotationRoll = function(state)
-        Gun.editRotationRoll = state
-    end,
-
-    enableRotationYaw = function(state)
-        Gun.editRotationYaw = state
-    end,
-
-    updateScroll = function(scroll)
-        if Gun.editionMode then
-            scroll = scroll * Gun.editPrecision
-            if Gun.editRotationYaw then
-                -- Rotating Yaw
-                Gun.padPlaceholder.tr.rot = QuatRotateQuat(Gun.padPlaceholder.tr.rot, QuatEuler(0, scroll, 0))
-            elseif Gun.editRotationRoll then
-                -- Rotating Roll
-                Gun.padPlaceholder.tr.rot = QuatRotateQuat(Gun.padPlaceholder.tr.rot, QuatEuler(scroll, 0, 0))
-            elseif Gun.editRotationPitch then
-                -- Rotating Pitch
-                Gun.padPlaceholder.tr.rot = QuatRotateQuat(Gun.padPlaceholder.tr.rot, QuatEuler(0, 0, scroll))
-            else
-                -- Moving closer to/away from player
-                Gun.padPlaceholder.distFromPlayer = clamp(Gun.padPlaceholder.distFromPlayer + (scroll * 0.1), 1, 100)
-            end
-        end
-    end,
-
...
-
-
-Gun.updatePadPlaceholder = function()
-    -- In edition, force the placeholder in front of the player camera
-    if Gun.editionMode or not Gun.padPlaceholder.initialized then
-        Gun.padPlaceholder.initialized = true
-        Gun.padPlaceholder.update()
-    end
-    Gun.padPlaceholder.draw()
 end
...
+    --local offsetWithSpeed = VecAdd(offsetVector, VecScale(GetPlayerVelocity(playerId), 1/30))
     local spritePos = VecAdd(playerTr.pos, offsetVector)
     local spriteRot = QuatRotateQuat(playerTr.rot, toolTr.rot)
     return Transform(spritePos, spriteRot)
 end
 
-Gun.draw = function()
-    
-    local size = 0.5
-    local padSprite = LoadSprite("img/SquareBlank.png")
-    local spriteTr = Gun.getSpriteTr()
-    local color = Gun.padPlaceholder.getColor()
-    DrawSprite(padSprite, spriteTr, size, size, color.r, color.g, color.b, 1)
-
-end
...
# Migration Report: scripts\pad.lua
...
```diff
--- original/scripts\pad.lua
+++ patched/scripts\pad.lua
@@ -1,60 +1,16 @@
-Pad = {}
-Pad.pads = {} -- Holds common data for all pads.
-Pad_padsList = {}
--- When set to true, pads ignore player
-Pad.ignorePlayer = false
-Pad.wasGodMode = false
-
-Pad.ability = {
-    selected = 1,
-    next = 0,
...
-        {
-            name = "Trampoline",
-            icon = "MOD/img/icon_trampoline.png",
-            color = Vec(200/255, 1, 200/255)
-        },
-        {
-            name = "Bouncepad",
-            icon = "MOD/img/icon_bouncepad.png",
-            color = Vec(170/255, 170/255, 1)
-        },
-        {
-            name = "Jumppad",
-            icon = "MOD/img/icon_jumppad.png",
-            color = Vec(1, 1, 170/255)
-        },
-        {
-            name = "Antipad",
-            icon = "MOD/img/icon_antipad.png",
...
-}
-
-for i=1,#Pad.ability.list do
-    local item = Pad.ability.list[i]
-    Pad.ability.indexByName[item.name] = i
-end
-
--- Size difference between the bounce pad itself and the animation square around it
+#version 2
 function Pad.getBounceSquareRatio()
     return 1.1
 end
 
--- How big the pad should grow during animation
 function Pad.getBounceAnimRatio()
     return 1.2
 end
 
--- Minimum speed allowed before a body gets ignored by a pad
 function Pad.getMinSpeedBeforeIgnore()
     return 1
 end
 
--- Get the pad color based on pad type
 function Pad.getColor(padType)
     local index = padType and Pad.ability.indexByName[padType] or Pad.ability.selected
     --DebugPrint((padType and padType or "nil").." index="..index)
@@ -68,7 +24,6 @@
     return color
 end
 
--- Get the position of the 4 corners of the pad
 function Pad.getCorners(pad)
     if pad.corners == nil then
         local tr = pad.tr
@@ -83,7 +38,6 @@
     return pad.corners
 end
...
--- Get the bounding box of the pad
 function Pad.getBoundingBox(pad)
     if pad.bbox == nil then
         local corners = Pad.getCorners(pad)
@@ -112,17 +66,15 @@
     return pad.bbox
 end
 
--- Get center point of the player
 function Pad.getPlayerCenter()
...
-
 function Pad.run()
     -- If no fall damage enabled, force player health when falling too fast
     local shouldGodMode = false
     if regGetBool('tool.noFallDamage') then
...
-Pad.pads.instances = {}
-
---- Create a pad object and store it in the list of pads.
 function Pad.pads.instances.create(tr, padType)
...
---- Spawn a pad into the world.
 function Pad.pads.instances.spawn(tr, padType)
     Pad.pads.instances.create(tr, padType)
     if regGetBool('pad.soundOn') then
@@ -221,7 +167,6 @@
     end
 end
 
---- Mark a pad for removal to safely remove it near the end of the frame.
 function Pad.pads.instances.markPadForRemoval(pad)
     pad.removePad = true
     if regGetBool('pad.soundOn') then
@@ -230,7 +175,6 @@
     dbp('Pad marked for removal: ' .. pad.id .. ' ' ..sfnTime())
 end
 
---- Mark pads closest to the player for removal.
 function Pad.pads.instances.removeClosestPads(count)
     count = count or 1
     local playerPos = Pad.getPlayerCenter()
@@ -262,7 +206,6 @@
 
 end
 
---- Safely delete all pads that are marked for removal.
 function Pad.pads.instances.removeMarkedPads()
 
     TableRemove(Pad_padsList, function(t, i, j)
@@ -272,14 +215,6 @@
...
-Pad.pads.physics = {}
-
--- Process the physics of a single pad.
 function Pad.pads.physics.process(pad)
     local bbox = Pad.getBoundingBox(pad)
     local didBounce = false
@@ -314,14 +249,14 @@
     end
     -- Process player in range and make it bounce if needed
     local playerBbox = Pad.getPlayerBbox()
-    local playerVel = GetPlayerVelocity()
+    local playerVel = GetPlayerVelocity(playerId)
     if AabbCheckOverlap(playerBbox.min, playerBbox.max, bbox.min, bbox.max) and not Pad.ignorePlayer then
         bodiesInRange["player"] = true
         if pad.ignoredBodies["player"] == nil then
             local newVelocity = Pad.pads.physics.tryBounce(pad, playerBbox.min, playerBbox.max, playerVel)
             if newVelocity ~= nil then
                 -- Bounce success, apply new velocity to player
-                SetPlayerVelocity(newVelocity)
+                SetPlayerVelocity(playerId, newVelocity)
                 didBounce = true
                 -- If body bounced but speed was realy slow, ignore it for next frames
                 if VecLength(playerVel) < Pad.getMinSpeedBeforeIgnore() then
@@ -342,7 +277,6 @@
 
 end
 
--- Check if the conditions to bounce an item off a pad are met, and if they are returns the bounce angle
 function Pad.pads.physics.tryBounce(pad, bboxMin, bboxMax, bodyVel)
     local corners = Pad.getCorners(pad)
     local bodyCenter = AabbGetCenterPos(bboxMin, bboxMax)
@@ -418,7 +352,6 @@
     return nil
 end
 
--- Returns a vector between the center of a bbox and its surface, aiming for target point
 function Pad.pads.physics.getCenterToBboxSurfaceAimingAt(bodyCenter, bboxMin, bboxMax, target, stopAtTarget)
     stopAtTarget = stopAtTarget or false
     local bboxLimit = target
@@ -432,7 +365,6 @@
     return bboxSurfacePoint, bodyCenterToBboxSurface
 end
 
--- Assuming point and square are on the same 3D plane, tells if point is inside the square
 function Pad.pads.physics.isPointInSquare(point, corners)
     local side1 = VecSub(corners[2], corners[1])
     local side2 = VecSub(corners[4], corners[1])
@@ -442,8 +374,6 @@
     return res
 end
 
-
--- Manage ignored bodies, removing those that are no longer in pad bbox
 function Pad.pads.physics.manageIgnoredBody(pad, bodiesInRange)
     local bodiesToRemove = {}
     for body in pairs(pad.ignoredBodies) do
@@ -456,7 +386,6 @@
     end
 end
 
--- Check if a body should still be ignored, if not remove it from ignored bodies list
 function Pad.pads.physics.checkIfBodyStillIgnored(pad, body, bodyCenterNow)
     -- A body is no longer ignored if its center is now on the other side of the pad
     local bodyCenterWhenIgnored = pad.ignoredBodies[body]
@@ -472,17 +401,9 @@
     end
 end
...
-Pad.pads.lifetime = {}
-
 function Pad.pads.lifetime.process(pad)
     -- Lifespan of 0 is unlimited
-    if pad.timers.lifespan.delay > 0 then
+    if pad.timers.lifespan.delay ~= 0 then
         TimerRunTime(pad.timers.lifespan)
         -- If lifespan runs out, remove the pad
         if pad.timers.lifespan.time <= 0 then
@@ -490,12 +411,6 @@
         end
     end
 end
...
-Pad.pads.effects = {}
 ...
 function Pad.pads.effects.process()
     -- Render pads.
@@ -505,7 +420,6 @@
     end
 end
 
--- Initiate the bounce animation + sound
 function Pad.pads.effects.doBounce(pad)
     -- Limit sound played frequency
     if regGetBool('pad.soundOn') and pad.timers.bounceSound.time <= 0 then
@@ -515,7 +429,6 @@
     pad.visualSize = pad.size * Pad.getBounceAnimRatio()
     TimerResetTime(pad.timers.bounceAnim)
 end
-
 
 function Pad.pads.effects.renderPad(pad)
     local color = Pad.getColor(pad.padType)
@@ -523,7 +436,7 @@
     -- Pad sprite.
     DrawSprite(padSprite, pad.tr, pad.visualSize, pad.visualSize, color.r, color.g, color.b, 1, true)
     -- Animate on bounce
-    if pad.timers.bounceAnim.time > 0 then
+    if pad.timers.bounceAnim.time ~= 0 then
         -- Pad anim square is visible all the length of the timer
         local squareSize = pad.visualSize * Pad.getBounceSquareRatio()
         DrawOutlineRectCentered(pad.tr, squareSize, squareSize, color)
@@ -531,4 +444,5 @@
         pad.visualSize = clamp(pad.visualSize * 0.9, pad.size, pad.visualSize)
         TimerRunTime(pad.timers.bounceAnim)
     end
-end+end
+
...
-    regSetFloat('tool.padPlaceholder.alpha' , 0.5)
-    regSetBool('tool.noFallDamage'          , false)
...
+    regSetFloat('tool.padPlaceholder.alpha' , 0.5, true)
+    regSetBool('tool.noFallDamage'          , false, true)
+    regSetBool('tool.useGrid'               , false, true)
+    regSetBool('tool.showAbilityText'       , true, true)
+    regSetBool('tool.showAbilityIcon'       , true, true)
 
     -- Pad
-    regSetBool('pad.soundOn'      , true)
-    regSetFloat('pad.size'        , 1)
-    regSetFloat('pad.jumpForce'   , 7)
-    regSetFloat('pad.bounceRatio' , 1)
-    regSetFloat('pad.lifespan'    , 0)
+    regSetBool('pad.soundOn'      , true, true)
+    regSetFloat('pad.size'        , 1, true)
+    regSetFloat('pad.jumpForce'   , 7, true)
+    regSetFloat('pad.bounceRatio' , 1, true)
+    regSetFloat('pad.lifespan'    , 0, true)
...
-
-        input = {
-            didPlacePad = function() return tool.tool.active() and InputPressed('lmb') end,
-            didReplacePad = function() return tool.tool.active() and not tool.tool.input.isEditing() and InputPressed('mmb') end,
-            didReset = function() return tool.tool.active() and InputPressed('k') end,
-            didDeletePad = function() return tool.tool.active() and InputPressed('l') end,
...
-            isRotatingRoll = function() return tool.tool.input.isEditing() and InputDown('x') end,
-            isRotatingYaw = function() return tool.tool.input.isEditing() and InputDown('b') end,
-            isRotatingPitch = function() return tool.tool.input.isEditing() and InputDown('v') end,
-            getScroll = function() return tool.tool.input.isEditing() and InputValue( "mousewheel" ) or 0 end,
-            isPadsIgnorePlayer = function() return tool.tool.active() and InputDown('ctrl') end,
-        },
-
-        setup = {
-            name = 'bouncepad',
...
-            do UiPush()
-
-                ui.slider.create('Pad preview transparency', 'tool.padPlaceholder.alpha', nil, 0, 1)
-                ui.padding.create(0, 30)
-
-                ui.checkBox.create('Show pad type', 'tool.showAbilityText')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Show pad icon', 'tool.showAbilityIcon')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Ignore fall damages', 'tool.noFallDamage')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Align Pads on grid', 'tool.useGrid')
-                ui.padding.create(0, 64) 
-
-            UiPop() end
-
...
-            do UiPush()
-
-                ui.slider.create('Size', 'pad.size', 'Meters', 1, 10)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Bounce multiplier', 'pad.bounceRatio', nil, 0.1, 2)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Jump force (Jumppad only)', 'pad.jumpForce', nil, 1, 100)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Lifespan (0 = infinite)', 'pad.lifespan', 'Seconds', 0, 60)
-                ui.padding.create(0, 30)
-
-                ui.checkBox.create('Sound', 'pad.soundOn')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
-            do UiPush()
-
-                ui.padding.create(900, -20)
-                UiAlign('center top')
-                local color = Pad.getColor("Trampoline")
-                UiColor(color.r,color.g,color.b,color.a)
-                UiFont('bold.ttf', 32)
-                UiText('Trampoline')
-                ui.padding.create(0, 32)
-                UiColor(1,1,1, 1)
-                UiFont('regular.ttf', font_normal/1.5)
-                UiText('Objects bounce naturally, keeping their momentum')
-                ui.padding.create(0, 32)
-                UiImage('MOD/img/bounce_trampoline.png')
-                ui.padding.create(0, 220)
-
-                local color = Pad.getColor("Jumppad")
-                UiColor(color.r,color.g,color.b,color.a)
-                UiFont('bold.ttf', 32)
-                UiText('Jumppad')
-                ui.padding.create(0, 32)
-                UiColor(1,1,1, 1)
-                UiFont('regular.ttf', font_normal/1.5)
-                UiText('Objects bounce straight, always with the same force')
-                ui.padding.create(0, 32)
-                UiImage('MOD/img/bounce_jumppad.png')
-
-            UiPop() end
-
-            do UiPush()
-
-                ui.padding.create(1300, -20)
-                UiAlign('center top')
-                local color = Pad.getColor("Bouncepad")
-                UiColor(color.r,color.g,color.b,color.a)
-                UiFont('bold.ttf', 32)
-                UiText('Bouncepad')
-                ui.padding.create(0, 32)
-                UiColor(1,1,1, 1)
-                UiFont('regular.ttf', font_normal/1.5)
-                UiText('Objects bounce straight, keeping their momentum')
-                ui.padding.create(0, 32)
-                UiImage('MOD/img/bounce_bouncepad.png')
-                ui.padding.create(0, 220)
-
-                local color = Pad.getColor("Antipad")
-                UiColor(color.r,color.g,color.b,color.a)
-                UiFont('bold.ttf', 32)
-                UiText('Antipad')
-                ui.padding.create(0, 32)
-                UiColor(1,1,1, 1)
-                UiFont('regular.ttf', font_normal/1.5)
-                UiText('Objects bounce back, keeping their momentum')
-                ui.padding.create(0, 32)
-                UiImage('MOD/img/bounce_antipad.png')
-
-            UiPop() end
-
-        UiPop() end
...
-            UiText('[T]: Change Pad type (hold)')
-            ui.padding.create(0, 40)
-            UiText('[Left Click]: Place a Pad')
-            ui.padding.create(0, 40)
-            UiText('[Middle Click]: Replace closest Pad')
-            ui.padding.create(0, 40)
-            UiText('[L]: Remove closest Pad')
-            ui.padding.create(0, 40)
-            UiText('[K]: Remove all Pads')
-            ui.padding.create(0, 40)
-            UiText('[Ctrl]: Player noclip Pads (hold)')
-            ui.padding.create(0, 40)
-            UiText('[Right Click]: Move Pad preview in front of you (hold)')
-            ui.padding.create(0, 40)
-            UiText('[Right Click]+[Scroll]: Move Pad preview closer/further')
-            ui.padding.create(0, 40)
-            UiText('[Right Click]+[X]+[Scroll]: Rotate Pad preview pitch')
-            ui.padding.create(0, 40)
-            UiText('[Right Click]+[B]+[Scroll]: Rotate Pad preview roll')
-            ui.padding.create(0, 40)
-            UiText('[Right Click]+[V]+[Scroll]: Rotate Pad preview yaw')
-            ui.padding.create(0, 40)
-            UiText('[Right Click]+[Middle Click]: Reinitialize Pad preview')
-            ui.padding.create(0, 40)
-            UiText('[Right Click]+[Ctrl]: Increase move precision (hold)')
-            ui.padding.create(0, 40)
-
-        UiPop() end
-        
...
-            UiFont('regular.ttf', font_normal)
-            UiText('- Pads will bounce any moving object or player, regardless of their size or mass')
-            ui.padding.create(0, 80)
-            UiText('- Pads will bounce on both sides')
-            ui.padding.create(0, 80)
-            UiText('- Pads will only bounce physical objects (not bullets or lasers)')
-            ui.padding.create(0, 80)
-            UiText('- Pads are unbreakable (immune to explosions, lasers, weapons, etc.)')
-            ui.padding.create(0, 80)
-            UiText('- If an object moves too slow, Pads will ignore it to avoid flickering')
-            ui.padding.create(0, 80)
-            UiText('- Values changed in the options only apply to future Pads, not existing ones')
-            ui.padding.create(0, 80)
-
-        UiPop() end
-        
...
-
-
---- Draw options shorcut key hint at bottom left.
 function uiDrawKeyHint(addHeight)
...
---- Draw pad type above tool name.
-function uiDrawPadType()
+-
+
+ction uiDrawPadType()
...
@@ -511,7 +303,7 @@
             ...
             UiPush()
-                if Pad.ability.highlightTimer > 0 then
+                if Pad.ability.highlightTimer ~= 0 then
                     local a = Pad.ability.highlightTimer
                     UiColor(1, 1, 1, a)
...
-function uiDrawAbilityWheel()
+f
+
+ction uiDrawAbilityWheel()
 
     uiDrawWheel(Pad.ability)
 
 end
 
-function uiDrawWheel(category)
+f
+
+ction uiDrawWheel(category)
     local imgW = 100
     local imgH = 100
...
-function uiDrawWheelItem(list, index, hovered)
+f
+
+ction uiDrawWheelItem(list, index, hovered)
...
+-
+
+ction uiManageGameOptions()
 
     if tool.tool.active() then
         -- Cannot open menu if other menu open
@@ -647,8 +446,9 @@
...
-function uiIsMenuOpen()
+f
+
+ction uiIsMenuOpen()
 
     return UI_GAME or UI_ABILITY
...
+ction ui.container.create(w, h, c, a)
...
+ction ui.tabView.tab.create(text)
...
+ction ui.slider.create(title, registryPath, valueText, min, max, w, h, fontSize, inline, intSlider)
 
     local hasTitle = not(title == nil or title == '')
     local intSlider = intSlider or false
@@ -799,9 +569,9 @@
...
+ction ui.checkBox.create(title, registryPath)
...
+u
+
+ction ui.colorPicker.create(title, registryPath, w, h, fontSize, useAlpha)
...
+u
+
+ction ui.button.create(text, textColor, borderColor, bgColor, border, w, h)
     local button = nil
     do UiPush()
...
@@ -1119,7 +1141,7 @@
         --DrawSprite(padSprite, tr, w, h, 0, 0, 0, 1)
     end
