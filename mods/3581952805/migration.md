...
+++ patched/main.lua
@@ -1,100 +1,13 @@
---rmwkrissvector custom tool example
-
...
-RECOIL_BASE_HIP   = 0.3   -- per-shot pitch in hip
-RECOIL_BASE_ADS   = 0.2   -- per-shot pitch in ADS
...
-VISUAL_SCALE_HIP  = 0.5   -- viewmodel tilt multiplier (hip)
-VISUAL_SCALE_ADS  = 0.5   -- viewmodel tilt multiplier (ADS)
...
-	RegisterTool("rmwkrissvector", "Kriss Vector", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwkrissvector.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwkrissvector", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
+tion tick(dt)
 	...
 	--Check if rmwkrissvector is selected
@@ -183,7 +94,7 @@
 		local d = GetToolBody()
...
+
+tion draw()
 if GetString("game.player.tool") == "rmwkrissvector" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwkrissvector", "Kriss Vector", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwkrissvector.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
