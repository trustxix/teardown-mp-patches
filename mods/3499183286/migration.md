...
-
--- ===== Recoil limits (put near your recoil tunables) =====
-MAX_CAM_PITCH = MAX_CAM_PITCH or 16   -- hard cap for camera kick up/down (deg)
-MAX_CAM_YAW   = MAX_CAM_YAW   or 12    -- hard cap for camera left/right (deg)
-
-MAX_PER_SHOT_PITCH = MAX_PER_SHOT_PITCH or 2.0  -- optional: cap how much a single shot can add
-MAX_PER_SHOT_YAW   = MAX_PER_SHOT_YAW   or 1.2
-
--- Visual (gun model) caps; can be lower than camera caps so the gun looks controlled
-MAX_VIS_PITCH = MAX_VIS_PITCH or 30
-MAX_VIS_YAW   = MAX_VIS_YAW   or 15
-
--- ========= /Recoil config =========
-
-
...
@@ -67,59 +12,11 @@
 	local gt = GetBodyTransform(GetToolBody())
 	local linkpos = TransformToParentPoint(gt, Vec(0.18, 0.09, -0.3))
 	local fwdpos = TransformToParentPoint(gt, Vec(6+math.random()*4, 0.5+math.random()*4, -0.65+math.random()*4))
-	local direction = VecAdd(GetPlayerVelocity(), VecSub(fwdpos, linkpos))
+	local direction = VecAdd(GetPlayerVelocity(playerId), VecSub(fwdpos, linkpos))
 	links = Spawn("MOD/vox/links.xml", Transform(linkpos, QuatEuler(math.random(0, 90), math.random(0, 90), math.random(0, 90))))
 	SetBodyVelocity(links[1], direction)
 end
 
-function init()
-	--Register tool and enable it
-	RegisterTool("rmwmg3", "MG3", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwmg3.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwmg3", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
@@ -162,8 +57,6 @@
 	recoilYaw   = clamp(recoilYaw   + yawKick,-MAX_CAM_YAW,   MAX_CAM_YAW)
 end
 
-
--- Per-tick: apply camera recoil (small) & recover
 function ApplyRecoilTick(dt, aiming, reloading, attachmentmenu)
 	-- FPS only: apply offset; TPS/menus: clear offset
 	if not GetBool("game.thirdperson") and not attachmentmenu and not reloading then
@@ -183,9 +76,7 @@
...
-function ApplyVisualGunRecoil(dt, aiming, baseTransform)
+tion ApplyVisualGunRecoil(dt, aiming, baseTransform)
 	local scale = aiming and VISUAL_SCALE_ADS or VISUAL_SCALE_HIP
 	-- clamp visual tilt so the gun never rolls away
 	local vp = clamp(recoilPitch * scale, -MAX_VIS_PITCH, MAX_VIS_PITCH)
@@ -195,11 +86,9 @@
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwmg3" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwmg3", "MG3", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwmg3.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
...
-			
-			local optic = opticssight[13]
-                if reloading then ---Magazine
...
-                    RemoveTag(optic, "invisible")
-					
-					SetShapeLocalTransform(optic, Transform(Vec(-0.025, 0.14, -0.1), QuatEuler(-90, 0, 0)))
-				else
-				SetTag(optic, "invisible")
-					
-					
...
-                if optics == 2 then ---Acog2Ret
-                    RemoveTag(optic, "invisible")
-					SetShapeLocalTransform(optic, Transform(Vec(0.0078, 0.214, -0.25), QuatEuler(-90, 0, 45)))
-				else
-				SetTag(optic, "invisible")
-                end
-				local optic = opticssight[10]
-                if optics == 2 then ---Acog3Ret
-                    RemoveTag(optic, "invisible")
-					SetShapeLocalTransform(optic, Transform(Vec(0.0078, 0.213, -0.245), QuatEuler(-90, 0, 45)))
-				else
-				SetTag(optic, "invisible")
-                end
-				local optic = opticssight[11]
-                if optics == 3 then ---Holo1Ret
-                    RemoveTag(optic, "invisible")
-					SetShapeLocalTransform(optic, Transform(Vec(0.0078, 0.213, -0.2), QuatEuler(-90, 0, 45)))
-				else
-				SetTag(optic, "invisible")
-                end
-				local optic = opticssight[12]
-                if optics == 3 then ---Holo2Ret
-                    RemoveTag(optic, "invisible")
-					SetShapeLocalTransform(optic, Transform(Vec(0.0078, 0.211, -0.19), QuatEuler(-90, 0, 45)))
-				else
-				SetTag(optic, "invisible")
-                end
