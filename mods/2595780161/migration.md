...
-
-function init()
-	RegisterTool("vector", "Vector", "MOD/vox/vector.vox", 3)
-	SetBool("game.tool.vector.enabled", true)
-	SetFloat("game.tool.vector.ammo", 101)
-
-
...
-		PlaySound(suppressedgunsound, GetPlayerTransform().pos, 0.6, false)
+		PlaySound(suppressedgunsound, GetPlayerTransform(playerId).pos, 0.6, false)
 	elseif muzzle == "muzzle2" then
-		PlaySound(gunsound, GetPlayerTransform().pos, 0.8, false)
+		PlaySound(gunsound, GetPlayerTransform(playerId).pos, 0.8, false)
 	else
...
+		PlaySound(reloadsound, GetPlayerTransform(playerId).pos, 0.5, false)
 ...
 function SelectFire()
 	if selectfire == 0 or selectfire == 3 then
...
@@ -669,7 +350,7 @@
 	...
 	local casingpos = TransformToParentPoint(gt, Vec(0.55, -0.5, -1.3))
 	local fwdpos = TransformToParentPoint(gt, Vec(6+math.random()*4, 0.5+math.random()*4, -0.8+math.random()*4))
...
-
-function tick(dt)
-	if GetString("game.player.tool") == "vector" and GetPlayerVehicle() == 0 then
...
-				Shoot()
-			end
-		elseif InputDown("lmb") and not reloading and selectfire == 2 and not selectattachments and not selectmag and selectfireTimer == 0 and GetPlayerGrabShape() == 0 and not InputDown("shift") and inspectTimer <= 0 and magcheckTimer == 0 and not jammed and jamclearTimer == 0 and burstshots < 2 then
-			if grenadelauncherammo > 0 and grenadelauncher then
-				ShootGrenade()
-			elseif not grenadelauncher and ammo > 0 then
-				Shoot()
...
-
-		
-		if InputDown("rmb") and selectfire > 0 and selectfireTimer == 0 and not selectattachments and not selectmag and GetPlayerGrabShape() == 0 and not InputDown("shift") and not selectmag and magcheckTimer == 0 and jamclearTimer == 0 then
...
-		end
-
-		if InputPressed(meleeKey) and not reloading and not ironsight and shootTimer <= 0 and selectfire > 0 and stock ~= "removed" and not InputDown("shift") then
-			meleeTimer = 0.8
-			inspectTimer = 0
-		end
-
...
-		end
-
-		if InputPressed(inspectKey) and not reloading and not InputDown("shift") and selectfireTimer <= 0 and not ironsight and selectfire > 0 and not selectattachments and not selectmag and magcheckTimer == 0 and magcheckTimer == 0 and jamclearTimer == 0 then
...
-
-		if ironsight then
-			recoilMax = 6*muzzleFactor*gripfactory*barrelFactory*stockFactor
-		else
...
-			local boltoffset = Vec(0, -0.025, 0)
-			local selectoroffset = Vec(-0.025, 0.005, 0)
-			local sightoffset = Vec(0, 0, -0.005)
...
-				if toprail == "holo" then
-					local gt = GetBodyTransform(GetToolBody())
-					local sightcenter = Vec(0.275, -0.325, -1.1)
...
-
-					local holopoint = VecAdd(VecAdd(camtrans.pos, VecScale(VecNormalize(VecSub(camtrans.pos, hitpoint)), -sightdist)), VecScale(GetPlayerVelocity(), 0.02))
-
...
-					local gt = GetBodyTransform(GetToolBody())
-					local sightcenter = Vec(0.275, -0.2125, -1.25)
-					local gunpos = TransformToParentPoint(gt, sightcenter)
-					local fwdpos = TransformToParentVec(gt, Vec(0, 0, -1))
-					local hit, dist = QueryRaycast(gunpos, fwdpos, 500, 0, true)
-					local hitpoint = VecAdd(gunpos, VecScale(fwdpos, dist))
...
-					scopepoint = TransformToParentTransform(btrans, Transform(Vec(0.2745, -0.252, -1.05), QuatEuler(0, 180, 0)))
...
-				-- offset.pos = VecAdd(offset.pos, Vec(0, 0, recoilTimer2/2))
-				-- offset.rot = QuatRotateQuat(offset.rot, QuatEuler(recoilTimer2*5, 0, 0))
-			end
-			if recoilTimer > 0 then
-				recoilTimer = recoilTimer - dt
-				boltoffset = Vec(0, -0.025, recoilTimer*7)
-			end
-			local rx, ry, rz, rr1, rr2, rr3 = (rnd2+rnd5)/400, (recoilAngle/-250)-(rnd1+rnd4)/400-recoilTimer2/5, recoilAngle/50+recoilTimer2/2+recoilAngle/125, recoilAngle+(rnd1+rnd4)/4+recoilTimer2*5, (rnd2+rnd5)/4, 0
...
-				boltoffset = Vec(0, -0.025, 1-jamclearTimer)
...
-				else
-					magoffset = Vec(0, -(0.6-magoutTimer)*4, (0.6-magoutTimer))
-				end
...
-				else
-					magoffset = Vec(0, -maginTimer*4, maginTimer)
-				end
-				maginTimer = maginTimer - dt/reloadFactor
-			end
-			if magcheckTimer > 3.4 then
-				magoffset = Vec(0, -(4-magcheckTimer)*2, (4-magcheckTimer)*0.5)
-			elseif magcheckTimer > 2.8 then
-				magoffset = Vec(-(3.4-magcheckTimer)*0.5, (3.4-magcheckTimer)*2-1.2, (3.4-magcheckTimer)*0.5+0.3)
-			elseif magcheckTimer > 1.2 then
-				magoffset = Vec(-0.3, 0, 0.6)
-			elseif magcheckTimer > 0.6 then
-				magoffset = Vec(-magcheckTimer*0.5+0.3, magcheckTimer*2-1.2-1.2, magcheckTimer*0.5+0.3-0.3)
-			elseif magcheckTimer > 0 then
-				magoffset = Vec(0, -magcheckTimer*2, magcheckTimer*0.5)
-			end
...
-					if ammo == 0 then
-						if magoutTimer > 0 then
-							x1, y1, z1, rotx1, roty1, rotz1 = 1.1, 0.25, 0, -5, 10, -50
-						elseif maginTimer > 0 or reloadTimer > 0.7 then
-							x1, y1, z1, rotx1, roty1, rotz1 = 1.2, 0.3, -0.1, -5, 10, -40
-						elseif reloadTimer < 0.7 then
-							x1, y1, z1, rotx1, roty1, rotz1 = 2, 0, 0.2, 10, 35, -60
-						end
-					else
...
-					elseif reloadTimer < 0.6 and ammo == 0 then
-						x1, y1, z1, rotx1, roty1, rotz1 = 0.35, -0.3, 0, 15, 25, -10
-					end
-				else
-					if magoutTimer > 0 or magcheckTimer > 0 then
-						x1, y1, z1, rotx1, roty1, rotz1 = 0.1, -0.05, 0, 0, 0, -20
-					elseif maginTimer > 0 or (reloadTimer > 0.4 and ammo > 0) or (reloadTimer > 1 and ammo == 0) then
-						x1, y1, z1, rotx1, roty1, rotz1 = 0.3, 0.1, 0, 0, 0, -35
-					elseif reloadTimer < 0.6 and ammo == 0 then
-						x1, y1, z1, rotx1, roty1, rotz1 = 0.3, 0.1, -0.1, -5, 10, 10
-					end
-				end
-			else
-				if magoutTimer > 0 or magcheckTimer > 0 then
-					x1, y1, z1, rotx1, roty1, rotz1 = 0.1, 0.05, 0, 10, -10, -20
-				elseif maginTimer > 0 or (reloadTimer > 0.4 and ammo > 0) then
-					x1, y1, z1, rotx1, roty1, rotz1 = 0.1, 0, 0, 5, 0, -10
-				elseif reloadTimer < 0.6 and ammo == 0 then
-					x1, y1, z1, rotx1, roty1, rotz1 = 0.65, -0.2, 0, 10, 30, -25
-				end
-			end
-			end
-
...
-			elseif InputDown("shift") and not reloading then
-				x3, y3, z3, rotx3, roty3, rotz3 = 0.15, -0.7, 0.7, 50, 0, -10
...
-				boltTrans3 = GetShapeLocalTransform(bolt3)
-				selectorTrans = GetShapeLocalTransform(selector)
-				selectorTrans2 = GetShapeLocalTransform(selector2)
...
-
-			mwt = TransformCopy(magwellTrans)
-			mwt.pos = VecAdd(mwt.pos, magwelloffset)
-
-			bt2.pos = VecAdd(bt2.pos, Vec(0.025, 0.025, 0))
-
-			if reloadTimer < 0.3 and reloadTimer > 0.2 and reloading and ammo == 0 then
-				bt2.rot = QuatRotateQuat(bt2.rot, QuatEuler(0, 0, -10*(0.3-reloadTimer)/0.1))
-			elseif reloadTimer < 0.2 and reloadTimer > 0.1 and reloading and ammo == 0 then
-				bt2.rot = QuatRotateQuat(bt2.rot, QuatEuler(0, 0, -10*(reloadTimer-0.1)/0.1))
-			end
-
-			if jamclearTimer < 1 and jamclearTimer > 0.7 then
-				bt3.pos = VecAdd(bt3.pos, Vec(0.1*(1-jamclearTimer)/0.3, 0, 0.3*(1-jamclearTimer)/0.3))
-				bt3.rot = QuatRotateQuat(bt3.rot, QuatEuler(0, 0, 90*(1-jamclearTimer)/0.3))
-			end
-
-			if selectfireTimer < 0.4 then
-				if selectfire == 1 then
-					st.rot = QuatRotateQuat(st.rot, QuatEuler(0, 0, 0))
-					st.pos = VecAdd(st.pos, Vec(0, 0, 0))
-				elseif selectfire == 2 then
-					st.rot = QuatRotateQuat(st.rot, QuatEuler(40, 0, 0))
-					st.pos = VecAdd(st.pos, Vec(0, -0.05, -0.025))
-				elseif selectfire == 3 then
-					st.rot = QuatRotateQuat(st.rot, QuatEuler(90, 0, 0))
-					st.pos = VecAdd(st.pos, Vec(0, -0.05, -0.1))
-				else
-					st.rot = QuatRotateQuat(st.rot, QuatEuler(0, 0, 0))
-					st.pos = VecAdd(st.pos, Vec(0, 0, 0))
-				end
-				if selectfire > 0 then
-					st2.rot = QuatRotateQuat(st2.rot, QuatEuler(90, 0, 0))
-					st2.pos = VecAdd(st2.pos, Vec(0, -0.05, -0.1))
-				else
-					st2.rot = QuatRotateQuat(st2.rot, QuatEuler(0, 0, 0))
-					st2.pos = VecAdd(st2.pos, Vec(0, 0, 0))
-				end
-			elseif selectfireTimer > 0 then
...
-			lightFactor = 1
-			if muzzle == "muzzle1" then
-				spt.pos = VecAdd(spt.pos, Vec(0.025, 0, -barrellength))
-				lightFactor = 0.4
-				muzzlelength = 0.8
-			else
-				spt.pos = hidePos
...
-				mbt.pos = VecAdd(mbt.pos, Vec(0.075, -0.045, -barrellength))
...
-			end
-			if toprail == "scope" then
-				sct.pos = Vec(0.2, -0.425, -0.85)
-			else
-				sct.pos = hidePos
-			end
-			if toprail == "holo" then
-				ht.pos = Vec(0.175, -0.425, -1)
-				ht2.pos = Vec(0.225, -0.35, -1)
-			else
-				ht.pos = hidePos
-				ht2.pos = hidePos
-			end
...
-			end
-
-			sct.pos = VecAdd(sct.pos, sightoffset)
-			ht.pos = VecAdd(ht.pos, sightoffset)
-			ht2.pos = VecAdd(ht2.pos, sightoffset)
-			rdt.pos = VecAdd(rdt.pos, sightoffset)
-			rdt2.pos = VecAdd(rdt2.pos, sightoffset)
-
-			if stock == "removed" then
-				stt.pos = Vec(0.5, -0.95, -1.15)
...
-				stockFactor = 1
-			end
-			rt.pos = VecAdd(rt.pos, Vec(-0.025, 0.025, -0.005))
-			mwt.pos = VecAdd(mwt.pos, Vec(0, 0.16, 0.135))
-			mwt.rot = QuatRotateQuat(mwt.rot, QuatEuler(-15, 0, 0))
-
-			if mag == "" or mag == "mag0" then
-				magsize = 17
-				reloadFactor = 0.9
-				SetShapeLocalTransform(mag0, Transform(VecAdd(mt.pos, Vec(-0.025, 0.03, 0.15)), mt.rot))
...
-				magsize = 33
-				reloadFactor = 1.1
-				SetShapeLocalTransform(mag1, Transform(VecAdd(mt.pos, Vec(-0.025, -0.375, 0.25)), mt.rot))
...
-				magsize = 50
-				reloadFactor = 1.3
-				SetShapeLocalTransform(mag2, Transform(VecAdd(mt.pos, Vec(-0.175, -0.35, 0.3)), mt.rot))
-				SetShapeLocalTransform(mag0, hideTrans)
-				SetShapeLocalTransform(mag1, hideTrans)
-				SetShapeLocalTransform(mag3, hideTrans)
-			elseif mag == "mag3" then
-				magsize = 100
-				reloadFactor = 1.5
-				SetShapeLocalTransform(mag3, Transform(VecAdd(mt.pos, Vec(-0.475, -0.35, 0.3)), mt.rot))
-				SetShapeLocalTransform(mag0, hideTrans)
-				SetShapeLocalTransform(mag1, hideTrans)
-				SetShapeLocalTransform(mag2, hideTrans)
-			end
...
-				gripfactorx = 0.85
-				gripfactory = 0.6
-				gt1.pos = Vec(0.2, -0.85+guardheight, -1.6-guardlength)
-				gt1_2.pos = Vec(0.225, -1.05+guardheight, -1.625-guardlength)
-			else
-				gt1.pos = hidePos
-				gt1_2.pos = hidePos
-			end
...
-				gripfactorx = 0.6
-				gripfactory = 0.85
-				gt2.pos = Vec(0.225, -0.9+guardheight, -1.5-guardlength)
-			else
-				gt2.pos = hidePos
-			end
-			if grip == "grip3" then
...
-			if side == "side1" then
-				if guard == "guard2" then
-					sdt1.pos = VecAdd(sdt1.pos, Vec(0, 0, -0.45))
-				else
-					sdt1.pos = sdt1.pos
-				end
-			else
...
-			if side == "side2" then
-				if guard == "guard2" then
-					sdt2.pos = VecAdd(sdt2.pos, Vec(0.025, 0.025, -0.45))
-				else
-					sdt2.pos = VecAdd(sdt2.pos, Vec(0.025, 0.025, 0))
-				end
-			else
-				sdt2.pos = hidePos
-			end
...
-
-			if guard == "" then
-				gdt0.pos = Vec(0.175, -0.825, -1.35)
-				gdt0_2.pos = Vec(0.2, -0.7, -1.35)
-				guardlength = 0
-				guardheight = -0.025
-			else
-				gdt0.pos = hidePos
...
-				gdt1.pos = Vec(0.225, -0.725, -1.35)
-				gdt1_2.pos = Vec(0.2, -0.7, -1.35)
-				guardlength = -0.1
-				guardheight = 0
-			else
-				gdt1.pos = hidePos
...
-			end
-			if guard == "guard2" then
-				gdt2.pos = Vec(0.175, -0.725, -1.5)
-				gdt2_2.pos = Vec(0.2, -0.7, -1.35)
-				guardlength = 0.3
-				guardheight = 0
-			else
-				gdt2.pos = hidePos
...
-			SetShapeLocalTransform(bolt3, bt3)
-			SetShapeLocalTransform(selector, st)
-			SetShapeLocalTransform(selector2, st2)
...
-			SetShapeLocalTransform(magwell, mwt)
-		end
-
-		if selectattachments then
-			clickedmag = clickedmag1 or clickedmag2 or clickedmag3
-			if clickedmag and selectattachments and not InputPressed(attachmentsKey) then
-				Reload()
-			end
-		end
...
-			end
-
-			if selectfireTimer < 0.55 and selectfireTimer > 0 and not selectsoundplaying and selectfire ~= 1 then
...
-		end
-		local x2, y2, z2, rotx2, roty2, rotz2 = 0.175, -0.85, -1.8-guardlength, 0, 0, 0
-		if reloading and ammo == 0 and reloadTimer < 0.3 then
-			x2, y2, z2, rotx2, roty2, rotz2 = 0.1, -0.7, -0.4+bt2.pos[3], 0, 0, 0
-		elseif reloading then
-			x2, y2, z2, rotx2, roty2, rotz2 = -0.05+mt.pos[1], 0.3+mt.pos[2], -0.05+mt.pos[3], 0, 0, 0
-		elseif selectfireTimer > 0 then
-			x2, y2, z2, rotx2, roty2, rotz2 = 0.175, -0.825, -0.9, 0, 0, 0
-		elseif InputDown("shift") and VecLength(GetPlayerVelocity()) > 2 then
-			x2, y2, z2, rotx2, roty2, rotz2 = -0.7-swayx*0.2, -1.6, -1+swayx*0.2, 0, 0, 0
-		end
-
...
-
-
-		sightattachpoint = TransformToParentPoint(btrans, Vec(0.25, -0.15, -0.75))
-		muzzleattachpoint = TransformToParentPoint(btrans, Vec(0.25, -0.35, -2.5-barrellength))
...
-		end
-	end
-	if GetString("game.player.tool") == "vector" and grenadelauncher and not selectattachments then
-		UiPush()
...
-		UiPop()
-	end
-	if GetString("game.player.tool") == "vector" and sideattachment and side == "side3" then
-		UiPush()
...
-	end
-
-	if realistic and selectmag and GetString("game.player.tool") == "vector" then
-		hoverindex=0
-		UiPush()
...
-	if selectattachments and GetString("game.player.tool") == "vector" and GetPlayerVehicle() == 0 then
-	hoverindex=0
-		UiPush()
...
-					UiTranslate(-70,0)
-					curx,cury=curx-70,cury
-					clickedholo = AttachmentButton("toprail","sight3",true,{curx,cury},{"Red Dot Sight","1x magnification with open and borderless sight picture."})
-					UiTranslate(-70,0)
-				UiPop()
-			end
-		UiPop()
...
-				UiPush()
-					UiAlign("center middle")
-					clickedbarrel1 = AttachmentButton("barrel","barrel1",true,{curx,cury},{"Short Barrel","Decreased accuracy but less vertical recoil."})
-					UiTranslate(-100,0)
-					curx,cury=curx-70,cury
-					clickedbarrel2 = AttachmentButton("barrel","barrel2",true,{curx,cury},{"Extended Barrel","Increased accuracy and damage but more vertical recoil."})
-					UiTranslate(-70,0)
-				UiPop()
-			end
-		UiPop()
...
-				UiPush()
-					UiAlign("center middle")
-					clickedguard1 = AttachmentButton("guard","guard1",true,{curx,cury},{"Short Guard","For short barrels."})
-					UiTranslate(-100,0)
-					curx,cury=curx-70,cury
-					if barrel == "barrel2" then
-						clickedguard2 = AttachmentButton("guard","guard2",true,{curx,cury},{"Long Guard","For long barrels."})
-					end
-					UiTranslate(-70,0)
-				UiPop()
-			end
...
-	end
-	end
-	if selectattachments and GetString("game.player.tool") == "vector" and GetPlayerVehicle() == 0 or GetBool("level.optionstriggered") and GetString("game.player.tool") == "vector" then
-	hoverindex=0
-	UiPush()
...
-				UiPush()
-					UiAlign("center middle")
-					clickedstockremoved = AttachmentButton("stock","removed",true,{curx,cury},{"Fold Stock","Folds stock for a smaller profile weapon."})
-				UiPop()
-			end
-		UiPop()
-	end
...
-
-	------OPTIONS UI------
-	if GetBool("level.optionstriggered") and GetString("game.player.tool") == "vector" then
-	UiPush()
-		UiAlign("center middle")
...
+    RegisterTool("vector", "Vector", "MOD/vox/vector.vox", 3)
+    SetBool("game.tool.vector.enabled", true, true)
+    SetFloat("game.tool.vector.ammo", 101, true)
+    ------INITIALISE OPTIONS MENU------
+    damageoption = GetInt("savegame.mod.damage")
+    if damageoption < 50 then
+    	damageoption = 100
...
+    if GetString("game.player.tool") == "vector" and GetPlayerVehicle(playerId) == 0 then
...
+    			Shoot()
+    		end
+    	elseif InputDown("lmb") and not reloading and selectfire == 2 and not selectattachments and not selectmag and selectfireTimer == 0 and GetPlayerGrabShape(playerId) == 0 and not InputDown("shift") and inspectTimer <= 0 and magcheckTimer == 0 and not jammed and jamclearTimer == 0 and burstshots < 2 then
+    		if grenadelauncherammo > 0 and grenadelauncher then
+    			ShootGrenade()
+    		elseif not grenadelauncher and ammo ~= 0 then
+    			Shoot()
...
+    	end
+
+    	if InputDown("rmb") and selectfire > 0 and selectfireTimer == 0 and not selectattachments and not selectmag and GetPlayerGrabShape(playerId) == 0 and not InputDown("shift") and not selectmag and magcheckTimer == 0 and jamclearTimer == 0 then
+    		if InputPressed("rmb") then
...
+    				local sightdist = VecLength(VecSub(camtrans.pos, gunpos))
+
+    				local holopoint = VecAdd(VecAdd(camtrans.pos, VecScale(VecNormalize(VecSub(camtrans.pos, hitpoint)), -sightdist)), VecScale(GetPlayerVelocity(playerId), 0.02))
+
+    				local holotrans = Transform(holopoint, QuatLookAt(camtrans.pos, hitpoint))
+
+    				if VecLength(VecSub(TransformToParentPoint(gt, sightcenter), holopoint)) < 0.1 then
...
+    	elseif selectfireTimer ~= 0 then
+    		x2, y2, z2, rotx2, roty2, rotz2 = 0.175, -0.825, -0.9, 0, 0, 0
+    	elseif InputDown("shift") and VecLength(GetPlayerVelocity(playerId)) > 2 then
+    		x2, y2, z2, rotx2, roty2, rotz2 = -0.7-swayx*0.2, -1.6, -1+swayx*0.2, 0, 0, 0
+    	end
+
+    	h1p1l = INS(true, x2, y2, z2, rotx2, roty2, rotz2).pos
...
+    ------ATTACHMENTS MENU------
+    if not realistic or GetBool("level.optionstriggered") then
+    if selectattachments and GetString("game.player.tool") == "vector" and GetPlayerVehicle(playerId) == 0 then
+    hoverindex=0
+    	UiPush()
+    		local x,y,dist=UiWorldToPixel(sightattachpoint)
+    		if dist ~= 0 then
...
+    end
+    end
+    if selectattachments and GetString("game.player.tool") == "vector" and GetPlayerVehicle(playerId) == 0 or GetBool("level.optionstriggered") and GetString("game.player.tool") == "vector" then
+    hoverindex=0
+    UiPush()
+    	local x,y,dist=UiWorldToPixel(stockattachpoint)
+    		if dist ~= 0 then
...
@@ -1,9 +1,13 @@
-function tick(dt)
-	SetString("game.player.tool","vector")
-	SetBool("level.optionstriggered",true)
+#version 2
+function server.tick(dt)
+    for _, playerId in ipairs(GetAllPlayers()) do
+        SetString("game.player.tool","vector", true)
