...
-    regSetString('world.generate'   , "floor + walls")
-    regSetInt('world.floors'        , 1)
...
+    regSetString('world.generate'   , "floor + walls", true)
+    regSetInt('world.floors'        , 1, true)
 
     -- Floor
-    regSetBool('floor.unbreakable'  , false)
-    regSetString('floor.material'   , "masonry")
-    regSetInt('floor.color.r'       , 127)
-    regSetInt('floor.color.g'       , 127)
-    regSetInt('floor.color.b'       , 127)
-    regSetInt('floor.color.a'       , 255)
-    regSetString('floor.texture'    , "none")
...
-    regSetInt('floor.rampProba'     , 10)
+    regSetBool('floor.unbreakable'  , false, true)
+    regSetString('floor.material'   , "masonry", true)
+    regSetInt('floor.color.r'       , 127, true)
+    regSetInt('floor.color.g'       , 127, true)
+    regSetInt('floor.color.b'       , 127, true)
+    regSetInt('floor.color.a'       , 255, true)
+    regSetString('floor.texture'    , "none", true)
...
+    regSetInt('floor.rampProba'     , 10, true)
 
     -- Walls
-    regSetBool('walls.unbreakable'  , false)
-    regSetString('walls.material'   , "masonry")
-    regSetInt('walls.color.r'       , 200)
-    regSetInt('walls.color.g'       , 200)
-    regSetInt('walls.color.b'       , 200)
-    regSetInt('walls.color.a'       , 255)
-    regSetString('walls.texture'    , "none")
...
-    regSetInt('walls.count'         , 1)
-    regSetInt('walls.proba'         , 100)
+    regSetBool('walls.unbreakable'  , false, true)
+    regSetString('walls.material'   , "masonry", true)
+    regSetInt('walls.color.r'       , 200, true)
+    regSetInt('walls.color.g'       , 200, true)
+    regSetInt('walls.color.b'       , 200, true)
+    regSetInt('walls.color.a'       , 255, true)
+    regSetString('walls.texture'    , "none", true)
...
+    regSetInt('walls.count'         , 1, true)
+    regSetInt('walls.proba'         , 100, true)
 
     -- Ceiling
-    regSetBool('ceiling.unbreakable'  , false)
-    regSetString('ceiling.material'   , "masonry")
-    regSetInt('ceiling.color.r'       , 55)
-    regSetInt('ceiling.color.g'       , 55)
-    regSetInt('ceiling.color.b'       , 55)
...
-    regSetBool('ceiling.transparent'  , false)
-    regSetInt('ceiling.thickness'     , 4)
+    regSetBool('ceiling.unbreakable'  , false, true)
+    regSetString('ceiling.material'   , "masonry", true)
+    regSetInt('ceiling.color.r'       , 55, true)
+    regSetInt('ceiling.color.g'       , 55, true)
+    regSetInt('ceiling.color.b'       , 55, true)
...
+    regSetBool('ceiling.transparent'  , false, true)
...
-    dbw("Player Pos", pos)
-    dbw("Chunk Pos", WorldGen.chunkPos)
-
-    local spiralLimit = WorldGen.renderDistance * 4
-    local dir = 1
-
-    local currX = 0
-    local currY = 0
...
-        local jumpAmount = floatToInt(i / 2)
-
-        if dir < 4 then
-            dir = dir + 1
-        else
...
-        if dir == 1 then
-            --Up
-            for j=1, jumpAmount do
-                RenderChunk(currX + WorldGen.chunkPos[1], currY + WorldGen.chunkPos[3])
-                currY = currY + 1
-            end
-        elseif dir == 2 then
-            --Right
...
-end
-
-function GetChunkTag(posX, posY)
-    return "chunk_"..string.format("%.0f", posX).."_"..string.format("%.0f", posY)
-end
-
-function IsGeneratedShape(shape)
-    return HasTag(shape, "generatedFloor") or HasTag(shape, "generatedWall") or HasTag(shape, "generatedCeiling")
-end
-
-function RenderChunk(inputX, inputY)
-    if WorldGen.chunkIndexes[inputX] == nil then
-        WorldGen.chunkIndexes[inputX] = {}
-        dbp("Expanding X to "..inputX)
-    end
-
-    if WorldGen.chunkIndexes[inputX][inputY] == nil then
-        WorldGen.chunkIndexes[inputX][inputY] = false
-        dbp("Expanding Y to "..inputY)
-    end
-
-    if (not WorldGen.chunkIndexes[inputX][inputY]) and (WorldGen.bufferSpeed <= 0) then
-        WorldGen.bufferSpeed = WorldGen.maxBufferSpeed
-        WorldGen.chunkIndexes[inputX][inputY] = true
-        -- Here you can load your own .vox or .xml file containing one chunk
-        -- In this case I will be generating the xml from code
-        local generationType = regGetString('world.generate')
-        local floors = regGetInt('world.floors')
-        for floor=1,floors do
-            local level = floor-1
-            local isLastFloor = (floor == floors)
-            RenderFloor(inputX, inputY, level)
-            if (generationType == 'floor + walls') or (generationType == 'floor + walls + ceiling') then
-                RenderWalls(inputX, inputY, level)
-            end
-            if generationType == 'floor + walls + ceiling' then
-                if isLastFloor then
-                    RenderCeiling(inputX, inputY, level)
-                end
-            end
-        end
-    end
-end
-
-function RenderFloor(inputX, inputY, level)
...
-    local a = regGetBool('floor.transparent') and 0.5 or 1 -- Any value other than 1 is the same transparency (0 cause a bug)
-    local textureIndex = WorldGen.texturesIndex[regGetString('floor.texture')]
-    -- textureIndex = inputY
-    local textureOpacity = regGetInt('floor.textureOpacity')/100
-    local reflective = regGetInt('floor.reflective')/100
-    local roughness = regGetInt('floor.roughness')/100
-    local metallic = regGetInt('floor.metallic')/100
-    local emissive = regGetInt('floor.emissive')/100*32
-    local thickness = regGetInt('floor.thickness')
-    local height = regGetInt('walls.height')
-    local levelOffset = level * height * 0.1
-    local tags = "generatedFloor "..GetChunkTag(inputX, inputY)
-    if regGetBool('floor.unbreakable') then
-        tags = tags.." unbreakable"
-    end
-    local rampPercent = regGetInt('floor.rampProba')/100
-    -- Turn floor into a ramp if requested
-    local rampSizeVoxels = WorldGen.chunkSizeVoxels
-    local rampAngle = QuatEuler(0, 0, 0)
-    local xOffset = 0
-    local yOffset = 0
-    -- No ramp on level 0 and no ramp on center chunk of first floor (which would overlap player spawn)
-    if (level > 0) and ((inputX ~= 0) or (inputY ~= 0) or (level > 1)) and(rdmf() <= rampPercent) then
-        local radianToDegree = 180 / math.pi
-        rampSizeVoxels = math.ceil(math.sqrt((WorldGen.chunkSizeVoxels * WorldGen.chunkSizeVoxels) + (height * height)))
-        local tiltAngle = math.atan(height / WorldGen.chunkSizeVoxels) * radianToDegree
-        local yawAngle = rdm(0, 3) * 90
-        if yawAngle == 90 then
-            yOffset = WorldGen.chunkSize
-        elseif yawAngle == 180 then
-            xOffset = WorldGen.chunkSize
-            yOffset = WorldGen.chunkSize
-        elseif yawAngle == 270 then
-            xOffset = WorldGen.chunkSize
-        end
-        rampAngle = QuatEuler(tiltAngle, yawAngle, 0)
-    end
-    local xmlString = "<voxbox \
-    size='"..WorldGen.chunkSizeVoxels.." "..thickness.." "..rampSizeVoxels.."' \
-    material='"..material.."' \
-    color='"..r.." "..g.." "..b.." "..a.."' \
-    pbr='"..reflective.." "..roughness.." "..metallic.." "..emissive.."' \
-    texture='"..textureIndex.." "..textureOpacity.."' \
-    tags='"..tags.."' \
-    />"
-    --dbp("floor XML="..xmlString)
-    Spawn(xmlString, Transform(Vec((inputX * WorldGen.chunkSize) - (WorldGen.chunkSize/2) + xOffset, (-thickness*0.1) + levelOffset, (inputY * WorldGen.chunkSize)  - (WorldGen.chunkSize/2) + yOffset), rampAngle), true)
-end
-
-function RenderWalls(inputX, inputY, level)
...
-    local a = regGetBool('walls.transparent') and 0.5 or 1 -- Any value other than 1 is the same transparency (0 cause a bug)
-    local textureIndex = WorldGen.texturesIndex[regGetString('walls.texture')]
-    -- textureIndex = inputY
-    local textureOpacity = regGetInt('walls.textureOpacity')/100
-    local reflective = regGetInt('walls.reflective')/100
-    local roughness = regGetInt('walls.roughness')/100
-    local metallic = regGetInt('walls.metallic')/100
-    local emissive = regGetInt('walls.emissive')/100*32
-    local thickness = regGetInt('walls.thickness')
-    local height = regGetInt('walls.height')
-    local levelOffset = level * height * 0.1
-    local tags = "generatedWall "..GetChunkTag(inputX, inputY)
-    if regGetBool('walls.unbreakable') then
-        tags = tags.." unbreakable"
-    end
-
-    local wallsToRender = GetWallsToRender()
-    -- wallsToRender = {"north"}
-    for i,direction in pairs(wallsToRender) do
-        local rot = QuatEuler(0, 0, 0)
-        local xOffset = 0
-        local yOffset = 0
-        if direction == "north" then
-            --r, g, b = 1, 0, 0
-            xOffset = -(thickness * 0.1) / 2
-            yOffset = -(thickness * 0.1) / 2
-        end
-        if direction == "east" then
-            --r, g, b = 0, 1, 0
-            rot = QuatEuler(0, -90, 0)
-            xOffset = ((thickness * 0.1) / 2) + WorldGen.chunkSize
-            yOffset = -(thickness * 0.1) / 2
-        end
-        if direction == "south" then
-            --r, g, b = 0, 0, 1
-            xOffset = -(thickness * 0.1) / 2
-            yOffset = -(thickness * 0.1) / 2 + WorldGen.chunkSize
-        end
...
-
-        local xmlString = "<voxbox \
-        size='"..(WorldGen.chunkSizeVoxels+thickness).." "..height.." "..thickness.."' \
-        material='"..material.."' \
-        color='"..r.." "..g.." "..b.." "..a.."' \
-        pbr='"..reflective.." "..roughness.." "..metallic.." "..emissive.."' \
-        texture='"..textureIndex.." "..textureOpacity.."' \
-        tags='"..tags.."' \
-        />"
-        --dbp("wall XML="..xmlString)
-        Spawn(xmlString, Transform(Vec((inputX * WorldGen.chunkSize) - (WorldGen.chunkSize/2) + xOffset, levelOffset, (inputY * WorldGen.chunkSize) - (WorldGen.chunkSize/2) + yOffset), rot), true)
-    end
-    
-end
-
-function GetWallsToRender()
-
-    local nbWalls = regGetInt('walls.count')
-    local wallsPercent = regGetInt('walls.proba')/100
-    local wallsPlaceable = {1, 2, 3, 4}
-    local wallsToRender = {}
-    -- Try to render as many walls as requested for the current chunk
-    for i=1,nbWalls do
-        local wallIndex = rdm(1, #wallsPlaceable)
-        -- For each wall, only render it if the probability is reached
-        if rdmf() <= wallsPercent then
-            wallsToRender[#wallsToRender+1]=WorldGen.directions[wallsPlaceable[wallIndex]]
-            table.remove(wallsPlaceable, wallIndex)
-        end
-    end
...
-
+#version 2
 function RenderCeiling(inputX, inputY, level)
...
-
-                ui.slider.create('Generate', 'world.generate', '', nil, nil, nil, nil, nil, nil, nil, {"floor", "floor + walls", "floor + walls + ceiling"})
-                ui.padding.create(0, 50)
-
-                ui.slider.create('Floors', 'world.floors', '', 1, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 50)
-
-                UiColor(1, 0.6, 0.6, 1)
-                UiAlign('left middle')
-                UiText("CAUTION: Multiple floors will lag the game faster,\ntry to reduce render distance to reduce the lag")
-                ui.padding.create(0, 70)
-
-            UiPop() end
-
...
-                UiColor(1, 1, 1, 1)
-                UiAlign('left middle')
-                UiText("This mod builds upon Thompson937's Minecraft Superflat World")
-                ui.padding.create(0, 30)
-                UiText("and adds the ability to generate walls and ceiling")
-                ui.padding.create(0, 30)
-                UiText("as well as fully customizable materials and textures.")
-                ui.padding.create(0, 40)
-                UiText("Although there is no actual procedural work here (only random walls) this mod is")
-                ui.padding.create(0, 30)
-                UiText("built as a framework to simplify the implementation of actual procedural tiles,")
-                ui.padding.create(0, 30)
-                UiText("as well as a way to easily test various chunk configurations.")
-                ui.padding.create(0, 40)
-                UiText("Feel free to reuse and build upon it, leaving a mention to this mod ;)")
-                ui.padding.create(0, 40)
-
-            UiPop() end
-
...
-            ui.padding.create(0, 50)
-
-            ui.slider.create('Probability for a floor to become a ramp', 'floor.rampProba', '%', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 50)
-           
-        UiPop() end
-        
...
-            ui.padding.create(0, 50)
-
-            ui.slider.create('Count per chunk', 'walls.count', 'Walls', 1, 4, nil, nil, nil, nil, true)
-            ui.padding.create(0, 50)
-
-            ui.slider.create('Probability to generate', 'walls.proba', '%', 1, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 50)
-
-        UiPop() end
-        
...
+function GetSegmentBetweenLines(point1, dir1, point2, dir2)
         ...
         local segPoint1 = nil
@@ -1127,10 +1141,12 @@
         local dist = VecDist(segPoint1, segPoint2)
         return dist, segPoint1, segPoint2
     end
