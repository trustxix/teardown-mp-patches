...
-#include "scripts/singularity.lua"
...
-		uiDrawKeyHint()
-        uiDrawSingularityType()
-    else
-		hintTimer = 6
-	end
-    uiManageGameOptions()
-    uiManageAbilityWheel()
-
...
+
 function dbaabb(min, max, r, g ,b ,a) if db then AabbDraw(min, max, r, g ,b ,a) end end
+
...
+
 function dds(transform, radius, sides, info)
     if db then
         local tr1 = TransformCopy(transform)
@@ -59,3 +65,4 @@
     dbw('TOOL rot', Gun.singularityCursor.tr.rot)
     dbw('TOOL pos', Gun.singularityCursor.getPos())
 end
+
...
# Migration Report: scripts\gun.lua
...
```diff
--- original/scripts\gun.lua
+++ patched/scripts\gun.lua
@@ -1,153 +1,6 @@
-Gun = {}
-
-Gun.editionMode = false
-Gun.callingSingularity = false
-Gun.editPrecision = 5
-Gun.handSphereRot = QuatEuler(0, 0, 0)
-Gun.handSphereAngle = 0
-Gun.handSphereRotSpeed = 100
-
-Gun.singularityCursor = {
-    tr = Transform(Vec(1, 0, 0), QuatEuler(90, 0, 0)),
-    defaultDist = 7,
-    distFromPlayer = 7,
-    lastPlayerRot = QuatEuler(0, 0, 0),
-
-    getPos = function()
-        -- Get placeholder actual position
-        local playerTr = GetPlayerEyeTransform()
-        --local playerPos = VecAdd(playerTr.pos, VecScale(GetPlayerVelocity(), 1/30))
-        local singularityPos = VecAdd(playerTr.pos, VecScale(Gun.singularityCursor.tr.pos, Gun.singularityCursor.distFromPlayer))
-        -- If cursor collides with the world, get impact point
-        if regGetBool('tool.cursorCollide') then
-            local playerToCursor = VecSub(singularityPos, playerTr.pos)
-            local hit, dist, normal, shape = QueryRaycast(playerTr.pos, VecNormalize(playerToCursor), VecLength(playerToCursor))
-            if hit then
-                singularityPos = VecAdd(playerTr.pos, VecResize(playerToCursor, dist))
-            end
-        end
-        -- If pos should be aligned on grid, only accept integer pos
-        if regGetBool('tool.useGrid') then
-            singularityPos = Vec(floatToInt(singularityPos[1]), floatToInt(singularityPos[2]), floatToInt(singularityPos[3]))
-        end
-        return singularityPos
-    end,
-
-    getRot = function()
-        -- Only take into account player pitch and yaw (not roll)
-        local roll, yaw, pitch = GetQuatEuler(Gun.singularityCursor.lastPlayerRot)
-        local singularityRot = QuatRotateQuat(QuatEuler(0, yaw, pitch), Gun.singularityCursor.tr.rot)
-        -- If rot should be aligned on grid, ignore player rot
-        if regGetBool('tool.useGrid') then
-            singularityRot = Gun.singularityCursor.tr.rot
-        end
-        return singularityRot
-    end,
-
-    getPosTransform = function()
-        return Transform(Gun.singularityCursor.getPos(), Gun.singularityCursor.getRot())
-    end,
-
-    getSize = function()
-        return regGetFloat('singularity.size')
-    end,
-
-    getColor = function()
-        local color = Singularity.getColor()
-        color.a = Gun.editionMode and 1 or regGetFloat('tool.cursor.alpha')
-        return color
-    end,
-
-    update = function()
-        local playerTr = GetPlayerEyeTransform()
-        local offsetVector = QuatRotateVec(playerTr.rot, Vec(0, 0, -1))
-        Gun.singularityCursor.tr.pos = offsetVector
-        Gun.singularityCursor.lastPlayerRot = playerTr.rot
-    end,
-
-    draw = function()
-        local size = Gun.singularityCursor.getSize()
-        local color = Gun.singularityCursor.getColor()
-        local info = {
-            r = color.r,
-            g = color.g,
-            b = color.b,
...
-            writeZ = true
-        }
-        visual.drawsphere( Gun.singularityCursor.getPosTransform(), size, 0, 200 * size, info )
-        local info = {
-            r = color.r,
-            g = color.g,
-            b = color.b,
-            a = oscillate() * color.a * 0.5,
-            writeZ = false
-        }
-        visual.drawsphere( Gun.singularityCursor.getPosTransform(), size, 0, 200 * size, info )
-        
...
-
+#version 2
 function Gun.getSingularityType()
     return Singularity.ability.list[Singularity.ability.selected].name
-end
-
-Gun.actions = {
-
-    callSingularity = function()
-        if not UI_GAME then
-            Singularity.onSingularityCalled()
-        end
-    end,
-
-    enableCallingSingularity = function(state)
-        Gun.callingSingularity = state and not UI_GAME
-    end,
-
-    reinitSingularityCursor = function()
-        Gun.singularityCursor.tr = Transform(Vec(1, 0, 0), QuatEuler(90, 0, 0))
-        Gun.singularityCursor.distFromPlayer = Gun.singularityCursor.defaultDist
-    end,
-
-    enableEdition = function(state)
-        Gun.editionMode = state
-        SetBool("game.input.locktool", state)
-    end,
-
-    setEditPrecision = function(value)
-        Gun.editPrecision = value
-    end,
-
-    updateScroll = function(scroll)
-        if Gun.editionMode then
-            scroll = scroll * Gun.editPrecision
-            -- Moving closer to/away from player
-            Gun.singularityCursor.distFromPlayer = clamp(Gun.singularityCursor.distFromPlayer + (scroll * 0.1), 0, 100)
-        end
-    end,
-
-    removeClosest = function()
-        local removedSingularitiesTransform = Singularity.singularities.instances.removeClosestSingularities()
-        if #removedSingularitiesTransform > 0 then
-            beep()
-            dbp('Closest Singularity was removed.' .. sfnTime())
-        end
-    end,
-
-}
-
-
-Gun.updateSingularityCursor = function()
-    Gun.singularityCursor.update()
-    Gun.singularityCursor.draw()
-end
-
--- Update hand sphere rotation
-Gun.update = function()
-
-    Gun.handSphereAngle = Gun.handSphereAngle + (Gun.handSphereRotSpeed*GetTimeStep())
-    Gun.handSphereRot = QuatEuler(Gun.handSphereAngle, Gun.handSphereAngle, Gun.handSphereAngle)
-
 end
...
@@ -163,75 +16,9 @@
     local playerTr = GetPlayerEyeTransform()
     local toolTr = Gun.getTr()
     local offsetVector = QuatRotateVec(playerTr.rot, toolTr.pos)
-    --local offsetWithSpeed = VecAdd(offsetVector, VecScale(GetPlayerVelocity(), 1/30))
+    --local offsetWithSpeed = VecAdd(offsetVector, VecScale(GetPlayerVelocity(playerId), 1/30))
     local spherePos = VecAdd(playerTr.pos, offsetVector)
     local sphereRot = QuatRotateQuat(playerTr.rot, toolTr.rot)
     return Transform(spherePos, sphereRot)
 end
...
-
-    local size = 0.5
-    local sphereTr = Gun.getSphereTr()
-    Gun.drawHandSphere(sphereTr)
-
-end
-
-
-Gun.drawHandSphere = function(transform)
-
-    local radius = 0.2
-    local sides = 40
-    local subCircles = 1
-    local angle = 90 / (subCircles+1)
-    local tr = TransformCopy(transform)
-    visual.drawpolygon( tr, radius, nil, sides, Gun.randColorInfo(), true)
-    local trL = TransformCopy(tr)
-    local trR = TransformCopy(tr)
-    for i = 1, subCircles do
-        trL.rot = QuatRotateQuat(trL.rot, QuatEuler(angle, 0, 0))
-        visual.drawpolygon( trL, radius, nil, sides, Gun.randColorInfo(), true)
-        trR.rot = QuatRotateQuat(trR.rot, QuatEuler(-angle, 0, 0))
-        visual.drawpolygon( trR, radius, nil, sides, Gun.randColorInfo(), true)
-    end
-    local tr2 = TransformCopy(tr)
-    tr2.rot = QuatRotateQuat(tr2.rot, QuatEuler(90, 0, 0))
-    visual.drawpolygon( tr2, radius, nil, sides, Gun.randColorInfo(), true)
-    trL = TransformCopy(tr2)
-    trR = TransformCopy(tr2)
-    for i = 1, subCircles do
-        trL.rot = QuatRotateQuat(trL.rot, QuatEuler(0, 0, angle))
-        visual.drawpolygon( trL, radius, nil, sides, Gun.randColorInfo(), true)
-        trR.rot = QuatRotateQuat(trR.rot, QuatEuler(0, 0, -angle))
-        visual.drawpolygon( trR, radius, nil, sides, Gun.randColorInfo(), true)
-    end
-    local tr3 = TransformCopy(tr2)
-    tr3.rot = QuatRotateQuat(tr3.rot, QuatEuler(0, 0, 90))
-    visual.drawpolygon( tr3, radius, nil, sides, Gun.randColorInfo(), true)
-    trL = TransformCopy(tr3)
-    trR = TransformCopy(tr3)
-    for i = 1, subCircles do
-        trL.rot = QuatRotateQuat(trL.rot, QuatEuler(angle, 0, 0))
-        visual.drawpolygon( trL, radius, nil, sides, Gun.randColorInfo(), true)
-        trR.rot = QuatRotateQuat(trR.rot, QuatEuler(-angle, 0, 0))
...
-end
-
--- Pick a random color from the singularity colors
-Gun.randColorInfo = function()
-    
-    local item = Singularity.ability.list[rdm(1, #Singularity.ability.list)]
...
-    regSetBool('tool.useGrid'      , false)
-    regSetBool('tool.cursorCollide', false)
-    regSetFloat('tool.cursor.alpha', 0.5)
-    regSetBool('tool.showAbilityText'       , true)
-    regSetBool('tool.showAbilityIcon'       , true)
+    regSetBool('tool.debugMode'    , false, true)
+    regSetBool('tool.useGrid'      , false, true)
+    regSetBool('tool.cursorCollide', false, true)
+    regSetFloat('tool.cursor.alpha', 0.5, true)
+    regSetBool('tool.showAbilityText'       , true, true)
+    regSetBool('tool.showAbilityIcon'       , true, true)
 
     -- Singularity
-    regSetBool('singularity.soundOn'           , true)
-    regSetBool('singularity.affectPlayer'      , true)
-    regSetString('singularity.antimatterAffect', "Hard")
-    regSetFloat('singularity.size'             , 2)
-    regSetFloat('singularity.force'            , 10)
-    regSetFloat('singularity.accelSlowRatio'   , 20)
-    regSetFloat('singularity.moveSpeed'        , 10)
-    regSetFloat('singularity.moveAccel'        , 5)
-    regSetBool('singularity.followPlayer'      , false)
+    regSetBool('singularity.soundOn'           , true, true)
+    regSetBool('singularity.affectPlayer'      , true, true)
+    regSetString('singularity.antimatterAffect', "Hard", true)
+    regSetFloat('singularity.size'             , 2, true)
+    regSetFloat('singularity.force'            , 10, true)
+    regSetFloat('singularity.accelSlowRatio'   , 20, true)
+    regSetFloat('singularity.moveSpeed'        , 10, true)
+    regSetFloat('singularity.moveAccel'        , 5, true)
+    regSetBool('singularity.followPlayer'      , false, true)
...
# Migration Report: scripts\singularity.lua
...
```diff
--- original/scripts\singularity.lua
+++ patched/scripts\singularity.lua
@@ -1,75 +1,4 @@
-Singularity = {}
-Singularity.singularities = {} -- Holds common data for all singularities.
-Singularity.singularitiesList = {}
-
-Singularity.wasGodMode = false
-Singularity.protectPlayer = false
-
-Singularity.ability = {
-    selected = 1,
-    next = 0,
...
-        {
-            name = "Antimatter",
-            icon = "MOD/img/antimatter.png",
-            color = Vec(212/255, 212/255, 212/255)
-        },
-        {
-            name = "Frozen",
-            icon = "MOD/img/frozen.png",
-            color = Vec(170/255, 170/255, 1)
-        },
-        {
-            name = "Acceleration",
-            icon = "MOD/img/acceleration.png",
-            color = Vec(1, 1, 170/255)
-        },
-        {
-            name = "Deceleration",
-            icon = "MOD/img/deceleration.png",
-            color = Vec(1, 200/255, 170/255)
-        },
-        {
-            name = "Attraction",
-            icon = "MOD/img/attraction.png",
-            color = Vec(200/255, 225/255, 200/255)
-        },
-        {
-            name = "Repulsion",
-            icon = "MOD/img/repulsion.png",
-            color = Vec(170/255, 1, 170/255)
-        },
-        {
-            name = "Propulsion",
-            icon = "MOD/img/propulsion.png",
-            color = Vec(170/255, 1, 1)
-        },
-        {
-            name = "Antigravity",
-            icon = "MOD/img/antigravity.png",
-            color = Vec(1, 170/255, 1)
-        },
-        {
-            name = "Protection",
-            icon = "MOD/img/protection.png",
-            color = Vec(1, 170/255, 200/255)
-        },
-        {
-            name = "Combustion",
-            icon = "MOD/img/combustion.png",
-            color = Vec(1, 170/255, 170/255)
-        }
-    },
-    indexByName = {}
-}
-
-for i=1,#Singularity.ability.list do
-    local item = Singularity.ability.list[i]
-    Singularity.ability.indexByName[item.name] = i
-end
-
--- Get the singularity color based on singularity type
+#version 2
 function Singularity.getColor(singularityType)
     local index = singularityType and Singularity.ability.indexByName[singularityType] or Singularity.ability.selected
     --DebugPrint((singularityType and singularityType or "nil").." index="..index)
@@ -83,8 +12,6 @@
     return color
 end
 
-
--- Get the bounding box of the singularity
 function Singularity.getBoundingBox(singularity)
     local pos = singularity.tr.pos
...
--- Get bounding box of the player
 function Singularity.getPlayerBbox()
...
--- Tells if a sngularity type is global (does not need to apply to eachcolliding body separately)
 function Singularity.isGlobalType(singularityType)
     return (singularityType == "Antimatter") or (singularityType == "Combustion")
 end
 
--- Do some initializations when player start calling a Singularity
 function Singularity.onSingularityCalled()
 
     -- If no singularity on the map yet, spawn one
@@ -134,7 +58,6 @@
     return Singularity.singularitiesList[1]
...
@@ -160,7 +83,7 @@
                 singularity.targetPos = VecAdd(singularity.tr.pos, expectedMovement)
                 moving = true
             -- If in range reset move speed unless distance is 0
-            elseif cursorDist > 0 then
+            elseif cursorDist ~= 0 then
                 singularity.moveSpeed = regGetFloat('singularity.moveSpeed')
             end
         end
@@ -201,10 +124,6 @@
...
-
-Singularity.singularities.instances = {}
-
---- Create a singularity object and store it in the list of singularities.
 function Singularity.singularities.instances.create(tr, singularityType)
...
---- Spawn a singularity into the world.
 function Singularity.singularities.instances.spawn(tr, singularityType)
     Singularity.singularities.instances.create(tr, singularityType)
     if regGetBool('singularity.soundOn') then
@@ -237,7 +155,6 @@
     end
 end
 
---- Mark a singularity for removal to safely remove it near the end of the frame.
 function Singularity.singularities.instances.markSingularityForRemoval(singularity)
     singularity.removeSingularity = true
     if regGetBool('singularity.soundOn') then
@@ -246,7 +163,6 @@
     dbp('Singularity marked for removal: ' .. singularity.id .. ' ' ..sfnTime())
 end
 
---- Mark singularities closest to the player for removal.
 function Singularity.singularities.instances.removeClosestSingularities(count)
     count = count or 1
...
---- Safely delete all singularities that are marked for removal.
 function Singularity.singularities.instances.removeMarkedSingularities()
 
     TableRemove(Singularity.singularitiesList, function(t, i, j)
@@ -292,21 +207,13 @@
...
-
-
-Singularity.singularities.physics = {}
-
--- Process the physics of a single singularity.
 function Singularity.singularities.physics.process(singularity)
     
     -- Move the singularity to its target position
     local distToTarget = VecDist(singularity.targetPos, singularity.tr.pos)
     -- dbp("dist="..distToTarget)
     singularity.velocity = 0
-    if distToTarget > 0 then
+    if distToTarget ~= 0 then
         -- Move to target
         local moveDir = VecSub(singularity.targetPos, singularity.tr.pos)
         local dist = math.min(distToTarget, singularity.moveSpeed * GetTimeStep())
@@ -357,7 +264,6 @@
 
 end
 
--- Return all bodies in the singularity (apply query filter before calling this)
 function Singularity.singularities.physics.getBodiesInSingularity(singularity)
     local bodiesFound = {}
     local impactPoints = {}
@@ -383,7 +289,6 @@
     return bodiesFound, impactPoints
 end
 
--- Update the singularity type (when existing some types, special actions must be done)
 function Singularity.singularities.physics.updateType(singularity, newType)
     
     if singularity.singularityType == newType then
@@ -396,7 +301,6 @@
 
 end
 
--- Apply the singularity effect to a body
 function Singularity.singularities.physics.applyEffectToBody(singularity, body)
     if singularity.singularityType == "Frozen" then
         SetBodyDynamic(body, false)
@@ -413,7 +317,6 @@
     end
 end
 
--- Do not apply the singularity effect to a body any more
 function Singularity.singularities.physics.removeEffectFromBody(singularity, body)
     if singularity.singularityType == "Frozen" then
...
--- Apply the singularity effect globally
 function Singularity.singularities.physics.applyGlobalEffect(singularity)
     if singularity.singularityType == "Antimatter" then
         local pos = singularity.tr.pos
@@ -453,10 +355,9 @@
     end
 end
 
--- Apply the singularity effect to the player
 function Singularity.singularities.physics.applyEffectToPlayer(singularity)
     if singularity.singularityType == "Antimatter" then
-        SetPlayerHealth(math.max(GetPlayerHealth() - (0.5 * GetTimeStep()), 0))
+        SetPlayerHealth(playerId, math.max(GetPlayerHealth(playerId) - (0.5 * GetTimeStep()), 0))
     elseif singularity.singularityType == "Frozen"
         or singularity.singularityType == "Acceleration"
         or singularity.singularityType == "Deceleration"
@@ -464,15 +365,14 @@
         or singularity.singularityType == "Repulsion"
         or singularity.singularityType == "Propulsion"
         or singularity.singularityType == "Antigravity" then
-        SetPlayerVelocity(Singularity.singularities.physics.getVelocityForEffect(singularity, GetPlayerCenter(), GetPlayerVelocity()))
+        SetPlayerVelocity(playerId, Singularity.singularities.physics.getVelocityForEffect(singularity, GetPlayerCenter(), GetPlayerVelocity(playerId)))
     elseif singularity.singularityType == "Protection" then
         Singularity.protectPlayer = true
     elseif singularity.singularityType == "Combustion" then
-        SetPlayerHealth(math.max(GetPlayerHealth() - (0.25 * GetTimeStep()), 0))
-    end
-end
-
--- Do not apply the singularity effect to a body any more
+        SetPlayerHealth(playerId, math.max(GetPlayerHealth(playerId) - (0.25 * GetTimeStep()), 0))
+    end
+end
+
 function Singularity.singularities.physics.getVelocityForEffect(singularity, pos, vel)
     local force = regGetFloat('singularity.force')
     local accel = regGetFloat('singularity.accelSlowRatio')
@@ -500,7 +400,6 @@
     return nil
 end
 
--- Manage bodies that are no longer in range
 function Singularity.singularities.physics.managePrevBodies(singularity, bodiesInRange)
     local bodiesToRemove = {}
     for body in pairs(singularity.prevBodies) do
@@ -511,14 +410,6 @@
     singularity.prevBodies = bodiesInRange
 end
...
-
-
-Singularity.singularities.effects = {}
-
 function Singularity.singularities.effects.process()
     -- Render singularities.
     for i = 1, #Singularity.singularitiesList do
@@ -526,7 +417,6 @@
         Singularity.singularities.effects.renderSingularity(singularity) -- Animated singularity
     end
 end
-
 
 function Singularity.singularities.effects.renderSingularity(singularity)
     local color = Singularity.getColor(singularity.singularityType)
@@ -632,4 +522,5 @@
...
-        
-        input = {
-            didCallSingularity = function() return tool.tool.active() and InputPressed('lmb') end,
-            callingSingularity = function() return tool.tool.active() and InputDown('lmb') end,
-            didReinit = function() return tool.tool.input.isEditing() and InputPressed('mmb') end,
-            isEditing = function() return tool.tool.active() and InputDown('rmb') end,
-            isPrecision = function() return tool.tool.input.isEditing() and InputDown('ctrl') end,
-            getScroll = function() return tool.tool.input.isEditing() and InputValue( "mousewheel" ) or 0 end,
-            didRemove = function() return tool.tool.active() and InputPressed('l') end,
-        },
-
-        setup = {
-            name = 'singularity',
...
-            RegisterTool(tool.tool.setup.name, tool.tool.setup.title)
...
@@ -112,7 +81,7 @@
     local handR = "hand_r"
     local handRTr = GetBoneWorldTransform(animator, handR)
     local playerEyeTr = GetPlayerEyeTransform()
-    local playerVel = GetPlayerVelocity()
+    local playerVel = GetPlayerVelocity(playerId)
     local playerForward = VecForward(playerEyeTr.rot)
     local playerUp = VecUp(playerEyeTr.rot)
     local playerDown = VecScale(playerUp, -1)
@@ -128,4 +97,5 @@
         rightTr = TransformToParentTransform(rightTr, adj)
     end
     SetAnimatorTransformIK(animator, "shoulder_r", handR, rightTr, 1, 0.9, false, false)
-end+end
+
...
+#version 2
 function uiDrawOptions()
     ...
     local w = UiWidth()
@@ -49,7 +45,6 @@
     ui.padding.create(singularityW, singularityH)
...
-
     ...
     local tabContentH = contH - title_h - options_tabH - options_tabH - singularityH * 6.5
     ui.padding.create(0, options_tabH)
     ui.container.create(contW - singularityW*4, tabContentH, ui.colors.g3, 1)
 
-
     -- Tab content
     ui.padding.create(singularityW, singularityH2)
     ui.container.create(contW - singularityW*6, tabContentH - singularityH*3, ui.colors.g1, 1)
@@ -203,190 +193,11 @@
...
-            do UiPush()
-
-                ui.checkBox.create('Align cursor on grid', 'tool.useGrid')
-                ui.padding.create(0, 64) 
-
-                ui.checkBox.create('Cursor collides with the world', 'tool.cursorCollide')
-                ui.padding.create(0, 64) 
-
-                ui.slider.create('Cursor opacity', 'tool.cursor.alpha',  nil, 0, 1)
-                ui.padding.create(0, 30)
-
-                ui.checkBox.create('Show singularity type', 'tool.showAbilityText')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Show singularity icon', 'tool.showAbilityIcon')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Sound', 'singularity.soundOn')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-            do UiPush()
-
-                ui.slider.create('Antimatter dissolves', 'singularity.antimatterAffect', 'Materials', nil, nil, nil, nil, nil, nil, nil, {"Soft", "Medium", "Hard"})
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Size', 'singularity.size', 'Meters', 0.1, 5)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Applied force', 'singularity.force', nil, 1, 100)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Applied accel/decel', 'singularity.accelSlowRatio', nil, 10, 100)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Initial call speed', 'singularity.moveSpeed', nil, 1, 50)
-                ui.padding.create(0, 25)
-                UiColor(0.6,0.6,0.6, 1)
-                UiAlign('left middle')
-                UiText('Note: Lower than Size x 10 for optimal interactions')
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Call acceleration', 'singularity.moveAccel', nil, 0, 50)
-                ui.padding.create(0, 30)
-
-            UiPop() end
-
...
-                ui.padding.create(800, 0)
-                do UiPush()
-                    ui.checkBox.create('Singularity affect player', 'singularity.affectPlayer')
-                UiPop() end
-                do UiPush()
-                    ui.padding.create(400, 0)
-                    ui.checkBox.create('Singularity follows you\n(at cursor distance)', 'singularity.followPlayer')
-                UiPop() end
-                ui.padding.create(0, 120)
-
-                UiFont('bold.ttf', 32)
-                UiColor(1,1,1, 1)
-                UiText('SINGULARITY TYPES')
-                ui.padding.create(0, 40)
-                UiFont('regular.ttf', font_normal)
-                uiColorByType('Antimatter')
-                UiText('Antimatter: Destroy materials of the selected hardness')
-                ui.padding.create(0, 40)
-                uiColorByType('Frozen')
-                UiText('Frozen: Freeze objects in time')
-                ui.padding.create(0, 40)
-                uiColorByType('Acceleration')
-                UiText('Acceleration: Increase objects velocity (accel param)')
-                ui.padding.create(0, 40)
-                uiColorByType('Deceleration')
-                UiText('Deceleration: Decrease objects velocity (decel param)')
-                ui.padding.create(0, 40)
-                uiColorByType('Attraction')
-                UiText('Attraction: Attract objects to the center (force param)')
-                ui.padding.create(0, 40)
-                uiColorByType('Repulsion')
-                UiText('Repulsion: Push objects away from the center (force param)')
-                ui.padding.create(0, 40)
-                uiColorByType('Propulsion')
-                UiText('Propulsion: Throw objects in one direction (force param)')
-                ui.padding.create(0, 40)
-                uiColorByType('Antigravity')
-                UiText('Antigravity: Negate objects gravity')
-                ui.padding.create(0, 40)
-                uiColorByType('Protection')
-                UiText('Protection: Make objects unbreakable')
-                ui.padding.create(0, 40)
-                uiColorByType('Combustion')
-                UiText('Combustion: Ignite objects (only flammable ones)')
-                ui.padding.create(0, 40)
-
-            UiPop() end
-
...
-            UiFont('regular.ttf', font_normal)
-            UiText('[T]: Change Singularity type (hold)')
-            ui.padding.create(0, 60)
-            UiText('[LMB]: Call the Singularity to the cursor (hold to call it faster)')
-            ui.padding.create(0, 60)
-            UiText('[RMB]+[Scroll]: Move cursor closer/further')
-            ui.padding.create(0, 60)
-            UiText('[RMB]+[MMB]: Reinitialize cursor')
-            ui.padding.create(0, 60)
-            UiText('[RMB]+[Ctrl]: Increase move precision (hold)')
-            ui.padding.create(0, 60)
-            UiText('[L]: Remove Singularity')
-            ui.padding.create(0, 60)
-
-        UiPop() end
-        
...
-            ui.padding.create(0, 60)
-            UiFont('regular.ttf', font_normal)
-            UiText('- Once called for the first time, the Singularity remains on the map forever')
-            ui.padding.create(0, 60)
-            UiText('- The Singularity effect will apply to any object in contact with it')
-            ui.padding.create(0, 60)
-            UiText('- Each Singularity type has its own effect (see Singularity tab)')
-            ui.padding.create(0, 60)
-            UiText('- The Singularity can move through anything')
-            ui.padding.create(0, 60)
-
-        UiPop() end
-        
...
-}
-
 function uiColorByType(type)
     local color = Singularity.getColor(type)
...
-
-
---- Draw singularity type above tool name.
 function uiDrawSingularityType()
 
     if regGetBool('tool.showAbilityText') then
@@ -454,7 +262,7 @@
             local dt = GetTimeStep()
     
             UiPush()
-                if Singularity.ability.highlightTimer > 0 then
+                if Singularity.ability.highlightTimer ~= 0 then
                     local a = Singularity.ability.highlightTimer
                     UiColor(1, 1, 1, a)
...
---- Manage when to open and close the ability menu.
 function uiManageAbilityWheel()
...
-    function VecResize(vec, size) return ((VecLength(vec) == 0) and vec or VecScale(VecNormalize(vec), size)) end
...
+function AabbClosestEdge(pos, shape)
         ...
         local bCenterY = AabbGetCenterPos(shapeAabbMin, shapeAabbMax)[2]
@@ -291,8 +312,8 @@
         end
...
+function AabbGetShapeCenterTopPos(shape, addY)
         addY = addY or 0
         local mi, ma = GetShapeBounds(shape)
         local v =  AabbGetCenterPos(mi,ma)
         v[2] = ma[2] + addY
         return v
     end
...
-
-    function DrawLineD(p0, p1, color, debug)
+function DrawLineD(p0, p1, color, debug)
         if debug then
             DebugLine(p0, p1, color.r, color.g, color.b, color.a)
         else
@@ -976,7 +996,7 @@
         end
     end
 
-    function DrawOutlineRect(tr, w, h, color, debug)
+function DrawOutlineRect(tr, w, h, color, debug)
         local downLeftCorner = tr.pos
         local downRightCorner = TransformToParentPoint(tr, Vec(w, 0, 0))
         local topRightCorner = TransformToParentPoint(tr, Vec(w, h, 0))
@@ -991,7 +1011,7 @@
         --DrawSprite(singularitiesprite, tr, w, h, 0, 0, 0, 1)
     end
 
-    function DrawOutlineRectCentered(tr, w, h, color, debug)
+function DrawOutlineRectCentered(tr, w, h, color, debug)
         local downLeftCorner = TransformToParentPoint(tr, Vec(-w/2, -h/2, 0))
         local downRightCorner = TransformToParentPoint(tr, Vec(w/2, -h/2, 0))
         local topRightCorner = TransformToParentPoint(tr, Vec(w/2, h/2, 0))
@@ -1006,66 +1026,63 @@
         --DrawSprite(singularitiesprite, tr, w, h, 0, 0, 0, 1)
     end
