...
-	RegisterTool("rmwg3a1", "G3A1", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwg3a1.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwg3a1", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwg3a1" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwg3a1", "G3A1", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwg3a1.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
...
-
-	--Check if rmwg3a1 is selected
-	if GetString("game.player.tool") == "rmwg3a1" then
...
-			end
-			
-			if GetString("game.player.tool") == "rmwg3a1" then --- When Zero then its empty
-			
-			local optic = opticssight[10]
-                if reloading then ---Charginghandle
-					SetShapeLocalTransform(optic, Transform(Vec(-0.03, 0.15, -0.4-0.2), QuatEuler(-90, 0, 0)))
-				else
-					SetShapeLocalTransform(optic, Transform(Vec(-0.03, 0.14, -0.55-0.2), QuatEuler(-90, 0, 0)))
+function server.init()
+    RegisterTool("rmwg3a1", "G3A1", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwg3a1.enabled", true, true)
+    angle = 0
...
+                    if optics == 1 or optics == 2 or optics == 3 then ---Carry Handle
+                        RemoveTag(optic, "invisible")
+
+    				else
+    				SetTag(optic, "invisible")
...
+                        RemoveTag(optic, "invisible")
+
+    					SetShapeLocalTransform(optic, Transform(Vec(-0.05, 0.16, -0.15), QuatEuler(-90, 0, 0)))
+    				else
+    				SetTag(optic, "invisible")
+
+                    end
...
-			
-			
-		if GetBool("game.player.canusetool") and InputDown("usetool") and not attachmentmenu and GetInt("game.tool.gun.ammo") > 0 and not low and magazine > 0 and fireswitch and not reloading and semi < 1 then
...
-					if muzzlec == 0 then
-					DrawSprite(muzzleflash, mt, 0.2, 0.15, 1, 1, 1, 1, true)
-					end
-					SetCameraOffsetTransform(Transform(Vec(x, y, 0.04), QuatEuler(x, y, 1)), true)
...
-		end
-		if aiming then
-		toolAnimator.offsetTransform = (Transform(Vec(-0.1085, 0.073, recoil/2), QuatEuler(0, 0, 0)))
-		tickToolAnimator(toolAnimator, dt)
-		if optics == 2 then
-		toolAnimator.offsetTransform = (Transform(Vec(-0.108, 0.006, recoil/2), QuatEuler(0, 0, 0)))
-		tickToolAnimator(toolAnimator, dt)
-		end
-		if optics == 3 then
-		toolAnimator.offsetTransform = (Transform(Vec(-0.108, 0.018, recoil/2), QuatEuler(0, 0, 0)))
-		tickToolAnimator(toolAnimator, dt)
-		end
...
-function draw()
-
-if GetString("game.player.tool") == "rmwg3a1" and not fireswitch and not reloading and not attachmentmenu then
-		UiPush()
...
-			UiText(magazine.." (9x19mm Parabellum) Auto")
-				
-				
-			end
-
-if GetString("game.player.tool") == "rmwg3a1" and fireswitch and not reloading and not attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(1, 1, 1)
-			UiFont("bold.ttf", 32)
-			UiTextOutline(0,0,0,1,0.1)
-			UiText(magazine.." (9x19mm Parabellum) Semi")
-			
-			end
-			
-if GetString("game.player.tool") == "rmwg3a1" and reloading and not attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(1, 1, 1)
...
-				
-			end
-if GetString("game.player.tool") == "rmwg3a1" and not reloading and attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(0.8, 0.8, 0.8)
