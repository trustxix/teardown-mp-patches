...
-	reddotsprite = LoadSprite("MOD/sprites/reddot.png")
...
-	RegisterTool("rmwbren2", "CZ Bren 2", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwbren2.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwbren2", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
@@ -256,7 +162,7 @@
 			if GetString("game.player.tool") == "rmwbren2" then --- When Zero then its empty
 			
 			local optic = opticssight[10]
-                if charginghandletime > 0 then ---Charginghandle
+                if charginghandletime ~= 0 then ---Charginghandle
 					SetShapeLocalTransform(optic, Transform(Vec(-0.06, 0.12, -0.13), QuatEuler(-90, 0, 0)))
 				else
 					SetShapeLocalTransform(optic, Transform(Vec(-0.06, 0.12, -0.23), QuatEuler(-90, 0, 0)))
@@ -505,7 +411,7 @@
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwbren2" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
+    RegisterTool("rmwbren2", "CZ Bren 2", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwbren2.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
