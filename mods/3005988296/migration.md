...
+#version 2
 function manager.cleanupScripts()
     local keys = regListKeys("melt.scripts")
     for i=1,#keys do
@@ -24,4 +8,13 @@
     regClearKey("melt.scripts")
 end
...
# Migration Report: scripts\melter.lua
...
```diff
--- original/scripts\melter.lua
+++ patched/scripts\melter.lua
@@ -1,336 +1,4 @@
-#include "../scripts/debug.lua"
-#include "../scripts/registry.lua"
-#include "../scripts/ui.lua"
...
-        radius = 0,
-        jointedBodies = {},
-        melterLoop = LoadLoop("light/buzz0.ogg"),-- TODO
-        countdownLoop = LoadLoop("alarm1.ogg"),
-        breakSound = LoadSound("light/spark0.ogg"),
-        isSelected = false,
-        visuTimer = 0,
-    }
-}
-
-function init()
-    local mi = Melter_info[1]
-    checkRegInitialized()
-    -- Gather information required for the script to work from body tags
-    mi.body = FindBody("melterBody")
-    mi.shape = FindShape("melterShape")
-    -- We know the script is the parent of the body in prefab
-    mi.script = mi.body-1
-    if not Melter.isManagerStarted() then
-        Melter.startManager()
-    end
-    if mi.body == 0 then
-        Melter.abortInitWithError("melterBody")
-        return
-    end
-    if mi.shape == 0 then
-        Melter.abortInitWithError("melterShape")
-        return
-    end
-    local melterType = GetTagValue(mi.body, "melterType")
-    if melterType == "" then
-        Melter.abortInitWithError("melterType")
-        return
-    end
-    tool.ability.type = melterType
-    mi.startDelay = regGetInt('melter.startDelay')
-    mi.range = ternary(Melter.isGrenade(), regGetInt('melter.grenadeRange'), regGetInt('melter.laserRange'))
-    mi.radius = regGetInt('melter.laserRadius')
-end
-
-function Melter.abortInitWithError(tagMissing)
-    local mi = Melter_info[1]
-    local oldDb = db
-    db = true
-    dbp("[Melter] Could not initialize object: missing or malformed tag '"..tagMissing.."'")
-    db = oldDb
-    mi.broken = true
-end
-
...
-        return
-    end
-    local newBody = GetShapeBody(mi.shape)
-    if mi.body ~= newBody then
-        mi.body = newBody
-    end
-    mi.broken = IsShapeBroken(mi.shape)
-    if mi.broken then
-        PlaySound(mi.breakSound, mi.center, 1)
-        RemoveTag(mi.body, "unmeltable")
-        RemoveTag(mi.shape, "unmeltable")
-        return
-    end
-    SetShapeEmissiveScale(mi.shape, 0)
-    mi.center = AabbGetShapeCenterPos(mi.shape)
-    mi.jointedBodies = GetJointedBodies(mi.body)
-    -- Update timers
-    local dt = GetTimeStep()
-    if mi.delay > 0 then
-        mi.delay = mi.delay - dt
-        if regGetBool('melt.soundOn') then
-            PlayLoop(mi.countdownLoop, mi.center, 1)
-        end
-        -- Blink while activating
-        local scale = mi.delay - math.floor(mi.delay)
-        SetShapeEmissiveScale(mi.shape, scale)
-    end
-    if mi.visuTimer > 0 then
-        mi.visuTimer = mi.visuTimer - dt
-    end
-    -- Manage actions of player on the device
-    Melter.manageKeys()
-    -- Update range visuals
-    Melter.updateVisuals()
-    -- Do melting effect if active
-    tool.ability.active = mi.active and (mi.delay <= 0)
-    if not tool.ability.active then
-        return
-    end
-    SetShapeEmissiveScale(mi.shape, 1)
-    -- If genade, levitate above ground
-    Melter.levitateGrenade()
-    -- Update unmeltable bodies
-    Melter.updateUnmeltableBodies()
-    -- A valid shape is required by meltShape(), it will be overriden
-    if not IsShapeValid(tool.tool.aimShape) then
-        tool.tool.aimShape = mi.shape
-    end
-    -- Melt shapes in range
-    local hitPoints = tool.ability.meltShape()
-    -- Play melt sound and device sound
-    if regGetBool('melt.soundOn') then
-        for i=1, #hitPoints do
-            PlayLoop(tool.effect.meltLoop, hitPoints[i], 1)
-        end
-        PlayLoop(mi.melterLoop, mi.center, 1)
-    end
-end
-
-function update()
...
-    if hit then
-        mi.isSelected = (shape == mi.shape)
-    end
-    if mi.isSelected and InputPressed(regGetString('key.toggleActive')) then
-        Melter.toggle()
-    end
-    if mi.isSelected and InputPressed(regGetString('key.prop1Add')) then
-        if Melter.isGrenade() then
-            mi.startDelay = mi.startDelay + 1
-            if mi.startDelay > 10 then
-                mi.startDelay = 0
-            end
-            regSetInt('melter.startDelay', mi.startDelay)
-        else
-            mi.radius = mi.radius + 1
-            if mi.radius > 100 then
-                mi.radius = 0
-            end
-            regSetInt('melter.laserRadius', mi.radius)
-            Melter.showVisuals()
-        end
-    end
-    if mi.isSelected and InputPressed(regGetString('key.prop1Rem')) then
-        if Melter.isGrenade() then
-            mi.startDelay = mi.startDelay - 1
-            if mi.startDelay < 0 then
-                mi.startDelay = 10
-            end
-            regSetInt('melter.startDelay', mi.startDelay)
-        else
...
-        end
-    end
-    if mi.isSelected and InputPressed(regGetString('key.prop2Add')) then
-        mi.range = mi.range + 10
-        if mi.range > 100 then
-            mi.range = 0
-        end
-        regSetInt(ternary(Melter.isGrenade(), 'melter.grenadeRange', 'melter.laserRange'), mi.range)
-        Melter.showVisuals()
-    end
-    if mi.isSelected and InputPressed(regGetString('key.prop2Rem')) then
-        mi.range = mi.range - 10
-        if mi.range < 0 then
-            mi.range = 100
-        end
...
-        Melter.showVisuals()
-    end
-    if InputPressed(regGetString('key.activateAll')) then
-        Melter.activate()
-    end
-    if InputPressed(regGetString('key.deactivateAll')) then
-        Melter.deactivate()
-    end
-end
-
-function Melter.showVisuals()
-    local mi = Melter_info[1]
-    mi.visuTimer = 3
-end
-
-function Melter.updateVisuals()
-    local mi = Melter_info[1]
-    if mi.visuTimer <= 0 then
-        return
-    end
-    -- exceptionally enforce debug to always draw these
-    local oldDb = db
-    db = true
-    local info = {
-        r = 1,
...
-        writeZ = false
-    }
-    local info2 = DeepCopy(info)
-    info2.writeZ = true
-    local tr = TransformCopy(GetShapeWorldTransform(mi.shape))
-    if Melter.isGrenade() then
-        -- For grenade show range
-        local radius = Melter.getRange()
-        tr.pos = mi.center
-        dds(tr, radius, nil, info)
-        dds(tr, radius, nil, info2)
-    else
-        -- For laser show range and radius
-        local range = Melter.getRange()
-        local dir = Melter.getLaserDir()
-        local endPos = VecAdd(mi.center, VecResize(dir, range))
-        -- Draw center line
-        dbl(mi.center, endPos, info.r, info.g, info.b, info.a)
-        DrawLine(mi.center, endPos, info.r, info.g, info.b, info.a)
-        -- Draw edge circles
-        local radius = Melter.getRadius()
-        local tr = Transform(mi.center, QuatRotateQuat(QuatLookAt(mi.center, endPos), QuatEuler(90, 0, 0)))
-        ddc(tr, radius, nil, info)
-        ddc(tr, radius, nil, info2)
-        local tr2 = TransformCopy(tr)
-        tr2.pos = endPos
-        ddc(tr2, radius, nil, info)
-        ddc(tr2, radius, nil, info2)
-    end
-    db = oldDb
-end
-
-function Melter.isGrenade()
-    return tool.ability.getType() == "MelterGrenade"
-end
-
-function Melter.isLaser()
-    return tool.ability.getType() == "MelterLaser"
-end
-
-function Melter.getLaserDir()
-    local mi = Melter_info[1]
-    local tr = GetShapeWorldTransform(mi.shape)
-    return TransformToParentVec(tr, Vec(0, 1, 0))
-end
-
-function Melter.printInfinite(val)
-    if val == 0 then
-        return "Inf"
-    end
-    return val
-end
-
-function Melter.getRange()
-    local mi = Melter_info[1]
-    local range = mi.range * 0.1
-    if Melter.isGrenade() then
-        range = range / 2
-    end
-    if range == 0 then
...
-end
-
-function Melter.getRadius()
-    local mi = Melter_info[1]
-    local radius = mi.radius * 0.1 / 2
-    if radius == 0 then
-        radius = 1000000
-    end
-    return radius
-end
-
-function Melter.levitateGrenade()
-    local mi = Melter_info[1]
-    if not Melter.isGrenade() then
-        return
-    end
-    if not regGetBool('melter.levitation') then
-        return
-    end
-    -- If not alone in its body or if jointed to anything, no levitation
-    local shapes = GetBodyShapes(mi.body)
-    if (#shapes > 1) or (#mi.jointedBodies > 1) then
-        return
-    end
-    -- Trace down to ground
-    local maxDist = Melter.getRange() / 2
-    QueryRejectBody(mi.body)
-    local hit, dist, normal, shape = QueryRaycast(mi.center, Vec(0, -1, 0), maxDist)
-    if hit then
-        -- If ground hit, levitate up
-        local vel = GetBodyVelocity(mi.body)
-        vel[2] = 1
-        SetBodyVelocity(mi.body, vel)
-    end
-end
-
-function Melter.updateUnmeltableBodies()
-    local mi = Melter_info[1]
-    tool.ability.unmeltableBodies = {}
-    tool.ability.unmeltableBodies[mi.body] = true
-    for i=1,#mi.jointedBodies do
-        tool.ability.unmeltableBodies[mi.jointedBodies[i]] = true
-    end
-end
-
+#version 2
 function Melter.toggle()
     local mi = Melter_info[1]
     if mi.active then
@@ -365,8 +33,6 @@
                             </group> \
...
-    regSetInt("melt.scriptManager", entities[1])
+    regSetInt("melt.scriptManager", entities[1], true)
 end
...
+#version 2
 function modReset()
     ...
     -- Melt
-    regSetBool('melt.soundOn'    , true)
-    regSetBool('melt.effectOn'   , true)
-    regSetBool('melt.particlesOn', true)
-    regSetBool('melt.outlineOn'  , true)
-    regSetBool('melt.cursorOn'   , true)
-    regSetBool('melt.debugMode'  , false)
-    regSetInt('melt.speed'       , 2)
-    regSetBool('melt.allowDrip'  , false)
-    regSetBool('melt.atCursor'   , false)
-    regSetBool('melt.inCircle'   , false)
-    regSetBool('melt.zerog'      , false)
-    regSetBool('melt.gravity'    , true)
-    regSetBool('melt.slimy'      , false)
-    regSetBool('melt.superfluid' , false)
-    regSetBool('melt.focus'      , false)
-    regSetBool('melt.repulsive'  , false)
-    regSetBool('melt.mirror'     , false)
-    regSetInt('melt.radius'      , 20)
-    regSetInt('melt.color.r'     , 255)
-    regSetInt('melt.color.g'     , 51)
-    regSetInt('melt.color.b'     , 51)
-    regSetInt('melt.saturation'  , 200)
-    regSetBool('melt.showAbilityText', true)
-    regSetBool('melt.showAbilityIcon', true)
+    regSetBool('melt.soundOn'    , true, true)
+    regSetBool('melt.effectOn'   , true, true)
+    regSetBool('melt.particlesOn', true, true)
+    regSetBool('melt.outlineOn'  , true, true)
+    regSetBool('melt.cursorOn'   , true, true)
+    regSetBool('melt.debugMode'  , false, true)
+    regSetInt('melt.speed'       , 2, true)
+    regSetBool('melt.allowDrip'  , false, true)
+    regSetBool('melt.atCursor'   , false, true)
+    regSetBool('melt.inCircle'   , false, true)
+    regSetBool('melt.zerog'      , false, true)
+    regSetBool('melt.gravity'    , true, true)
+    regSetBool('melt.slimy'      , false, true)
+    regSetBool('melt.superfluid' , false, true)
+    regSetBool('melt.focus'      , false, true)
+    regSetBool('melt.repulsive'  , false, true)
+    regSetBool('melt.mirror'     , false, true)
+    regSetInt('melt.radius'      , 20, true)
+    regSetInt('melt.color.r'     , 255, true)
+    regSetInt('melt.color.g'     , 51, true)
+    regSetInt('melt.color.b'     , 51, true)
+    regSetInt('melt.saturation'  , 200, true)
+    regSetBool('melt.showAbilityText', true, true)
+    regSetBool('melt.showAbilityIcon', true, true)
 
     -- Spawnable melters
-    regSetInt('melter.startDelay'  , 0)
-    regSetBool('melter.levitation' , true)
-    regSetInt('melter.grenadeRange', 20)
-    regSetInt('melter.laserRange'  , 50)
-    regSetInt('melter.laserRadius' , 10)
-    regSetBool('melter.implode'    , false)
-    regSetBool('melter.explode'    , false)
+    regSetInt('melter.startDelay'  , 0, true)
+    regSetBool('melter.levitation' , true, true)
+    regSetInt('melter.grenadeRange', 20, true)
+    regSetInt('melter.laserRange'  , 50, true)
+    regSetInt('melter.laserRadius' , 10, true)
+    regSetBool('melter.implode'    , false, true)
+    regSetBool('melter.explode'    , false, true)
 
     -- Key bindings
     regInitKey('key.melt'       , "lmb")
     regInitKey('key.pattern'    , "t")
     regInitKey('key.options'    , "o")
-    regSetBool('melt.globalKeys', false)
+    regSetBool('melt.globalKeys', false, true)
 
     regInitKey('key.toggleActive' , "e")
     regInitKey('key.activateAll'  , "")
@@ -73,21 +74,22 @@
...
-
-        input = {
-            didStartSpell = function() return InputPressed(regGetString('key.melt')) end,
-            didStopSpell = function() return InputReleased(regGetString('key.melt')) end,
-        },
-
-        setup = {
-            name = 'melt',
...
+#version 2
 function tool.run()
     ...
     local globalKeys = regGetBool('melt.globalKeys') 
@@ -49,7 +19,7 @@
     if tool.tool.active() or globalKeys then
         -- Store player pos
         tool.tool.playerPos = GetPlayerCenter()
-        tool.tool.eyePos = GetPlayerCameraTransform().pos
+        tool.tool.eyePos = GetPlayerCameraTransform(playerId).pos
         tool.ability.active = IsShapeValid(tool.tool.aimShape)
         -- Manage tool visibility
         local toolBody = GetToolBody()
@@ -108,13 +78,13 @@
 
 function tool.tool.updateAimPos()
     -- Get point fixed distance in front of player
-    local playerTr = GetPlayerCameraTransform()
+    local playerTr = GetPlayerCameraTransform(playerId)
     tool.tool.aimDir = TransformToParentVec(playerTr, Vec(0, 0, -1))
     tool.tool.aimPos = VecAdd(playerTr.pos, VecResize(tool.tool.aimDir, 10))
     -- If selecting shape at cursor, update selected shape
     if tool.ability.active and regGetBool('melt.atCursor') then
         local maxDist = 1000000
-        local playerTr = GetPlayerCameraTransform()
+        local playerTr = GetPlayerCameraTransform(playerId)
         local direction = TransformToParentVec(playerTr, Vec(0, 0, -1))
...
-        {
-            name = "Point",
-            icon = "MOD/img/point.png",
-            color = Vec(1, 1, 170/255)
-        },
-        {
-            name = "Flat",
-            icon = "MOD/img/flat.png",
-            color = Vec(1, 170/255, 170/255)
-        },
-        {
-            name = "Circular",
-            icon = "MOD/img/circular.png",
-            color = Vec(1, 1, 85/255)
-        },
-        {
-            name = "Surround",
-            icon = "MOD/img/surround.png",
-            color = Vec(1, 85/255, 85/255)
-        },
-        {
-            name = "Cone",
-            icon = "MOD/img/cone.png",
-            color = Vec(1, 1, 0)
-        },
-        {
-            name = "Laser",
-            icon = "MOD/img/laser.png",
-            color = Vec(1, 0, 0)
-        }
-    },
-    indexByName = {},
...
@@ -755,7 +636,7 @@
     ...
     -- Randomly pick an available spot to move the voxel
     local newVoxelIndex = nil
-    if #voxelIndexPrioCandidates > 0 then
+    if #voxelIndexPrioCandidates ~= 0 then
         newVoxelIndex = voxelIndexPrioCandidates[rdm(1, #voxelIndexPrioCandidates)]
     else
         newVoxelIndex = voxelIndexCandidates[rdm(1, #voxelIndexCandidates)]
@@ -909,7 +790,7 @@
     -- Browse voxels on the selected axis until a non-empty voxel is found
     for i=0,maxIndex do
         local type, r, g, b, a, entry = GetShapeMaterialAtIndex(tool.tool.aimShape, voxelVecIndex[1], voxelVecIndex[2], voxelVecIndex[3])
-        if entry > 0 then
+        if entry ~= 0 then
             voxelToMelt = VecCopy(voxelVecIndex)
             break
         end
@@ -922,8 +803,6 @@
     return voxelToMelt
...
--- Alternate method using Thomasims shape content functions
--- (full shape rewrite was way too laggy even without any voxel change)
 function tool.ability.meltShapeContent(shape)
     -- Find X voxels to melt this tick
     local voxelsToMelt = math.pow(10, regGetFloat('melt.speed'))
@@ -1085,7 +964,7 @@
 function tool.ability.start()
     -- Get shape at cursor position
     local maxDist = 1000000
-    local playerTr = GetPlayerCameraTransform()
+    local playerTr = GetPlayerCameraTransform(playerId)
     local direction = TransformToParentVec(playerTr, Vec(0, 0, -1))
     local hit, dist, normal, shape = QueryRaycast(playerTr.pos, direction, maxDist)
     if hit and tool.ability.isShapeMeltable(shape) then
@@ -1137,43 +1016,6 @@
     end
     return dir, secondaryDirs
 end
-
-tool.effect = {
-    meltLoop = LoadLoop("MOD/snd/lava.ogg"),
-    fadeInOut = 0,
-    flameColors = {
-        {
-            232/255.0,
...
-}
 ...
 function tool.effect.getFlameColor()
     return tool.effect.flameColors[rdm(1, #tool.effect.flameColors)]
@@ -1225,4 +1067,5 @@
     ParticleSticky(0)
     ParticleCollide(1)
...
-            do UiPush()
-
-                ui.slider.create('Melting speed', 'melt.speed', 'vox/tick', 0, 6, nil, nil, nil, nil, true, nil, 
-                function(rawValue)
...
-
-                ui.slider.create('Melting radius (for Cone/Laser)', 'melt.radius', 'vox', 1, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                ui.checkBox.create('Melt all objects in circle (for Cone/Laser)', 'melt.inCircle')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Melted voxels fall faster (but may float more often)', 'melt.allowDrip')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Mirror (Objects also melt from the opposite direction)', 'melt.mirror')
-                ui.padding.create(0, 64)
-
-                ui.colorPicker.create('Screen effect color', 'melt.color')
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Screen effect saturation', 'melt.saturation', '%', 0, 200, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-            UiPop() end
-
-            do UiPush()
-
-                ui.padding.create(625, 0)
-
-                ui.checkBox.create('Zero-G (Melted voxels flow in any direction)', 'melt.zerog')
-                ui.padding.create(0, 64)
-                
-                ui.checkBox.create('Fall (Melted voxels fall down)', 'melt.gravity')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Slimy (Melted voxels regroup near object center)', 'melt.slimy')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Superfluid (Melted voxels spread away from object center)', 'melt.superfluid')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Focus (Melted voxels regroup near cursor center)', 'melt.focus')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Repulsive (Melted voxels spread away from cursor center)', 'melt.repulsive')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
-            do UiPush()
-
-                ui.padding.create(1275, 0)
-                
-                do UiPush()
-                    ui.padding.create(175, 0)
-                    ui.checkBox.create('Debug Mode', 'melt.debugMode')
-                UiPop() end
-                ui.checkBox.create('Sound', 'melt.soundOn')
-                ui.padding.create(0, 64)
-
-                do UiPush()
-                    ui.padding.create(175, 0)
-                    ui.checkBox.create('Outile effect', 'melt.outlineOn')
-                UiPop() end
-                ui.checkBox.create('Screen effect', 'melt.effectOn')
-                ui.padding.create(0, 64)
-
-                do UiPush()
-                    ui.padding.create(175, 0)
-                    ui.checkBox.create('Display cursor', 'melt.cursorOn')
-                UiPop() end
-                ui.checkBox.create('Particles effect', 'melt.particlesOn')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Show melting pattern type', 'melt.showAbilityText')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Show melting pattern icon', 'melt.showAbilityIcon')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Always melt the object at cursor', 'melt.atCursor')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-        do UiPush()
-
-            UiText('CONTROLS for spawnable melters')
-            ui.padding.create(0, 50)
-            UiFont('regular.ttf', font_normal)
-            ui.keyBinding.create('Toggle melter activity', 'key.toggleActive', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Activate all melters', 'key.activateAll', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Deactivate all melters', 'key.deactivateAll', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Increase property 1', 'key.prop1Add', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Decrease property 1', 'key.prop1Rem', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Increase property 2', 'key.prop2Add', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Decrease property 2', 'key.prop2Rem', true)
-            ui.padding.create(0, 50)
-
-        UiPop() end
-
...
-
-            ui.checkBox.create('Grenades levitate when active', 'melter.levitation')
-            ui.padding.create(0, 64)
-
-            ui.checkBox.create('Implode (Melted voxels regroup near melter center)', 'melter.implode')
-            ui.padding.create(0, 64)
-
-            ui.checkBox.create('Explode (Melted voxels spread away from melter center)', 'melter.explode')
-            ui.padding.create(0, 64)
-
-        UiPop() end
-
...
-            ui.padding.create(0, 50)
-            UiFont('regular.ttf', font_normal)
-            ui.keyBinding.create('Melt (hold)', 'key.melt', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Change melting pattern (hold)', 'key.pattern', true)
-            ui.padding.create(0, 50)
...
-            UiText('', true)
-            ui.padding.create(0, 30)
-            ui.checkBox.create('Make melt and pattern change keys always available', 'melt.globalKeys')
-            ui.padding.create(0, 64)
-
-        UiPop() end
-
...
-            UiFont('regular.ttf', font_normal)
-            ui.padding.create(0, 50)
-            UiText('- You can melt only one object at a time')
-            ui.padding.create(0, 50)
-            UiText('- The melting object remain the same as long as you hold the key down')
-            ui.padding.create(0, 50)
-            UiText('- Objects melt in one direction at a time (down)')
-            ui.padding.create(0, 50)
-            UiText('- Fully melted objects will end up as a flat puddle')
-            ui.padding.create(0, 50)
-            UiText('- Voxels from the melted object may end up floating in the air')
-            ui.padding.create(0, 50)
-
-        UiPop() end
-        
...
---- Draw spell type above tool name.
 function uiDrawSpellType()
     ...
     if regGetBool('melt.showAbilityText') then
@@ -450,7 +239,7 @@
             local dt = GetTimeStep()
...
--- Draw key hint on currently hovered melter
 function uiDrawMelterHint()
     local mi = Melter_info[1]
     if (not mi.isSelected) or mi.broken then
@@ -594,7 +382,6 @@
...
---- Manage when to open and close the ability menu.
 function uiManageAbilityWheel()
     ...
     if tool.tool.active() or regGetBool('melt.globalKeys') then
@@ -652,40 +438,15 @@
