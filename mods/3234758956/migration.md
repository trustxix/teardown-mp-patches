...
+#version 2
 function modReset()
     ...
     -- Global parameters
-    regSetBool('sss.debugMode'        , false)
-    regSetInt('sss.soundVolume'       , 50)
-    regSetInt('tool.pour.velocity'    , 0)
-    regSetBool('tool.pour.overlap'    , false)
-    regSetBool('tool.pour.screenshake', true)
-    regSetBool('tool.hide'            , false)
-    regSetBool('tool.keepSpin'        , false)
+    regSetBool('sss.debugMode'        , false, true)
+    regSetInt('sss.soundVolume'       , 50, true)
+    regSetInt('tool.pour.velocity'    , 0, true)
+    regSetBool('tool.pour.overlap'    , false, true)
+    regSetBool('tool.pour.screenshake', true, true)
+    regSetBool('tool.hide'            , false, true)
+    regSetBool('tool.keepSpin'        , false, true)
 
     -- Physical characteristics and appearance
-    regSetFloat('sss.density'     , 1)
-    regSetInt('sss.friction'      , 100)
-    regSetInt('sss.restitution'   , 10)
-    regSetBool('sss.solid'        , true)
-    regSetBool('sss.unbreakable'  , false)
-    regSetBool('sss.explosive'    , false)
-    regSetBool('sss.ghost'        , false)
-    regSetBool('sss.hollow'       , true)
-    regSetInt('sss.size'          , 10)
-    regSetString('sss.material'   , "metal")
-    regSetInt('sss.color.r'       , 255)
-    regSetInt('sss.color.g'       , 255)
-    regSetInt('sss.color.b'       , 255)
-    regSetString('sss.texture'    , "none")
-    regSetInt('sss.textureOpacity', 0)
-    regSetInt('sss.reflective'    , 100)
-    regSetInt('sss.roughness'     , 0)
-    regSetInt('sss.metallic'      , 0)
-    regSetInt('sss.emissive'      , 0)
-    regSetString('sss.loadSelected'    , "none")
+    regSetFloat('sss.density'     , 1, true)
+    regSetInt('sss.friction'      , 100, true)
+    regSetInt('sss.restitution'   , 10, true)
+    regSetBool('sss.solid'        , true, true)
+    regSetBool('sss.unbreakable'  , false, true)
+    regSetBool('sss.explosive'    , false, true)
+    regSetBool('sss.ghost'        , false, true)
+    regSetBool('sss.hollow'       , true, true)
+    regSetInt('sss.size'          , 10, true)
+    regSetString('sss.material'   , "metal", true)
+    regSetInt('sss.color.r'       , 255, true)
+    regSetInt('sss.color.g'       , 255, true)
+    regSetInt('sss.color.b'       , 255, true)
+    regSetString('sss.texture'    , "none", true)
+    regSetInt('sss.textureOpacity', 0, true)
+    regSetInt('sss.reflective'    , 100, true)
+    regSetInt('sss.roughness'     , 0, true)
+    regSetInt('sss.metallic'      , 0, true)
+    regSetInt('sss.emissive'      , 0, true)
+    regSetString('sss.loadSelected'    , "none", true)
     local savedSssSkins = regListKeys('sss.saved')
-    if #savedSssSkins > 0 then
-        regSetString('sss.loadSelected', savedSssSkins[1])
+    if #savedSssSkins ~= 0 then
+        regSetString('sss.loadSelected', savedSssSkins[1], true)
     end
-    regSetString('sss.saveName'        , "")
+    regSetString('sss.saveName'        , "", true)
...
-        setup = {
-            name = 'sss',
-            title = 'Solid Sphere Summoner',
-            voxPath = 'MOD/vox/hand.vox',
-        },
-        -- Tool projectiles
-        shootTimer = 0,
-        spawnSound = LoadSound("MOD/snd/spawn.ogg"),
-        chargeSound = LoadLoop("MOD/snd/charge.ogg"),
-        shootSound = LoadSound("MOD/snd/shoot.ogg"),
-        grabSound = LoadSound("MOD/snd/grab.ogg"),
-        deniedSound = LoadSound("error.ogg"),
-        spinSound = LoadLoop("MOD/snd/spin.ogg"),
-        texturesIndex = {},
-        hitShape = nil,
-	    lightTimer = 0,
-        toolPos = nil,
-        chargeSpeed = 10, -- Speed gained per second charging
-        maxSpeed = 100, -- Maximum charge speed allowed
-        shootSpeed = 0,
-        grabbedItem = nil,
-        holdLocalOffsetToPlayer = Vec(),
-        holdLocalObjectVec = Vec(),
-        holdLocalObjectTr = Transform(),
-        lastThrownBody = nil,
-        lastThrownVelocity = nil,
-        lastThrowSuccess = 0,
-        selectedAxisIndex = 1,
-        axisDisplayTimer = 0,
-        aimAxisIndex = 1,
...
-        spinSpeed = 0,
-        maxSpinSpeed = 30,
-        wasToolHidden = regGetBool('tool.hide'),
-        lastWorldDir = Vec(),
-        toolBackup = nil,
-    }
-}
-
-tool.allAxis = {"x", "y", "z"}
-tool.axisToVec = {x=Vec(1, 0, 0), y=Vec(0, 1, 0), z=Vec(0, 0, 1)}
-tool.allAxisVec = {
-    Vec(1, 0, 0),
-    Vec(0, 1, 0),
-    Vec(0, 0, 1),
...
-tool.input = {}
+#version 2
 function tool.input.didSummon()
     return InputPressed(regGetString('key.reload'))
 end
+
 function tool.input.startCharging()
     return InputPressed(regGetString('key.shoot'))
 end
+
 function tool.input.isCharging()
     return InputDown(regGetString('key.shoot'))
 end
+
 function tool.input.didShoot()
     return InputReleased(regGetString('key.shoot'))
 end
+
 function tool.input.didGrab()
     return InputPressed(regGetString('key.grab'))
 end
+
 function tool.input.didSelect()
     return InputPressed(regGetString('key.select'))
 end
+
 function tool.input.didAim()
     return InputPressed(regGetString('key.aim'))
 end
+
 function tool.input.startSpinning()
     return InputPressed(regGetString('key.spin'))
 end
+
 function tool.input.isSpinning()
     return InputDown(regGetString('key.spin'))
 end
+
 function tool.input.didReverse()
     return InputPressed(regGetString('key.reverse'))
 end
+
...
@@ -135,7 +93,7 @@
 			...
 			ti.toolPos = TransformToParentPoint(toolTrans, Vec(-0.3, -0.2, -2.4))
 
-			if ti.shootTimer > 0 then
+			if ti.shootTimer ~= 0 then
 				local t = Transform()
 				t.pos = VecAdd(posOffset, Vec(0, 0, ti.shootTimer*3))
 				t.rot = QuatEuler(ti.shootTimer*100, 0, 0)
@@ -187,7 +145,7 @@
                 SetToolHandPoseLocalTransform(rightTr, nil)
             end
 
-			if ti.lightTimer > 0 then
+			if ti.lightTimer ~= 0 then
 				PointLight(ti.toolPos, 1, 1, 1, 0.5)
 				ti.lightTimer = ti.lightTimer - dt
 			end
@@ -294,7 +252,7 @@
     end
     tool.grabItem(sphereBody)
     -- Play sound if needed
-    if vol > 0 then
+    if vol ~= 0 then
         PlaySound(ti.spawnSound, ti.toolPos, vol)
     end
     return true
@@ -348,7 +306,7 @@
     end
     -- Play sound if needed
     local vol = regGetInt('sss.soundVolume') / 100
-    if vol > 0 then
+    if vol ~= 0 then
         local pitch = ratio(ti.shootSpeed, 0, 100, 0, 3)
         PlayLoop(ti.chargeSound, ti.toolPos, vol, true, pitch)
     end
@@ -380,7 +338,7 @@
     ti.lastThrowSuccess = 0
     -- Play sound if needed
     local vol = regGetInt('sss.soundVolume') / 100
-    if vol > 0 then
+    if vol ~= 0 then
         local pitch = ratio(ti.shootSpeed, 0, 100, 0, 3)
         PlaySound(ti.shootSound, ti.toolPos, vol, true, pitch)
     end
@@ -471,7 +429,7 @@
             tool.grabItem(body)
             -- Play sound if needed
...
-            if vol > 0 then
+            if vol ~= 0 then
                 PlaySound(ti.grabSound, ti.toolPos, vol)
             end
         end
@@ -538,18 +496,18 @@
     ConstrainPosition(ti.grabbedItem, 0, center, holdPos)
     local alignedRot = TransformToParentTransform(playerTr, ti.holdLocalObjectTr).rot
     local worldDir = Vec(0, 0, 0)
-    if ti.spinSpeed > 0 then
+    if ti.spinSpeed ~= 0 then
         alignedRot, worldDir = tool.getAlignedRot()
         ti.lastWorldDir = worldDir
     end
     ConstrainOrientation(ti.grabbedItem, 0, bodyTr.rot, alignedRot)
-    if ti.spinSpeed > 0 then
+    if ti.spinSpeed ~= 0 then
         local selectedAxisVec = tool.getAxisVec()
         local worldDir = TransformToParentVec(bodyTr, selectedAxisVec)
         ConstrainAngularVelocity(ti.grabbedItem, 0, worldDir, ti.spinSpeed * ti.spinDir)
         -- Play sound if needed
         local vol = regGetInt('sss.soundVolume') / 100
-        if vol > 0 then
+        if vol ~= 0 then
             local pitch = ratio(ti.spinSpeed, 0, 30, 0, 3)
             PlayLoop(ti.spinSound, ti.toolPos, vol, true, pitch)
         end
@@ -674,7 +632,6 @@
     return sphereBody
 end
...
--- Get number of points to get a sphere of radius radius with dist between each point
 function tool.getSpherePoints(tr, radius, dist, lastNbSamples, isDebug)
     -- Scale max dist to match world actual distance
     dist = dist * 0.4
@@ -691,7 +648,6 @@
...
--- Finds the number of points to draw a sphere with the minimum distance between any two point being lower or equal than maxDist
 function tool.findSamples(transform, radius, maxDist, sampleStart)
     sampleStart = sampleStart or 2
     sampleStart = math.max(sampleStart, 2)
@@ -859,4 +815,5 @@
     end
     beep()
     dbp('All spheres removed.')
-end+end
+
...
-            do UiPush()
-
-                ui.slider.create('Spheres initial shoot speed', 'tool.pour.velocity', 'm/s', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 40)
-                ui.checkBox.create('Screen shake when charging', 'tool.pour.screenshake')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Allow summon if colliding', 'tool.pour.overlap')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Endless spin (for last object launched)', 'tool.keepSpin')
-                ui.padding.create(0, 64)
-               
-            UiPop() end
-
...
-
-                ui.slider.create('Sound volume', 'sss.soundVolume', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 40)
-                ui.checkBox.create('Debug Mode', 'sss.debugMode')
-                ui.padding.create(0, 64)
...
-            do UiPush()
-
-                local nameToLoad = regGetString('sss.loadSelected')
-                local savedSssSkins = regListKeys('sss.saved')
-                local hasSavedSssSkins = #savedSssSkins > 0
-                -- Makes sure table is never empty (slider doesn't like empty tables)
-                if not hasSavedSssSkins then
-                    savedSssSkins = {"none"}
-                end
-                -- Makes sure currently selected name is always part of the list
-                if TableFind(savedSssSkins, nameToLoad) == nil then
-                    regSetString('sss.loadSelected', savedSssSkins[1])
-                end
-                ui.slider.create('Saved Sphere Skins', 'sss.loadSelected', '', nil, nil, nil, nil, nil, nil, nil, savedSssSkins)
-                nameToLoad = regGetString('sss.loadSelected')
-                ui.padding.create(0, 50)
-
-                do UiPush()
-
-                    local loadButton = ui.button.create('Load', whiteColor, whiteColor, nil, nil, nil, nil, not hasSavedSssSkins)
-                    if loadButton then
-                        regSetString('sss.material', regGetString('sss.saved.'..nameToLoad..'.material'))
-                        regSetInt('sss.color.r', regGetInt('sss.saved.'..nameToLoad..'.color.r'))
-                        regSetInt('sss.color.g', regGetInt('sss.saved.'..nameToLoad..'.color.g'))
-                        regSetInt('sss.color.b', regGetInt('sss.saved.'..nameToLoad..'.color.b'))
-                        regSetString('sss.texture', regGetString('sss.saved.'..nameToLoad..'.texture'))
-                        regSetInt('sss.textureOpacity', regGetInt('sss.saved.'..nameToLoad..'.textureOpacity'))
-                        regSetInt('sss.reflective', regGetInt('sss.saved.'..nameToLoad..'.reflective'))
-                        regSetInt('sss.roughness', regGetInt('sss.saved.'..nameToLoad..'.roughness'))
-                        regSetInt('sss.metallic', regGetInt('sss.saved.'..nameToLoad..'.metallic'))
-                        regSetInt('sss.emissive', regGetInt('sss.saved.'..nameToLoad..'.emissive'))
-                    end
-                    ui.padding.create(ui.button.lastWidth + padW, 0)
-
-                    local deleteButton = ui.button.create('Delete', whiteColor, whiteColor, nil, nil, nil, nil, not hasSavedSssSkins)
-                    if deleteButton then
-                        regClearKey('sss.saved.'..nameToLoad)
-                    end
-                    ui.padding.create(ui.button.lastWidth + padW, 0)
-
-                    local saveName = regGetString('sss.saveName')
-                    local saveButton = ui.button.create('Save as', whiteColor, whiteColor, nil, nil, nil, nil, string.len(saveName) == 0)
-                    if saveButton then
-                        regSetString('sss.saved.'..saveName..'.material', regGetString('sss.material'))
-                        regSetInt('sss.saved.'..saveName..'.color.r', regGetInt('sss.color.r'))
-                        regSetInt('sss.saved.'..saveName..'.color.g', regGetInt('sss.color.g'))
-                        regSetInt('sss.saved.'..saveName..'.color.b', regGetInt('sss.color.b'))
-                        regSetString('sss.saved.'..saveName..'.texture', regGetString('sss.texture'))
-                        regSetInt('sss.saved.'..saveName..'.textureOpacity', regGetInt('sss.textureOpacity'))
-                        regSetInt('sss.saved.'..saveName..'.reflective', regGetInt('sss.reflective'))
-                        regSetInt('sss.saved.'..saveName..'.roughness', regGetInt('sss.roughness'))
-                        regSetInt('sss.saved.'..saveName..'.metallic', regGetInt('sss.metallic'))
-                        regSetInt('sss.saved.'..saveName..'.emissive', regGetInt('sss.emissive'))
-                    end
-                    ui.padding.create(ui.button.lastWidth + padW, -20)
-
-                UiPop() end
-                ui.padding.create(0, 34)
-                ui.lineEdit.create('', 'sss.saveName', nil, nil, true)
-                ui.padding.create(0, 64)
-
-                ui.slider.create('Size (>30 will lag)', 'sss.size', 'Vox', 1, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                ui.slider.create('Sphere mass multiplier', 'sss.density', 'x', -3, 3, nil, nil, nil, nil, nil, nil, 
-                function(rawValue)
-                    -- Return display value for given raw value
-                    return math.pow(10, rawValue)
-                end)
-                ui.padding.create(0, 40)
-                ui.slider.create('Sphere friction (0 = ice)', 'sss.friction', '%', 0, 300, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                ui.slider.create('Sphere bounciness (190 = perfect bounce)', 'sss.restitution', '%', 0, 300, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-               
-            UiPop() end
-
...
-                ui.padding.create(600, 0)
-
-                ui.slider.create('Material (physics and sound)', 'sss.material', '', nil, nil, nil, nil, nil, nil, nil, materials)
-                ui.padding.create(0, 50)
-
-                ui.colorPicker.create('Color', 'sss.color', nil, nil, nil, false)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Texture (visual and sound)', 'sss.texture', '', nil, nil, nil, nil, nil, nil, nil, textures)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Texture opacity', 'sss.textureOpacity', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Reflective', 'sss.reflective', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Roughness (reflection blur)', 'sss.roughness', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Metallic (color tint)', 'sss.metallic', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-    
-                ui.slider.create('Emissive (glow)', 'sss.emissive', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-    
-            UiPop() end
-
...
-
-                ui.checkBox.create('Solid spheres (no break on impact)', 'sss.solid')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Unbreakable spheres', 'sss.unbreakable')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Explosive spheres', 'sss.explosive')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Ghost spheres (no collision with each other)', 'sss.ghost')
-                ui.padding.create(0, 64)
-                ui.checkBox.create('Hollow spheres (reduce lag)', 'sss.hollow')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-            ui.keyBinding.create('Summon', 'key.reload', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Shoot (hold to charge, release to launch)', 'key.shoot', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Grab', 'key.grab', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Select axis', 'key.select', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Orient axis', 'key.aim', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Spin around axis (hold)', 'key.spin', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Reverse spin direction', 'key.reverse', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Stop spin', 'key.stop', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Remove all spheres', 'key.remove', true)
-            ui.padding.create(0, 50)
...
-            UiFont('regular.ttf', font_normal)
-            ui.padding.create(0, 50)
-            UiText('- Summon as many spheres as you like', true)
-            UiText('', true)
-            UiText('- Launch them with more or less force', true)
-            UiText('', true)
-            UiText('- You can to grab and launch any dynamic object', true)
-            UiText('', true)
-            UiText('- Larger spheres are smoother but lag more', true)
-            UiText('', true)
-            UiText('- Full spheres (non hollow) are more sturdy but lag more', true)
-            UiText('', true)
-
