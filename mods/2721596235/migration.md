...
-#include "scripts/black_hole.lua"
...
+        if not sounds or not sounds.play or not sounds.play.launch then
+            initSounds()
+            initTimers()
+        end
+        runTimers()
...
+       if not regGetBool('perfo.renderBlackHoleParticles') then
+           BlackHole.blackHoles.effects.renderBlackHolesIdleSimple()
+       end
 
-    -- Fix broken sound after quicksave reload
-    if not sounds or not sounds.play or not sounds.play.launch then
-        initSounds()
-        initTimers()
...
# Migration Report: scripts\black_hole.lua
...
```diff
--- original/scripts\black_hole.lua
+++ patched/scripts\black_hole.lua
@@ -1,12 +1,4 @@
-BlackHole = {}
--- Make the list a global variable or it won't be saved properly
-BlackHole_blackHolesList = {}
-BlackHole_info = {
-    {
-        rainbowIndex = 0,
-    }
...
-
+#version 2
 function BlackHole.run()
     --> Process blackHoles.
     local xxlMode = regGetBool('black_hole.xxlMode')
@@ -46,7 +38,7 @@
 
     -- Manage max Black Holes
     local maxBlackHoles = regGetInt('perfo.maxBlackHoles')
-    if maxBlackHoles > 0 then
+    if maxBlackHoles ~= 0 then
         for i=1,#BlackHole_blackHolesList do
             if #BlackHole_blackHolesList - i >= maxBlackHoles then
                 BlackHole.blackHoles.instances.closeBlackHole(BlackHole_blackHolesList[i])
@@ -61,12 +53,6 @@
...
-
-
-BlackHole.blackHoles = {} -- Holds common data for all blackHoles.
-BlackHole.blackHoles.instances = {}
-
---- Create a black hole object and store it in the list of blackHoles.
 function BlackHole.blackHoles.instances.create(tr)
...
---- Spawn a black hole into the world.
 function BlackHole.blackHoles.instances.spawn(pos)
 
     BlackHole.blackHoles.instances.create(Transform(pos, QuatEuler(0,0,0)))
 
 end
 
---- Mark black hole to play the closing animation, then be removed
 function BlackHole.blackHoles.instances.closeBlackHole(black_hole)
     black_hole.closeBlackHole = true
     -- dbp('Black Hole closing: ' .. black_hole.id .. ' ' ..sfnTime())
 end
 
---- Mark a black hole for removal to safely remove it near the end of the frame.
 function BlackHole.blackHoles.instances.markBlackHoleForRemoval(black_hole)
     black_hole.removeBlackHole = true
     dbp('Black Hole marked for removal: ' .. black_hole.id .. ' ' ..sfnTime())
 end
 
---- Mark black holes closest to the player for removal.
 function BlackHole.blackHoles.instances.removeClosestBlackHoles(count)
     count = count or 1
...
---- Move the closest black hole to position
 function BlackHole.blackHoles.instances.moveClosestBlackHole(pos)
     if #BlackHole_blackHolesList == 0 or regGetBool('black_hole.followPlayer') then
         return false
@@ -150,7 +131,6 @@
     return true
 end
 
---- Get a list of black holes sorted by distance to a given point (by default, the player location)
 function BlackHole.blackHoles.instances.getSortedBlackHolesList(center)
     center = center or GetPlayerCenter()
     local blackHolesOrderedByDist = {}
@@ -172,7 +152,6 @@
     return blackHolesOrderedByDist
 end
 
---- Safely delete all black holes that are marked for removal.
 function BlackHole.blackHoles.instances.removeMarkedBlackHoles()
 
     TableRemove(BlackHole_blackHolesList, function(t, i, j)
@@ -182,15 +161,6 @@
...
-
-
-BlackHole.blackHoles.gravityWell = {}
-
--- Processes the gravity well of a black_hole.
 function BlackHole.blackHoles.gravityWell.process(black_hole)
 
     BlackHole.blackHoles.gravityWell.move(black_hole)
@@ -200,20 +170,18 @@
 
 end
 
--- Get the size of the well based on absolute and relative sizes
 function BlackHole.blackHoles.gravityWell.getGravityWellSize(black_hole)
     local relativeSize = black_hole.size * regGetInt('black_hole.gravityWellRatio') / 100
-    if relativeSize > 0 then
+    if relativeSize ~= 0 then
         relativeSize = relativeSize + black_hole.size
     end
     local absoluteSize = regGetInt('black_hole.gravityWellRadius')
-    if absoluteSize > 0 then
+    if absoluteSize ~= 0 then
         absoluteSize = absoluteSize + black_hole.size
     end
     return math.max(absoluteSize, relativeSize)
 end
...
--- Finds the number of points to draw a sphere with the minimum distance between any two point being lower or equal than maxDist
 function BlackHole.blackHoles.gravityWell.findSamples(transform, radius, maxDist, sampleStart)
...
--- Get extended sphere radius and points
 function BlackHole.blackHoles.gravityWell.getExtendedSphereInfos(black_hole)
     local extendedSphereUnitRadius = regGetFloat('black_hole.extendedSphereUnitRadius')
     local extendedSphereSize = black_hole.size - extendedSphereUnitRadius
@@ -262,7 +229,6 @@
     return extendedSphereSize, extendedSphereSamples, extendedSpherePoints
 end
 
--- Get number of points to get a sphere of radius radius with dist between each point
 function BlackHole.blackHoles.gravityWell.getSpherePoints(tr, radius, dist, lastNbSamples, isDebug)
     local nbSamples = BlackHole.blackHoles.gravityWell.findSamples(tr, radius, dist, lastNbSamples)
...
--- Move black hole center if needed
 function BlackHole.blackHoles.gravityWell.move(black_hole)
     if regGetBool('black_hole.followPlayer') then
         black_hole.targetPos = GetPlayerCenter()
@@ -309,11 +274,10 @@
     return newPos
 end
 
--- Pull objects in the gravity well radius
 function BlackHole.blackHoles.gravityWell.pullObjects(black_hole)
 
     local bodiesInRange = nil
-    if black_hole.gravityWellSize > 0 then -- Black Hole gravity well is active, pull objects in
+    if black_hole.gravityWellSize ~= 0 then -- Black Hole gravity well is active, pull objects in
 
         local min = VecAdd(black_hole.tr.pos, Vec(-black_hole.gravityWellSize, -black_hole.gravityWellSize, -black_hole.gravityWellSize))
         local max = VecAdd(black_hole.tr.pos, Vec(black_hole.gravityWellSize, black_hole.gravityWellSize, black_hole.gravityWellSize))
@@ -347,9 +311,9 @@
             if dist < black_hole.gravityWellSize then
...
--- Destroy any object in the black hole radius
 function BlackHole.blackHoles.gravityWell.destroyObjects(black_hole, bodies)
     -- Destroy matter in the center
     MakeHole(black_hole.tr.pos, black_hole.size, black_hole.size, black_hole.size, true)
     local extendedSphereUnitRadius = regGetFloat('black_hole.extendedSphereUnitRadius')
     -- Only destroy points on extended sphere one by one (or N by N using setting)
-    if black_hole.extendedSphereSamples > 0 then
+    if black_hole.extendedSphereSamples ~= 0 then
         -- Reset index if more samples than last time
         if black_hole.extendedSphereSamples ~= black_hole.lastExtendedSphereSamples then
             black_hole.extendedSphereIndex = 0
@@ -377,7 +340,7 @@
         end
     end
     -- Destroy random points around black hole if requested
-    if regGetBool('black_hole.randomDestruction') and black_hole.gravityWellSize > 0 then
+    if regGetBool('black_hole.randomDestruction') and black_hole.gravityWellSize ~= 0 then
         if black_hole.timers.destructionDelay.time <= 0 then
             local gravityWellExternalSize = black_hole.gravityWellSize - black_hole.size
             local size = math.min(gravityWellExternalSize/2, extendedSphereUnitRadius)
@@ -444,7 +407,7 @@
...
@@ -483,14 +446,13 @@
     ...
     -- Cleanup stuck shapes
-    if shapesRemaining >= 100 and #stuckShapes > 0 then
+    if shapesRemaining >= 100 and #stuckShapes ~= 0 then
         for i=1,#stuckShapes do
             Delete(stuckShapes[i])
         end
     end
 end
 
--- Make black hole grow
 function BlackHole.blackHoles.gravityWell.grow(black_hole)
     -- Only grow if grow time is not reached
     local xxlMode = regGetBool('black_hole.xxlMode')
@@ -503,19 +465,13 @@
                 newSize = black_hole.size + 5
             end
             local growMaxSize = regGetInt('perfo.growMaxSize')
-            if growMaxSize > 0 then
+            if growMaxSize ~= 0 then
                 newSize = clamp(newSize, newSize, growMaxSize)
             end
             black_hole.size = newSize
         end
     end
 end
-
...
-
-
-BlackHole.blackHoles.lifetime = {}
 
 function BlackHole.blackHoles.lifetime.process(black_hole)
     -- Grow time of 0 is unlimited
@@ -528,18 +484,12 @@
     end
      -- Max grow size of 0 is unlimited
      local growMaxSize = regGetInt('perfo.growMaxSize')
-     if growMaxSize > 0 then
+     if growMaxSize ~= 0 then
         if black_hole.size >= growMaxSize and regGetBool('perfo.closeAtGrowMaxSize') then
             BlackHole.blackHoles.instances.closeBlackHole(black_hole)
         end
     end
 end
-
...
-
-
-BlackHole.blackHoles.effects = {}
 
 function BlackHole.blackHoles.effects.process()
     local bhi = BlackHole_info[1]
@@ -559,7 +509,6 @@
         end
...
-
     ...
     -- Render black hole projectiles.
     for i = 1, #projectiles do
@@ -577,7 +526,6 @@
     bhi.rainbowIndex = ((h + (GetTimeStep() / 4)) * 360) % 360
 end
 
---- Draw ui dots instead of particles based on.
 function BlackHole.blackHoles.effects.renderBlackHolesIdleSimple()
 
     do UiPush()
@@ -643,7 +591,7 @@
     -- end
 
     -- -- Idle extended sphere
-    -- if black_hole.extendedSphereSamples > 0 then
+    -- if black_hole.extendedSphereSamples ~= 0 then
     --     local size = regGetFloat('black_hole.extendedSphereUnitRadius')
     --     for i=1,#black_hole.extendedSpherePoints do
     --         local pos = black_hole.extendedSpherePoints[i]
@@ -754,4 +702,5 @@
         }
     end
     return color
...
-            }, false)
-
-            if regGetBool('black_hole.soundOn') then
-                sounds.play.launch(GetPlayerCenter(), 0.5)
-            end
-        end
-    end,
-
...
-
-        if not UI_GAME then
-            if regGetInt('black_hole.movementSpeed') > 0 then
-                local moved = Gun.actions.moveClosestHole()
-                if moved and regGetBool('black_hole.soundOn') then
-                    sounds.play.move(GetPlayerCenter(), 0.5)
-                end
-            end
-        end
-        
-    end,
-
-    -- Creates a black hole projectile.
-    createShot = function()
-        local toolTr = TransformCopy(GetBodyTransform(GetToolBody()))
-        toolTr.pos = TransformToParentPoint(toolTr, Vec(0,0.2,-1))
-
-        local spreadMult = (math.random() - 0.5) * regGetFloat('tool.launch.spread')
-        local t = GetCameraTransform()
-        local transform = TransformToParentVec(t, Vec(0, 0, -1))
-        local hit, dist, normal, shape = QueryRaycast(t.pos, transform, 1000000)
-        local targetPos = VecAdd(t.pos, VecScale(transform, 1000000))
-        if hit then
-            targetPos = VecAdd(t.pos, VecScale(transform, dist))
-        end
...
-                math.random()*spreadMult))
-
-        createProj(toolTr, projectiles, projPresets.blackHoleProj)
-    end,
-
-    -- Moves the closest black hole to cursor position.
-    moveClosestHole = function()
-        local pos = GetCameraTransform().pos
-        local direction = GetPlayerLookDir()
-        local hit, dist, normal, shape = QueryRaycast(pos, direction, 1000000)
-        local targetPos = VecAdd(pos, VecScale(direction, 1000000))
-        if hit then
-            targetPos = VecAdd(pos, VecScale(direction, dist))
-        end
-        return BlackHole.blackHoles.instances.moveClosestBlackHole(targetPos)
-    end,
-
-    reset = function()
-        if #BlackHole_blackHolesList > 0 then
-            projectiles = {}
-            BlackHole.blackHoles.instances.removeClosestBlackHoles(#BlackHole_blackHolesList)
-        end
-        beep()
-        dbp('Black Holes have been reset.' .. sfnTime())
-    end,
-
-    removeClosest = function()
-        local removedBlackHolesTransform = BlackHole.blackHoles.instances.removeClosestBlackHoles()
-        if #removedBlackHolesTransform > 0 then
-            beep()
-            dbp('Closest Black Hole was removed.' .. sfnTime())
-        end
-    end,
-
-}
...
# Migration Report: scripts\projectiles.lua
...
```diff
--- original/scripts\projectiles.lua
+++ patched/scripts\projectiles.lua
@@ -1,5 +1,4 @@
-projectiles = {}
-
+#version 2
 function manageProjectiles()
     --print("projectiles="..TableToStringRecursive(projectiles))
...
+#version 2
 function modReset()
     ...
     -- Tool
-    regSetFloat('tool.launch.gravity'   , 0)
-    regSetFloat('tool.launch.rate'      , 1600)
-    regSetFloat('tool.launch.spread'    , 0)
-    regSetFloat('tool.launch.velocity'  , 0.5)
-    regSetBool('tool.debugMode'         , false)
+    regSetFloat('tool.launch.gravity'   , 0, true)
+    regSetFloat('tool.launch.rate'      , 1600, true)
+    regSetFloat('tool.launch.spread'    , 0, true)
+    regSetFloat('tool.launch.velocity'  , 0.5, true)
+    regSetBool('tool.debugMode'         , false, true)
 
     -- BlackHole
-    regSetInt('black_hole.gravityWellRatio'           , 200)
-    regSetInt('black_hole.gravityWellRadius'          , 0)
-    regSetFloat('black_hole.gravityWellForce'         , 0.7)
-    regSetBool('black_hole.gravityWellOnPlayer'       , false)
-    regSetFloat('black_hole.growSpeed'                , 0.2)
-    regSetBool('black_hole.constantGrowSpeed'         , false)
-    regSetBool('black_hole.xxlMode'                   , false)
-    regSetFloat('black_hole.extendedSphereUnitRadius' , 5)
-    regSetBool('black_hole.soundOn'                   , true)
-    regSetBool('black_hole.randomDestruction'         , false)
-    regSetFloat('black_hole.randomDestructionDelay'   , 0)
-    regSetBool('black_hole.randomDestClosest'         , false)
-    regSetInt('black_hole.movementSpeed'              , 10)
-    regSetBool('black_hole.followPlayer'              , false)
-    regSetInt('black_hole.color.r'                    , 0)
-    regSetInt('black_hole.color.g'                    , 0)
-    regSetInt('black_hole.color.b'                    , 0)
-    regSetInt('black_hole.color.a'                    , 255)
-    regSetFloat('black_hole.glow'                     , 0)
-    regSetBool('black_hole.rainbow'                   , false)
-    regSetInt('black_hole.particleColor.r'            , 255)
-    regSetInt('black_hole.particleColor.g'            , 255)
-    regSetInt('black_hole.particleColor.b'            , 255)
-    regSetInt('black_hole.particleColor.a'            , 255)
-    regSetBool('black_hole.particleNegative'          , true)
+    regSetInt('black_hole.gravityWellRatio'           , 200, true)
+    regSetInt('black_hole.gravityWellRadius'          , 0, true)
+    regSetFloat('black_hole.gravityWellForce'         , 0.7, true)
+    regSetBool('black_hole.gravityWellOnPlayer'       , false, true)
+    regSetFloat('black_hole.growSpeed'                , 0.2, true)
+    regSetBool('black_hole.constantGrowSpeed'         , false, true)
+    regSetBool('black_hole.xxlMode'                   , false, true)
+    regSetFloat('black_hole.extendedSphereUnitRadius' , 5, true)
+    regSetBool('black_hole.soundOn'                   , true, true)
+    regSetBool('black_hole.randomDestruction'         , false, true)
+    regSetFloat('black_hole.randomDestructionDelay'   , 0, true)
+    regSetBool('black_hole.randomDestClosest'         , false, true)
+    regSetInt('black_hole.movementSpeed'              , 10, true)
+    regSetBool('black_hole.followPlayer'              , false, true)
+    regSetInt('black_hole.color.r'                    , 0, true)
+    regSetInt('black_hole.color.g'                    , 0, true)
+    regSetInt('black_hole.color.b'                    , 0, true)
+    regSetInt('black_hole.color.a'                    , 255, true)
+    regSetFloat('black_hole.glow'                     , 0, true)
+    regSetBool('black_hole.rainbow'                   , false, true)
+    regSetInt('black_hole.particleColor.r'            , 255, true)
+    regSetInt('black_hole.particleColor.g'            , 255, true)
+    regSetInt('black_hole.particleColor.b'            , 255, true)
+    regSetInt('black_hole.particleColor.a'            , 255, true)
+    regSetBool('black_hole.particleNegative'          , true, true)
 
     -- Performance
-    regSetBool('perfo.renderBlackHoleParticles' , true)
-    regSetFloat('perfo.growTime'                , 0)
-    regSetBool('perfo.closeAtGrowTime'          , false)
-    regSetInt('perfo.growMaxSize'               , 0)
-    regSetBool('perfo.closeAtGrowMaxSize'       , false)
-    regSetInt('perfo.maxBlackHoles'             , 1)
-    regSetInt('perfo.maxDestructionPerTick'     , 1)
-    regSetBool('perfo.cleanupStuckShapes'       , false)
-    regSetFloat('perfo.sphereQuality'           , 10.0)
+    regSetBool('perfo.renderBlackHoleParticles' , true, true)
+    regSetFloat('perfo.growTime'                , 0, true)
+    regSetBool('perfo.closeAtGrowTime'          , false, true)
+    regSetInt('perfo.growMaxSize'               , 0, true)
+    regSetBool('perfo.closeAtGrowMaxSize'       , false, true)
+    regSetInt('perfo.maxBlackHoles'             , 1, true)
+    regSetInt('perfo.maxDestructionPerTick'     , 1, true)
+    regSetBool('perfo.cleanupStuckShapes'       , false, true)
+    regSetFloat('perfo.sphereQuality'           , 10.0, true)
...
# Migration Report: scripts\timers.lua
...
```diff
--- original/scripts\timers.lua
+++ patched/scripts\timers.lua
@@ -1,7 +1,4 @@
-timers = {}
-timer = {time, delay}
-
...
@@ -11,3 +8,4 @@
 function initTimers()
     timers.gun = { shoot = { time = 0, delay = 60 / regGetFloat('tool.launch.rate') } }
 end
+
...
-        body = nil,
-        blackHoleShape= nil,
-        blackHoleCenterTr = nil,
-        angle = 0,
-
-
-        input = {
-            didShoot = function() return tool.tool.active() and InputPressed(regGetString('key.shoot')) end,
-            didOpen = function() return tool.tool.active() and InputPressed(regGetString('key.open')) end,
-            didMove = function() return tool.tool.active() and InputPressed(regGetString('key.move')) end,
-            didReset = function() return tool.tool.active() and InputPressed(regGetString('key.closeAll')) end,
-            didRemove = function() return tool.tool.active() and InputPressed(regGetString('key.closeOne')) end,
-        },
-
-        setup = {
-            name = 'black_hole_launcher',
-            title = 'BHL-X42',
-            voxPath = 'MOD/vox/black_hole_launcher.vox',
-        },
-
-        active = function()
...
+#version 2
 function tool.run()
     ...
     if tool.tool.input.didShoot() then -- Shooting.
@@ -108,9 +70,6 @@
...
-            do UiPush()
-
-                ui.slider.create('Launch Velocity', 'tool.launch.velocity', 'm/s', 0.01, 1)
...
-
-                ui.checkBox.create('Sound', 'black_hole.soundOn')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-            do UiPush()
-
-                ui.slider.create('Grow speed', 'black_hole.growSpeed', 'm/s', 0.1, 10)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Gravity well size (proportionnal to black hole size)', 'black_hole.gravityWellRatio', '%', 0, 500, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Gravity well size (fixed from black hole surface)', 'black_hole.gravityWellRadius', 'Meters', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Gravity well force', 'black_hole.gravityWellForce', '', 0.1, 2)
-                ui.padding.create(0, 30)
-
-                ui.checkBox.create('Gravity well affect player', 'black_hole.gravityWellOnPlayer')
-                ui.padding.create(0, 64)
-
-                ui.slider.create('Movement speed', 'black_hole.movementSpeed', 'Vox/s', 0, 300, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-                UiColor(0.6,0.6,0.6, 1)
-                UiAlign('left middle')
-                UiText('Note: It is recommended to set max size at 5', true)
-                UiText('in perfo tab when using movable black holes', true)
-
-                ui.checkBox.create('Follow player', 'black_hole.followPlayer')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-
-                ui.checkBox.create('Constant grow speed', 'black_hole.constantGrowSpeed')
-                ui.padding.create(0, 60)
-                UiColor(1,0.6,0.6, 1)
-                UiAlign('left middle')
-                UiText('Caution: Some matter inside the black hole', true)
-                UiText('may not be destoryed when enabled', true)
-
-                ui.checkBox.create('XXL Mode (grows really big, really fast)', 'black_hole.xxlMode')
-                ui.padding.create(0, 60)
-                UiColor(0.6,0.6,0.6, 1)
-                UiAlign('left middle')
-                UiText('Note: If it lags, set Gravity well size to 0', true)
-
-                ui.checkBox.create('Destroy buildings near', 'black_hole.randomDestruction')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Only buildings in direct line of sight from BH', 'black_hole.randomDestClosest')
-                ui.padding.create(0, 64)
-
-                ui.slider.create('Building destruction delay', 'black_hole.randomDestructionDelay', 'Seconds', 0, 1)
-                ui.padding.create(0, 30)
-                UiColor(0.6,0.6,0.6, 1)
-                UiAlign('left middle')
-                UiText('Note: If building destruction lags your game,', true)
-                UiText('enable "Cleanup stuck debris" in perfo tab', true)
-
-            UiPop() end
-
-            do UiPush()
-
-                ui.padding.create(1090, 0)
-
-                ui.colorPicker.create('Color', 'black_hole.color', nil, nil, nil, true)
-                ui.padding.create(0, 44)
-
-                ui.slider.create('Glow', 'black_hole.glow', '', 0, 5)
-                ui.padding.create(0, 30)
-
-                ui.checkBox.create('Particles use negative of BH color', 'black_hole.particleNegative')
-                ui.padding.create(0, 74)
-
-                ui.colorPicker.create('Particles color', 'black_hole.particleColor', nil, nil, nil, true)
-                ui.padding.create(0, 44)
-
-                do UiPush()
-                    ui.padding.create(350, 0)
-                    ui.checkBox.create('Rainbow hole', 'black_hole.rainbow')
-                UiPop() end
-
-                
-
...
-        do UiPush()
-
-            ui.slider.create('Max Black Holes on map (0 = infinite)', 'perfo.maxBlackHoles', 'Black Holes', 0, 10, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-
-            ui.slider.create('Black Hole grow time (0 = infinite)', 'perfo.growTime', 'Seconds', 0, 60)
-            ui.padding.create(0, 30)
-
-            ui.checkBox.create('Close Black Hole when grow time reached', 'perfo.closeAtGrowTime')
-            ui.padding.create(0, 64)
-
-            ui.slider.create('Black Hole max size (0 = infinite)', 'perfo.growMaxSize', 'Meters', 0, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-            UiColor(0.6,0.6,0.6, 1)
-            UiAlign('left middle')
-            UiText('Note: size < 5 = Best perfo, size < 10 = Best sphere shape', true)
-
-            ui.checkBox.create('Close Black Hole when max size reached', 'perfo.closeAtGrowMaxSize')
-            ui.padding.create(0, 64)
-
-            ui.slider.create('Black Hole destruction per tick', 'perfo.maxDestructionPerTick', '', 1, 100, nil, nil, nil, nil, true)
-            ui.padding.create(0, 30)
-            UiColor(0.6,0.6,0.6, 1)
-            UiAlign('left middle')
-            UiText('Note: Higher values will let Black Holes grow faster, but lag more', true)
-
-        UiPop() end
-
-
...
-            ui.padding.create(600, 0)
-
-            ui.checkBox.create('Render Black Hole Particles', 'perfo.renderBlackHoleParticles')
-            ui.padding.create(0, 64)
-
-            ui.slider.create('Black Hole sphere smoothness', 'perfo.sphereQuality', '', 1.0, 10.0)
-            ui.padding.create(0, 30)
-
-            ui.checkBox.create('Cleanup stuck debris', 'perfo.cleanupStuckShapes')
-            ui.padding.create(0, 64)
-
-        UiPop() end
-
...
-            ui.keyBinding.create('Launch a Black Hole', 'key.shoot', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Open launched Black Hole', 'key.open', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Move closest Black Hole to cursor', 'key.move', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Close the closest Black Hole', 'key.closeOne', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Close all Black Holes', 'key.closeAll', true)
-            ui.padding.create(0, 50)
...
-            ui.padding.create(0, 50)
-            UiFont('regular.ttf', font_normal)
-            UiText('- Black Holes will slowly attract any loose object in their gravity well radius')
-            ui.padding.create(0, 50)
-            UiText('- Black Holes will progressively destroy buildings in their gravity well radius if enabled')
-            ui.padding.create(0, 50)
-            UiText('- Black Holes will destroy any matter inside them (including vehicles)')
-            ui.padding.create(0, 50)
-            UiText('- Black Holes have no effect on player')
-            ui.padding.create(0, 50)
-            UiText('- Black Holes will slowly grow in size')
-            ui.padding.create(0, 50)
-
-        UiPop() end
-
...
-ui.slider = {}
 ...
 function ui.slider.create(title, registryPath, valueText, min, max, w, h, fontSize, inline, intSlider)
 
@@ -609,9 +330,9 @@
     if intSlider then val = floatToInt(val) end
     if done then
         if intSlider then
