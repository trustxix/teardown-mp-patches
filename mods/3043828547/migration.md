...
-#include "scripts/spawn_helper.lua"
...
-    regSetInt('tool.previewRes', 60)
-    regSetBool('tool.picSpawn' , false)
+    regSetBool('tool.debugMode', false, true)
+    regSetBool('tool.soundOn'  , true, true)
+    regSetInt('tool.previewRes', 60, true)
+    regSetBool('tool.picSpawn' , false, true)
 
     -- Key bindings
     regInitKey('key.save'   , "lmb")
@@ -37,21 +38,22 @@
...
# Migration Report: scripts\spawn_helper.lua
...
```diff
--- original/scripts\spawn_helper.lua
+++ patched/scripts\spawn_helper.lua
@@ -1,43 +1,4 @@
...
-
+#version 2
 function SpawnHelper.tick(dt)
 	local shi = SpawnHelper_info[1]
 	dbw("spawnPlacement", shi.spawnPlacement)
@@ -51,10 +12,10 @@
 
 	if not shi.spawnPlacement then return end
...
+	SetBool("game.disableinteract", true, true)
 	...
 	shi.spawnDist = clamp(shi.spawnDist + InputValue("mousewheel"), 2, 8+shi.spawnRadius)
@@ -76,7 +37,7 @@
 	if InputPressed("pause") then SpawnHelper.spawnAbort() return end
 
 	if shi.spStatic then
...
+		SetBool("game.player.disableinput", true, true)
 		...
 		local realX = InputValue("camerax")
 		local realY = InputValue("cameray")*2.5
@@ -158,7 +119,7 @@
 			shi.spawnRot = shi.spawnRot + totalInput * rotSpeed
 		end
 	elseif shi.spOffset then
-		SetBool("game.player.disableinput", true)
+		SetBool("game.player.disableinput", true, true)
...
@@ -169,7 +130,7 @@
 		...
 		shi.spawnOffsetZ = shi.spawnOffsetZ - totalInput*offsetFactor
 	end
-	if not shi.spawnPlacement then SetString("game.player.tool", shi.spawnPreviousTool) end
+	if not shi.spawnPlacement then SetString("game.player.tool", shi.spawnPreviousTool, true) end
 end
 
 function SpawnHelper.draw(dt)
@@ -366,7 +327,7 @@
 	shi.spawnEntities = {}
 	shi.spawnFile = ""
 	shi.spawnIndex = nil
-	SetString("game.player.tool", shi.spawnPreviousTool)
+	SetString("game.player.tool", shi.spawnPreviousTool, true)
 end
 
 function SpawnHelper.spawn(file, entities, index)
@@ -425,8 +386,9 @@
 		end
 	end
 	shi.spawnPreviousTool = GetString("game.player.tool")
-	SetString("game.player.tool", "none")
+	SetString("game.player.tool", "none", true)
 	shi.spawnPlacement = true
 	shi.spawnFile = file
 	shi.spawnIndex = index
 end
+
...
-        selectedShapes = {},
-        flashTimer = 0,
-        flashSound = LoadSound("MOD/snd/flash.ogg"),
-        selectSound = LoadSound("spawn/select.ogg"),
-        saveIndex = 1,
-    }
-}
-
-tool.input = {}
+#version 2
 function tool.input.didSave()
     return tool.active() and (not tool.deleteMode()) and InputPressed(regGetString('key.save'))
 end
+
 function tool.input.didSelect()
     return tool.active() and InputPressed(regGetString('key.select'))
 end
+
 function tool.input.didClear()
     return tool.active() and InputPressed(regGetString('key.clear'))
 end
+
 function tool.input.didDelete()
     return tool.active() and tool.deleteMode() and InputPressed(regGetString('key.save'))
 end
...
@@ -155,7 +139,7 @@
     ...
     -- Draw flash effect
-    if ti.flashTimer > 0 then
+    if ti.flashTimer ~= 0 then
         ti.flashTimer = ti.flashTimer - GetTimeStep()
         for i=1,#ti.lastSavedShapes do
             DrawShapeHighlight(ti.lastSavedShapes[i], ti.flashTimer)
@@ -171,7 +155,7 @@
         ti.saveIndex = ti.saveIndex + 1
     end
     local gallerySlot = "gallery."..ti.saveIndex
-    regSetString(gallerySlot..".name", "unnamed")
+    regSetString(gallerySlot..".name", "unnamed", true)
     -- Take a picture of the prop and save it
     local side = regGetInt("tool.previewRes")
     local camX = side
@@ -196,16 +180,16 @@
         end
     end
     local jsonPreview = json.encode(pixels)
-    regSetString(gallerySlot..".preview", jsonPreview)
+    regSetString(gallerySlot..".preview", jsonPreview, true)
 
     -- Save body data
     local body = GetShapeBody(shape)
     local bodyTags = tool.getTagsStr(body)
-    regSetString(gallerySlot..".body.tags", bodyTags)
+    regSetString(gallerySlot..".body.tags", bodyTags, true)
 
     local bodyTr = TransformToLocalTransform(playerTr, GetBodyTransform(body))
     local jsonBodyTr = json.encode(bodyTr)
-    regSetString(gallerySlot..".body.tr", jsonBodyTr)
+    regSetString(gallerySlot..".body.tr", jsonBodyTr, true)
 
     -- Save main shape data
     tool.saveShape(gallerySlot..".shape", shape, body)
@@ -218,15 +202,15 @@
 
 function tool.saveShape(path, shape, body)
     local shapeTags = tool.getTagsStr(shape)
-    regSetString(path..".tags", shapeTags)
+    regSetString(path..".tags", shapeTags, true)
 
     local shapeContent = GetShapeContent(shape)
     local jsonShapeContent = json.encode(shapeContent)
-    regSetString(path..".content", jsonShapeContent)
+    regSetString(path..".content", jsonShapeContent, true)
 
     local shapePalette = GetShapePaletteContent(shape)
     local jsonShapePalette = json.encode(shapePalette)
-    regSetString(path..".palette", jsonShapePalette)
+    regSetString(path..".palette", jsonShapePalette, true)
 
     local size = {
         x = nil,
@@ -236,12 +220,12 @@
     }
     size.x, size.y, size.z, size.s = GetShapeSize(shape)
     local jsonShapeSize = json.encode(size)
-    regSetString(path..".size", jsonShapeSize)
+    regSetString(path..".size", jsonShapeSize, true)
 
     local shapeTr = GetShapeWorldTransform(shape)
     local localTr = TransformToLocalTransform(GetBodyTransform(body), shapeTr)
     local jsonShapeTr = json.encode(localTr)
-    regSetString(path..".tr", jsonShapeTr)
+    regSetString(path..".tr", jsonShapeTr, true)
...
@@ -249,7 +233,7 @@
     ...
     local jsonTexture = json.encode(texture)
-    regSetString(path..".texture", jsonTexture)
+    regSetString(path..".texture", jsonTexture, true)
 end
 
 function tool.loadProp(index)
@@ -355,7 +339,7 @@
         if i > 1 then
             listStr = listStr.." "
         end
...
-                ui.slider.create("Page", "ui.galleryPage", "/"..galleryPages, 1, galleryPages, 250, nil, nil, nil, true)
...
-
-                ui.lineEdit.create('', 'ui.searchFilter', 300, nil, true)
-                ui.padding.create(0, 74)
-
-                local spawnButton = ui.button.create("Search", whiteColor, whiteColor)
-                if spawnButton then
-                    uiUpdateFilteredItems()
-                end
-    
-            UiPop() end
-
...
-                local message = ""
-                if #allItems == 0 then
-                    message = "No prop saved yet ):"
-                else
-                    message = "No prop matches searched words"
-                end
-                do UiPush()
-
...
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Spawn props with picture angle', 'tool.picSpawn')
-                ui.padding.create(0, 64)
-
-                ui.slider.create('Preview image resolution', 'tool.previewRes', 'Pixels', 1, 200, nil, nil, nil, nil, true)
-                ui.padding.create(0, 50)
-
-                UiAlign('left middle')
-                UiColor(0.6,0.6,0.6, 1)
-                UiText('CAUTION: Too high resolution may lag your whole game,', true)
-                UiText('delete the props taken with high res to fix', true)
-
-            UiPop() end
-
-            do UiPush()
...
-            ui.padding.create(0, 50)
-            UiFont('regular.ttf', font_normal)
-            ui.keyBinding.create('Save prop', 'key.save', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Select/deselect prop', 'key.select', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Clear selection', 'key.clear', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Delete prop', 'key.delete', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Gallery', 'key.options', true)
-            ui.padding.create(0, 50)
-            UiColor(0.6,0.6,0.6, 1)
...
-            UiFont('regular.ttf', font_normal)
-            ui.padding.create(0, 50)
-            UiText('- Only single shapes can be saved in the Gallery (not complex objects)', true)
-            UiText('', true)
-            UiText('- Props texture will not be saved (limitation of current modding API)', true)
-            UiText('', true)
-            UiText('- Props saved in the Gallery can be loaded any number of times, on any map', true)
-            UiText('', true)
-            UiText('- You can rename props in the Gallery after saving them', true)
-            UiText('', true)
-            UiText('- To capture a group of props, your cursor must be on one of them', true)
-            UiText('', true)
-            UiText('- Loaded group props will stick together as a single entity', true)
-            UiText('', true)
-            UiText('- The delete action can delete any object, not only those spawned from the Gallery', true)
-            UiText('', true)
-            UiText('- Using delete when multiple props are selected will delete all of them', true)
-            UiText('', true)
-        UiPop() end
-
-    end,
...
---- Draw prop preview reticle
 function uiDrawReticle()
...
-ui.galleryCard = {}
 function ui.galleryCard.create(uiIndex)
