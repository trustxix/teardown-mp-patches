...
-RECOIL_BASE_HIP   = 2   -- per-shot pitch in hip
-RECOIL_BASE_ADS   = 2   -- per-shot pitch in ADS
-RECOIL_YAW_RANGE  = 1   -- +/- horizontal drift per shot (random)
...
-VISUAL_SCALE_ADS  = 2   -- viewmodel tilt multiplier (ADS)
-CAMERA_SCALE_HIP  = 1   -- camera tilt multiplier (hip)
-CAMERA_SCALE_ADS  = 1   -- camera tilt multiplier (ADS)
-
...
-	RegisterTool("rmwaa12", "AA12", "MOD/prefab/minigun.xml")
-	SetBool("game.tool.rmwaa12.enabled", true)
-
-	angle = 0
-	angVel = 0
-	coolDown = 0
...
-	SetToolHaptic("rmwaa12", toolHaptic);
-
-	
-	oldPipePos = Vec()
-	particleTimer = 0
...
@@ -248,7 +158,7 @@
 			if GetString("game.player.tool") == "rmwaa12" then --- When Zero then its empty
 			
 			local optic = opticssight[10]
-                if charginghandletime > 0 then ---Charginghandle
+                if charginghandletime ~= 0 then ---Charginghandle
 					SetShapeLocalTransform(optic, Transform(Vec(-0.06, 0.165, -0.33), QuatEuler(-90, 0, 0)))
 				else
 					SetShapeLocalTransform(optic, Transform(Vec(-0.06, 0.165, -0.23), QuatEuler(-90, 0, 0)))
@@ -469,7 +379,7 @@
 					smoke = math.min(1.0, smoke + 0.1)
 					coolDown = 0.2
 					charginghandletime = 0.06
...
+tion draw()
 ...
 if GetString("game.player.tool") == "rmwaa12" and not reloading and not attachmentmenu and GetBool("game.thirdperson") and not low then
...
-
 if GetString("game.player.tool") == "rmwaa12" and not fireswitch and not reloading and not attachmentmenu then
 		UiPush()
...
+    RegisterTool("rmwaa12", "AA12", "MOD/prefab/minigun.xml")
+    SetBool("game.tool.rmwaa12.enabled", true, true)
+    angle = 0
+    angVel = 0
+    coolDown = 0
+    smoke = 0
...
-
-	--Check if rmwaa12 is selected
-	if GetString("game.player.tool") == "rmwaa12" then
...
+                        RemoveTag(optic, "invisible")
+
+    					SetShapeLocalTransform(optic, Transform(Vec(-0.05, 0.2, -0.19), QuatEuler(-90, 0, 0)))
+    				else
+    				SetTag(optic, "invisible")
+
+                    end
...
+                        RemoveTag(optic, "invisible")
+
+    					SetShapeLocalTransform(optic, Transform(Vec(-0.025, 0.2, -0.19), QuatEuler(-90, 0, 0)))
+    				else
+    				SetTag(optic, "invisible")
+
+                    end
...
-			end
-			
-			if InputPressed("r") and magazine < 21 and not InputDown("usetool") and not reloading and not attachmentmenu then
-				reloadtimer = 2.3
...
-					Shoot(p, d, "shotgun", 0.15)
-					magazine = magazine - 1
-					semi = semi + 1
-					ShakeCamera(recoilshake)
-					if muzzlec == 0 then
-					DrawSprite(muzzleflash, mt, 0.2, 0.2, 1, 1, 1, 1, true)
-					end
-					SetCameraOffsetTransform(Transform(Vec(x, y, 0.06), QuatEuler(x, y, 2)), true)
...
-
-		--Check if firing
-		if GetBool("game.player.canusetool") and InputDown("usetool") and not attachmentmenu and not reloading and GetInt("game.tool.gun.ammo") > 0 and magazine > 0 and not fireswitch and not low then
-			angVel = math.min(1000, angVel + dt*9000)	
-			if angVel == 1000 then
-				if coolDown < 0 then
-                    local _, p, _, d = GetPlayerAimInfo(mt.pos)
...
-		tickToolAnimator(toolAnimator, dt)
-		if optics == 2 then
-		toolAnimator.offsetTransform = (Transform(Vec(-0.108, -0.035, recoil/2), QuatEuler(0, 0, 0)))
-		tickToolAnimator(toolAnimator, dt)
-		end
-		if optics == 3 then
-		toolAnimator.offsetTransform = (Transform(Vec(-0.108, -0.022, recoil/2), QuatEuler(0, 0, 0)))
-		tickToolAnimator(toolAnimator, dt)
-		end
...
-			UiText(magazine.." (7.62x51mm Nato) Auto")
-				
-				
-			end
-
-if GetString("game.player.tool") == "rmwaa12" and fireswitch and not reloading and not attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(1, 1, 1)
-			UiFont("bold.ttf", 32)
-			UiTextOutline(0,0,0,1,0.1)
-			UiText(magazine.." (7.62x51mm Nato) Semi")
-			
-			end
-			
-if GetString("game.player.tool") == "rmwaa12" and reloading and not attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(1, 1, 1)
...
-				
-			end
-if GetString("game.player.tool") == "rmwaa12" and not reloading and attachmentmenu then
-		UiPush()
-			UiTranslate(UiCenter(), UiHeight()-60)
-			UiAlign("center middle")
-			UiColor(0.8, 0.8, 0.8)
-			UiFont("bold.ttf", 31)
-			UiTextOutline(0,0,0,1,0.1)
-			UiText("Attachments: [O-Sights] [P-Underbarrel] [K-Muzzle] [L-Rails] [H-Hands]")
-				
-			end
