# Migration Report: dragon\scripts\dragon_ai.lua
...
```diff
--- original/dragon\scripts\dragon_ai.lua
+++ patched/dragon\scripts\dragon_ai.lua
@@ -1,42 +1,4 @@
-DragonAI = {}
--- Nested object to make sure quick save will keep state
-DragonAI_info = {
-    {
-        hunger = regGetInt("dragon.hungerDelay") * 60,
-        stamina = regGetInt("dragon.staminaDelay") * 60,
-        flyTimer = 0,
-        flightDir = nil,
-        flyTarget = nil,
...
-
+#version 2
 function DragonAI.run()
     local dcInfo = DragonController_info[1]
     local daInfo = DragonAI_info[1]
@@ -47,7 +9,7 @@
 
     -- If enemy, track player last known position at all time
     if dcInfo.isEnemy then
-        QueryRejectVehicle(GetPlayerVehicle())
+        QueryRejectVehicle(GetPlayerVehicle(playerId))
         local playerPos = GetPlayerCenter()
         daInfo.isPlayerVisible = DragonController.isPosVisible(playerPos, nil, true)
         if daInfo.isPlayerVisible then
@@ -115,17 +77,17 @@
                 for i=1,#enemiesList do
                     local enemyTag = enemiesList[i]
                     local preyShapes = FindShapes(enemyTag, true)
-                    if #preyShapes > 0 then
+                    if #preyShapes ~= 0 then
                         anyPotentialPrey = true
                     end
                     DragonAI.addPotentialPreys(visiblePreys, preyShapes, true)
                     local preyBodies = FindBodies(enemyTag, true)
-                    if #preyBodies > 0 then
+                    if #preyBodies ~= 0 then
                         anyPotentialPrey = true
                     end
                     DragonAI.addPotentialPreys(visiblePreys, preyBodies, true)
                     local preyVehicles = FindVehicles(enemyTag, true)
-                    if #preyVehicles > 0 then
+                    if #preyVehicles ~= 0 then
                         anyPotentialPrey = true
                     end
                     DragonAI.addPotentialPreys(visiblePreys, preyVehicles, true)
@@ -135,7 +97,7 @@
                     -- Return true to keep the value, or false to discard it.
                     return DragonAI.isEnemy(t[i]);
                 end) 
-                if #visiblePreys > 0 then
+                if #visiblePreys ~= 0 then
                     -- Pick a random prey in the list and attack it
                     DragonAI.startAutoAttack(nil, visiblePreys[rdm(1, #visiblePreys)], false, true)
                     daInfo.huntTimer = 0
@@ -160,7 +122,7 @@
     elseif dcInfo.state == "search" then
         if daInfo.searchTarget == "player" then
             -- If looking for player, then find landing spot near the player
-            local playerPos = GetPlayerTransform().pos
+            local playerPos = GetPlayerTransform(playerId).pos
             local hit, dist, normal, shape, hitPos = DragonController.trace(Vec(0, -1, 0), 1000000, playerPos)
             if hit and (not IsPointInWater(hitPos)) then
                 DragonAI.setSearchSpot(DragonAI.getLandingSpotNear(hitPos, 5))
@@ -201,17 +163,17 @@
                 for i=1,#foodList do
                     local foodTag = foodList[i]
                     local preyShapes = FindShapes(foodTag, true)
-                    if #preyShapes > 0 then
+                    if #preyShapes ~= 0 then
                         anyPotentialPrey = true
                     end
                     DragonAI.addPotentialPreys(visiblePreys, preyShapes)
                     local preyBodies = FindBodies(foodTag, true)
-                    if #preyBodies > 0 then
+                    if #preyBodies ~= 0 then
                         anyPotentialPrey = true
                     end
                     DragonAI.addPotentialPreys(visiblePreys, preyBodies)
                     local preyVehicles = FindVehicles(foodTag, true)
-                    if #preyVehicles > 0 then
+                    if #preyVehicles ~= 0 then
                         anyPotentialPrey = true
                     end
                     DragonAI.addPotentialPreys(visiblePreys, preyVehicles)
@@ -226,7 +188,7 @@
                     -- We will try to hunt again when the timer runs out
                     DragonAI.eatItem(nil, true)
                     DragonController.setObjective("fly")
-                elseif #visiblePreys > 0 then
+                elseif #visiblePreys ~= 0 then
                     -- Pick a random prey in the list
                     DragonController.setShapeToGrab(visiblePreys[rdm(1, #visiblePreys)])
                 end
@@ -433,25 +395,25 @@
     end
 
     -- Process timers
-    if daInfo.hunger > 0 then
+    if daInfo.hunger ~= 0 then
         daInfo.hunger = daInfo.hunger - dt
     end
     if isRecovering then
         daInfo.stamina = daInfo.stamina + (dt * 60)
-    elseif daInfo.stamina > 0 then
+    elseif daInfo.stamina ~= 0 then
         daInfo.stamina = daInfo.stamina - dt
     end
-    if daInfo.eatTimer > 0 then
+    if daInfo.eatTimer ~= 0 then
         daInfo.eatTimer = daInfo.eatTimer - dt
         -- After holding food item in mouth for a few seconds, actually eat it
         if (daInfo.eatTimer <= 0) and DragonAI.isFood(dcInfo.grabbedShape) then
             DragonController.dropItem()
         end
     end
-    if daInfo.huntTimer > 0 then
+    if daInfo.huntTimer ~= 0 then
         daInfo.huntTimer = daInfo.huntTimer - dt
     end
...
@@ -613,7 +575,7 @@
     -- Manage dragon flight altitude
     if relativeAltitude < 0 then
         DragonController.addAction("UP")
-    elseif relativeAltitude > 0 then
+    elseif relativeAltitude ~= 0 then
         if not stabilize then
             DragonController.addAction("DOWN")
         end
@@ -792,7 +754,7 @@
             DragonController.setObjective("fly", true)
             daInfo.flyTarget = hitPoint
         end
-    elseif isGrabbing and hitShape > 0 then
+    elseif isGrabbing and hitShape ~= 0 then
         -- If player was already grabbing and is getting impatient and gabs same object again, just land asap
         if (dcInfo.state == "search") and (daInfo.searchTarget == "object") and (hitShape == dcInfo.shapeToGrab) and (not dcInfo.isBiting) then
             daInfo.fasterLanding = true
@@ -803,7 +765,7 @@
         DragonController.setShapeToGrab(hitShape)
         DragonAI.resetSearch()
         dcInfo.isBiting = false
-    elseif isBiting and hitShape > 0 then
+    elseif isBiting and hitShape ~= 0 then
         -- Ask the dragon to search for a landing spot near the object to bite
         DragonController.setObjective("search", true)
         daInfo.searchTarget = "object"
@@ -1093,7 +1055,7 @@
     end
     daInfo.lastTurnOverWater = dcInfo.isOverWater or dcInfo.isOutOfMap
     if shouldSeekPlayer then
-        local playerDir = VecFlatten(VecSub(GetPlayerTransform().pos, center))
+        local playerDir = VecFlatten(VecSub(GetPlayerTransform(playerId).pos, center))
         if daInfo.flightDir then
             -- If going away from player, do a hard turn of 90 deg
             local angleToPlayer = VecAngleBetween(playerDir, daInfo.flightDir)
@@ -1120,7 +1082,6 @@
     end
 end
 
--- Is this shape or body an edible item for the dragon
 function DragonAI.isFood(shapeOrBody)
     -- No matter what dragons cannot eat each other
     if not DragonAI.isValidTarget(shapeOrBody) then
@@ -1144,7 +1105,6 @@
     return false
 end
 
--- Is this shape or body part of an edible item for the dragon
 function DragonAI.isFoodPart(shapeOrBody)
     local body = shapeOrBody
     if DragonAI.isFood(shapeOrBody) then
@@ -1177,7 +1137,6 @@
     return nil
 end
 
--- Is this shape or body an enemy for the dragon
 function DragonAI.isEnemy(shapeOrBody)
     -- No matter what dragons cannot eat each other
     if not DragonAI.isValidTarget(shapeOrBody) then
@@ -1215,7 +1174,6 @@
     return false
...
--- Is this shape or body a valid target
 function DragonAI.isValidTarget(shapeOrBody)
     -- Cannot target invalid/destroyed objects
     local isValid = ternary(GetEntityType(shapeOrBody) == "shape", IsShapeValid(shapeOrBody), IsBodyValid(shapeOrBody))
@@ -1229,7 +1187,6 @@
     return true
 end
 
--- Is this shape or body blacklisted
 function DragonAI.isBlacklisted(shapeOrBody)
     -- If item is in the blacklist, then ignore it
     local blacklist = DragonAI.getBlacklist()
@@ -1241,7 +1198,6 @@
     return false
 end
 
--- Is this shape or body part of a blacklisted item for the dragon
 function DragonAI.isBlacklistedPart(shapeOrBody)
     local body = shapeOrBody
     if DragonAI.isBlacklisted(shapeOrBody) then
@@ -1268,25 +1224,21 @@
     return IsBodyDynamic(shapeOrBody)
 end
 
--- Get the list of food tags
 function DragonAI.getFoodList()
     local foodStr = regGetString('dragon.foodList')
     return sSplit(foodStr, ' ')
 end
 
--- Get the list of enemy tags
 function DragonAI.getEnemiesList()
     local enemiesStr = regGetString('dragon.enemiesList')
     return sSplit(enemiesStr, ' ')
 end
 
--- Get the list of blacklisted tags
 function DragonAI.getBlacklist()
     local blacklistStr = regGetString('dragon.blacklist')
     return sSplit(blacklistStr, ' ')
 end
 
--- Add the potential preys to the main list if they are in the field of view of the dragon
 function DragonAI.addPotentialPreys(allPreys, newPreys, rejectTransparent)
     for i=1,#newPreys do
...
--- Eats the given item
 function DragonAI.eatItem(shape, force)
     local daInfo = DragonAI_info[1]
     local deleted = false
@@ -1385,7 +1336,7 @@
     if daInfo.attackTarget ~= nil then
         local targetPos = Vec()
         local targetVelocity = Vec()
-        if daInfo.attackTarget > 0 then
+        if daInfo.attackTarget ~= 0 then
             -- If attack target is a valid shape, then track that shape
             targetPos = AabbGetShapeCenterPos(daInfo.attackTarget)
             local body = GetShapeBody(daInfo.attackTarget)
@@ -1398,8 +1349,8 @@
         else
...
@@ -1418,7 +1369,7 @@
                 desiredPos = futurePos
             end
         else
-            if daInfo.attackTarget > 0 then
+            if daInfo.attackTarget ~= 0 then
                 -- Aim at closest target point
                 local hit, hitPos = GetShapeClosestPoint(daInfo.attackTarget, headPos)
                 if hit then
@@ -1452,7 +1403,6 @@
     return VecScale(sumVec, 1.0/nbVelocities)
 end
 
--- Initial recursive version I came up with
 function DragonAI.leadShot_old(projPos, projSpeed, targetPos, targetVelocity, distToTarget)
     -- Prevent a potential infinite loop case
     if VecLength(targetVelocity) >= projSpeed then
@@ -1472,7 +1422,6 @@
     end
 end
 
--- New version suggested by Mathias on Discord
 function DragonAI.leadShot(projPos, projSpeed, targetPos, targetVelocity)
...
--- If the player is holding an object that is visible for the dragon and that object is food, return the food shape
 function DragonAI.getFoodHeldByPlayer()
-    local grabbedShape = GetPlayerGrabShape()
+    local grabbedShape = GetPlayerGrabShape(playerId)
     if not grabbedShape then
         return nil
     end
@@ -1518,4 +1466,5 @@
         return nil
...
# Migration Report: dragon\scripts\dragon_controller.lua
...
```diff
--- original/dragon\scripts\dragon_controller.lua
+++ patched/dragon\scripts\dragon_controller.lua
@@ -1,446 +1,4 @@
...
-#include "dragon_vehicle.lua"
-#include "dragon_ai.lua"
-
-DragonController = {}
--- Nested object to make sure quick save will keep state
-DragonController_info = {
-    {
-        bodyParts = {
-            body = FindBody("body"),
-            head = FindBody("head"),
...
-            legs = {
-                frontLeft = {
-                    FindBody("legFrontLeft1"),
-                    FindBody("legFrontLeft2"),
-                    FindBody("legFrontLeft3"),
-                },
-                frontRight = {
-                    FindBody("legFrontRight1"),
-                    FindBody("legFrontRight2"),
-                    FindBody("legFrontRight3"),
-                },
-                backLeft = {
-                    FindBody("legBackLeft1"),
-                    FindBody("legBackLeft2"),
-                    FindBody("legBackLeft3"),
-                },
-                backRight = {
-                    FindBody("legBackRight1"),
-                    FindBody("legBackRight2"),
-                    FindBody("legBackRight3"),
-                },
-            },
-            wings = {
-                left = {
...
-        },
-        bodyLights = {
-            eyeLeft = FindLight("lightEyeLeft"),
-            eyeRight = FindLight("lightEyeRight"),
-            mouth = FindLight("lightMouth"),
-        },
-        bodyJoints = {
-            tail = {
-                FindJoint("jointBodyToTail1"),
-                FindJoint("jointTail1ToTail2"),
-                FindJoint("jointTail2ToTail3"),
-                FindJoint("jointTail3ToTail4"),
-                FindJoint("jointTail4ToTail5"),
-                FindJoint("jointTail5ToTail6"),
-                FindJoint("jointTail7ToTail8"),
-                FindJoint("jointTail8ToTail9"),
-                FindJoint("jointTail9ToTail10"),
-                FindJoint("jointTail10ToTail11"),
-                FindJoint("jointTail11ToTail12"),
-            },
-            neck = {
-                FindJoint("jointBodyToNeck1"),
-                FindJoint("jointNeck1ToNeck2"),
-                FindJoint("jointNeck2ToNeck3"),
-                FindJoint("jointNeck3ToNeck4") or FindJoint("jointNeck3ToHead"),
-                FindJoint("jointNeck4ToNeck5"),
-                FindJoint("jointNeck5ToHead"),
-            },
-            jaw = {
-                FindJoint("jointHeadToJaw1"),
-                FindJoint("jointHeadToJaw2"),
-            },
-            legs = {
-                frontLeft = {
-                    FindJoint("jointBodyToLegFrontLeft1"),
-                    FindJoint("jointFrontLeft1ToLegFrontLeft2"),
-                    FindJoint("jointFrontLeft2ToLegFrontLeft3"),
-                },
-                frontRight = {
-                    FindJoint("jointBodyToLegFrontRight1"),
-                    FindJoint("jointFrontRight1ToLegFrontRight2"),
-                    FindJoint("jointFrontRight2ToLegFrontRight3"),
-                },
-                backLeft = {
-                    FindJoint("jointBodyToLegBackLeft1"),
-                    FindJoint("jointLegBackLeft1ToLegBackLeft2"),
-                    FindJoint("jointLegBackLeft2ToLegBackLeft3"),
-                },
-                backRight = {
-                    FindJoint("jointBodyToLegBackRight1"),
-                    FindJoint("jointLegBackRight1ToLegBackRight2"),
-                    FindJoint("jointLegBackRight2ToLegBackRight3"),
-                },
-            },
-            wings = {
-                left = {
-                    FindJoints("jointBodyToWingLeft1"),
-                    FindJoints("jointWingLeft1ToWingLeft2"),
-                    FindJoints("jointWingLeft2ToWingLeft3"),
-                    FindJoints("jointWingLeft3ToWingLeft4"),
-                },
-                right = {
-                    FindJoints("jointBodyToWingRight1"),
-                    FindJoints("jointWingRight1ToWingRight2"),
-                    FindJoints("jointWingRight2ToWingRight3"),
-                    FindJoints("jointWingRight3ToWingRight4"),
-                },
-            },
-        },
-        sounds = {
-            breath = LoadLoop("MOD/dragon/sounds/breath.ogg", 10),
-            roar = LoadSound("MOD/dragon/sounds/roar0.ogg", 1000),
-            flap = LoadSound("MOD/dragon/sounds/flap0.ogg", 50),
-            success = LoadSound("MOD/dragon/sounds/success.ogg", 100),
-            fireball = {
-                LoadLoop("MOD/dragon/sounds/fireball1.ogg", 10),
-                LoadLoop("MOD/dragon/sounds/fireball2.ogg", 10),
-                LoadLoop("MOD/dragon/sounds/fireball3.ogg", 10),
-                LoadLoop("MOD/dragon/sounds/fireball4.ogg", 10),
-                LoadLoop("MOD/dragon/sounds/fireball5.ogg", 10),
-                LoadLoop("MOD/dragon/sounds/fireball6.ogg", 10),
-            },
-            growl = LoadLoop("MOD/dragon/sounds/growl.ogg", 10),
-            purr = LoadLoop("MOD/dragon/sounds/purr.ogg", 10),
-            deathrattle = LoadSound("MOD/dragon/sounds/deathrattle.ogg", 1000),
-        },
-        type = nil,
-        vehicle = FindVehicle("tameableDragon"),
-        bodyPartsCount = 0,
-        height = 0,
-        radius = 0,
-        isDead = false,
-        isPlayerRiding = false,
-        isTamed = GetBoolParam("isTamed", false),
-        isEnemy = GetBoolParam("isEnemy", false),
-        landDist = nil,
-        isOutOfMap = false,
-        isOverWater = false,
-        lastLandPos = nil,
...
-        wingMidAngle = 0,
-        flapDir = "down",
-        wingsTipCollide=true,
-        ticksWithoutAutoCorrect = 0,
-        ticksSinceFlapEnd = 0,
-        scratchLevel = 0,
-        maxArmor = 0,
-        armor = 0,
...
-        timeSinceContact = 0,
-        predictedSpeed = 0,
-        walkSpeedFactor = 0.25,
-    }
-}
-
--- Possible states/objectives = {"fly", "attack", "search", "land", "grab", "bite", "rest"}
-
-function init()
-    checkRegInitialized()
-    initDebug()
...
-    updateDebug()
-    local dcInfo = DragonController_info[1]
-    -- If the dragon was somehow partially or completely destroyed, don't attempt to control it any more
-    if dcInfo.isDead then
-        return
-    end
-    -- If spawn is not allowed, delete the dragon
-    DragonController.checkSpawnAllowed()
-    DragonController.updateData()
-    -- Death state is updated by updateData(), don't go further if dragon is dead
-    if dcInfo.isDead then
-        if dcInfo.lastKnownPos then
-            -- Play death sound
-            PlaySound(dcInfo.sounds.deathrattle, dcInfo.lastKnownPos, 10)
-        end
-        return
-    end
-    -- Either the player or the AI select the next objective and desired movement
-    DragonVehicle.run()
-    DragonAI.run()
-
-    -- Draw debug info
-    if db then
-        local name = "Dragon "..dcInfo.vehicle
-        dbw(name.." State", dcInfo.state)
-        dbw(name.." Actions", json.encode(dcInfo.actions))
-        dbw(name.." Focus", json.encode(dcInfo.focus))
-        dbw(name.." Speed", VecLength(GetBodyVelocity(dcInfo.bodyParts.body)))
-        dbw(name.." FlapDir", dcInfo.flapDir)
-        dbw(name.." WingAngle", dcInfo.wingAngle)
-        dbw(name.." FireballCharge", dcInfo.fireballCharge)
-        dbw(name.." ShapeToGrab", dcInfo.shapeToGrab)
-        dbw(name.." GrabbedShape", dcInfo.grabbedShape)
-        dbw(name.." LandDist", dcInfo.landDist)
-        dbw(name.." Biting", dcInfo.isBiting)
-        dbw(name.." ShouldWalk", DragonController.shouldWalk())
-        
-        local color = {
-            r = 1,
-            g = 1,
...
-            writeZ = false
-        }
-        if dcInfo.isPlayerRiding then
-            color.a = 0.5
-        end
-        local center = DragonController.getDragonCenter()
-        local headCenter = DragonController.getDragonHeadCenter()
-        local top = VecAdd(center, Vec(0, dcInfo.height, 0))
-        local bottom = VecAdd(center, Vec(0, -dcInfo.height, 0))
-        local color1 = DeepCopy(color)
-        if dcInfo.isTamed then
-            color1.r = 0.5
-            color1.g = 0.5
-            color1.b = 1
-        end
...
-        local forward = DragonController.getForwardDir()
...
-            color4.g = 0
-            color4.b = 0
-            dbl(headCenter, dcInfo.focus, color4.r, color4.g, color4.b, color4.a)
-        end
-    end
-    -- The controller applies this objective and move the dragon
-    DragonController.run()
-    -- Update last known position
-    dcInfo.lastKnownPos = DragonController.getDragonCenter()
-end
-
-function draw()
-    local dcInfo = DragonController_info[1]
-    uiDrawDebugMessage()
-    -- If the dragon was somehow partially or completely destroyed
-    if dcInfo.isDead then
-        return
-    end
-    DragonVehicle.showKeyHints()
-    uiDrawDragonArmor()
-    uiDrawTamingHints()
-    uiDrawHappyMeter()
-end
-
-function DragonController.init()
-    local dcInfo = DragonController_info[1]
-    -- Script garbage collector management
-    dcInfo.script = dcInfo.vehicle-1
-    dcInfo.type = GetTagValue(dcInfo.vehicle, "dragonType")
-    if not DragonController.isManagerStarted() then
-        DragonController.startManager()
-    end
-    -- Cleanup parts that were not found
-    DragonController.cleanupUnusedParts()
-    --DebugWatch("type", GetEntityType(dcInfo.script))
-    -- Init armor
-    dcInfo.maxArmor = DragonController.getMaxArmor()
-    dcInfo.armor = dcInfo.maxArmor
-    -- Compute height and radius from the dragon idle position at spawn
-    -- They represent a cylinder of radius 'radius' and height '2*height' centered on the center of the main body peice
-    dcInfo.height = 0
-    dcInfo.radius = 0
-    local center = DragonController.getDragonCenter()
-    local jointedBodies = GetJointedBodies(dcInfo.bodyParts.body)
-    dcInfo.bodyPartsCount = #jointedBodies
-    for i=1,#jointedBodies do
-        local body = jointedBodies[i]
-        local shapes = GetBodyShapes(body)
-        local tr, min, max = GetBodyObb(body)
...
-            if radius > dcInfo.radius then
-                dcInfo.radius = radius
-            end
-            local height = math.abs(corner[2] - center[2])
-            if height > dcInfo.height then
-                dcInfo.height = height
-            end
-        end
-        -- Make dragon unbreakable if it has armor
-        if dcInfo.armor > 0 then
-            SetTag(body, "unbreakable")
-        end
-    end
-    -- Store dragon radius for spawner to avoid collisions
-    regSetFloat("dragon.estimatedRadius", dcInfo.radius)
-    -- Set flight height
-    dcInfo.flightHeight = center[2]
-    -- Initialize collision filters
-    DragonController.enforceCollisionFilters()
-    -- Initialize feet for walking
-    DragonController.hoverInit()
-    DragonController.feetInit()
-end
-
-function DragonController.enforceCollisionFilters()
-    local dcInfo = DragonController_info[1]
-    -- Disable collision between wing and back leg
-    local bodyAttachedWingBodies = DragonController.getWingSideBodies("left", 1)
-    bodyAttachedWingBodies = DragonController.getWingSideBodies("right", 1, bodyAttachedWingBodies)
-    bodyAttachedWingBodies[#bodyAttachedWingBodies+1] = dcInfo.bodyParts.legs.backRight[1]
-    bodyAttachedWingBodies[#bodyAttachedWingBodies+1] = dcInfo.bodyParts.legs.backLeft[1]
-    --DebugWatch("bodyAttachedWingBodies", json.encode(bodyAttachedWingBodies))
-    for i=1,#bodyAttachedWingBodies do
-        local shapes = GetBodyShapes(bodyAttachedWingBodies[i])
-        for j=1,#shapes do
-            SetShapeCollisionFilter(shapes[j], 2, 255-2)
-        end
-    end
-    -- Disable collision between top and bottom jaw
-    local jawBodies = dcInfo.bodyParts.jaw
-    for i=1,#jawBodies do
-        local shapes = GetBodyShapes(jawBodies[i])
-        for j=1,#shapes do
-            SetShapeCollisionFilter(shapes[j], 2, 255-2)
-        end
-    end
-    -- Disable collision between wings tip and everything when landing Fantasy Dragon
-    if dcInfo.type == "fantasy" then
-        DragonController.wingsTipCollide(not dcInfo.isLanded)
-    end 
-    -- Make feet not collide legs by default until death
-    DragonController.feetCollideLegs(false)
-end
-
-function DragonController.wingsTipCollide(enableCollision)
-    local wingsTipBodies = DragonController.getWingSideBodies("left", 2)
-    wingsTipBodies = DragonController.getWingSideBodies("right", 2, wingsTipBodies)
-    --DebugWatch("wingsTipBodies", json.encode(wingsTipBodies))
-    for i=1,#wingsTipBodies do
-        local shapes = GetBodyShapes(wingsTipBodies[i])
-        for j=1,#shapes do
-            SetShapeCollisionFilter(shapes[j], 1, enableCollision and 255 or 0)
-        end
-    end
-end
-
-function DragonController.cleanupUnusedParts()
-    local dcInfo = DragonController_info[1]
-    local objsToCleanup = {"bodyParts", "bodyLights", "boduJoints"}
-    for i=1,#objsToCleanup do
-        local object = dcInfo[objsToCleanup[i]]
-        DragonController.recursiveCleanup(object)
-    end
-end
-
+#version 2
 function DragonController.recursiveCleanup(object)
     if type(object) == "table" then
         for key,value in pairs(object) do
@@ -506,7 +64,7 @@
     or (GetEntityType(selectedDragon) ~= "vehicle")
     or (not HasTag(selectedDragon, "tameableDragon"))
     or HasTag(selectedDragon, "isDead") then
-        if selectedDragon > 0 then
+        if selectedDragon ~= 0 then
             DragonController.unselectDragon()
         end
         selectedDragon = DragonController.selectDragon()
@@ -584,14 +142,14 @@
 end
 
 function DragonController.unselectDragon()
-    regSetInt('dragon.selected', 0)
+    regSetInt('dragon.selected', 0, true)
 end
 
 function DragonController.selectDragon()
     local dcInfo = DragonController_info[1]
     -- Set current dragon as the selected one, only works if it is tamed
     if dcInfo.isTamed then
-        regSetInt('dragon.selected', dcInfo.vehicle)
+        regSetInt('dragon.selected', dcInfo.vehicle, true)
         return dcInfo.vehicle
     end
     return nil
@@ -651,7 +209,7 @@
     if dcInfo.isDead then
         -- Kick player if he was riding
         if dcInfo.isPlayerRiding then
-            SetPlayerVehicle(0)
+            SetPlayerVehicle(playerId, 0)
             dcInfo.isPlayerRiding = false
         end
         -- Drop grabbed item (to avoid keeping it unbreakable if it was)
@@ -661,7 +219,7 @@
         -- Add tag so that any other script can easily know it's dead
         SetTag(dcInfo.vehicle, "isDead")
         -- Mark script for removal
-        regSetBool("dragon.scripts."..dcInfo.script, true)
+        regSetBool("dragon.scripts."..dcInfo.script, true, true)
         return
     end
     -- Cannot be both tamed and enemy, if it happen only keep isEnemy
@@ -669,7 +227,7 @@
         dcInfo.isTamed = false
     end
     -- Is the player riding the dragon
-    local v = GetPlayerVehicle()
+    local v = GetPlayerVehicle(playerId)
     local wasPlayerRiding = dcInfo.isPlayerRiding
     dcInfo.isPlayerRiding = (v == dcInfo.vehicle)
     if dcInfo.isPlayerRiding ~= wasPlayerRiding then
@@ -692,7 +250,7 @@
     end
     if not dcInfo.lastLandPos then
         -- If no land under dragon, use player location as initial land distance
-        dcInfo.lastLandPos = GetPlayerTransform().pos
+        dcInfo.lastLandPos = GetPlayerTransform(playerId).pos
     end
     if not dcInfo.landDist then
         -- If no land detected, use last known land position to compute distance
@@ -725,7 +283,7 @@
                 writeZ = false
             }
             dds(Transform(dcInfo.mouthCenter, GetBodyTransform(dcInfo.bodyParts.head).rot), regGetFloat("dragon.grabRange"), nil, color)
-            if dcInfo.shapeToGrab > 0 then
+            if dcInfo.shapeToGrab ~= 0 then
                 HighlightFullBody(GetShapeBody(dcInfo.shapeToGrab), color.r, color.g, color.b, color.a)
             end
         end
@@ -761,13 +319,13 @@
         if dcInfo.shapeToGrab == 0 then
             newClosestPoint = GetPlayerCenter()
             -- If vehicle driven, get closest pos to vehicle instead
-            local vehicle = GetPlayerVehicle()
-            if vehicle > 0 then
+            local vehicle = GetPlayerVehicle(playerId)
+            if vehicle ~= 0 then
                 local mainBody = GetVehicleBody(vehicle)
                 targetShape = GetMainShape(mainBody)
             end
         end
-        if targetShape > 0 then
+        if targetShape ~= 0 then
             local hit2, point = GetShapeClosestPoint(targetShape, dcInfo.mouthCenter)
             if hit2 then
                 newClosestPoint = point
@@ -775,7 +333,7 @@
         end
         -- When tryig to bite the player, if shapeToGrabClosestPoint is already defined, only update it if it's visible
         if (dcInfo.shapeToGrab == 0) and (dcInfo.shapeToGrabClosestPoint ~= nil) then
-            if not DragonController.isPosVisible(newClosestPoint, GetPlayerVehicle(), true) then
+            if not DragonController.isPosVisible(newClosestPoint, GetPlayerVehicle(playerId), true) then
                 newClosestPoint = dcInfo.shapeToGrabClosestPoint
             end
         end
@@ -785,7 +343,6 @@
     dcInfo.shapeToGrabClosestPoint = newClosestPoint
 end
 
--- Returns true if the given position is visible by the dragon
 function DragonController.isPosVisible(pos, rejected, rejectTransparent)
     -- Check if pos is in front of the dragon
     local forward = DragonController.getForwardDir()
@@ -833,11 +390,11 @@
     if (dragonSkin ~= "") and not unlockedSkins[dragonSkin] then
         -- Ad the script will be destroyed by manager, use global debug message log
         if dragonSkin == "bronze" then
-            regSetString("dragon.dbm", "To unlock Bronze Dragon: Ride a Wild Dragon and stay on its back when it changes direction!")
+            regSetString("dragon.dbm", "To unlock Bronze Dragon: Ride a Wild Dragon and stay on its back when it changes direction!", true)
         elseif dragonSkin == "silver" then
-            regSetString("dragon.dbm", "To unlock Silver Dragon: Tame a Wild Dragon!")
+            regSetString("dragon.dbm", "To unlock Silver Dragon: Tame a Wild Dragon!", true)
         elseif dragonSkin == "gold" then
-            regSetString("dragon.dbm", "To unlock Gold Dragon: Tame a Wild Dragon without feeding it!")
+            regSetString("dragon.dbm", "To unlock Gold Dragon: Tame a Wild Dragon without feeding it!", true)
         end
         Delete(dcInfo.bodyParts.body)
     end
@@ -1571,7 +1128,7 @@
         end
     else
         -- Wiggle tail when happy
-        if dcInfo.scratchLevel > 0 then
+        if dcInfo.scratchLevel ~= 0 then
             dcInfo.tailAngle = dcInfo.tailAngle + (dcInfo.tailWiggleDir * dt * dcInfo.scratchLevel)
             if dcInfo.tailAngle > 1 then
                 dcInfo.tailWiggleDir = -1
@@ -1805,7 +1362,7 @@
             DragonController.setShapeToGrab(closestShape)
         end
     end
-    if regGetBool('dragon.pullGrab') and dcInfo.shapeToGrab and dcInfo.shapeToGrab > 0 then
+    if regGetBool('dragon.pullGrab') and dcInfo.shapeToGrab and dcInfo.shapeToGrab ~= 0 then
         local bodyToGrab = GetShapeBody(dcInfo.shapeToGrab)
         if IsBodyDynamic(bodyToGrab) then
             local objToMouth = VecSub(dcInfo.mouthCenter, dcInfo.shapeToGrabClosestPoint)
@@ -1908,7 +1465,7 @@
                         DragonController.setObjective("fly")
                     end
                 end
...
+                if dcInfo.stateTimer ~= 0 then
                     ...
                     if DragonController.isInGrabRange(2) then
@@ -1936,7 +1493,7 @@
                     DragonController.setObjective("fly")
                 end
             end
...
+            if dcInfo.stateTimer ~= 0 then
                 ...
                 if DragonController.isInGrabRange(2) then
@@ -1950,12 +1507,12 @@
     elseif dcInfo.state == "rest" then
         -- If dragon tamed and player is nearby, extend timer
         if dcInfo.isTamed then
-            local playerPos = GetPlayerTransform().pos
+            local playerPos = GetPlayerTransform(playerId).pos
             if VecDist(playerPos, center) <= dcInfo.radius then
                 dcInfo.stateTimer = regGetInt('dragon.staminaDelay')
             end
         end
-        if dcInfo.stateTimer > 0 then
+        if dcInfo.stateTimer ~= 0 then
             ...
             -- Rest is over, restart flying
@@ -1980,12 +1537,12 @@
     local headCenter = DragonController.getDragonHeadCenter()
     local breathVolume = 0.5
     -- Tune down breath if purring or we can't hear it
-    if dcInfo.scratchLevel > 0 then
+    if dcInfo.scratchLevel ~= 0 then
         breathVolume = 0.5/(dcInfo.scratchLevel)
     end
     PlayLoop(dcInfo.sounds.breath, headCenter, breathVolume)
     -- Process roar timer
-    if dcInfo.roarTimer > 0 then
+    if dcInfo.roarTimer ~= 0 then
         dcInfo.roarTimer = dcInfo.roarTimer - dt
     end
     -- Determine if should growl or purr (when player touching it)
@@ -2049,8 +1606,8 @@
 
 function DragonController.getPlayerGrabbedBodyPart()
     local dcInfo = DragonController_info[1]
...
-
-
 function DragonController.hasAnyTags(body, tags)
     for i=1,#tags do
         if HasTag(body, tags[i]) then
@@ -2165,15 +1720,12 @@
     PointLight(pos, r, g, b, 10)
 end
 
--- Return integer between 1 and 6 representing the fireball power (and radius)
 function DragonController.getFireballLevel(charge)
     local dcInfo = DragonController_info[1]
     charge = charge or dcInfo.fireballCharge
     return math.floor(charge) + 1
 end
 
--- Place the fireball on the center point between each jaw,
--- following an orthogonal line away from each jaw 
 function DragonController.getMouthCenter()
     local dcInfo = DragonController_info[1]
     local topCenter = AabbGetBodyCenterPos(dcInfo.bodyParts.jaw[1])
@@ -2254,7 +1806,6 @@
     PointLight(pos, r, g, b, 10 * radius)
 end
 
--- Instantiates a proj and adds it to the projectiles table.
 function DragonController.createProj(transform, level)
     local dcInfo = DragonController_info[1]
     local fireballSpeed = regGetInt('dragon.fireballSpeed') -- m/s
@@ -2328,7 +1879,6 @@
     proj.drop = proj.drop + (proj.dropIncrement * dt)
 end
 
--- Is this shape or body part of the current dragon body
 function DragonController.isCurrentDragonBodyPart(shapeOrBody)
     local dcInfo = DragonController_info[1]
     if GetEntityType(shapeOrBody) == "shape" then
@@ -2337,7 +1887,6 @@
     return GetBodyVehicle(shapeOrBody) == dcInfo.vehicle
 end
 
--- Is this shape or body part of a dragon body
 function DragonController.isDragonBodyPart(shapeOrBody)
     return DragonController.isTagInherited(shapeOrBody, "tameableDragon")
 end
@@ -2422,7 +1971,7 @@
     -- If player in hole range, damage it
     local playerCenter = GetPlayerCenter()
     if VecDist(pos, playerCenter) <= radius then
...
@@ -2541,10 +2090,10 @@
         ...
         -- Apply force to player too
-        local playerPos = GetPlayerTransform().pos
+        local playerPos = GetPlayerTransform(playerId).pos
         if DragonController.isInWingsWind(playerPos) then
             local wingWindVel = DragonController.getWingWindVelocity(playerPos, true)
-            SetPlayerVelocity(VecAdd(GetPlayerVelocity(), VecScale(wingWindVel, dt * 2)))
+            SetPlayerVelocity(playerId, VecAdd(GetPlayerVelocity(playerId), VecScale(wingWindVel, dt * 2)))
         end
         -- Render wind particles
         DragonController.renderWingWindParticles()
@@ -2568,7 +2117,6 @@
     return true
 end
 
--- Taken from winddust.lua and adapted
 function DragonController.getWingWindVelocity(pos, flat)
     local center = DragonController.getDragonCenter()
     local dir = VecSub(pos, center)
@@ -2592,7 +2140,7 @@
 function DragonController.rndVec(t)
 	return Vec(DragonController.rnd(-t, t), DragonController.rnd(-t, t), DragonController.rnd(-t, t))
 end
- 
+
 function DragonController.getParticlePos(rMin, rMax, life)
 	local camPos = GetCameraTransform().pos
 	local p = DragonController.rndVec(1)
@@ -2706,7 +2254,6 @@
     end
 end
 
-
 function DragonController.hitByExplosion(strength, pos)
 	-- Check if one of the dragon parts was actually in the explosion area
     local center = DragonController.getDragonCenter()
@@ -2784,7 +2331,7 @@
     local jointedBodies = GetJointedBodies(dcInfo.bodyParts.body)
     for i=1,#jointedBodies do
...
-    regSetInt("dragon.scriptManager", entities[1])
+    regSetInt("dragon.scriptManager", entities[1], true)
 end
 
 function DragonController.drawCursor(tr, radius, normal, info)
@@ -2838,11 +2385,6 @@
...
-
-----------------------------------------------------------
--- Walking management, taken and adapted from robot.lua --
-----------------------------------------------------------
 
 function DragonController.getFeetBodies()
     local dcInfo = DragonController_info[1]
@@ -2888,7 +2430,7 @@
     local dcInfo = DragonController_info[1]
 	local f = DragonController.getFeetBodies()
     local bodyTr = GetBodyTransform(dcInfo.bodyParts.body)
-	if #f > 0 then
+	if #f ~= 0 then
 		dcInfo.distTarget = 0
 		for i=1, #f do
...
-	-- if dcInfo.contact > 0 then
+	-- if dcInfo.contact ~= 0 then
 	-- 	local maxAngVel = 10.0 / dcInfo.contact
...
-	if dcInfo.contact > 0 then
+	if dcInfo.contact ~= 0 then
 		dcInfo.timeSinceContact = 0
 	else
 		dcInfo.timeSinceContact = dcInfo.timeSinceContact + dt
@@ -3158,7 +2700,7 @@
 
 function DragonController.hoverFloat()
     local dcInfo = DragonController_info[1]
-	if dcInfo.contact > 0 then
+	if dcInfo.contact ~= 0 then
 		local d = clamp(dcInfo.distTarget - dcInfo.currentDist, -0.2, 0.2)
 		local v = d * 10
         local mass = GetFullBodyMass(dcInfo.bodyParts.body)
@@ -3192,4 +2734,3 @@
 	end
 end
...
# Migration Report: dragon\scripts\dragon_vehicle.lua
...
```diff
--- original/dragon\scripts\dragon_vehicle.lua
+++ patched/dragon\scripts\dragon_vehicle.lua
@@ -1,19 +1,4 @@
-DragonVehicle = {}
--- Nested object to make sure quick save will keep state
-DragonVehicle_info = {
-    {
-        wasPlayerRiding = false,
-        unseatDelay = regGetInt('dragon.unseatTime'),
-        unseatTimer = 0,
-        unseatAttempts = 0,
-        unseatDirection = nil,
-        heartRelativePos = 0,
-        heartSpeed = 0,
-        heartTimer = 0,
-        heartSprite = LoadSprite("MOD/dragon/images/heart.png"),
-    }
-}
-
+#version 2
 function DragonVehicle.run()
     local dcInfo = DragonController_info[1]
     local dvInfo = DragonVehicle_info[1]
@@ -35,7 +20,7 @@
         DragonVehicle.onDragonMounted()
     end
     -- Hide default vehicle controls
-    SetBool("game.vehicle.interactive", false)
+    SetBool("game.vehicle.interactive", false, true)
     -- Select actions for the dragon based on player inputs
     DragonVehicle.selectActions()
     -- If dragon is not tamed, try to unseat the player
@@ -43,7 +28,6 @@
 
     dvInfo.wasPlayerRiding = true
 end
-
 
 function DragonVehicle.onDragonMounted()
     local dcInfo = DragonController_info[1]
@@ -139,11 +123,10 @@
         return false
     end
     unlockedSkinsTab[#unlockedSkinsTab+1] = skinName
-    regSetString('dragon.unlockedSkins', TableToStr(unlockedSkinsTab))
+    regSetString('dragon.unlockedSkins', TableToStr(unlockedSkinsTab), true)
     return true
 end
 
-hintTimer = 6
 function DragonVehicle.showKeyHints()
     local dcInfo = DragonController_info[1]
     if dcInfo.isPlayerRiding then
@@ -170,7 +153,7 @@
     if DragonController.getRelativeAltitude() < 0 then
         DragonController.addAction("UP")
     end
-    if dvInfo.unseatTimer > 0 then
+    if dvInfo.unseatTimer ~= 0 then
         -- Keep the same direction for a certain time
         dvInfo.unseatTimer = dvInfo.unseatTimer - GetTimeStep()
     else
@@ -192,9 +175,9 @@
             end
         else
             -- If the player couldn't stop the dragon, eject him from the dragon's back
-            SetPlayerVehicle(0)
+            SetPlayerVehicle(playerId, 0)
             -- Throw player in the direction of the dragon movement
-            SetPlayerVelocity(VecScale(GetBodyVelocity(dcInfo.bodyParts.body), 2))
+            SetPlayerVelocity(playerId, VecScale(GetBodyVelocity(dcInfo.bodyParts.body), 2))
             DragonController.roar()
         end
     end
@@ -219,7 +202,7 @@
         if InputDown("crouch") then DragonController.addAction("DOWN") end
         if InputDown("usetool") then DragonController.addAction("FIRE") end
         --dbw("cameraTransform", json.encode(GetCameraTransform()))
-        --dbw("playerCameraTransform", json.encode(GetPlayerCameraTransform()))
+        --dbw("playerCameraTransform", json.encode(GetPlayerCameraTransform(playerId)))
         --dbw("playerEyeTransform", json.encode(GetPlayerEyeTransform()))
         local playerTr = GetCameraTransform()
         local headCenter = DragonController.getDragonHeadCenter()
@@ -263,7 +246,7 @@
     local dcInfo = DragonController_info[1]
     local dvInfo = DragonVehicle_info[1]
     -- Show the smoke heart on top of the dragon body center
-    if dvInfo.heartTimer > 0 then
+    if dvInfo.heartTimer ~= 0 then
         local dt = GetTimeStep()
         dvInfo.heartTimer = dvInfo.heartTimer - dt
         dvInfo.heartRelativePos = dvInfo.heartRelativePos + (dvInfo.heartSpeed * dt)
@@ -321,4 +304,5 @@
 function DragonVehicle.increaseUnseatDelay()
     local dvInfo = DragonVehicle_info[1]
     dvInfo.unseatDelay = dvInfo.unseatDelay + 0.25
-end+end
+
...
-#include "scripts/debug.lua"
-#include "scripts/utility.lua"
-#include "scripts/dragon_spawner.lua"
-#include "scripts/registry.lua"
-#include "scripts/umf.lua"
-#include "scripts/json.lua"
-#include "scripts/ui.lua"
...
+function server.init()
     ...
     checkRegInitialized()
     initDebug()
-    regSetFloat("dragon.estimatedRadius", 1)
-
+    regSetFloat("dragon.estimatedRadius", 1, true)
     -- Retrocompatibility for old unlocking system
     if regGetBool('dragon.manualSpawnAllowed') then
-        regSetString('dragon.unlockedSkins', TableToStr({"bronze", "silver", "gold"}))
+        regSetString('dragon.unlockedSkins', TableToStr({"bronze", "silver", "gold"}), true)
         regClearKey('dragon.manualSpawnAllowed')
     end
-
 end
...
+#version 2
 function initDebug()
     db = regGetBool('dragon.debugMode')
 end
...
# Migration Report: scripts\dragon_spawner.lua
...
```diff
--- original/scripts\dragon_spawner.lua
+++ patched/scripts\dragon_spawner.lua
@@ -1,6 +1,4 @@
-DragonSpawner = {}
-
--- This file attempt to spawn one dragon in the world at all time, so if the dragon dies, it is replaced
+#version 2
 function DragonSpawner.run()
     -- List all dragons on the map
     local dragons = FindVehicles("tameableDragon")
@@ -18,7 +16,6 @@
     end
 end
 
--- Spawn a dragon up in the sky and load its state from registry
 function DragonSpawner.spawn()
     -- Always spawn behind player (45 deg up in the air at flight altitude)
     local flightAltitude = regGetInt("dragon.flightAltitude")
@@ -77,4 +74,5 @@
     else
         return "dragon"
     end
...
-function tick()
-    -- Manage scripts
-    local keys = regListKeys("dragon.scripts")
-    for i=1,#keys do
-        local script = tonumber(keys[i])
-        Delete(script)
+function server.tick(dt)
+    for _, playerId in ipairs(GetAllPlayers()) do
+        local keys = regListKeys("dragon.scripts")
...
+        end
+        regClearKey("dragon.scripts")
+        -- Manage global debug messages
+        local message = regGetString("dragon.dbm")
+        if message ~= "" then
+            dbm(message)
+            regClearKey("dragon.dbm")
+        end
+        updateDebug()
     end
-    regClearKey("dragon.scripts")
...
+#version 2
 function modReset()
     ...
     -- Dragon
-    regSetBool('dragon.debugMode'          , false)
-    regSetBool('dragon.showActiveKeys'     , true)
+    regSetBool('dragon.debugMode'          , false, true)
+    regSetBool('dragon.showActiveKeys'     , true, true)
     regInitKey('dragon.spawnType'          , "wild")
     regInitKey('dragon.spawnBodyType'      , "Ender Dragon")
-    regSetInt('dragon.spawnCount'          , 1)
-    regSetInt('dragon.flightAltitude'      , 100)
-    regSetInt('dragon.flightAltitudeOffset', 10)
-    regSetInt('dragon.hungerDelay'         , 5)
-    regSetInt('dragon.staminaDelay'        , 20)
-    regSetBool('dragon.cautiousFlight'     , true)
-    regSetBool('dragon.cautiousLanding'    , true)
-    regSetBool('dragon.lineOfSightGrab'    , true)
-    regSetBool('dragon.lineOfSightHunt'    , true)
-    regSetBool('dragon.alignedAttack'      , false)
-    -- regSetBool('dragon.manualSpawnAllowed' , false)
-    -- regSetString('dragon.spawnType'        , "wild")
+    regSetInt('dragon.spawnCount'          , 1, true)
+    regSetInt('dragon.flightAltitude'      , 100, true)
+    regSetInt('dragon.flightAltitudeOffset', 10, true)
+    regSetInt('dragon.hungerDelay'         , 5, true)
+    regSetInt('dragon.staminaDelay'        , 20, true)
+    regSetBool('dragon.cautiousFlight'     , true, true)
+    regSetBool('dragon.cautiousLanding'    , true, true)
+    regSetBool('dragon.lineOfSightGrab'    , true, true)
+    regSetBool('dragon.lineOfSightHunt'    , true, true)
+    regSetBool('dragon.alignedAttack'      , false, true)
+    -- regSetBool('dragon.manualSpawnAllowed' , false, true)
+    -- regSetString('dragon.spawnType'        , "wild", true)
     -- regClearKey('dragon.unlockedSkins')
-    -- regSetString('dragon.spawnBodyType'      , "Ender Dragon")
-    regSetBool('dragon.unbreakableGrab'    , false)
-    regSetBool('dragon.pullGrab'           , false)
-    regSetInt('dragon.unseatTime'          , 3)
-    regSetInt('dragon.maxUnseatAttempts'   , 5)
-    regSetFloat('dragon.rotSpeed'          , 3.0)
-    regSetFloat('dragon.accelSpeed'        , 2.0) -- 5.0
-    regSetInt('dragon.maxSpeed'            , 20)
-    regSetFloat('dragon.brakeFactor'       , 3.0)
-    regSetBool('dragon.canTakeDamages'     , true)
-    regSetBool('dragon.showArmor'          , false)
-    regSetInt('dragon.armor'               , 1000)
-    regSetInt('dragon.armorResistance'     , 100)
-    regSetBool('dragon.applyArmor'         , false)
-    regSetFloat("dragon.grabRange"         , 1.0)
-    -- regSetInt('dragon.selected'             , 0)
-    regSetBool('dragon.highlightSelected'  , false)
-    regSetBool('dragon.highlightExecuting' , true)
-    regSetInt('dragon.maxAimAngle'         , 15)
-    regSetInt('dragon.fireballSpeed'       , 60)
-    regSetString('dragon.foodList'         , "meatBone fish fishBody frog forg mose raddit pig_body bird capybara bunny")
-    regSetString('dragon.enemiesList'      , "head body flyerfp chopper GrowthNodeBody PlantHeart SeedBody SpawnerBody drone dronesNest")
-    regSetString('dragon.blacklist'        , "fly DimasDogCompanion friendly bone")
-    regSetInt('dragon.minEnemySize'        , 50)
-    regSetBool('dragon.showGrabCursor'     , true)
-    regSetInt('dragon.biteChances'         , 20)
+    -- regSetString('dragon.spawnBodyType'      , "Ender Dragon", true)
+    regSetBool('dragon.unbreakableGrab'    , false, true)
+    regSetBool('dragon.pullGrab'           , false, true)
+    regSetInt('dragon.unseatTime'          , 3, true)
+    regSetInt('dragon.maxUnseatAttempts'   , 5, true)
+    regSetFloat('dragon.rotSpeed'          , 3.0, true)
+    regSetFloat('dragon.accelSpeed'        , 2.0, true) -- 5.0
+    regSetInt('dragon.maxSpeed'            , 20, true)
+    regSetFloat('dragon.brakeFactor'       , 3.0, true)
+    regSetBool('dragon.canTakeDamages'     , true, true)
+    regSetBool('dragon.showArmor'          , false, true)
+    regSetInt('dragon.armor'               , 1000, true)
+    regSetInt('dragon.armorResistance'     , 100, true)
+    regSetBool('dragon.applyArmor'         , false, true)
+    regSetFloat("dragon.grabRange"         , 1.0, true)
+    -- regSetInt('dragon.selected'             , 0, true)
+    regSetBool('dragon.highlightSelected'  , false, true)
+    regSetBool('dragon.highlightExecuting' , true, true)
+    regSetInt('dragon.maxAimAngle'         , 15, true)
+    regSetInt('dragon.fireballSpeed'       , 60, true)
+    regSetString('dragon.foodList'         , "meatBone fish fishBody frog forg mose raddit pig_body bird capybara bunny", true)
+    regSetString('dragon.enemiesList'      , "head body flyerfp chopper GrowthNodeBody PlantHeart SeedBody SpawnerBody drone dronesNest", true)
+    regSetString('dragon.blacklist'        , "fly DimasDogCompanion friendly bone", true)
+    regSetInt('dragon.minEnemySize'        , 50, true)
+    regSetBool('dragon.showGrabCursor'     , true, true)
+    regSetInt('dragon.biteChances'         , 20, true)
 
     -- Key bindings
     regInitKey('key.call'   , "x")
@@ -78,21 +79,22 @@
...
-------------------------------------------------------------------------------------------------
-cheatCode = {'t','a','m','e','t','h','e','d','r','a','g','o','n'}
-cheatIndex = 1
-
+#version 2
...
-dragonTypes = {"wild", "enemy", "tamed"}
-dragonBodyTypes = {"Ender Dragon", "Skeletal Dragon", "Zombie Dragon", "Christmas Dragon", "Ghost Dragon", "Fantasy Dragon"}
-
-options_tabs_render = {
-
-    Dragon = function()
...
-                    ui.checkBox.create('Debug Mode', 'dragon.debugMode')
-                UiPop() end
-
-                ui.checkBox.create('Show grab/bite cursor', 'dragon.showGrabCursor')
-                ui.padding.create(0, 64)
-                
-                ui.slider.create('Maximum flight speed', 'dragon.maxSpeed', 'm/s', 1, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Flight altitude', 'dragon.flightAltitude', 'Meters', 10, 200, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                do UiPush()
-                    ui.padding.create(200, 0)
-                    ui.checkBox.create('Show armor of Dragon at cursor', 'dragon.showArmor')
-                UiPop() end
-
-                ui.checkBox.create('Can take damages', 'dragon.canTakeDamages')
-                ui.padding.create(0, 64)
-
-                do UiPush()
-                    ui.padding.create(325, 0)
-                    local whiteColor = {r=1, g=1, b=1, a=1}
-                    local armorResetButton = ui.button.create('Apply', whiteColor, whiteColor, nil, nil, nil, nil, nil, font_normal)
-                    if armorResetButton then
-                        regSetBool("dragon.applyArmor", true)
-                    else
-                        regSetBool("dragon.applyArmor", false)
-                    end
-                UiPop() end
-
-                ui.slider.create('Armor resistance', 'dragon.armorResistance', '%', 0, 100, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                do UiPush()
-                    ui.padding.create(300, 0)
-                    ui.checkBox.create('Highlight Dragon actions', 'dragon.highlightExecuting')
-                UiPop() end
-                ui.checkBox.create('Highlight selected Dragon', 'dragon.highlightSelected')
-                ui.padding.create(0, 64)
-
-                ui.slider.create('Hunger delay', 'dragon.hungerDelay', 'min', 1, 120, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Rest delay', 'dragon.staminaDelay', 'min', 1, 120, nil, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-            UiPop() end
-
...
-
-                ui.checkBox.create('Try to avoid obstacles in flight', 'dragon.cautiousFlight')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Try to avoid breaking too much stuff when landing', 'dragon.cautiousLanding')
-                ui.padding.create(0, 64)
-                
-                ui.checkBox.create('Only attack aligned targets', 'dragon.alignedAttack')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Only grab items in line of sight', 'dragon.lineOfSightGrab')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Only hunt preys in line of sight', 'dragon.lineOfSightHunt')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Show Dragon intent when riding + taming hints', 'dragon.showActiveKeys')
-                ui.padding.create(0, 64)
-
-            UiPop() end
-
...
-
-                ui.checkBox.create('Make objects grabbed unbreakable', 'dragon.unbreakableGrab')
-                ui.padding.create(0, 64)
-
-                ui.checkBox.create('Pull objects being grabbed', 'dragon.pullGrab')
-                ui.padding.create(0, 64)
-
-                ui.lineEdit.create('Food tags (Dragons eat only those)', 'dragon.foodList')
-                ui.padding.create(0, 64)
-
-                ui.lineEdit.create('Enemy tags (Dragons attack only those)', 'dragon.enemiesList')
-                ui.padding.create(0, 64)
-
-                ui.lineEdit.create('Ignored tags (Dragons never target those)', 'dragon.blacklist')
-                ui.padding.create(0, 64)
-
-                ui.slider.create('Ignore enemies smaller than', 'dragon.minEnemySize', 'Voxels', 0, 100, 300, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Chances to use bite attack', 'dragon.biteChances', '%', 0, 100, 300, nil, nil, nil, true)
-                ui.padding.create(0, 30)
-
-            UiPop() end
-
...
-                UiFont('bold.ttf', 32)
-                UiAlign('left middle')
-                UiText('When mod enabled:', true)
-                ui.slider.create('Type of Dragon AI to spawn', 'dragon.spawnType', '', nil, nil, nil, nil, nil, nil, nil, dragonTypes)
-                ui.padding.create(0, 30)
-
-                -- If more skins are unlocked, add them to the list
-                local unlockedBodyTypes = DeepCopy(dragonBodyTypes)
-                local unlockedSkins = TableToSet(StrToTable(regGetString('dragon.unlockedSkins')))
-                if unlockedSkins["bronze"] then
-                    unlockedBodyTypes[#unlockedBodyTypes+1] = "Bronze Dragon"
-                end
-                if unlockedSkins["silver"] then
-                    unlockedBodyTypes[#unlockedBodyTypes+1] = "Silver Dragon"
-                end
-                if unlockedSkins["gold"] then
-                    unlockedBodyTypes[#unlockedBodyTypes+1] = "Gold Dragon"
-                end
-                ui.slider.create('Type of Dragon body to spawn', 'dragon.spawnBodyType', '', nil, nil, nil, nil, nil, nil, nil, unlockedBodyTypes)
-                ui.padding.create(0, 30)
-
-                ui.slider.create('Number of Dragons to spawn', 'dragon.spawnCount', 'Dragons', 0, 10, nil, nil, nil, nil, true)
-                ui.padding.create(0, 40)
-
-                UiColor(1,0.6,0.6, 1)
-                UiAlign('left middle')
-                UiText('WARNING: Too many Dragons will probably cause lag', true)
-
-            UiPop() end
-
-        UiPop() end
...
-            -- UiText('Only the selected Tamed Dragon will accept orders')
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Order to land near you', 'key.call', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Order to fly to position', 'key.send', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Order to throw a fireball', 'key.fire', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Order to bite item', 'key.bite', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Order to grab/drop item', 'key.grab', true)
-            ui.padding.create(0, 50)
-            ui.keyBinding.create('Remove all Dragons', 'key.remove', true)
-            ui.padding.create(0, 50)
...
-            UiFont('regular.ttf', font_normal)
-            ui.padding.create(0, 50)
-            UiText('- There is 3 types of Dragons:', true)
-            UiText('-- Wild Dragons: Will fly around peacefully, you can try to tame them by mounting them', true)
-            UiText('-- Enemy Dragons: Will attack you on sight, they cannot be tamed', true)
-            UiText('-- Tamed Dragons: Will obey your orders and can be mounted at will', true)
-            UiText('', true)
-            UiText('- Dragons will most of the time be flying and will rarely look for a spot to land and rest', true)
-            UiText('', true)
-            UiText('- Dragons likes to eat smaller animals, they may try to catch one from time to time', true)
-            UiText('', true)
-            UiText('- If you hold one of those in your hand and the Dragon can see it, it will come to eat it', true)
-            UiText('', true)
-            UiText('- Dragons will attack back if they are shot or damaged by explosions', true)
-            UiText('', true)
-            UiText('- Mouse over a Tamed Dragon to select it', true)
-            UiText('', true)
-            UiText('- Dragons will roar if they get hurt or if they cannot execute an order', true)
-            UiText('', true)
-            UiText('- When mounting a Wild Dragon, it will rush in random directions,', true)
-            UiText('    you need to counter its movements several times to tame it', true)
-
-        UiPop() end
-
-    end,
...
---- Draw vehicle keys hint
 function uiDrawVehicleKeyHint(isTamed)
...
---- Draw dragon armor bar
 function uiDrawDragonArmor()
     if not regGetBool("dragon.showArmor") then
         return
@@ -551,7 +310,6 @@
     UiPop()
 end
...
---- Draw taming hints
 function uiDrawTamingHints()
     if not regGetBool('dragon.showActiveKeys') then
         return
@@ -584,7 +342,6 @@
     UiPop()
 end
...
---- Draw last debug message
 function uiDrawDebugMessage()
     ...
     if not lastDebugMessages then
@@ -606,7 +363,6 @@
 
 end
 
---- Draw happy meter
 function uiDrawHappyMeter()
